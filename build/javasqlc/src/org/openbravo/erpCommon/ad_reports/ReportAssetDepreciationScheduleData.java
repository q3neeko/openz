 ...
 */
  public static ReportAssetDepreciationScheduleData[] select(ConnectionProvider connectionProvider, String lang, String client, String DateFrom, String DateTo, String assetSeaKey, String assetDescription, String cAssetCategoryId, String Org)    throws ServletException {
    return select(connectionProvider, lang, client, DateFrom, DateTo, assetSeaKey, assetDescription, cAssetCategoryId, Org, 0, 0);
  }

/**
Method used for search(HTML) and PDF 
 */
  public static ReportAssetDepreciationScheduleData[] select(ConnectionProvider connectionProvider, String lang, String client, String DateFrom, String DateTo, String assetSeaKey, String assetDescription, String cAssetCategoryId, String Org, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "    SELECT A.NAME AS ASSETNAME,A.DESCRIPTION AS ASSETDESCRIPTION,to_char(am.dateacct,'YYYY') AS FIELDFISCALYEAR,zssi_strDate(AM.STARTDATE,?) AS STARTDATE," +
      "    zssi_strDate(AM.ENDDATE,?) AS ENDDATE,coalesce(AML.Amortizationamt,0) AS AMOUNT,C.ISO_CODE AS CURRENCY, coalesce(a.assetvalueamt,0) AS initAmt,coalesce(a.assetvalueamt,0)-coalesce(a.depreciatedvalue,0) as currentamt, COALESCE(zssi_strDate(A.AMORTIZATIONSTARTDATE,?), '') AS AMORTIZATIONSTARTDATE, COALESCE(zssi_strDate(a_get_AmortizationenddateOrAlternative(A.A_ASSET_ID),?), '') AS AMORTIZATIONENDDATE" +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectReportAssetDepreciationScheduleData.startdate = UtilSql.getValue(result, "startdate");
        objectReportAssetDepreciationScheduleData.enddate = UtilSql.getValue(result, "enddate");
        ...
        objectReportAssetDepreciationScheduleData.amortizationstartdate = UtilSql.getValue(result, "amortizationstartdate");
        objectReportAssetDepreciationScheduleData.amortizationenddate = UtilSql.getValue(result, "amortizationenddate");
  ...
  public static ReportAssetDepreciationScheduleData[] set()    throws ServletException {
    ...
    objectReportAssetDepreciationScheduleData[0].amortizationstartdate = "";
    objectReportAssetDepreciationScheduleData[0].amortizationenddate = "";
    return objectReportAssetDepreciationScheduleData;
  }
}
