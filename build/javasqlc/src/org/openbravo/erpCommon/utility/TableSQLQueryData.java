...
static Logger log4j = Logger.getLogger(TableSQLQueryData.class);
  ...
  public String sessionvarname;
  public String sessionvarvalue;
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("columnname"))
      ...
      return tablename;
    else if (fieldName.equalsIgnoreCase("parentkeycolumn"))
      return parentkeycolumn;
    else if (fieldName.equalsIgnoreCase("parenttabid"))
      return parenttabid;
    else if (fieldName.equalsIgnoreCase("sessionvarname"))
      return sessionvarname;
    else if (fieldName.equalsIgnoreCase("sessionvarvalue"))
  ...
  public static TableSQLQueryData[] selectRelationStructure(ConnectionProvider connectionProvider, String adLanguage, String adRoleID, String adTabId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        coalesce(afi.template,f.template,ad_datatype_guiengine_template_mapping(c.ad_reference_id)) as template," +
      "        coalesce(afi.ad_field_v_id,f.ad_field_v_id) as ad_field_v_id," +
      ...
      "        c.FieldLength, " +
      "        coalesce(coalesce(afi.DefaultValue,f.DefaultValue),c.DefaultValue) as  DefaultValue, " +
      "        c.IsKey, c.IsParent, f.required as ismandatory, 'Y' as IsUpdateable," +
      "        c.ReadOnlyLogic, " +
      "        case when coalesce(afi.isidentifiercolumn,'N')='Y' then 'Y' else c.IsIdentifier end as IsIdentifier, case when c.IsKey='Y' then 999 else f.SeqNo end as seqno, c.IsTranslated, c.IsEncrypted, null as VFormat," +
      "        null as ValueMin, null as ValueMax, case when coalesce(afi.isfiltercolumn,f.isfiltercolumn)='NON' then f.isfiltercolumn else coalesce(afi.isfiltercolumn,f.isfiltercolumn) end  as IsSelectionColumn, c.AD_Process_ID, c.IsSessionAttr," +
      "        'N' as IsSecondaryKey, 'N' as IsDesencryptable, c.AD_CallOut_ID, ad_getFieldText(f.ad_field_v_id,?) AS Name," +
      "        f.AD_FieldGroup_ID, case coalesce(afi.visiblesetting,'') when 'VISIBLE' then 'Y' when 'HIDDEN' then 'N' else f.IsDisplayed end as IsDisplayed, " +
      "        coalesce(afi.DisplayLogic,f.DisplayLogic) as DisplayLogic, coalesce(afi.gridlength,f.gridlength,f.DisplayLength) as DisplayLength, f.IsReadOnly," +
      ...
      "        '' AS WindowType, '' AS IsSOTrx, ''AS Window_Name,'' as ad_column_id,'' as ismaxrowsparam, 0 as maxrowsparam," +
      "        '' AS Window_Name_Trl, t.name AS Tab_Name, '' AS Tab_Name_Trl, t.AD_Window_ID, '' AS TableName,'' as parentkeycolumn,'' as parenttabid,'' as sessionVarName,'' as sessionVarValue," +
      "         case when c.IsKey='Y' then 999 else COALESCE(afi.gridseqno,f.gridseqno,f.SeqNo,100) end as gridseqno," +
      ...
      "            AND c.IsActive = 'Y'" +
      "            AND (c.IsKey = 'Y' or (case coalesce(afi.visiblesetting,'') when 'VISIBLE' then 'Y' when 'HIDDEN' then 'N' else f.IsDisplayed='Y' end and c.IsEncrypted='N'))" +
      "            ORDER BY gridseqno";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectTableSQLQueryData.template = UtilSql.getValue(result, "template");
        objectTableSQLQueryData.adFieldVId = UtilSql.getValue(result, "ad_field_v_id");
        ...
        objectTableSQLQueryData.adColumnId = UtilSql.getValue(result, "ad_column_id");
        objectTableSQLQueryData.ismaxrowsparam = UtilSql.getValue(result, "ismaxrowsparam");
        objectTableSQLQueryData.maxrowsparam = UtilSql.getValue(result, "maxrowsparam");
        ...
        objectTableSQLQueryData.parentkeycolumn = UtilSql.getValue(result, "parentkeycolumn");
        objectTableSQLQueryData.parenttabid = UtilSql.getValue(result, "parenttabid");
        objectTableSQLQueryData.sessionvarname = UtilSql.getValue(result, "sessionvarname");
        objectTableSQLQueryData.sessionvarvalue = UtilSql.getValue(result, "sessionvarvalue");
  ...
  public static TableSQLQueryData[] selectRelationStructureAudit(ConnectionProvider connectionProvider, String adLanguage, String adTabId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      "        SELECT c.ColumnName, " +
      "        case when upper(c.columnname) in ('UPDATED','CREATED') then '16' else c.ad_Reference_id end as ad_Reference_id, c.AD_Reference_Value_ID, c.AD_Val_Rule_ID," +
  ...
  public static TableSQLQueryData[] selectWindowDefinition(ConnectionProvider connectionProvider, String adLanguage, String adTabId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "       tb.TableName ," +
      "       t.whereclause,t.ad_column_id," +
      "       case when coalesce(i.ismaxrowsparam,'') ='Y' then 'Y' when  coalesce(i.ismaxrowsparam,'') ='N' then 'N' else t.ismaxrowsparam end as ismaxrowsparam," +
      "       coalesce(i.maxrowsparam,t.maxrowsparam) as maxrowsparam," +
      "       (select ad_tab_id from ad_tab where ad_window_id=w.ad_window_id and tablevel=t.tablevel-1 and seqno<t.seqno order by seqno desc limit 1) as parenttabid," +
      "       (select columnname from ad_column where iskey='Y' and ad_table_id =(select ad_table_id from ad_tab where ad_window_id=w.ad_window_id and tablevel=t.tablevel-1 and seqno<t.seqno order by seqno desc limit 1)) as parentkeycolumn" +
      "       FROM AD_Tab t left join AD_Tab_TRL t_trl on t.AD_Tab_ID = t_trl.AD_Tab_ID " +
      "                        AND t_trl.AD_Language = ?" +
      "                     left join ad_tab_instance i on i.ad_tab_id=t.ad_tab_id," +
...
Name of the columns parent of the tab
 */
  public static TableSQLQueryData[] selectTabSessionFields(ConnectionProvider connectionProvider, String tab, String keyname, String keyvalue)    throws ServletException {
    return selectTabSessionFields(connectionProvider, tab, keyname, keyvalue, 0, 0);
  }

/**
Name of the columns parent of the tab
 */
  public static TableSQLQueryData[] selectTabSessionFields(ConnectionProvider connectionProvider, String tab, String keyname, String keyvalue, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select sessionVarName,sessionVarValue from ad_getSessionVarsFromTab(?,?,?)";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, tab);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, keyname);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, keyvalue);
