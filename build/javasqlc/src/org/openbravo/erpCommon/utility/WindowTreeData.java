...
static Logger log4j = Logger.getLogger(WindowTreeData.class);
  ...
  public static WindowTreeData[] selectTrl(ConnectionProvider connectionProvider, String adUserId, String adLanguage, String editable, String parentId, String nodeId, String adTreeId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "                                                      AND tn.Node_ID=tb.Node_ID" +
      "                                                      AND tb.AD_User_ID  = ? , " +
      "             AD_Menu m left join ad_menu_trl t on t.ad_menu_id=m.ad_menu_id and t.ad_language=? left join" +
      "             ad_menu_trl_instance ti on ti.ad_menu_id=m.ad_menu_id and ti.ad_language= ? ," +
      "             AD_Module Mo" +
      "        WHERE ";
    strSql = strSql + ((editable.equals("editable"))?" tn.IsActive='Y' AND m.isActive='Y' AND  ":"");
    strSql = strSql + ((parentId==null || parentId.equals(""))?"":" tn.Parent_ID = ? AND  ");
  ...
  public static WindowTreeData[] selectTreeByID(ConnectionProvider connectionProvider, String treeid)    throws ServletException {
    return selectTreeByID(connectionProvider, treeid, 0, 0);
  }

  public static WindowTreeData[] selectTreeByID(ConnectionProvider connectionProvider, String treeid, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "         SELECT AD_Client_ID as Node_ID, AD_Tree_ID AS ID, NAME" +
      "        FROM AD_Tree " +
      "        WHERE AD_Tree_id = ?        ";
