...
static Logger log4j = Logger.getLogger(WindowAccessData.class);
  ...
  public static String getWindowType(ConnectionProvider connectionProvider, String adWindowId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "         select windowtype from ad_window where ad_window_id = ?";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "windowtype");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
