...
static Logger log4j = Logger.getLogger(PreferencesData.class);
  ...
  public static String initBWA(Connection conn, ConnectionProvider connectionProvider, String bwaHeaderId, String datefrom, String dateto, String adOrgId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "          SELECT zsfi_bwainit(?,to_date(?),to_date(?),?)";
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, bwaHeaderId);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "zsfi_bwainit");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
