...
static Logger log4j = Logger.getLogger(CustomerData.class);
  ...
  public static CustomerData[] select(ConnectionProvider connectionProvider, String OrderId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectCustomerData.phone = UtilSql.getValue(result, "phone");
        objectCustomerData.email = UtilSql.getValue(result, "email");
        objectCustomerData.shippingtype = UtilSql.getValue(result, "shippingtype");
        objectCustomerData.countrycode = UtilSql.getValue(result, "countrycode");
  ...
  public static CustomerData[] selectinvoicedata(ConnectionProvider connectionProvider, String OrderId)    throws ServletException {
    return selectinvoicedata(connectionProvider, OrderId, 0, 0);
  }

  public static CustomerData[] selectinvoicedata(ConnectionProvider connectionProvider, String OrderId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "         select b.value,b.owncodeatpartnersite,l.address1,l.address2,l.postal,l.city,c.name as country,u.firstname,u.lastname,coalesce(bpl.deviant_bp_name,b.name) as bpname," +
      "                         u.phone, u.email,coalesce(b.shippingtype,(case when c.countrycode='DE' then 'V01PAK' else 'V53WPAK' end)) as shippingtype,c.countrycode" +
