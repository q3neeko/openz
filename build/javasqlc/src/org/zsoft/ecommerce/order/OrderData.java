...
static Logger log4j = Logger.getLogger(OrderData.class);
  ...
  public static String ChangeOrder2FullfillmentWarehouse(Connection conn, ConnectionProvider connectionProvider, String cOrderId, String channel, String shopId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "                SELECT zsse_changeOrder2FullfillmentWarehouse(?,?,?) FROM DUAL";
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "zsse_changeorder2fullfillmentwarehouse");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
  ...
  public static String ActivateOrder2Fullfillment(Connection conn, ConnectionProvider connectionProvider, String cOrderId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "                SELECT zsse_ActivateOrder2Fullfillment(?) FROM DUAL";
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "zsse_activateorder2fullfillment");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
