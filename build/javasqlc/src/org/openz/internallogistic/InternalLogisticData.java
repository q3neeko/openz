...
static Logger log4j = Logger.getLogger(InternalLogisticData.class);
  ...
  public static InternalLogisticData[] selectlowerTransfer(ConnectionProvider connectionProvider, String language, String consumption, String locator)    throws ServletException {
    return selectlowerTransfer(connectionProvider, language, consumption, locator, 0, 0);
  }

  public static InternalLogisticData[] selectlowerTransfer(ConnectionProvider connectionProvider, String language, String consumption, String locator, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        Select  coalesce(s.snr_internal_consumptionline_id,l.m_internal_consumptionline_id) as snr_masterdata_id, " +
      "                zssi_getproductname(l.m_product_id,?) as pdcmaterialconsumptionproduct," +
  ...
  public static String ils_createtransfer(ConnectionProvider connectionProvider, String consumption, String targetlocator)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, consumption);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, targetlocator);

      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "ils_createinternaltransfer");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
