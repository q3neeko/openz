 ...
 */
  public static UserAssignmentData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adRoleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_User_Roles.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_User_Roles.AD_Role_ID, " +
      "(CASE WHEN AD_User_Roles.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_User_Roles left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_User_Roles.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Role_ID, Name from AD_Role) table2 on (AD_User_Roles.AD_Role_ID = table2.AD_Role_ID) left join (select AD_User_ID, Name from AD_User) table3 on (AD_User_Roles.AD_User_ID = table3.AD_User_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static UserAssignmentData[] set(String adRoleId, String createdby, String createdbyr, String adUserId, String updatedby, String updatedbyr, String adClientId, String adUserRolesId, String adOrgId, String isactive)    throws ServletException {
