 ...
 */
  public static FormAccessData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adRoleId, String key, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Form_Access.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Form_Access.AD_Role_ID, " +
      "(CASE WHEN AD_Form_Access.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Form_Access left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Form_Access.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Role_ID, Name from AD_Role) table2 on (AD_Form_Access.AD_Role_ID = table2.AD_Role_ID) left join (select AD_Form_ID, Name from AD_Form) table3 on (AD_Form_Access.AD_Form_ID = table3.AD_Form_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static FormAccessData[] set(String adRoleId, String isreadwrite, String adFormId, String createdby, String createdbyr, String adFormAccessId, String adOrgId, String adClientId, String isactive, String updatedby, String updatedbyr)    throws ServletException {
