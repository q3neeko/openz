...
static Logger log4j = Logger.getLogger(RoleData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return themer;
    else if (fieldName.equalsIgnoreCase("targetmain"))
      return targetmain;
    else if (fieldName.equalsIgnoreCase("targetmenu"))
      return targetmenu;
 ...
 */
  public static RoleData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        AD_Role.AD_Org_ID, " +
      "(CASE WHEN AD_Role.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Role.Name, " +
      "AD_Role.Description, " +
      ...
      "(CASE WHEN AD_Role.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "AD_Role.AmtApproval, " +
      "AD_Role.AD_Tree_Menu_ID, " +
      "(CASE WHEN AD_Role.AD_Tree_Menu_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Tree_Menu_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Role left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Role.AD_Org_ID = table1.AD_Org_ID) left join ad_ref_list_v list1 on (AD_Role.UserLevel = list1.value and list1.ad_reference_id = '226' and list1.ad_language = ?)  left join (select C_Currency_ID, ISO_Code from C_Currency) table2 on (AD_Role.C_Currency_ID = table2.C_Currency_ID) left join (select AD_Tree_ID, Name from AD_Tree) table3 on (AD_Role.AD_Tree_Menu_ID =  table3.AD_Tree_ID) left join ad_ref_list_v list2 on (AD_Role.Theme = list2.value and list2.ad_reference_id = '800102' and list2.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectRoleData.targetmain = UtilSql.getValue(result, "targetmain");
        objectRoleData.targetmenu = UtilSql.getValue(result, "targetmenu");
...
Create a registry
 */
  public static RoleData[] set(String userlevel, String ismanual, String createdby, String createdbyr, String targetmenu, String adOrgId, String orglist, String amtapproval, String adClientId, String name, String description, String isactive, String clientlist, String btnGeneraterole, String updatedby, String updatedbyr, String processing, String sessionexpiration, String adRoleId, String cCurrencyId, String adTreeMenuId, String targetmain, String theme, String showinfomenu)    throws ServletException {
    ...
    objectRoleData[0].targetmain = targetmain;
    objectRoleData[0].targetmenu = targetmenu;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE AD_Role" +
      "        SET AD_Org_ID = (?) , Name = (?) , Description = (?) , IsActive = (?) , UserLevel = (?) , Showinfomenu = (?) , C_Currency_ID = (?) , AmtApproval = TO_NUMBER(?) , AD_Tree_Menu_ID = (?) , Sessionexpiration = TO_NUMBER(?) , Theme = (?) , Targetmain = (?) , Targetmenu = (?) , btn_GenerateRole = (?) , Processing = (?) , AD_Role_ID = (?) , ClientList = (?) , IsManual = (?) , OrgList = (?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, targetmain);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, targetmenu);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO AD_Role " +
      "        (AD_Org_ID, Name, Description, IsActive, UserLevel, Showinfomenu, C_Currency_ID, AmtApproval, AD_Tree_Menu_ID, Sessionexpiration, Theme, Targetmain, Targetmenu, btn_GenerateRole, Processing, AD_Role_ID, ClientList, IsManual, OrgList, AD_Client_ID, created, createdby, updated, updatedBy)" +
