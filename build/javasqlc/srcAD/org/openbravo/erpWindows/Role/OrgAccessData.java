 ...
 */
  public static OrgAccessData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adRoleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Role_OrgAccess.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Role_OrgAccess.AD_Role_ID, " +
      "(CASE WHEN AD_Role_OrgAccess.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      "COALESCE(AD_Role_OrgAccess.IsActive, 'N') AS IsActive, " +
      "AD_Role_OrgAccess.AD_Role_OrgAccess_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Role_OrgAccess left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Role_OrgAccess.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Role_ID, Name from AD_Role) table2 on (AD_Role_OrgAccess.AD_Role_ID = table2.AD_Role_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static OrgAccessData[] set(String adRoleId, String adRoleOrgaccessId, String adOrgId, String createdby, String createdbyr, String adClientId, String isactive, String updatedby, String updatedbyr)    throws ServletException {
