 ...
 */
  public static WorkflowAccessData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adRoleId, String key, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Workflow_Access.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Workflow_Access.AD_Role_ID, " +
      "(CASE WHEN AD_Workflow_Access.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Workflow_Access left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Workflow_Access.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Role_ID, Name from AD_Role) table2 on (AD_Workflow_Access.AD_Role_ID = table2.AD_Role_ID) left join (select AD_Workflow_ID, Name from AD_Workflow) table3 on (AD_Workflow_Access.AD_Workflow_ID = table3.AD_Workflow_ID) left join (select AD_Workflow_ID,AD_Language, Name from AD_Workflow_TRL) tableTRL3 on (table3.AD_Workflow_ID = tableTRL3.AD_Workflow_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static WorkflowAccessData[] set(String adRoleId, String adClientId, String adWorkflowId, String adOrgId, String createdby, String createdbyr, String isactive, String isreadwrite, String updatedby, String updatedbyr, String adWorkflowAccessId)    throws ServletException {
