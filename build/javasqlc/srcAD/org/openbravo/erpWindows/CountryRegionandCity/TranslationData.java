 ...
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCountryId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Country_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Country_Trl.C_Country_ID, " +
      "(CASE WHEN C_Country_Trl.C_Country_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_Country_IDR, " +
      "C_Country_Trl.AD_Language, " +
      "(CASE WHEN C_Country_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Country_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Country_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select C_Country_ID, Name from C_Country) table2 on (C_Country_Trl.C_Country_ID = table2.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL2 on (table2.C_Country_ID = tableTRL2.C_Country_ID and tableTRL2.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table3 on (C_Country_Trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TranslationData[] set(String cCountryId, String displaysequence, String description, String regionname, String adOrgId, String adClientId, String cCountryTrlId, String isactive, String updatedby, String updatedbyr, String adLanguage, String name, String istranslated, String createdby, String createdbyr)    throws ServletException {
