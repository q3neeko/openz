 ...
 */
  public static DependencyData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adModuleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Module_Dependency.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Module_Dependency.IsActive, 'N') AS IsActive, " +
      "AD_Module_Dependency.AD_Module_ID, " +
      "(CASE WHEN AD_Module_Dependency.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Module_Dependency.AD_Dependent_Module_ID, " +
      "(CASE WHEN AD_Module_Dependency.AD_Dependent_Module_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Dependent_Module_IDR, " +
      "AD_Module_Dependency.StartVersion, " +
      "AD_Module_Dependency.EndVersion, " +
      "COALESCE(AD_Module_Dependency.IsIncluded, 'N') AS IsIncluded, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Module_Dependency left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Module_Dependency.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Module_Dependency.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join (select AD_Module_ID, Name from AD_Module) table4 on (AD_Module_Dependency.AD_Dependent_Module_ID =  table4.AD_Module_ID)" +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static DependencyData[] set(String adModuleId, String endversion, String adDependentModuleId, String updatedby, String isactive, String startversion, String createdby, String adModuleDependencyId, String adClientId, String adOrgId, String isincluded)    throws ServletException {
