 ...
 */
  public static NamingExceptionsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adModuleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Exceptions.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Exceptions.AD_Module_ID, " +
      "(CASE WHEN AD_Exceptions.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      ...
      "AD_Exceptions.Name2, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Exceptions left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Exceptions.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Exceptions.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join ad_ref_list_v list1 on (AD_Exceptions.Type = list1.value and list1.ad_reference_id = '1722C8B5D1BC4B349332167C3E3A4561' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static NamingExceptionsData[] set(String adModuleId, String adClientId, String type, String adExceptionsId, String name1, String adOrgId, String isactive, String name2, String updatedby, String updatedbyr, String createdby, String createdbyr)    throws ServletException {
