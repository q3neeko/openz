 ...
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String AD_Module_ID, String adModuleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Module_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Module_Trl.IsActive, 'N') AS IsActive, " +
      "AD_Module_Trl.AD_Module_ID, " +
      "(CASE WHEN AD_Module_Trl.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Module_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Module_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Module_Trl.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static TranslationData[] set(String adModuleId, String license, String createdby, String referencedatainfo, String isactive, String adModuleTrlId, String updateinfo, String istranslated, String adLanguage, String description, String adClientId, String adOrgId, String help, String updatedby)    throws ServletException {
