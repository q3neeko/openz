 ...
 */
  public static AlertRuleData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_AlertRule.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_AlertRule.Name, " +
      "COALESCE(AD_AlertRule.IsActive, 'N') AS IsActive, " +
      "AD_AlertRule.Sql, " +
      "AD_AlertRule.FilterClause, " +
      "AD_AlertRule.AD_Tab_ID, " +
      "(CASE WHEN AD_AlertRule.AD_Tab_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS AD_Tab_IDR, " +
...
Create a registry
 */
  public static AlertRuleData[] set(String adTabId, String deactvatewhennotapplied, String isactive, String sql, String adOrgId, String updatedby, String updatedbyr, String adClientId, String createdby, String createdbyr, String filterclause, String fixed, String name, String sendemailtouser, String type, String adAlertruleId)    throws ServletException {
