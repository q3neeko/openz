 ...
 */
  public static AlertRecipientData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adAlertruleId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_AlertRecipient.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_AlertRecipient.AD_AlertRule_ID, " +
      "(CASE WHEN AD_AlertRecipient.AD_AlertRule_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_AlertRule_IDR, " +
      "COALESCE(AD_AlertRecipient.IsActive, 'N') AS IsActive, " +
      "AD_AlertRecipient.AD_Role_ID, " +
      "(CASE WHEN AD_AlertRecipient.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      "AD_AlertRecipient.AD_User_ID, " +
      "(CASE WHEN AD_AlertRecipient.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "COALESCE(AD_AlertRecipient.Sendemail, 'N') AS Sendemail, " +
      "AD_AlertRecipient.AD_Alertrecipient_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_AlertRecipient left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_AlertRecipient.AD_Org_ID = table1.AD_Org_ID) left join (select AD_AlertRule_ID, Name from AD_AlertRule) table2 on (AD_AlertRecipient.AD_AlertRule_ID = table2.AD_AlertRule_ID) left join (select AD_AlertRule_ID,AD_Language, Name from AD_AlertRule_TRL) tableTRL2 on (table2.AD_AlertRule_ID = tableTRL2.AD_AlertRule_ID and tableTRL2.AD_Language = ?)  left join (select AD_Role_ID, Name from AD_Role) table3 on (AD_AlertRecipient.AD_Role_ID = table3.AD_Role_ID) left join (select AD_User_ID, Name from AD_User) table4 on (AD_AlertRecipient.AD_User_ID = table4.AD_User_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static AlertRecipientData[] set(String adAlertruleId, String isactive, String createdby, String createdbyr, String adClientId, String adAlertrecipientId, String adOrgId, String sendemail, String adUserId, String adRoleId, String updatedby, String updatedbyr)    throws ServletException {
