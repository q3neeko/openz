 ...
 */
  public static LocationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Location.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      ...
      "(CASE WHEN C_Location.C_Region_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Region_IDR, " +
      "C_Location.C_Country_ID, " +
      "(CASE WHEN C_Location.C_Country_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_Country_IDR, " +
      "C_Location.Postal_Add, " +
      "C_Location.C_Location_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Location left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Location.AD_Org_ID = table1.AD_Org_ID) left join (select C_Region_ID, Name from C_Region) table2 on (C_Location.C_Region_ID = table2.C_Region_ID) left join (select C_Country_ID, Name from C_Country) table3 on (C_Location.C_Country_ID = table3.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL3 on (table3.C_Country_ID = tableTRL3.C_Country_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
