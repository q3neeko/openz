 ...
 */
  public static DocumentDefinitionData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_DocType.AD_Org_ID, " +
      "(CASE WHEN C_DocType.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_DocType.Name, " +
      ...
      "(CASE WHEN C_DocType.GL_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS GL_Category_IDR, " +
      ...
      "(CASE WHEN C_DocType.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      ...
      "(CASE WHEN C_DocType.C_DocTypeInvoice_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS C_DocTypeInvoice_IDR, " +
      "C_DocType.C_DocTypeShipment_ID, " +
      "(CASE WHEN C_DocType.C_DocTypeShipment_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS C_DocTypeShipment_IDR, " +
      "COALESCE(C_DocType.IsDocNoControlled, 'N') AS IsDocNoControlled, " +
      "C_DocType.DocNoSequence_ID, " +
      "(CASE WHEN C_DocType.DocNoSequence_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS DocNoSequence_IDR, " +
      "C_DocType.C_DocType_ID, " +
      "C_DocType.DocumentCopies, " +
      "        ? AS LANGUAGE " +
      "        FROM C_DocType left join (select AD_Org_ID, Name from AD_Org) table1 on (C_DocType.AD_Org_ID = table1.AD_Org_ID) left join (select GL_Category_ID, Name from GL_Category) table2 on (C_DocType.GL_Category_ID = table2.GL_Category_ID) left join ad_ref_list_v list1 on (C_DocType.DocBaseType = list1.value and list1.ad_reference_id = '183' and list1.ad_language = ?)  left join (select AD_Table_ID, TableName from AD_Table) table3 on (C_DocType.AD_Table_ID = table3.AD_Table_ID) left join ad_ref_list_v list2 on (C_DocType.DocSubTypeSO = list2.value and list2.ad_reference_id = '148' and list2.ad_language = ?)  left join (select C_DocType_ID, Name from C_DocType) table4 on (C_DocType.C_DocTypeInvoice_ID =  table4.C_DocType_ID) left join (select C_DocType_ID,AD_Language, Name from C_DocType_TRL) tableTRL4 on (table4.C_DocType_ID = tableTRL4.C_DocType_ID and tableTRL4.AD_Language = ?)  left join (select C_DocType_ID, Name from C_DocType) table5 on (C_DocType.C_DocTypeShipment_ID =  table5.C_DocType_ID) left join (select C_DocType_ID,AD_Language, Name from C_DocType_TRL) tableTRL5 on (table5.C_DocType_ID = tableTRL5.C_DocType_ID and tableTRL5.AD_Language = ?)  left join (select AD_Sequence_ID, Name from AD_Sequence) table6 on (C_DocType.DocNoSequence_ID =  table6.AD_Sequence_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static DocumentDefinitionData[] set(String cDoctypeId, String glCategoryId, String adTableId, String isdocnocontrolled, String isactive, String createdby, String createdbyr, String adClientId, String description, String documentnote, String docnosequenceId, String adOrgId, String docbasetype, String docsubtypeso, String documentcopies, String cDoctypeshipmentId, String name, String printname, String orgfiltered, String cDoctypeinvoiceId, String isdefault, String updatedby, String updatedbyr, String issotrx, String cDoctypeReversedId)    throws ServletException {
