...
static Logger log4j = Logger.getLogger(PaymentData.class);
  ...
  public String cBpBankaccountIdr;
  ...
  public String getField(String fieldName) {
    ...
    else if (fieldName.equalsIgnoreCase("c_bp_bankaccount_idr") || fieldName.equals("cBpBankaccountIdr"))
      return cBpBankaccountIdr;
 ...
 */
  public static PaymentData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cInvoiceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Debt_Payment.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Debt_Payment.C_Invoice_ID, " +
      "(CASE WHEN C_Debt_Payment.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table3.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "C_Debt_Payment.C_Settlement_Cancel_ID, " +
      "(CASE WHEN C_Debt_Payment.C_Settlement_Cancel_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.DocumentNo), ''))),'') ) END) AS C_Settlement_Cancel_IDR, " +
      ...
      "(CASE WHEN C_Debt_Payment.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Debt_Payment.C_Bp_Bankaccount_ID, " +
      "(CASE WHEN C_Debt_Payment.C_Bp_Bankaccount_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Iban), ''))),'') ) END) AS C_Bp_Bankaccount_IDR, " +
      ...
      "(CASE WHEN C_Debt_Payment.C_BankStatementLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.StmtAmt), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Name), ''))),'') ) END) AS C_BankStatementLine_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Debt_Payment left join (select C_Currency_ID, ISO_Code from C_Currency) table1 on (C_Debt_Payment.C_Currency_ID = table1.C_Currency_ID) left join (select AD_Org_ID, Name from AD_Org) table2 on (C_Debt_Payment.AD_Org_ID = table2.AD_Org_ID) left join (select C_Invoice_ID, DocumentNo, DateInvoiced, GrandTotal from C_Invoice) table3 on (C_Debt_Payment.C_Invoice_ID = table3.C_Invoice_ID) left join (select C_Settlement_ID, DocumentNo from C_Settlement) table4 on (C_Debt_Payment.C_Settlement_Cancel_ID =  table4.C_Settlement_ID) left join ad_ref_list_v list1 on (C_Debt_Payment.PaymentRule = list1.value and list1.ad_reference_id = '195' and list1.ad_language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table5 on (C_Debt_Payment.C_BPartner_ID = table5.C_BPartner_ID) left join (select C_BP_BankAccount_ID, Iban from C_BP_BankAccount) table6 on (C_Debt_Payment.C_Bp_Bankaccount_ID =  table6.C_BP_BankAccount_ID) left join (select C_CashBook_ID, Name from C_CashBook) table7 on (C_Debt_Payment.C_CashBook_ID = table7.C_CashBook_ID) left join (select C_CashLine_ID, C_Cash_ID, Line, Amount, Description from C_CashLine) table8 on (C_Debt_Payment.C_CashLine_ID = table8.C_CashLine_ID) left join (select C_Cash_ID, Name from C_Cash) table9 on (table8.C_Cash_ID = table9.C_Cash_ID) left join (select C_BankStatementLine_ID, Line, StmtAmt, C_BankStatement_ID from C_BankStatementLine) table10 on (C_Debt_Payment.C_BankStatementLine_ID = table10.C_BankStatementLine_ID) left join (select C_BankStatement_ID, Name from C_BankStatement) table11 on (table10.C_BankStatement_ID = table11.C_BankStatement_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectPaymentData.cBpBankaccountId = UtilSql.getValue(result, "c_bp_bankaccount_id");
        objectPaymentData.cBpBankaccountIdr = UtilSql.getValue(result, "c_bp_bankaccount_idr");
...
Create a registry
 */
  public static PaymentData[] set(String cInvoiceId, String cBankstatementlineId, String isreceipt, String cBpBankaccountId, String ispaid, String cBankaccountId, String adClientId, String cancelProcessed, String isdirectposting, String description, String isvalid, String createdby, String createdbyr, String cSettlementCancelId, String dateplanned, String adOrgId, String amount, String isautomaticgenerated, String statusInitial, String cProjectId, String cSettlementGenerateId, String cCurrencyId, String cCashlineId, String isactive, String discountamt, String writeoffamt, String cDebtPaymentId, String created, String isapproved, String cGlitemId, String ismanual, String cInvoiceIdr, String changesettlementcancel, String status, String cCashbookId, String paymentrule, String generateProcessed, String cOrderId, String withholdingamount, String cWithholdingId, String updatedby, String updatedbyr, String cBpartnerId, String cBpartnerIdr, String glitemamt)    throws ServletException {
    ...
    objectPaymentData[0].cBpBankaccountId = cBpBankaccountId;
    objectPaymentData[0].cBpBankaccountIdr = "";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET Amount = TO_NUMBER(?) , C_Currency_ID = (?) , WriteOffAmt = TO_NUMBER(?) , Discountamt = TO_NUMBER(?) , IsPaid = (?) , Isapproved = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , C_Invoice_ID = (?) , C_Settlement_Cancel_ID = (?) , PaymentRule = (?) , Dateplanned = TO_DATE(?) , C_BPartner_ID = (?) , C_Bp_Bankaccount_ID = (?) , Description = (?) , C_CashBook_ID = (?) , C_CashLine_ID = (?) , C_BankStatementLine_ID = (?) , C_Project_ID = (?) , C_Withholding_ID = (?) , GLItemAmt = TO_NUMBER(?) , Cancel_Processed = (?) , Changesettlementcancel = (?) , C_Debt_Payment_ID = (?) , IsActive = (?) , IsReceipt = (?) , C_Settlement_Generate_ID = (?) , C_BankAccount_ID = (?) , IsAutomaticGenerated = (?) , C_Glitem_ID = (?) , C_Order_ID = (?) , Generate_Processed = (?) , Isdirectposting = (?) , Status = (?) , Withholdingamount = TO_NUMBER(?) , IsManual = (?) , IsValid = (?) , Status_Initial = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Debt_Payment.C_Debt_Payment_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Amount, C_Currency_ID, WriteOffAmt, Discountamt, IsPaid, Isapproved, AD_Client_ID, AD_Org_ID, C_Invoice_ID, C_Settlement_Cancel_ID, PaymentRule, Dateplanned, C_BPartner_ID, C_Bp_Bankaccount_ID, Description, C_CashBook_ID, C_CashLine_ID, C_BankStatementLine_ID, Status_Initial, C_Project_ID, C_Withholding_ID, Cancel_Processed, Changesettlementcancel, C_Debt_Payment_ID, IsActive, IsReceipt, C_Settlement_Generate_ID, C_BankAccount_ID, IsAutomaticGenerated, C_Glitem_ID, C_Order_ID, Generate_Processed, GLItemAmt, Isdirectposting, Status, Withholdingamount, IsManual, IsValid, created, createdby, updated, updatedBy)" +
