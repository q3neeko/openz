 ...
 */
  public static OrgSchemaData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String ad_org_id, String adOrgId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Org_AcctSchema.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Org_AcctSchema.Isactive, 'N') AS Isactive, " +
      "AD_Org_AcctSchema.C_Acctschema_ID, " +
      "(CASE WHEN AD_Org_AcctSchema.C_Acctschema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Acctschema_IDR, " +
      "AD_Org_AcctSchema.C_Tax_ID, " +
      "(CASE WHEN AD_Org_AcctSchema.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Org_AcctSchema left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Org_AcctSchema.AD_Org_ID = table1.AD_Org_ID) left join (select C_Acctschema_ID, Name from C_Acctschema) table2 on (AD_Org_AcctSchema.C_Acctschema_ID = table2.C_Acctschema_ID) left join (select C_Tax_ID, Name from C_Tax) table3 on (AD_Org_AcctSchema.C_Tax_ID = table3.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL3 on (table3.C_Tax_ID = tableTRL3.C_Tax_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static OrgSchemaData[] set(String adOrgId, String updatedby, String cTaxId, String cAcctschemaId, String createdby, String isprnewaccount, String isactive, String adSequencePrId, String adOrgAcctschemaId, String subacctLength, String acctLength, String adClientId, String adSequenceBpId, String isbpnewaccount)    throws ServletException {
