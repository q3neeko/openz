 ...
 */
  public static ColumnData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adTableId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN ad_column_v.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "ad_column_v.AD_Table_ID, " +
      "(CASE WHEN ad_column_v.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "ad_column_v.AD_Module_ID, " +
      "(CASE WHEN ad_column_v.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "ad_column_v.Columnname, " +
      "ad_column_v.AD_Element_ID, " +
      "(CASE WHEN ad_column_v.AD_Element_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.ColumnName), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_Element_IDR, " +
      "ad_column_v.Description, " +
      "ad_column_v.Help, " +
      "COALESCE(ad_column_v.Isactive, 'N') AS Isactive, " +
      "ad_column_v.Fieldlength, " +
      "ad_column_v.AD_Reference_ID, " +
      "(CASE WHEN ad_column_v.AD_Reference_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS AD_Reference_IDR, " +
      "ad_column_v.AD_Val_Rule_ID, " +
      "(CASE WHEN ad_column_v.AD_Val_Rule_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS AD_Val_Rule_IDR, " +
      "ad_column_v.AD_Process_ID, " +
      "(CASE WHEN ad_column_v.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL8.Name IS NULL THEN TO_CHAR(table8.Name) ELSE TO_CHAR(tableTRL8.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      "ad_column_v.AD_Reference_Value_ID, " +
      "(CASE WHEN ad_column_v.AD_Reference_Value_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL9.Name IS NULL THEN TO_CHAR(table9.Name) ELSE TO_CHAR(tableTRL9.Name) END)), ''))),'') ) END) AS AD_Reference_Value_IDR, " +
      ...
      "(CASE WHEN ad_column_v.AD_Callout_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'') ) END) AS AD_Callout_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM ad_column_v left join (select AD_Org_ID, Name from AD_Org) table1 on (ad_column_v.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Table_ID, TableName from AD_Table) table2 on (ad_column_v.AD_Table_ID = table2.AD_Table_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table3 on (ad_column_v.AD_Module_ID = table3.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table4 on (table3.AD_Language =  table4.AD_Language) left join (select AD_Element_ID, ColumnName, Name from AD_Element) table5 on (ad_column_v.AD_Element_ID = table5.AD_Element_ID) left join (select AD_Reference_ID, Name from AD_Reference) table6 on (ad_column_v.AD_Reference_ID =  table6.AD_Reference_ID) left join (select AD_Reference_ID,AD_Language, Name from AD_Reference_TRL) tableTRL6 on (table6.AD_Reference_ID = tableTRL6.AD_Reference_ID and tableTRL6.AD_Language = ?)  left join (select AD_Val_Rule_ID, Name from AD_Val_Rule) table7 on (ad_column_v.AD_Val_Rule_ID = table7.AD_Val_Rule_ID) left join (select AD_Process_ID, Name from AD_Process) table8 on (ad_column_v.AD_Process_ID = table8.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL8 on (table8.AD_Process_ID = tableTRL8.AD_Process_ID and tableTRL8.AD_Language = ?)  left join (select AD_Reference_ID, Name from AD_Reference) table9 on (ad_column_v.AD_Reference_Value_ID =  table9.AD_Reference_ID) left join (select AD_Reference_ID,AD_Language, Name from AD_Reference_TRL) tableTRL9 on (table9.AD_Reference_ID = tableTRL9.AD_Reference_ID and tableTRL9.AD_Language = ?)  left join (select AD_Callout_ID, Name from AD_Callout) table10 on (ad_column_v.AD_Callout_ID = table10.AD_Callout_ID) left join ad_ref_list_v list1 on (ad_column_v.Developmentstatus = list1.value and list1.ad_reference_id = '800103' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ColumnData[] set(String adTableId, String help, String createdby, String createdbyr, String columnname, String isactive, String developmentstatus, String fieldlength, String readonlylogic, String adReferenceValueId, String iskey, String istranslated, String adProcessId, String isparent, String updatedby, String updatedbyr, String description, String adModuleId, String adClientId, String isencrypted, String isidentifier, String adCalloutId, String adReferenceId, String adValRuleId, String seqno, String issessionattr, String defaultvalue, String adElementId, String adColumnVId, String adOrgId)    throws ServletException {
