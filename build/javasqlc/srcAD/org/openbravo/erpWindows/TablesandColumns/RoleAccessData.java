 ...
 */
  public static RoleAccessData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adTableId, String key, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Table_Access.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Table_Access.AD_Table_ID, " +
      "(CASE WHEN AD_Table_Access.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "AD_Table_Access.AD_Role_ID, " +
      "(CASE WHEN AD_Table_Access.AD_Role_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Role_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Table_Access left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Table_Access.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Table_ID, TableName from AD_Table) table2 on (AD_Table_Access.AD_Table_ID = table2.AD_Table_ID) left join (select AD_Role_ID, Name from AD_Role) table3 on (AD_Table_Access.AD_Role_ID = table3.AD_Role_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static RoleAccessData[] set(String adTableId, String isreadonly, String adTableAccessId, String isactive, String createdby, String createdbyr, String isexclude, String adClientId, String adRoleId, String updatedby, String updatedbyr, String adOrgId)    throws ServletException {
