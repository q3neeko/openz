 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        C_Cash.AD_Client_ID, " +
      "C_Cash.AD_Org_ID, " +
      "(CASE WHEN C_Cash.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Cash.C_CashBook_ID, " +
      "(CASE WHEN C_Cash.C_CashBook_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_CashBook_IDR, " +
      "C_Cash.Name, " +
      "C_Cash.Description, " +
      "C_Cash.StatementDate, " +
      "C_Cash.DateAcct, " +
      "C_Cash.C_Project_ID, " +
      "(CASE WHEN C_Cash.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_Cash.C_Activity_ID, " +
      "(CASE WHEN C_Cash.C_Activity_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Activity_IDR, " +
      "C_Cash.C_Campaign_ID, " +
      "(CASE WHEN C_Cash.C_Campaign_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_Campaign_IDR, " +
      "C_Cash.AD_OrgTrx_ID, " +
      "C_Cash.User1_ID, " +
      "C_Cash.User2_ID, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Cash left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Cash.AD_Org_ID = table1.AD_Org_ID) left join (select C_CashBook_ID, Name from C_CashBook) table2 on (C_Cash.C_CashBook_ID = table2.C_CashBook_ID) left join (select C_Project_ID, Value, Name from C_Project) table3 on (C_Cash.C_Project_ID = table3.C_Project_ID) left join (select C_Activity_ID, Name from C_Activity) table4 on (C_Cash.C_Activity_ID = table4.C_Activity_ID) left join (select C_Campaign_ID, Name from C_Campaign) table5 on (C_Cash.C_Campaign_ID = table5.C_Campaign_ID) left join ad_ref_list_v list1 on (list1.ad_reference_id = '800024' and list1.ad_language = ?  AND C_Cash.Processed = TO_CHAR(list1.value)) left join ad_ref_list_v list2 on (list2.ad_reference_id = '234' and list2.ad_language = ?  AND C_Cash.Posted = TO_CHAR(list2.value))" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static HeaderData[] set(String endingbalance, String beginningbalance, String adOrgtrxId, String cActivityId, String cProjectId, String cCampaignId, String cCashId, String name, String statementdifference, String user2Id, String adClientId, String processing, String dateacct, String posted, String postedBtn, String processed, String processedBtn, String statementdate, String adOrgId, String cCashbookId, String isactive, String createdby, String createdbyr, String description, String updatedby, String updatedbyr, String user1Id)    throws ServletException {
