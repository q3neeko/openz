 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCashId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_CashLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_CashLine.C_Cash_ID, " +
      "(CASE WHEN C_CashLine.C_Cash_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Cash_IDR, " +
      "C_CashLine.Line, " +
      ...
      "(CASE WHEN C_CashLine.C_Glitem_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_Glitem_IDR, " +
      "C_CashLine.Amount, " +
      "C_CashLine.C_Currency_ID, " +
      "(CASE WHEN C_CashLine.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "C_CashLine.C_Charge_ID, " +
      "C_CashLine.C_Debt_Payment_ID, " +
      "(CASE WHEN C_CashLine.C_Debt_Payment_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(list2.name),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table5.Dateplanned, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Amount), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.IsReceipt), ''))),'') ) END) AS C_Debt_Payment_IDR, " +
      "C_CashLine.DiscountAmt, " +
      "C_CashLine.WriteOffAmt, " +
      "C_CashLine.C_Order_ID, " +
      "(CASE WHEN C_CashLine.C_Order_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table7.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.GrandTotal), ''))),'') ) END) AS C_Order_IDR, " +
      "C_CashLine.PayInAdvance, " +
      "C_CashLine.C_CashLine_ID, " +
      "COALESCE(C_CashLine.IsActive, 'N') AS IsActive, " +
      "        ? AS LANGUAGE " +
      "        FROM C_CashLine left join (select AD_Org_ID, Name from AD_Org) table1 on (C_CashLine.AD_Org_ID = table1.AD_Org_ID) left join (select C_Cash_ID, Name from C_Cash) table2 on (C_CashLine.C_Cash_ID = table2.C_Cash_ID) left join ad_ref_list_v list1 on (C_CashLine.CashType = list1.value and list1.ad_reference_id = '217' and list1.ad_language = ?)  left join (select C_Glitem_ID, Name from C_Glitem) table3 on (C_CashLine.C_Glitem_ID = table3.C_Glitem_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table4 on (C_CashLine.C_Currency_ID = table4.C_Currency_ID) left join (select C_Debt_Payment_ID, C_BPartner_ID, PaymentRule, Dateplanned, Amount, IsReceipt from C_Debt_Payment) table5 on (C_CashLine.C_Debt_Payment_ID = table5.C_Debt_Payment_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table6 on (table5.C_BPartner_ID = table6.C_BPartner_ID) left join ad_ref_list_v list2 on (table5.PaymentRule = list2.value and list2.ad_reference_id = '195' and list2.ad_language = ?)  left join (select C_Order_ID, DocumentNo, Name, DateOrdered, GrandTotal from C_Order) table7 on (C_CashLine.C_Order_ID = table7.C_Order_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String cCashId, String cCashlineId, String adOrgId, String updatedby, String updatedbyr, String isgenerated, String writeoffamt, String cCurrencyId, String cChargeId, String discountamt, String description, String cDebtPaymentId, String cDebtPaymentIdr, String payinadvance, String created, String isactive, String createdby, String createdbyr, String cGlitemId, String adClientId, String amount, String cashtype, String line, String cOrderId, String cOrderIdr)    throws ServletException {
