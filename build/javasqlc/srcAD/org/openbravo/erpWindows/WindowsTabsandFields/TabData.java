...
static Logger log4j = Logger.getLogger(TabData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return issorttab;
    else if (fieldName.equalsIgnoreCase("ismaxrowsparam"))
      return ismaxrowsparam;
    else if (fieldName.equalsIgnoreCase("maxrowsparam"))
      return maxrowsparam;
 ...
 */
  public static TabData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adWindowId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        AD_Tab.AD_Org_ID, " +
      "(CASE WHEN AD_Tab.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Tab.AD_Window_ID, " +
      "(CASE WHEN AD_Tab.AD_Window_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Window_IDR, " +
      "AD_Tab.AD_Module_ID, " +
      "(CASE WHEN AD_Tab.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Tab.Name, " +
      "AD_Tab.Description, " +
      "AD_Tab.Help, " +
      "COALESCE(AD_Tab.IsActive, 'N') AS IsActive, " +
      "AD_Tab.AD_Table_ID, " +
      "(CASE WHEN AD_Tab.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "AD_Tab.SeqNo, " +
      "AD_Tab.TabLevel, " +
      ...
      "COALESCE(AD_Tab.Ismaxrowsparam, 'N') AS Ismaxrowsparam, " +
      "AD_Tab.Maxrowsparam, " +
      "AD_Tab.AD_ColumnSortOrder_ID, " +
      "(CASE WHEN AD_Tab.AD_ColumnSortOrder_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.ColumnName), ''))),'') ) END) AS AD_ColumnSortOrder_IDR, " +
      "AD_Tab.AD_ColumnSortYesNo_ID, " +
      "(CASE WHEN AD_Tab.AD_ColumnSortYesNo_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.ColumnName), ''))),'') ) END) AS AD_ColumnSortYesNo_IDR, " +
      "AD_Tab.AD_Column_ID, " +
      "(CASE WHEN AD_Tab.AD_Column_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.ColumnName), ''))),'') ) END) AS AD_Column_IDR, " +
      "AD_Tab.AD_Process_ID, " +
      "(CASE WHEN AD_Tab.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL9.Name IS NULL THEN TO_CHAR(table9.Name) ELSE TO_CHAR(tableTRL9.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      ...
      "(CASE WHEN AD_Tab.AD_Relationtoolbar IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Value), ''))),'') ) END) AS AD_RelationtoolbarR, " +
      "AD_Tab.AD_Editiontoolbar, " +
      "(CASE WHEN AD_Tab.AD_Editiontoolbar IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Value), ''))),'') ) END) AS AD_EditiontoolbarR, " +
      ...
      "AD_Tab.AD_Image_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Tab left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Tab.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Window_ID, Name from AD_Window) table2 on (AD_Tab.AD_Window_ID = table2.AD_Window_ID) left join (select AD_Window_ID,AD_Language, Name from AD_Window_TRL) tableTRL2 on (table2.AD_Window_ID = tableTRL2.AD_Window_ID and tableTRL2.AD_Language = ?)  left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table3 on (AD_Tab.AD_Module_ID = table3.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table4 on (table3.AD_Language =  table4.AD_Language) left join (select AD_Table_ID, TableName from AD_Table) table5 on (AD_Tab.AD_Table_ID = table5.AD_Table_ID) left join ad_ref_list_v list1 on (AD_Tab.UIPattern = list1.value and list1.ad_reference_id = 'D15C950D445D408E8CC8135E530C246B' and list1.ad_language = ?)  left join (select AD_Column_ID, ColumnName from AD_Column) table6 on (AD_Tab.AD_ColumnSortOrder_ID =  table6.AD_Column_ID) left join (select AD_Column_ID, ColumnName from AD_Column) table7 on (AD_Tab.AD_ColumnSortYesNo_ID =  table7.AD_Column_ID) left join (select AD_Column_ID, ColumnName from AD_Column) table8 on (AD_Tab.AD_Column_ID = table8.AD_Column_ID) left join (select AD_Process_ID, Name from AD_Process) table9 on (AD_Tab.AD_Process_ID = table9.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL9 on (table9.AD_Process_ID = tableTRL9.AD_Process_ID and tableTRL9.AD_Language = ?)  left join ad_ref_list_v list2 on (AD_Tab.Lefttabsmode = list2.value and list2.ad_reference_id = 'EBB90F873C384B6C83574321FB08D8B6' and list2.ad_language = ?)  left join (select AD_Toolbar_ID, Value from ad_toolbar) table10 on (AD_Tab.AD_Relationtoolbar =  table10.AD_Toolbar_ID) left join (select AD_Toolbar_ID, Value from ad_toolbar) table11 on (AD_Tab.AD_Editiontoolbar =  table11.AD_Toolbar_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectTabData.ismaxrowsparam = UtilSql.getValue(result, "ismaxrowsparam");
        objectTabData.maxrowsparam = UtilSql.getValue(result, "maxrowsparam");
...
Create a registry
 */
  public static TabData[] set(String adWindowId, String adTableId, String uipattern, String isinfotab, String isreadonly, String lefttabsmode, String updatedby, String updatedbyr, String adImageId, String seqno, String importfields, String maxrowsparam, String issorttab, String name, String editionmanual, String adRelationtoolbar, String adProcessId, String description, String adModuleId, String hastree, String whereclause, String adColumnId, String manualservletclass, String isstandalone, String ismaxrowsparam, String relationmanual, String commitwarning, String adColumnsortorderId, String help, String orderbyclause, String adColumnsortyesnoId, String createcustom, String dropcustom, String tablevel, String editreference, String adOrgId, String issinglerow, String adTabId, String adClientId, String includedTabId, String isactive, String filterclause, String istranslationtab, String adEditiontoolbar, String processing, String createdby, String createdbyr)    throws ServletException {
    ...
    objectTabData[0].ismaxrowsparam = ismaxrowsparam;
    objectTabData[0].maxrowsparam = maxrowsparam;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE AD_Tab" +
      "        SET AD_Org_ID = (?) , AD_Window_ID = (?) , AD_Module_ID = (?) , Name = (?) , Description = (?) , Help = (?) , IsActive = (?) , AD_Table_ID = (?) , SeqNo = TO_NUMBER(?) , TabLevel = TO_NUMBER(?) , IsSingleRow = (?) , UIPattern = (?) , HasTree = (?) , IsInfoTab = (?) , IsTranslationTab = (?) , IsSortTab = (?) , Ismaxrowsparam = (?) , Maxrowsparam = TO_NUMBER(?) , AD_ColumnSortOrder_ID = (?) , AD_ColumnSortYesNo_ID = (?) , AD_Column_ID = (?) , AD_Process_ID = (?) , CommitWarning = (?) , WhereClause = (?) , FilterClause = (?) , OrderByClause = (?) , Lefttabsmode = (?) , AD_Relationtoolbar = (?) , AD_Editiontoolbar = (?) , Manualservletclass = (?) , Relationmanual = (?) , Editionmanual = (?) , Isstandalone = (?) , ImportFields = (?) , Processing = (?) , Createcustom = (?) , Dropcustom = (?) , EditReference = (?) , Included_Tab_ID = (?) , AD_Tab_ID = (?) , AD_Client_ID = (?) , AD_Image_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE AD_Tab.AD_Tab_ID = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, ismaxrowsparam);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, maxrowsparam);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO AD_Tab " +
      "        (AD_Org_ID, AD_Window_ID, AD_Module_ID, Name, Description, Help, IsActive, AD_Table_ID, SeqNo, TabLevel, IsSingleRow, UIPattern, HasTree, IsInfoTab, IsTranslationTab, IsSortTab, Ismaxrowsparam, Maxrowsparam, AD_ColumnSortOrder_ID, AD_ColumnSortYesNo_ID, AD_Column_ID, AD_Process_ID, CommitWarning, WhereClause, FilterClause, OrderByClause, Lefttabsmode, AD_Relationtoolbar, AD_Editiontoolbar, Manualservletclass, Relationmanual, Editionmanual, Isstandalone, ImportFields, Processing, Createcustom, Dropcustom, EditReference, Included_Tab_ID, AD_Tab_ID, AD_Client_ID, AD_Image_ID, created, createdby, updated, updatedBy)" +
