 ...
 */
  public static WindowData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "AD_Window.AD_Org_ID, " +
      "(CASE WHEN AD_Window.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Window.AD_Module_ID, " +
      "(CASE WHEN AD_Window.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Window.Name, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Window left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Window.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Window.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join ad_ref_list_v list1 on (AD_Window.WindowType = list1.value and list1.ad_reference_id = '108' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static WindowData[] set(String adOrgId, String isactive, String createdby, String createdbyr, String help, String adClientId, String isdefault, String adWindowId, String windowtype, String description, String name, String processing, String issotrx, String updatedby, String updatedbyr, String adImageId, String adModuleId)    throws ServletException {
