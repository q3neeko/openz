 ...
 */
  public static AccountingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductCategoryId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product_Category_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_Product_Category_Acct.M_Product_Category_ID, " +
      "(CASE WHEN M_Product_Category_Acct.M_Product_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_Product_Category_IDR, " +
      "M_Product_Category_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN M_Product_Category_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "COALESCE(M_Product_Category_Acct.IsActive, 'N') AS IsActive, " +
      "M_Product_Category_Acct.P_Asset_Acct, " +
      "(CASE WHEN M_Product_Category_Acct.P_Asset_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS P_Asset_AcctR, " +
      "M_Product_Category_Acct.P_Expense_Acct, " +
      "(CASE WHEN M_Product_Category_Acct.P_Expense_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Combination), ''))),'') ) END) AS P_Expense_AcctR, " +
      "M_Product_Category_Acct.P_Revenue_Acct, " +
      "(CASE WHEN M_Product_Category_Acct.P_Revenue_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Combination), ''))),'') ) END) AS P_Revenue_AcctR, " +
      "M_Product_Category_Acct.P_Cogs_Acct, " +
      "(CASE WHEN M_Product_Category_Acct.P_Cogs_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Combination), ''))),'') ) END) AS P_Cogs_AcctR, " +
      "M_Product_Category_Acct.P_PurchasePriceVariance_Acct, " +
      "M_Product_Category_Acct.P_InvoicePriceVariance_Acct, " +
      "(CASE WHEN M_Product_Category_Acct.P_InvoicePriceVariance_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Combination), ''))),'') ) END) AS P_InvoicePriceVariance_AcctR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Product_Category_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Product_Category_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_Category_ID, Name from M_Product_Category) table2 on (M_Product_Category_Acct.M_Product_Category_ID = table2.M_Product_Category_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (M_Product_Category_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (M_Product_Category_Acct.P_Asset_Acct = table4.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table5 on (M_Product_Category_Acct.P_Expense_Acct = table5.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table6 on (M_Product_Category_Acct.P_Revenue_Acct = table6.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table7 on (M_Product_Category_Acct.P_Cogs_Acct = table7.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table8 on (M_Product_Category_Acct.P_InvoicePriceVariance_Acct = table8.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
