 ...
 */
  public static AssignedProductsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String M_Product_Category_ID, String mProductCategoryId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Product left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Product.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_Category_ID, Name from M_Product_Category) table2 on (M_Product.M_Product_Category_ID = table2.M_Product_Category_ID) left join ad_ref_list_v list1 on (M_Product.ProductType = list1.value and list1.ad_reference_id = '270' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Product" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , M_Product_Category_ID = (?) , Value = (?) , Name = (?) , IsActive = (?) , IsSummary = (?) , UPC = (?) , Discontinued = (?) , Description = (?) , ProductType = (?) , DocumentNote = (?) , IsStocked = (?) , S_ExpenseType_ID = (?) , IsPurchased = (?) , IsSold = (?) , S_Resource_ID = (?) , Processing = (?) , ImageURL = (?) , DescriptionURL = (?) , VersionNo = (?) , GuaranteeDays = TO_NUMBER(?) , M_AttributeSet_ID = (?) , M_AttributeSetInstance_ID = (?) , DownloadURL = (?) , M_FreightCategory_ID = (?) , Classification = (?) , M_Locator_ID = (?) , M_Product_ID = (?) , C_UOM_ID = (?) , Volume = TO_NUMBER(?) , Weight = TO_NUMBER(?) , SKU = (?) , ShelfWidth = TO_NUMBER(?) , ShelfHeight = TO_NUMBER(?) , ShelfDepth = TO_NUMBER(?) , UnitsPerPallet = TO_NUMBER(?) , DiscontinuedBy = TO_DATE(?) , Help = (?) , SalesRep_ID = (?) , IsBOM = (?) , IsInvoicePrintDetails = (?) , IsPickListPrintDetails = (?) , isVerified = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_Product.M_Product_ID = ? " +
      "                 AND M_Product.M_Product_Category_ID = ? " +
      "        AND M_Product.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, M_Product_Category_ID, Value, Name, IsActive, IsSummary, UPC, Discontinued, Description, ProductType, DocumentNote, IsStocked, S_ExpenseType_ID, IsPurchased, IsSold, S_Resource_ID, Processing, ImageURL, DescriptionURL, VersionNo, GuaranteeDays, M_AttributeSet_ID, M_AttributeSetInstance_ID, DownloadURL, M_FreightCategory_ID, Classification, M_Locator_ID, M_Product_ID, C_UOM_ID, Volume, Weight, SKU, ShelfWidth, ShelfHeight, ShelfDepth, UnitsPerPallet, DiscontinuedBy, Help, SalesRep_ID, IsBOM, IsInvoicePrintDetails, IsPickListPrintDetails, isVerified, created, createdby, updated, updatedBy)" +
