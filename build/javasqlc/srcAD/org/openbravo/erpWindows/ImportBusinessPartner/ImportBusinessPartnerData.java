 ...
 */
  public static ImportBusinessPartnerData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN I_BPartner.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "I_BPartner.I_ErrorMsg, " +
      "I_BPartner.AD_Client_ID, " +
      "I_BPartner.AD_Org_ID, " +
      "(CASE WHEN I_BPartner.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "I_BPartner.Value, " +
      "I_BPartner.Name, " +
      "I_BPartner.Name2, " +
      ...
      "(CASE WHEN I_BPartner.C_BP_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BP_Group_IDR, " +
      "I_BPartner.C_BPartner_Location_ID, " +
      "(CASE WHEN I_BPartner.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "I_BPartner.Address1, " +
      "I_BPartner.Address2, " +
      "I_BPartner.Postal, " +
      ...
      "(CASE WHEN I_BPartner.C_Region_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_Region_IDR, " +
      "I_BPartner.CountryCode, " +
      "I_BPartner.C_Country_ID, " +
      "(CASE WHEN I_BPartner.C_Country_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_Country_IDR, " +
      "COALESCE(I_BPartner.Isheadquarter, 'N') AS Isheadquarter, " +
      "I_BPartner.Taxataddress, " +
      "I_BPartner.AD_User_ID, " +
      "(CASE WHEN I_BPartner.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "I_BPartner.Title, " +
      ...
      "(CASE WHEN I_BPartner.C_Greeting_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL8.Name IS NULL THEN TO_CHAR(table8.Name) ELSE TO_CHAR(tableTRL8.Name) END)), ''))),'') ) END) AS C_Greeting_IDR, " +
      "I_BPartner.Processing, " +
      "COALESCE(I_BPartner.Processed, 'N') AS Processed, " +
      "        ? AS LANGUAGE " +
      "        FROM I_BPartner left join (select C_BPartner_ID, Value, Name from C_BPartner) table1 on (I_BPartner.C_BPartner_ID = table1.C_BPartner_ID) left join (select AD_Org_ID, Name from AD_Org) table2 on (I_BPartner.AD_Org_ID = table2.AD_Org_ID) left join (select C_BP_Group_ID, Name from C_BP_Group) table3 on (I_BPartner.C_BP_Group_ID = table3.C_BP_Group_ID) left join (select C_BPartner_Location_ID, Name, Deviant_Bp_Name from C_BPartner_Location) table4 on (I_BPartner.C_BPartner_Location_ID = table4.C_BPartner_Location_ID) left join (select C_Region_ID, Name from C_Region) table5 on (I_BPartner.C_Region_ID = table5.C_Region_ID) left join (select C_Country_ID, Name from C_Country) table6 on (I_BPartner.C_Country_ID = table6.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL6 on (table6.C_Country_ID = tableTRL6.C_Country_ID and tableTRL6.AD_Language = ?)  left join (select AD_User_ID, Name from AD_User) table7 on (I_BPartner.AD_User_ID = table7.AD_User_ID) left join (select C_Greeting_ID, Name from C_Greeting) table8 on (I_BPartner.C_Greeting_ID = table8.C_Greeting_ID) left join (select C_Greeting_ID,AD_Language, Name from C_Greeting_TRL) tableTRL8 on (table8.C_Greeting_ID = tableTRL8.C_Greeting_ID and tableTRL8.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
