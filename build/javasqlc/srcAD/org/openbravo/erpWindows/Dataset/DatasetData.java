 ...
 */
  public static DatasetData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Dataset.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Dataset.IsActive, 'N') AS IsActive, " +
      "AD_Dataset.AD_Module_ID, " +
      "(CASE WHEN AD_Dataset.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Dataset.Value, " +
      "AD_Dataset.Name, " +
      ...
      "AD_Dataset.Export, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Dataset left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Dataset.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Dataset.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join ad_ref_list_v list1 on (AD_Dataset.Accesslevel = list1.value and list1.ad_reference_id = '5' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static DatasetData[] set(String exportallowed, String adDatasetId, String value, String description, String name, String export, String adModuleId, String isactive, String createdby, String updatedby, String accesslevel, String adOrgId, String adClientId)    throws ServletException {
