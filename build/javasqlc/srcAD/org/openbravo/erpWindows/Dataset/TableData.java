 ...
 */
  public static TableData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adDatasetId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Dataset_Table.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Dataset_Table.AD_Dataset_ID, " +
      "(CASE WHEN AD_Dataset_Table.AD_Dataset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Dataset_IDR, " +
      "AD_Dataset_Table.AD_Table_ID, " +
      "(CASE WHEN AD_Dataset_Table.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "COALESCE(AD_Dataset_Table.IsActive, 'N') AS IsActive, " +
      "AD_Dataset_Table.AD_Module_ID, " +
      "(CASE WHEN AD_Dataset_Table.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Dataset_Table left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Dataset_Table.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Dataset_ID, Name from AD_Dataset) table2 on (AD_Dataset_Table.AD_Dataset_ID = table2.AD_Dataset_ID) left join (select AD_Table_ID, TableName from AD_Table) table3 on (AD_Dataset_Table.AD_Table_ID = table3.AD_Table_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table4 on (AD_Dataset_Table.AD_Module_ID = table4.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table5 on (table4.AD_Language =  table5.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TableData[] set(String adDatasetId, String createdby, String adOrgId, String updatedby, String adTableId, String adDatasetTableId, String adClientId, String isbusinessobject, String includeallcolumns, String adModuleId, String excludeauditinfo, String isactive, String whereclause)    throws ServletException {
