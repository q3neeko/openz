 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCommissionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_CommissionLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_CommissionLine.C_Commission_ID, " +
      "(CASE WHEN C_CommissionLine.C_Commission_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Commission_IDR, " +
      ...
      "(CASE WHEN C_CommissionLine.C_BP_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BP_Group_IDR, " +
      "C_CommissionLine.C_BPartner_ID, " +
      "(CASE WHEN C_CommissionLine.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_CommissionLine.M_Product_Category_ID, " +
      "(CASE WHEN C_CommissionLine.M_Product_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS M_Product_Category_IDR, " +
      "C_CommissionLine.M_Product_ID, " +
      "(CASE WHEN C_CommissionLine.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "C_CommissionLine.C_SalesRegion_ID, " +
      "(CASE WHEN C_CommissionLine.C_SalesRegion_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_SalesRegion_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_CommissionLine left join (select AD_Org_ID, Name from AD_Org) table1 on (C_CommissionLine.AD_Org_ID = table1.AD_Org_ID) left join (select C_Commission_ID, Name from C_Commission) table2 on (C_CommissionLine.C_Commission_ID = table2.C_Commission_ID) left join (select C_BP_Group_ID, Name from C_BP_Group) table3 on (C_CommissionLine.C_BP_Group_ID = table3.C_BP_Group_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table4 on (C_CommissionLine.C_BPartner_ID = table4.C_BPartner_ID) left join (select M_Product_Category_ID, Name from M_Product_Category) table5 on (C_CommissionLine.M_Product_Category_ID = table5.M_Product_Category_ID) left join (select M_Product_ID, Value, Name from M_Product) table6 on (C_CommissionLine.M_Product_ID = table6.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL6 on (table6.M_Product_ID = tableTRL6.M_Product_ID and tableTRL6.AD_Language = ?)  left join (select C_SalesRegion_ID, Name from C_SalesRegion) table7 on (C_CommissionLine.C_SalesRegion_ID = table7.C_SalesRegion_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String cCommissionId, String updatedby, String updatedbyr, String line, String orgId, String cSalesregionId, String qtymultiplier, String adClientId, String amtmultiplier, String cBpartnerId, String cBpartnerIdr, String iscommissioninprice, String mProductCategoryId, String ispositiveonly, String commissionorders, String description, String salesvolumefrom, String createdby, String createdbyr, String isshareofturnover, String mProductId, String mProductIdr, String adOrgId, String convertedturnover, String cCommissionlineId, String isactive, String amtsubtract, String cBpGroupId, String qtysubtract)    throws ServletException {
