...
static Logger log4j = Logger.getLogger(HeaderData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return dateordered;
    else if (fieldName.equalsIgnoreCase("issent"))
      return issent;
      ...
      return issotrx;
    else if (fieldName.equalsIgnoreCase("discountcouponcode"))
 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_Order.POReference, " +
      "C_Order.DateOrdered, " +
      "COALESCE(C_Order.Issent, 'N') AS Issent, " +
      ...
      "(CASE WHEN C_Order.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Order.bpzipcode, " +
      "C_Order.BillTo_ID, " +
      ...
      "(CASE WHEN C_Order.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "C_Order.AD_User_ID, " +
      "(CASE WHEN C_Order.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "C_Order.SalesRep_ID, " +
      "(CASE WHEN C_Order.SalesRep_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS SalesRep_IDR, " +
      ...
      "(CASE WHEN C_Order.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      ...
      "(CASE WHEN C_Order.C_PaymentTerm_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL12.Name IS NULL THEN TO_CHAR(table12.Name) ELSE TO_CHAR(tableTRL12.Name) END)), ''))),'') ) END) AS C_PaymentTerm_IDR, " +
      "C_Order.C_Incoterms_ID, " +
      "(CASE WHEN C_Order.C_Incoterms_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table13.Name), ''))),'') ) END) AS C_Incoterms_IDR, " +
      "C_Order.Delivery_Location_ID, " +
      "(CASE WHEN C_Order.Delivery_Location_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table14.Name), ''))),'') ) END) AS Delivery_Location_IDR, " +
      ...
      "(CASE WHEN C_Order.M_Shipper_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table15.Name), ''))),'') ) END) AS M_Shipper_IDR, " +
      "C_Order.Weight, " +
      "C_Order.Weight_Uom, " +
      "(CASE WHEN C_Order.Weight_Uom IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL16.Name IS NULL THEN TO_CHAR(table16.Name) ELSE TO_CHAR(tableTRL16.Name) END)), ''))),'') ) END) AS Weight_UomR, " +
      ...
      "(CASE WHEN C_Order.M_Product_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL17.Name IS NULL THEN TO_CHAR(table17.Name) ELSE TO_CHAR(tableTRL17.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "C_Order.Qty, " +
      "C_Order.FreightAmt, " +
      "C_Order.C_Project_ID, " +
      "(CASE WHEN C_Order.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_Order.C_Projecttask_ID, " +
      "(CASE WHEN C_Order.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table20.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table20.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table19.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "C_Order.Orderselfjoin, " +
      "(CASE WHEN C_Order.Orderselfjoin IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table21.DocumentNo), ''))),'') ) END) AS OrderselfjoinR, " +
      "C_Order.Firstschedinvoicedate, " +
      "C_Order.DatePromised, " +
      "C_Order.SchedDeliveryDate, " +
      "C_Order.SchedTransactionDate, " +
      "COALESCE(C_Order.Isapproved, 'N') AS Isapproved, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectHeaderData.issent = UtilSql.getValue(result, "issent");
        ...
        objectHeaderData.deliverylocationtext = UtilSql.getValue(result, "deliverylocationtext");
        ...
        objectHeaderData.discountcouponcode = UtilSql.getValue(result, "discountcouponcode");
...
Create a registry
 */
  public static HeaderData[] set(String cBpartnerId, String cBpartnerIdr, String issent, String priorityrule, String generateproject, String billtoId, String cIncotermsId, String isselected, String freightamt, String mShipperId, String performanceperiodstart, String istaxincluded, String qty, String cProjecttaskId, String performanceperiodend, String cChargeId, String mProductId, String totallines, String ispaid, String enddate, String weightUom, String proposalstatus, String posted, String weight, String name, String chargeamt, String invoicefrequence, String scheddeliverydate, String cBpartnerLocationId, String internalnote, String isdiscountprinted, String totallinesonetime, String freightcostrule, String datepromised, String cPaymenttermId, String contractdate, String docstatus, String cProjectId, String cProjectIdr, String paymentrule, String generatetemplate, String transactiondate, String lostproposalfixedreason, String updatedby, String updatedbyr, String deliveryLocationId, String salesrepId, String poreference, String discountcouponcode, String mWarehouseId, String schedtransactiondate, String createdropshiporder, String deliverycomplete, String cOrderId, String isdelivered, String deliveryrule, String issotrx, String isselfservice, String createdby, String createdbyr, String dateordered, String cActivityId, String documentno, String monthlyDay, String completeordervalue, String copyfrompo, String isapproved, String subscriptionchangedate, String cCurrencyId, String adOrgId, String totalpaid, String bpzipcode, String grandtotalonetime, String cDoctypetargetId, String invoicerule, String closeproject, String transactionreference, String isinvoiceafterfirstcycle, String deliverynotes, String incotermsdescription, String mPricelistId, String invoicedamt, String processed, String deliveryviarule, String weeklyDay, String estpropability, String description, String docaction, String docactionBtn, String isprinted, String dateprinted, String grandtotal, String isinvoiced, String orderselfjoin, String iscompletelyinvoiced, String deliverylocationtext, String lostproposalreason, String isactive, String cDoctypeId, String adClientId, String adUserId, String quarterlyMonth, String firstschedinvoicedate, String processing, String yearlyMonth)    throws ServletException {
    ...
    objectHeaderData[0].issent = issent;
    ...
    objectHeaderData[0].deliverylocationtext = deliverylocationtext;
    ...
    objectHeaderData[0].discountcouponcode = discountcouponcode;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET AD_Org_ID = (?) , DocumentNo = (?) , POReference = (?) , DateOrdered = TO_DATE(?) , Issent = (?) , C_DocTypeTarget_ID = (?) , Name = (?) , C_DocType_ID = (?) , DocStatus = (?) , Description = (?) , Deliverynotes = (?) , Internalnote = (?) , C_BPartner_ID = (?) , bpzipcode = (?) , BillTo_ID = (?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , SalesRep_ID = (?) , M_PriceList_ID = (?) , IsDiscountPrinted = (?) , InvoiceRule = (?) , Estpropability = (?) , M_Warehouse_ID = (?) , C_Currency_ID = (?) , DeliveryRule = (?) , PriorityRule = (?) , PaymentRule = (?) , C_PaymentTerm_ID = (?) , C_Incoterms_ID = (?) , Delivery_Location_ID = (?) , DeliveryViaRule = (?) , M_Shipper_ID = (?) , Weight = TO_NUMBER(?) , Weight_Uom = (?) , FreightCostRule = (?) , M_Product_ID = (?) , Qty = TO_NUMBER(?) , FreightAmt = TO_NUMBER(?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Orderselfjoin = (?) , Firstschedinvoicedate = TO_DATE(?) , DatePromised = TO_DATE(?) , SchedDeliveryDate = TO_DATE(?) , SchedTransactionDate = TO_DATE(?) , Isapproved = (?) , TransactionDate = TO_DATE(?) , Contractdate = TO_DATE(?) , Enddate = TO_DATE(?) , Invoicefrequence = (?) , Isinvoiceafterfirstcycle = (?) , Quarterly_Month = (?) , Yearly_Month = (?) , Weekly_Day = (?) , Monthly_Day = TO_NUMBER(?) , Proposalstatus = (?) , Lostproposalfixedreason = (?) , Lostproposalreason = (?) , Totallinesonetime = TO_NUMBER(?) , Grandtotalonetime = TO_NUMBER(?) , TotalLines = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , InvoicedAmt = TO_NUMBER(?) , TotalPaid = TO_NUMBER(?) , CompleteOrderValue = TO_NUMBER(?) , IsCompletelyInvoiced = (?) , DeliveryComplete = (?) , Subscriptionchangedate = TO_DATE(?) , DocAction = (?) , CopyFromPO = (?) , Generatetemplate = (?) , Generateproject = (?) , Closeproject = (?) , Createdropshiporder = (?) , C_Charge_ID = (?) , Incotermsdescription = (?) , Deliverylocationtext = (?) , ChargeAmt = TO_NUMBER(?) , performanceperiodend = TO_DATE(?) , Posted = (?) , Processing = (?) , IsSOTrx = (?) , Discountcouponcode = (?) , IsSelected = (?) , IsSelfService = (?) , IsPrinted = (?) , IsInvoiced = (?) , performanceperiodstart = TO_DATE(?) , IsDelivered = (?) , Transactionreference = (?) , IsActive = (?) , AD_Client_ID = (?) , Ispaid = (?) , C_Order_ID = (?) , IsTaxIncluded = (?) , DatePrinted = TO_DATE(?) , Processed = (?) , C_Activity_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, issent);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, discountcouponcode);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (AD_Org_ID, DocumentNo, POReference, DateOrdered, Issent, C_DocTypeTarget_ID, Name, C_DocType_ID, DocStatus, Description, Deliverynotes, Internalnote, C_BPartner_ID, bpzipcode, BillTo_ID, C_BPartner_Location_ID, AD_User_ID, SalesRep_ID, M_PriceList_ID, IsDiscountPrinted, InvoiceRule, Estpropability, M_Warehouse_ID, C_Currency_ID, DeliveryRule, PriorityRule, PaymentRule, C_PaymentTerm_ID, C_Incoterms_ID, Delivery_Location_ID, DeliveryViaRule, M_Shipper_ID, Weight, Weight_Uom, FreightCostRule, M_Product_ID, Qty, FreightAmt, C_Project_ID, C_Projecttask_ID, Orderselfjoin, Firstschedinvoicedate, DatePromised, SchedDeliveryDate, SchedTransactionDate, Isapproved, TransactionDate, Contractdate, Enddate, Invoicefrequence, Isinvoiceafterfirstcycle, Quarterly_Month, Yearly_Month, Weekly_Day, Monthly_Day, Proposalstatus, Lostproposalfixedreason, Lostproposalreason, Totallinesonetime, Grandtotalonetime, TotalLines, GrandTotal, InvoicedAmt, TotalPaid, CompleteOrderValue, IsCompletelyInvoiced, DeliveryComplete, Subscriptionchangedate, DocAction, CopyFromPO, Generatetemplate, Generateproject, Closeproject, Createdropshiporder, C_Charge_ID, Incotermsdescription, Deliverylocationtext, ChargeAmt, performanceperiodend, Posted, Processing, IsSOTrx, Discountcouponcode, IsSelected, IsSelfService, IsPrinted, IsInvoiced, performanceperiodstart, IsDelivered, Transactionreference, IsActive, AD_Client_ID, Ispaid, C_Order_ID, IsTaxIncluded, DatePrinted, Processed, C_Activity_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), now(), ?, now(), ?)";
