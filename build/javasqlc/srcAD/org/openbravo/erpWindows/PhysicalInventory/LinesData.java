 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mInventoryId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_InventoryLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_InventoryLine.M_Inventory_ID, " +
      "(CASE WHEN M_InventoryLine.M_Inventory_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_Inventory_IDR, " +
      "M_InventoryLine.Line, " +
      "M_InventoryLine.M_Locator_ID, " +
      "(CASE WHEN M_InventoryLine.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      "M_InventoryLine.Description, " +
      "M_InventoryLine.M_Product_ID, " +
      "(CASE WHEN M_InventoryLine.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_InventoryLine.M_AttributeSetInstance_ID, " +
      "(CASE WHEN M_InventoryLine.M_AttributeSetInstance_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Description), ''))),'') ) END) AS M_AttributeSetInstance_IDR, " +
      ...
      "(CASE WHEN M_InventoryLine.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "M_InventoryLine.QtyBook, " +
      "M_InventoryLine.M_InventoryLine_ID, " +
      "COALESCE(M_InventoryLine.IsActive, 'N') AS IsActive, " +
      "        ? AS LANGUAGE " +
      "        FROM M_InventoryLine left join (select AD_Org_ID, Name from AD_Org) table1 on (M_InventoryLine.AD_Org_ID = table1.AD_Org_ID) left join (select M_Inventory_ID, Name from M_Inventory) table2 on (M_InventoryLine.M_Inventory_ID = table2.M_Inventory_ID) left join (select M_Locator_ID, Value from M_Locator) table3 on (M_InventoryLine.M_Locator_ID = table3.M_Locator_ID) left join (select M_Product_ID, Value, Name from M_Product) table4 on (M_InventoryLine.M_Product_ID = table4.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL4 on (table4.M_Product_ID = tableTRL4.M_Product_ID and tableTRL4.AD_Language = ?)  left join (select M_AttributeSetInstance_ID, Description from M_AttributeSetInstance) table5 on (M_InventoryLine.M_AttributeSetInstance_ID = table5.M_AttributeSetInstance_ID) left join (select C_UOM_ID, Name from C_UOM) table6 on (M_InventoryLine.C_UOM_ID = table6.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL6 on (table6.C_UOM_ID = tableTRL6.C_UOM_ID and tableTRL6.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String mInventoryId, String updatedby, String updatedbyr, String adOrgId, String description, String mProductUomId, String createdby, String createdbyr, String mLocatorId, String mLocatorIdr, String adClientId, String qtycount, String mProductId, String mProductIdr, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String quantityorder, String isactive, String cUomId, String value, String line, String mInventorylineId, String qtybook, String name, String quantityorderbook)    throws ServletException {
