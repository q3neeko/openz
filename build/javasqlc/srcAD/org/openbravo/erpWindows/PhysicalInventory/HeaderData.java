 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Inventory.M_PerpetualInv_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS M_PerpetualInv_IDR, " +
      "M_Inventory.MovementDate, " +
      "M_Inventory.C_Project_ID, " +
      "(CASE WHEN M_Inventory.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "M_Inventory.C_Activity_ID, " +
      "(CASE WHEN M_Inventory.C_Activity_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_Activity_IDR, " +
      "M_Inventory.C_Campaign_ID, " +
      "(CASE WHEN M_Inventory.C_Campaign_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_Campaign_IDR, " +
      "M_Inventory.AD_OrgTrx_ID, " +
      "(CASE WHEN M_Inventory.AD_OrgTrx_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS AD_OrgTrx_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Inventory left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Inventory.AD_Org_ID = table1.AD_Org_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table2 on (M_Inventory.M_Warehouse_ID =  table2.M_Warehouse_ID) left join (select M_PerpetualInv_ID, Name from M_PerpetualInv) table3 on (M_Inventory.M_PerpetualInv_ID = table3.M_PerpetualInv_ID) left join (select C_Project_ID, Value, Name from C_Project) table4 on (M_Inventory.C_Project_ID = table4.C_Project_ID) left join (select C_Activity_ID, Name from C_Activity) table5 on (M_Inventory.C_Activity_ID = table5.C_Activity_ID) left join (select C_Campaign_ID, Name from C_Campaign) table6 on (M_Inventory.C_Campaign_ID = table6.C_Campaign_ID) left join (select AD_Org_ID, value, Name from AD_Org) table7 on (M_Inventory.AD_OrgTrx_ID =  table7.AD_Org_ID) left join ad_ref_list_v list1 on (list1.ad_reference_id = '234' and list1.ad_language = ?  AND M_Inventory.Posted = TO_CHAR(list1.value))" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static HeaderData[] set(String updateqty, String createdby, String createdbyr, String updatedby, String updatedbyr, String posted, String postedBtn, String mInventoryId, String mPerpetualinvId, String generatelist, String description, String user2Id, String processing, String mWarehouseId, String cActivityId, String cCampaignId, String movementdate, String processed, String adOrgtrxId, String adClientId, String user1Id, String cProjectId, String cProjectIdr, String isactive, String adOrgId, String name)    throws ServletException {
