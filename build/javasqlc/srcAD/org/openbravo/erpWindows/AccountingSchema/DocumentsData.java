 ...
 */
  public static DocumentsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cAcctschemaTableId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_AcctSchema_Table_DocType.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(C_AcctSchema_Table_DocType.Isactive, 'N') AS Isactive, " +
      "C_AcctSchema_Table_DocType.C_Acctschema_Table_ID, " +
      "(CASE WHEN C_AcctSchema_Table_DocType.C_Acctschema_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Acctschema_Table_IDR, " +
      ...
      "(CASE WHEN C_AcctSchema_Table_DocType.AD_Createfact_Template_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_Createfact_Template_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM C_AcctSchema_Table_DocType left join (select AD_Org_ID, Name from AD_Org) table1 on (C_AcctSchema_Table_DocType.AD_Org_ID = table1.AD_Org_ID) left join (select C_Acctschema_Table_ID, C_AcctSchema_ID, AD_Table_ID from C_Acctschema_Table) table2 on (C_AcctSchema_Table_DocType.C_Acctschema_Table_ID = table2.C_Acctschema_Table_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (table2.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select AD_Table_ID, Name from AD_Table) table4 on (table2.AD_Table_ID =  table4.AD_Table_ID) left join ad_ref_list_v list1 on (C_AcctSchema_Table_DocType.Docbasetype = list1.value and list1.ad_reference_id = '183' and list1.ad_language = ?)  left join (select AD_Createfact_Template_ID, Name from AD_Createfact_Template) table5 on (C_AcctSchema_Table_DocType.AD_Createfact_Template_ID = table5.AD_Createfact_Template_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static DocumentsData[] set(String cAcctschemaTableId, String docbasetype, String adOrgId, String adClientId, String createdby, String cAcctschemaTableDoctypeId, String allownegative, String isactive, String updatedby, String adCreatefactTemplateId)    throws ServletException {
