 ...
 */
  public static AccountSchemaTablesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cAcctschemaId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_AcctSchema_Table.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(C_AcctSchema_Table.IsActive, 'N') AS IsActive, " +
      "C_AcctSchema_Table.C_AcctSchema_ID, " +
      "(CASE WHEN C_AcctSchema_Table.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "C_AcctSchema_Table.AD_Table_ID, " +
      "(CASE WHEN C_AcctSchema_Table.AD_Table_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Table_IDR, " +
      "C_AcctSchema_Table.AD_Createfact_Template_ID, " +
      "(CASE WHEN C_AcctSchema_Table.AD_Createfact_Template_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Createfact_Template_IDR, " +
      "C_AcctSchema_Table.Acctdescription, " +
      "        ? AS LANGUAGE " +
      "        FROM C_AcctSchema_Table left join (select AD_Org_ID, Name from AD_Org) table1 on (C_AcctSchema_Table.AD_Org_ID = table1.AD_Org_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table2 on (C_AcctSchema_Table.C_AcctSchema_ID = table2.C_AcctSchema_ID) left join (select AD_Table_ID, Name from AD_Table) table3 on (C_AcctSchema_Table.AD_Table_ID =  table3.AD_Table_ID) left join (select AD_Createfact_Template_ID, Name from AD_Createfact_Template) table4 on (C_AcctSchema_Table.AD_Createfact_Template_ID = table4.AD_Createfact_Template_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
