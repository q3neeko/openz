 ...
 */
  public static TaxData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cOrderId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_OrderTax.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_OrderTax.C_Tax_ID, " +
      "(CASE WHEN C_OrderTax.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_OrderTax left join (select AD_Org_ID, Name from AD_Org) table1 on (C_OrderTax.AD_Org_ID = table1.AD_Org_ID) left join (select C_Tax_ID, Name from C_Tax) table2 on (C_OrderTax.C_Tax_ID = table2.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL2 on (table2.C_Tax_ID = tableTRL2.C_Tax_ID and tableTRL2.AD_Language = ?)  left join (select C_Order_ID, DocumentNo, Name, DateOrdered, GrandTotal from C_Order) table3 on (C_OrderTax.C_Order_ID = table3.C_Order_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_OrderTax" +
      "        SET AD_Client_ID = (?) , Line = TO_NUMBER(?) , AD_Org_ID = (?) , C_Tax_ID = (?) , Isonetimeposition = (?) , C_Order_ID = (?) , TaxAmt = TO_NUMBER(?) , Reversetaxamt = TO_NUMBER(?) , TaxBaseAmt = TO_NUMBER(?) , IsActive = (?) , C_OrderTax_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_OrderTax.C_OrderTax_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, Line, AD_Org_ID, C_Tax_ID, Isonetimeposition, C_Order_ID, TaxAmt, Reversetaxamt, TaxBaseAmt, IsActive, C_OrderTax_ID, created, createdby, updated, updatedBy)" +
