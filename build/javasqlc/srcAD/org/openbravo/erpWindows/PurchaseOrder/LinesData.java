 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cOrderId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_OrderLine.M_Product_Po_ID, " +
      "(CASE WHEN C_OrderLine.M_Product_Po_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL11.Name IS NULL THEN TO_CHAR(table11.Name) ELSE TO_CHAR(tableTRL11.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.Name), ''))),'') ) END) AS M_Product_Po_IDR, " +
      ...
      "(CASE WHEN C_OrderLine.M_Shipper_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table13.Name), ''))),'') ) END) AS M_Shipper_IDR, " +
      ...
      "(CASE WHEN C_OrderLine.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "C_OrderLine.C_Project_ID, " +
      "(CASE WHEN C_OrderLine.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_OrderLine.C_Projecttask_ID, " +
      "(CASE WHEN C_OrderLine.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table19.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table19.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "C_OrderLine.Orderlineselfjoin, " +
      "(CASE WHEN C_OrderLine.Orderlineselfjoin IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table21.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table21.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table21.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table21.GrandTotal), ''))),'') ) END) AS OrderlineselfjoinR, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectLinesData.mProductPoId = UtilSql.getValue(result, "m_product_po_id");
        objectLinesData.mProductPoIdr = UtilSql.getValue(result, "m_product_po_idr");
 ...
 */
  public static LinesData[] set(String cOrderId, String mProductUomId, String mWarehouseId, String datepromised, String aAssetId, String isgrossprice, String qtyinvoiced, String securityqty, String calcfastestdeliverydate, String invoicedamt, String createdby, String createdbyr, String quantityorder, String discount, String linenetamt, String mOfferId, String pricelimit, String linegrossamt, String updatedby, String updatedbyr, String qtydelivered, String cOrderIdr, String directship, String qtyordered, String cProjecttaskId, String refOrderlineId, String cProjectId, String cProjectIdr, String ignoreresiduedate, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String cBpartnerLocationId, String isactive, String cancelpricead, String priceactual, String isdescription, String deliverycomplete, String mShipperId, String cOrderlineId, String cChargeId, String isonetimeposition, String dateinvoiced, String cBpartnerId, String cBpartnerIdr, String cProjectphaseId, String cUomId, String scheddeliverydate, String mProductId, String mProductIdr, String adOrgId, String freightamt, String chargeamt, String orderlineselfjoin, String mProductPoId, String linetaxamt, String qtyreserved, String dateordered, String sResourceassignmentId, String pricestd, String calloffqty, String line, String ispagebreak, String cCurrencyId, String datedelivered, String pricelist, String adClientId, String issummaryitem, String cTaxId, String description, String ignoreresidue)    throws ServletException {
    ...
    objectLinesData[0].mProductPoId = mProductPoId;
    objectLinesData[0].mProductPoIdr = "";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_OrderLine" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_Order_ID = (?) , Ispagebreak = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , DateOrdered = TO_DATE(?) , DatePromised = TO_DATE(?) , Scheddeliverydate = TO_DATE(?) , Line = TO_NUMBER(?) , M_Product_ID = (?) , Calcfastestdeliverydate = (?) , M_AttributeSetInstance_ID = (?) , C_Charge_ID = (?) , Description = (?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , QtyOrdered = TO_NUMBER(?) , C_UOM_ID = (?) , Securityqty = TO_NUMBER(?) , QtyDelivered = TO_NUMBER(?) , M_Product_Po_ID = (?) , QtyInvoiced = TO_NUMBER(?) , Invoicedamt = TO_NUMBER(?) , Ignoreresidue = (?) , Deliverycomplete = (?) , M_Shipper_ID = (?) , PriceActual = TO_NUMBER(?) , C_Currency_ID = (?) , PriceStd = TO_NUMBER(?) , CANCELPRICEAD = (?) , PriceList = TO_NUMBER(?) , Discount = TO_NUMBER(?) , FreightAmt = TO_NUMBER(?) , C_Tax_ID = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Orderlineselfjoin = (?) , LineNetAmt = TO_NUMBER(?) , Linegrossamt = TO_NUMBER(?) , Calloffqty = TO_NUMBER(?) , Ignoreresiduedate = TO_DATE(?) , IsOneTimePosition = (?) , IsActive = (?) , Issummaryitem = (?) , DateDelivered = TO_DATE(?) , DateInvoiced = TO_DATE(?) , C_OrderLine_ID = (?) , ChargeAmt = TO_NUMBER(?) , DirectShip = (?) , PriceLimit = TO_NUMBER(?) , Linetaxamt = TO_NUMBER(?) , Isgrossprice = (?) , S_ResourceAssignment_ID = (?) , Ref_OrderLine_ID = (?) , QtyReserved = TO_NUMBER(?) , M_Warehouse_ID = (?) , M_Offer_ID = (?) , IsDescription = (?) , C_Projectphase_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_OrderLine.C_OrderLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, C_Order_ID, Ispagebreak, C_BPartner_ID, C_BPartner_Location_ID, DateOrdered, DatePromised, Scheddeliverydate, Line, M_Product_ID, Calcfastestdeliverydate, M_AttributeSetInstance_ID, C_Charge_ID, Description, QuantityOrder, M_Product_Uom_Id, QtyOrdered, C_UOM_ID, Securityqty, QtyDelivered, M_Product_Po_ID, QtyInvoiced, Invoicedamt, Ignoreresidue, Deliverycomplete, M_Shipper_ID, PriceActual, C_Currency_ID, PriceStd, CANCELPRICEAD, PriceList, Discount, FreightAmt, C_Tax_ID, A_Asset_ID, C_Project_ID, C_Projecttask_ID, Orderlineselfjoin, LineNetAmt, Linegrossamt, Calloffqty, Ignoreresiduedate, IsOneTimePosition, IsActive, Issummaryitem, DateDelivered, DateInvoiced, C_OrderLine_ID, ChargeAmt, DirectShip, PriceLimit, Linetaxamt, Isgrossprice, S_ResourceAssignment_ID, Ref_OrderLine_ID, QtyReserved, M_Warehouse_ID, M_Offer_ID, IsDescription, C_Projectphase_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), (?), (?), (?), TO_DATE(?), TO_DATE(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), now(), ?, now(), ?)";
