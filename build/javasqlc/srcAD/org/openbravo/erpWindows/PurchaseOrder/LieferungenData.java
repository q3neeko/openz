 ...
 */
  public static LieferungenData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cOrderId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_InOut.M_Shipper_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS M_Shipper_IDR, " +
      ...
      "(CASE WHEN M_InOut.M_Warehouse_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS M_Warehouse_IDR, " +
      "M_InOut.AD_User_ID, " +
      "(CASE WHEN M_InOut.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "M_InOut.C_BPartner_ID, " +
      "(CASE WHEN M_InOut.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "M_InOut.C_BPartner_Location_ID, " +
      "M_InOut.DateAcct, " +
      "M_InOut.C_Order_ID, " +
      "(CASE WHEN M_InOut.C_Order_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table11.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.GrandTotal), ''))),'') ) END) AS C_Order_IDR, " +
      "M_InOut.DatePrinted, " +
      "M_InOut.FreightAmt, " +
      ...
      "(CASE WHEN M_InOut.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table12.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "M_InOut.CreateFrom, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_InOut" +
      "        SET User2_ID = (?) , User1_ID = (?) , AD_OrgTrx_ID = (?) , Posted = (?) , POReference = (?) , UpdateLines = (?) , DocumentNo = (?) , Islogistic = (?) , C_DocType_ID = (?) , GenerateLines = (?) , Description = (?) , MovementDate = TO_DATE(?) , Delivery_Location_ID = (?) , C_Project_ID = (?) , C_Campaign_ID = (?) , AD_Org_ID = (?) , C_Activity_ID = (?) , ShipDate = TO_DATE(?) , PickDate = TO_DATE(?) , Qty = TO_NUMBER(?) , TrackingNo = (?) , SalesRep_ID = (?) , DateOrdered = TO_DATE(?) , PriorityRule = (?) , M_Shipper_ID = (?) , DeliveryViaRule = (?) , DeliveryRule = (?) , M_Warehouse_ID = (?) , AD_User_ID = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , DateAcct = TO_DATE(?) , C_Order_ID = (?) , DatePrinted = TO_DATE(?) , FreightAmt = TO_NUMBER(?) , FreightCostRule = (?) , IsSOTrx = (?) , IsActive = (?) , MovementType = (?) , DocStatus = (?) , C_Invoice_ID = (?) , CreateFrom = (?) , Processed = (?) , IsPrinted = (?) , GenerateTo = (?) , Processing = (?) , M_InOut_ID = (?) , AD_Client_ID = (?) , DocAction = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_InOut.M_InOut_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO M_InOut " +
      "        (User2_ID, User1_ID, AD_OrgTrx_ID, Posted, POReference, UpdateLines, DocumentNo, Islogistic, C_DocType_ID, GenerateLines, Description, MovementDate, Delivery_Location_ID, C_Project_ID, C_Campaign_ID, AD_Org_ID, C_Activity_ID, ShipDate, PickDate, Qty, TrackingNo, SalesRep_ID, DateOrdered, PriorityRule, M_Shipper_ID, DeliveryViaRule, DeliveryRule, M_Warehouse_ID, AD_User_ID, C_BPartner_ID, C_BPartner_Location_ID, DateAcct, C_Order_ID, DatePrinted, FreightAmt, FreightCostRule, IsSOTrx, IsActive, MovementType, DocStatus, C_Invoice_ID, CreateFrom, Processed, IsPrinted, GenerateTo, Processing, M_InOut_ID, AD_Client_ID, DocAction, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_NUMBER(?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), TO_DATE(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
