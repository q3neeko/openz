 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_Order.A_Asset_ID, " +
      "(CASE WHEN C_Order.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table14.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "C_Order.C_Project_ID, " +
      "(CASE WHEN C_Order.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table15.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table15.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_Order.C_Projecttask_ID, " +
      "(CASE WHEN C_Order.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "C_Order.Orderselfjoin, " +
      "(CASE WHEN C_Order.Orderselfjoin IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.DocumentNo), ''))),'') ) END) AS OrderselfjoinR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET AD_Org_ID = (?) , DocumentNo = (?) , POReference = (?) , DateOrdered = TO_DATE(?) , Issent = (?) , C_DocTypeTarget_ID = (?) , Name = (?) , C_DocType_ID = (?) , DocStatus = (?) , Description = (?) , Deliverynotes = (?) , Internalnote = (?) , C_BPartner_ID = (?) , bpzipcode = (?) , Deliver2projectadress = (?) , BillTo_ID = (?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , SalesRep_ID = (?) , M_PriceList_ID = (?) , IsDiscountPrinted = (?) , M_Warehouse_ID = (?) , C_Currency_ID = (?) , PriorityRule = (?) , PaymentRule = (?) , C_PaymentTerm_ID = (?) , DeliveryViaRule = (?) , C_Incoterms_ID = (?) , Incotermsdescription = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Orderselfjoin = (?) , Isrecharge = (?) , DatePromised = TO_DATE(?) , SchedDeliveryDate = TO_DATE(?) , SchedTransactionDate = TO_DATE(?) , TransactionDate = TO_DATE(?) , Contractdate = TO_DATE(?) , Enddate = TO_DATE(?) , TotalLines = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , InvoicedAmt = TO_NUMBER(?) , TotalPaid = TO_NUMBER(?) , IsCompletelyInvoiced = (?) , DeliveryComplete = (?) , Ispaid = (?) , DocAction = (?) , CopyFromPO = (?) , ecommerceorder = (?) , FreightAmt = TO_NUMBER(?) , Posted = (?) , IsTaxIncluded = (?) , IsSelected = (?) , Monthly_Day = TO_NUMBER(?) , InvoiceRule = (?) , IsPrinted = (?) , M_Product_ID = (?) , Transactionreference = (?) , Quarterly_Month = (?) , IsInvoiced = (?) , IsSelfService = (?) , Proposalstatus = (?) , Subscriptionchangedate = TO_DATE(?) , IsDelivered = (?) , IsActive = (?) , AD_Client_ID = (?) , Estpropability = (?) , C_Order_ID = (?) , Weekly_Day = (?) , Delivery_Location_ID = (?) , Deliverylocationtext = (?) , Qty = TO_NUMBER(?) , Lostproposalfixedreason = (?) , Weight = TO_NUMBER(?) , Isinvoiceafterfirstcycle = (?) , Weight_Uom = (?) , Firstschedinvoicedate = TO_DATE(?) , Invoicefrequence = (?) , Generateproject = (?) , Totallinesonetime = TO_NUMBER(?) , CompleteOrderValue = TO_NUMBER(?) , Yearly_Month = (?) , Closeproject = (?) , C_Activity_ID = (?) , Grandtotalonetime = TO_NUMBER(?) , Generatetemplate = (?) , Lostproposalreason = (?) , C_Bidproject_ID = (?) , M_Shipper_ID = (?) , Processed = (?) , DatePrinted = TO_DATE(?) , DeliveryRule = (?) , FreightCostRule = (?) , IsSOTrx = (?) , Processing = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (AD_Org_ID, DocumentNo, POReference, DateOrdered, Issent, C_DocTypeTarget_ID, Name, C_DocType_ID, DocStatus, Description, Deliverynotes, Internalnote, C_BPartner_ID, bpzipcode, Deliver2projectadress, BillTo_ID, C_BPartner_Location_ID, AD_User_ID, SalesRep_ID, M_PriceList_ID, IsDiscountPrinted, M_Warehouse_ID, C_Currency_ID, PriorityRule, PaymentRule, C_PaymentTerm_ID, DeliveryViaRule, C_Incoterms_ID, Incotermsdescription, A_Asset_ID, C_Project_ID, C_Projecttask_ID, Orderselfjoin, Isrecharge, DatePromised, SchedDeliveryDate, SchedTransactionDate, TransactionDate, Contractdate, Enddate, TotalLines, GrandTotal, InvoicedAmt, TotalPaid, IsCompletelyInvoiced, DeliveryComplete, Ispaid, DocAction, CopyFromPO, ecommerceorder, FreightAmt, Posted, IsTaxIncluded, IsSelected, Monthly_Day, InvoiceRule, IsPrinted, M_Product_ID, Transactionreference, Quarterly_Month, IsInvoiced, IsSelfService, Proposalstatus, Subscriptionchangedate, IsDelivered, IsActive, AD_Client_ID, Estpropability, C_Order_ID, Weekly_Day, Delivery_Location_ID, Deliverylocationtext, Qty, Lostproposalfixedreason, Weight, Isinvoiceafterfirstcycle, Weight_Uom, Firstschedinvoicedate, Invoicefrequence, Generateproject, Totallinesonetime, CompleteOrderValue, Yearly_Month, Closeproject, C_Activity_ID, Grandtotalonetime, Generatetemplate, Lostproposalreason, C_Bidproject_ID, M_Shipper_ID, Processed, DatePrinted, DeliveryRule, FreightCostRule, IsSOTrx, Processing, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), TO_DATE(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), now(), ?, now(), ?)";
