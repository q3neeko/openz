 ...
 */
  public static ShipmentRoutingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mWarehouseId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Warehouse_Shipper.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      ...
      "(CASE WHEN M_Warehouse_Shipper.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "M_Warehouse_Shipper.SeqNo, " +
      "        ? AS LANGUAGE " +
      "        FROM M_Warehouse_Shipper left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Warehouse_Shipper.AD_Org_ID = table1.AD_Org_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table2 on (M_Warehouse_Shipper.M_Warehouse_ID = table2.M_Warehouse_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (M_Warehouse_Shipper.C_BPartner_ID = table3.C_BPartner_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ShipmentRoutingData[] set(String mWarehouseId, String createdby, String createdbyr, String adClientId, String updatedby, String updatedbyr, String adOrgId, String mWarehouseShipperId, String isactive, String seqno, String cBpartnerId, String cBpartnerIdr)    throws ServletException {
