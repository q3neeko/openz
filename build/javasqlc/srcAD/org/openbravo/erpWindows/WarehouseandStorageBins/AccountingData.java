 ...
 */
  public static AccountingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mWarehouseId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Warehouse_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_Warehouse_Acct.M_Warehouse_ID, " +
      "(CASE WHEN M_Warehouse_Acct.M_Warehouse_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_Warehouse_IDR, " +
      "M_Warehouse_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN M_Warehouse_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      ...
      "(CASE WHEN M_Warehouse_Acct.W_Differences_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS W_Differences_AcctR, " +
      "M_Warehouse_Acct.W_Revaluation_Acct, " +
      "M_Warehouse_Acct.M_Warehouse_Acct_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM M_Warehouse_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Warehouse_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table2 on (M_Warehouse_Acct.M_Warehouse_ID = table2.M_Warehouse_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (M_Warehouse_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (M_Warehouse_Acct.W_Differences_Acct = table4.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static AccountingData[] set(String mWarehouseId, String createdby, String createdbyr, String updatedby, String updatedbyr, String wDifferencesAcct, String wDifferencesAcctr, String mWarehouseAcctId, String adClientId, String isactive, String adOrgId, String cAcctschemaId, String wInvactualadjustAcct, String wInvactualadjustAcctr, String wInventoryAcct, String wInventoryAcctr, String wRevaluationAcct, String wRevaluationAcctr)    throws ServletException {
