 ...
 */
  public static CombinationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_ValidCombination.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      ...
      "(CASE WHEN C_ValidCombination.AD_Org_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_ValidCombination.Account_ID, " +
      "(CASE WHEN C_ValidCombination.Account_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS Account_IDR, " +
      "C_ValidCombination.C_Activity_ID, " +
      "(CASE WHEN C_ValidCombination.C_Activity_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Activity_IDR, " +
      "C_ValidCombination.M_Product_ID, " +
      "(CASE WHEN C_ValidCombination.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "C_ValidCombination.C_BPartner_ID, " +
      "(CASE WHEN C_ValidCombination.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_ValidCombination.C_Project_ID, " +
      "(CASE WHEN C_ValidCombination.C_Project_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_ValidCombination.C_Campaign_ID, " +
      "(CASE WHEN C_ValidCombination.C_Campaign_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS C_Campaign_IDR, " +
      "C_ValidCombination.AD_OrgTrx_ID, " +
      "C_ValidCombination.C_LocFrom_ID, " +
      "(CASE WHEN C_ValidCombination.C_LocFrom_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.City), ''))),'') ) END) AS C_LocFrom_IDR, " +
      "C_ValidCombination.C_LocTo_ID, " +
      "(CASE WHEN C_ValidCombination.C_LocTo_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.City), ''))),'') ) END) AS C_LocTo_IDR, " +
      "C_ValidCombination.C_SalesRegion_ID, " +
      "(CASE WHEN C_ValidCombination.C_SalesRegion_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Name), ''))),'') ) END) AS C_SalesRegion_IDR, " +
      "C_ValidCombination.User1_ID, " +
      "C_ValidCombination.User2_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM C_ValidCombination left join (select C_AcctSchema_ID, Name from C_AcctSchema) table1 on (C_ValidCombination.C_AcctSchema_ID = table1.C_AcctSchema_ID) left join (select AD_Org_ID, Name from AD_Org) table2 on (C_ValidCombination.AD_Org_ID =  table2.AD_Org_ID) left join (select C_ElementValue_ID, value, Name from C_ElementValue) table3 on (C_ValidCombination.Account_ID =  table3.C_ElementValue_ID) left join (select C_ElementValue_ID,AD_Language, Name from C_ElementValue_TRL) tableTRL3 on (table3.C_ElementValue_ID = tableTRL3.C_ElementValue_ID and tableTRL3.AD_Language = ?)  left join (select C_Activity_ID, Name from C_Activity) table4 on (C_ValidCombination.C_Activity_ID =  table4.C_Activity_ID) left join (select M_Product_ID, Value, Name from M_Product) table5 on (C_ValidCombination.M_Product_ID = table5.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL5 on (table5.M_Product_ID = tableTRL5.M_Product_ID and tableTRL5.AD_Language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table6 on (C_ValidCombination.C_BPartner_ID = table6.C_BPartner_ID) left join (select C_Project_ID, value, Name from C_Project) table7 on (C_ValidCombination.C_Project_ID =  table7.C_Project_ID) left join (select C_Campaign_ID, value, Name from C_Campaign) table8 on (C_ValidCombination.C_Campaign_ID =  table8.C_Campaign_ID) left join (select C_Location_ID, City from C_Location) table9 on (C_ValidCombination.C_LocFrom_ID =  table9.C_Location_ID) left join (select C_Location_ID, City from C_Location) table10 on (C_ValidCombination.C_LocTo_ID =  table10.C_Location_ID) left join (select C_SalesRegion_ID, value, Name from C_SalesRegion) table11 on (C_ValidCombination.C_SalesRegion_ID =  table11.C_SalesRegion_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
