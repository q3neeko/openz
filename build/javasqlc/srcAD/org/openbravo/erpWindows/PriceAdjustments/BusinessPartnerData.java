 ...
 */
  public static BusinessPartnerData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mOfferId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Offer_BPartner.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Offer_BPartner left join (select M_Offer_ID, Name from M_Offer) table1 on (M_Offer_BPartner.M_Offer_ID = table1.M_Offer_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table2 on (M_Offer_BPartner.C_BPartner_ID = table2.C_BPartner_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
