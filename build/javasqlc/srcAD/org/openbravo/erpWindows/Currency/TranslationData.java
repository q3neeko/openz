 ...
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCurrencyId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Currency_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Currency_Trl.C_Currency_ID, " +
      "(CASE WHEN C_Currency_Trl.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "C_Currency_Trl.AD_Language, " +
      "(CASE WHEN C_Currency_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Currency_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Currency_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table2 on (C_Currency_Trl.C_Currency_ID = table2.C_Currency_ID) left join (select AD_Language, Name from AD_Language) table3 on (C_Currency_Trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TranslationData[] set(String cCurrencyId, String cCurrencyTrlId, String updatedby, String updatedbyr, String adLanguage, String description, String istranslated, String adOrgId, String cursymbol, String isactive, String adClientId, String createdby, String createdbyr)    throws ServletException {
