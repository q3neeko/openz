 ...
 */
  public static ConversionRatesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String C_Currency_ID, String cCurrencyId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Conversion_Rate.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Conversion_Rate.C_Currency_ID, " +
      "(CASE WHEN C_Conversion_Rate.C_Currency_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "C_Conversion_Rate.C_Currency_ID_To, " +
      "(CASE WHEN C_Conversion_Rate.C_Currency_ID_To IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.ISO_Code), ''))),'') ) END) AS C_Currency_ID_ToR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Conversion_Rate left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Conversion_Rate.AD_Org_ID = table1.AD_Org_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table2 on (C_Conversion_Rate.C_Currency_ID =  table2.C_Currency_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table3 on (C_Conversion_Rate.C_Currency_ID_To =  table3.C_Currency_ID)" +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static ConversionRatesData[] set(String cCurrencyId, String adOrgId, String cConversionRateId, String conversionratetype, String validto, String updatedby, String updatedbyr, String cCurrencyIdTo, String validfrom, String multiplyrate, String isactive, String dividerate, String createdby, String createdbyr, String adClientId)    throws ServletException {
