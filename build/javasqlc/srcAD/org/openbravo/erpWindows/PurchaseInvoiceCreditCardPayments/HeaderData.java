 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Invoice_Creditcard_V.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Invoice_Creditcard_V.DocumentNo, " +
      "C_Invoice_Creditcard_V.C_DocTypeTarget_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.C_DocTypeTarget_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_DocTypeTarget_IDR, " +
      "C_Invoice_Creditcard_V.M_PriceList_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.M_PriceList_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS M_PriceList_IDR, " +
      "C_Invoice_Creditcard_V.C_BPartner_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.C_BPartner_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Invoice_Creditcard_V.C_BPartner_Location_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "C_Invoice_Creditcard_V.DateAcct, " +
      "C_Invoice_Creditcard_V.DateInvoiced, " +
      "C_Invoice_Creditcard_V.AD_User_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "C_Invoice_Creditcard_V.C_Currency_ID, " +
      "(CASE WHEN C_Invoice_Creditcard_V.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Invoice_Creditcard_V left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Invoice_Creditcard_V.AD_Org_ID = table1.AD_Org_ID) left join (select C_DocType_ID, Name from C_DocType) table2 on (C_Invoice_Creditcard_V.C_DocTypeTarget_ID =  table2.C_DocType_ID) left join (select C_DocType_ID,AD_Language, Name from C_DocType_TRL) tableTRL2 on (table2.C_DocType_ID = tableTRL2.C_DocType_ID and tableTRL2.AD_Language = ?)  left join (select M_PriceList_ID, Name from M_PriceList) table3 on (C_Invoice_Creditcard_V.M_PriceList_ID = table3.M_PriceList_ID) left join (select C_BPartner_ID, value, Name from C_BPartner) table4 on (C_Invoice_Creditcard_V.C_BPartner_ID =  table4.C_BPartner_ID) left join (select C_BPartner_Location_ID, Name, Deviant_Bp_Name from C_BPartner_Location) table5 on (C_Invoice_Creditcard_V.C_BPartner_Location_ID = table5.C_BPartner_Location_ID) left join (select AD_User_ID, Name from AD_User) table6 on (C_Invoice_Creditcard_V.AD_User_ID = table6.AD_User_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table7 on (C_Invoice_Creditcard_V.C_Currency_ID = table7.C_Currency_ID) left join ad_ref_list_v list1 on (C_Invoice_Creditcard_V.DocStatus = list1.value and list1.ad_reference_id = '131' and list1.ad_language = ?)  left join ad_ref_list_v list2 on (list2.ad_reference_id = '135' and list2.ad_language = ?  AND (CASE C_Invoice_Creditcard_V.DocAction WHEN '--' THEN 'CL' ELSE TO_CHAR(C_Invoice_Creditcard_V.DocAction) END) = list2.value) left join ad_ref_list_v list3 on (list3.ad_reference_id = '234' and list3.ad_language = ?  AND C_Invoice_Creditcard_V.Posted = TO_CHAR(list3.value))" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static HeaderData[] set(String cInvoiceCreditcardVId, String paymentrule, String adClientId, String mPricelistId, String docstatus, String issotrx, String createdby, String createdbyr, String transactiondate, String cBpartnerLocationId, String docaction, String docactionBtn, String cInvoiceId, String cBpartnerId, String posted, String postedBtn, String cActivityId, String cPaymenttermId, String cCurrencyId, String cDoctypeId, String dateinvoiced, String documentno, String isgrossinvoice, String processing, String isactive, String adUserId, String internalnote, String adOrgtrxId, String description, String updatedby, String updatedbyr, String grandtotal, String processed, String cDoctypetargetId, String adOrgId, String totallines, String created, String dateacct, String generateto)    throws ServletException {
