 ...
 */
  public static ColumnusedintablesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adElementId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "AD_Column.AD_Org_ID, " +
      "(CASE WHEN AD_Column.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Column.AD_Callout_ID, " +
      "AD_Column.ColumnName, " +
      "AD_Column.AD_Table_ID, " +
      "(CASE WHEN AD_Column.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "AD_Column.Description, " +
      "COALESCE(AD_Column.IsActive, 'N') AS IsActive, " +
      "AD_Column.DefaultValue, " +
      "AD_Column.AD_Reference_ID, " +
      "(CASE WHEN AD_Column.AD_Reference_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS AD_Reference_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Column left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Column.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Table_ID, TableName from AD_Table) table2 on (AD_Column.AD_Table_ID = table2.AD_Table_ID) left join (select AD_Reference_ID, Name from AD_Reference) table3 on (AD_Column.AD_Reference_ID =  table3.AD_Reference_ID) left join (select AD_Reference_ID,AD_Language, Name from AD_Reference_TRL) tableTRL3 on (table3.AD_Reference_ID = tableTRL3.AD_Reference_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ColumnusedintablesData[] set(String adElementId, String readonlylogic, String adValRuleId, String fieldlength, String istranslated, String adProcessId, String iskey, String adCalloutId, String help, String defaultvalue, String adClientId, String description, String adOrgId, String columnname, String updatedby, String updatedbyr, String isparent, String adReferenceId, String adColumnId, String isactive, String adTableId, String adReferenceValueId, String isidentifier, String createdby, String createdbyr)    throws ServletException {
