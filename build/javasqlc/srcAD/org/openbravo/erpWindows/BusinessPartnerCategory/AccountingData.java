 ...
 */
  public static AccountingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBpGroupId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_BP_Group_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_BP_Group_Acct.C_BP_Group_ID, " +
      "(CASE WHEN C_BP_Group_Acct.C_BP_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BP_Group_IDR, " +
      "C_BP_Group_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN C_BP_Group_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "COALESCE(C_BP_Group_Acct.IsActive, 'N') AS IsActive, " +
      "C_BP_Group_Acct.C_Receivable_Acct, " +
      "(CASE WHEN C_BP_Group_Acct.C_Receivable_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS C_Receivable_AcctR, " +
      "C_BP_Group_Acct.C_Prepayment_Acct, " +
      "C_BP_Group_Acct.PayDiscount_Exp_Acct, " +
      "C_BP_Group_Acct.WriteOff_Acct, " +
      "(CASE WHEN C_BP_Group_Acct.WriteOff_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Combination), ''))),'') ) END) AS WriteOff_AcctR, " +
      ...
      "(CASE WHEN C_BP_Group_Acct.V_Liability_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Combination), ''))),'') ) END) AS V_Liability_AcctR, " +
      ...
      "(CASE WHEN C_BP_Group_Acct.NotInvoicedReceipts_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Combination), ''))),'') ) END) AS NotInvoicedReceipts_AcctR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_BP_Group_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (C_BP_Group_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select C_BP_Group_ID, Name from C_BP_Group) table2 on (C_BP_Group_Acct.C_BP_Group_ID = table2.C_BP_Group_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (C_BP_Group_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (C_BP_Group_Acct.C_Receivable_Acct = table4.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table5 on (C_BP_Group_Acct.WriteOff_Acct = table5.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table6 on (C_BP_Group_Acct.V_Liability_Acct = table6.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table7 on (C_BP_Group_Acct.NotInvoicedReceipts_Acct = table7.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
