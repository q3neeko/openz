 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBpartnerId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN S_TimeExpenseLine_V.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "S_TimeExpenseLine_V.S_TimeExpense_ID, " +
      "(CASE WHEN S_TimeExpenseLine_V.S_TimeExpense_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateReport, 'DD-MM-YYYY')),'') ) END) AS S_TimeExpense_IDR, " +
      ...
      "(CASE WHEN S_TimeExpenseLine_V.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "S_TimeExpenseLine_V.Qty, " +
      "S_TimeExpenseLine_V.C_Currency_ID, " +
      "(CASE WHEN S_TimeExpenseLine_V.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "S_TimeExpenseLine_V.C_UOM_ID, " +
      "(CASE WHEN S_TimeExpenseLine_V.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      ...
      "(CASE WHEN S_TimeExpenseLine_V.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "S_TimeExpenseLine_V.TEL_C_Bpartner_ID, " +
      "(CASE WHEN S_TimeExpenseLine_V.TEL_C_Bpartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS TEL_C_Bpartner_IDR, " +
      "S_TimeExpenseLine_V.S_ResourceAssignment_ID, " +
      "S_TimeExpenseLine_V.C_ProjectPhase_ID, " +
      "S_TimeExpenseLine_V.C_ProjectTask_ID, " +
      "(CASE WHEN S_TimeExpenseLine_V.C_ProjectTask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS C_ProjectTask_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM S_TimeExpenseLine_V left join (select AD_Org_ID, Name from AD_Org) table1 on (S_TimeExpenseLine_V.AD_Org_ID = table1.AD_Org_ID) left join (select S_TimeExpense_ID, DocumentNo, C_BPartner_ID, DateReport from S_TimeExpense) table2 on (S_TimeExpenseLine_V.S_TimeExpense_ID = table2.S_TimeExpense_ID) left join (select C_BPartner_ID, Name from C_BPartner) table3 on (table2.C_BPartner_ID =  table3.C_BPartner_ID) left join (select M_Product_ID, Value, Name from M_Product) table4 on (S_TimeExpenseLine_V.M_Product_ID = table4.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL4 on (table4.M_Product_ID = tableTRL4.M_Product_ID and tableTRL4.AD_Language = ?)  left join (select C_Currency_ID, ISO_Code from C_Currency) table5 on (S_TimeExpenseLine_V.C_Currency_ID = table5.C_Currency_ID) left join (select C_UOM_ID, Name from C_UOM) table6 on (S_TimeExpenseLine_V.C_UOM_ID = table6.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL6 on (table6.C_UOM_ID = tableTRL6.C_UOM_ID and tableTRL6.AD_Language = ?)  left join (select C_Project_ID, Value, Name from C_Project) table7 on (S_TimeExpenseLine_V.C_Project_ID = table7.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table8 on (S_TimeExpenseLine_V.TEL_C_Bpartner_ID = table8.C_BPartner_ID) left join (select C_ProjectTask_ID, C_Project_ID, Name from C_ProjectTask) table9 on (S_TimeExpenseLine_V.C_ProjectTask_ID = table9.C_ProjectTask_ID) left join (select C_Project_ID, Value, Name from C_Project) table10 on (table9.C_Project_ID = table10.C_Project_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String cBpartnerId, String invoiceprice, String cProjectphaseId, String dateexpense, String cCurrencyId, String cInvoicelineId, String telCBpartnerId, String telCBpartnerIdr, String cActivityId, String updatedby, String updatedbyr, String note, String adOrgId, String cProjecttaskId, String cCampaignId, String isinvoiced, String qty, String convertedamt, String sTimeexpenseId, String createdby, String createdbyr, String adClientId, String line, String mProductId, String mProductIdr, String datereport, String expenseamt, String description, String sTimeexpenselineId, String cUomId, String cOrderlineId, String istimereport, String isactive, String sTimetypeId, String sResourceassignmentId, String cProjectId)    throws ServletException {
 ...
 */
  public static String selectDefD463F52ACB6611DD87FACF0742499ECD_0(ConnectionProvider connectionProvider, String TEL_C_Bpartner_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT  ( COALESCE(TO_CHAR(table1.Value), '')  || ' - ' || COALESCE(TO_CHAR(table1.Name), '') ) as TEL_C_Bpartner_ID FROM C_BPartner table1 WHERE table1.isActive='Y' AND table1.C_BPartner_ID = ?  ";
