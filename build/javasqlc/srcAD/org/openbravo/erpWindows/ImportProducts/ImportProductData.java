 ...
 */
  public static ImportProductData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "I_Product.AD_Org_ID, " +
      "(CASE WHEN I_Product.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "I_Product.Value, " +
      "I_Product.Name, " +
      ...
      "I_Product.X12DE355, " +
      "I_Product.C_UOM_ID, " +
      "(CASE WHEN I_Product.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "I_Product.ProductCategory_Value, " +
      "I_Product.M_Product_Category_ID, " +
      "(CASE WHEN I_Product.M_Product_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS M_Product_Category_IDR, " +
      ...
      "(CASE WHEN I_Product.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "I_Product.ISO_Code, " +
      "I_Product.C_Currency_ID, " +
      "(CASE WHEN I_Product.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "I_Product.PriceList, " +
      "I_Product.PricePO, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM I_Product left join (select M_Product_ID, Value, Name from M_Product) table1 on (I_Product.M_Product_ID = table1.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL1 on (table1.M_Product_ID = tableTRL1.M_Product_ID and tableTRL1.AD_Language = ?)  left join (select AD_Org_ID, Name from AD_Org) table2 on (I_Product.AD_Org_ID = table2.AD_Org_ID) left join (select C_UOM_ID, Name from C_UOM) table3 on (I_Product.C_UOM_ID = table3.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL3 on (table3.C_UOM_ID = tableTRL3.C_UOM_ID and tableTRL3.AD_Language = ?)  left join (select M_Product_Category_ID, Name from M_Product_Category) table4 on (I_Product.M_Product_Category_ID = table4.M_Product_Category_ID) left join ad_ref_list_v list1 on (I_Product.ProductType = list1.value and list1.ad_reference_id = '270' and list1.ad_language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table5 on (I_Product.C_BPartner_ID = table5.C_BPartner_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table6 on (I_Product.C_Currency_ID = table6.C_Currency_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static ImportProductData[] set(String pricelist, String iIsimported, String adClientId, String name, String orderMin, String vendorcategory, String discontinuedby, String unitsperpallet, String processing, String cUomId, String mProductId, String cBpartnerId, String cBpartnerIdr, String descriptionurl, String help, String bpartnerValue, String shelfdepth, String isoCode, String shelfheight, String isactive, String volume, String orderPack, String weight, String deliverytimePromised, String sku, String shelfwidth, String royaltyamt, String cCurrencyId, String manufacturer, String productcategoryValue, String value, String vendorproductno, String upc, String iProductId, String processed, String classification, String documentnote, String adOrgId, String producttype, String pricepo, String imageurl, String x12de355, String mProductCategoryId, String costperorder, String priceeffective, String description, String createdby, String createdbyr, String iErrormsg, String discontinued, String updatedby, String updatedbyr)    throws ServletException {
