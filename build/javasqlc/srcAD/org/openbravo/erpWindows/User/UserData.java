 ...
 */
  public static UserData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "AD_User.AD_Org_ID, " +
      "(CASE WHEN AD_User.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_User.Name, " +
      "AD_User.Description, " +
      "AD_User.Comments, " +
      "COALESCE(AD_User.IsActive, 'N') AS IsActive, " +
      "AD_User.C_BPartner_ID, " +
      "(CASE WHEN AD_User.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "AD_User.UserName, " +
      "AD_User.Password, " +
      "AD_User.AD_OrgTrx_ID, " +
      "AD_User.Default_Ad_Role_ID, " +
      "(CASE WHEN AD_User.Default_Ad_Role_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS Default_Ad_Role_IDR, " +
      "AD_User.Default_Ad_Language, " +
      "(CASE WHEN AD_User.Default_Ad_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS Default_Ad_LanguageR, " +
      "AD_User.Default_Ad_Client_ID, " +
      "(CASE WHEN AD_User.Default_Ad_Client_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS Default_Ad_Client_IDR, " +
      "AD_User.Default_Ad_Org_ID, " +
      "(CASE WHEN AD_User.Default_Ad_Org_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS Default_Ad_Org_IDR, " +
      "AD_User.Default_M_Warehouse_ID, " +
      "(CASE WHEN AD_User.Default_M_Warehouse_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS Default_M_Warehouse_IDR, " +
      "AD_User.Phone2, " +
      "AD_User.Birthday, " +
      ...
      "AD_User.AD_User_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_User left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_User.AD_Org_ID = table1.AD_Org_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table2 on (AD_User.C_BPartner_ID = table2.C_BPartner_ID) left join (select AD_Role_ID, Name from AD_Role) table3 on (AD_User.Default_Ad_Role_ID =  table3.AD_Role_ID) left join (select AD_Language, Name from AD_Language) table4 on (AD_User.Default_Ad_Language =  table4.AD_Language) left join (select AD_Client_ID, Name from AD_Client) table5 on (AD_User.Default_Ad_Client_ID =  table5.AD_Client_ID) left join (select AD_Org_ID, Name from AD_Org) table6 on (AD_User.Default_Ad_Org_ID =  table6.AD_Org_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table7 on (AD_User.Default_M_Warehouse_ID =  table7.M_Warehouse_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static UserData[] set(String cBpartnerLocationId, String defaultAdOrgId, String username, String email, String emailuser, String emailuserpw, String isactive, String adOrgId, String supervisorId, String fax, String password, String adClientId, String defaultMWarehouseId, String cGreetingId, String title, String firstname, String lastcontact, String comments, String phone, String lastname, String defaultAdLanguage, String defaultAdRoleId, String updatedby, String updatedbyr, String birthday, String processing, String adOrgtrxId, String name, String description, String cBpartnerId, String lastresult, String createdby, String createdbyr, String adUserId, String phone2, String defaultAdClientId)    throws ServletException {
