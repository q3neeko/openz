 ...
 */
  public static FormData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "AD_Form.AD_Org_ID, " +
      "(CASE WHEN AD_Form.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Form.AD_Module_ID, " +
      "(CASE WHEN AD_Form.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Form.Name, " +
      "AD_Form.Description, " +
      "AD_Form.Help, " +
      ...
      "(CASE WHEN AD_Form.AD_Toolbar_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'') ) END) AS AD_Toolbar_IDR, " +
      "AD_Form.AD_Form_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Form left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Form.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Form.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join ad_ref_list_v list1 on (AD_Form.AccessLevel = list1.value and list1.ad_reference_id = '5' and list1.ad_language = ?)  left join (select AD_Toolbar_ID, Value from AD_Toolbar) table4 on (AD_Form.AD_Toolbar_ID = table4.AD_Toolbar_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static FormData[] set(String isactive, String name, String description, String accesslevel, String adFormId, String adToolbarId, String adClientId, String classname, String adModuleId, String createdby, String createdbyr, String updatedby, String updatedbyr, String adOrgId, String help)    throws ServletException {
