 ...
 */
  public static ProceduresData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adExtensionPointsId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_EP_Procedures.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_EP_Procedures.AD_Extension_Points_ID, " +
      "(CASE WHEN AD_EP_Procedures.AD_Extension_Points_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Extension_Points_IDR, " +
      "AD_EP_Procedures.AD_Module_ID, " +
      "(CASE WHEN AD_EP_Procedures.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "COALESCE(AD_EP_Procedures.Isactive, 'N') AS Isactive, " +
      "AD_EP_Procedures.Procedurename, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_EP_Procedures left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_EP_Procedures.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Extension_Points_ID, Name from AD_Extension_Points) table2 on (AD_EP_Procedures.AD_Extension_Points_ID = table2.AD_Extension_Points_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table3 on (AD_EP_Procedures.AD_Module_ID = table3.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table4 on (table3.AD_Language =  table4.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
