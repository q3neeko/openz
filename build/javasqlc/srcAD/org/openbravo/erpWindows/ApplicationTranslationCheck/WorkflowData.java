 ...
 */
  public static WorkflowData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Workflow_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Workflow_Trl.AD_Workflow_ID, " +
      "(CASE WHEN AD_Workflow_Trl.AD_Workflow_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Workflow_IDR, " +
      "AD_Workflow_Trl.AD_Language, " +
      "(CASE WHEN AD_Workflow_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Workflow_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Workflow_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Workflow_ID, Name from AD_Workflow) table2 on (AD_Workflow_Trl.AD_Workflow_ID = table2.AD_Workflow_ID) left join (select AD_Workflow_ID,AD_Language, Name from AD_Workflow_TRL) tableTRL2 on (table2.AD_Workflow_ID = tableTRL2.AD_Workflow_ID and tableTRL2.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table3 on (AD_Workflow_Trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static WorkflowData[] set(String adLanguage, String help, String isactive, String istranslated, String adOrgId, String description, String createdby, String createdbyr, String name, String updatedby, String updatedbyr, String adWorkflowTrlId, String adClientId, String adWorkflowId)    throws ServletException {
