 ...
 */
  public static BOM8AE923EC99AA4BFA9A0DB84A6A751079Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String caAssetsstockedId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN CA_ASSETSSTOCKEDBOM.CA_Assetsstocked_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Qty), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Serialnumber), ''))),'') ) END) AS CA_Assetsstocked_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET CA_Assetsstocked_ID = (?) , M_Product_ID = (?) , Price = TO_NUMBER(?) , Qty = TO_NUMBER(?) , Amt = TO_NUMBER(?) , Description = (?) , Bomqty = TO_NUMBER(?) , CA_Assetsstockedbom_ID = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , Dateofsale = TO_DATE(?) , Isactive = (?) , updated = now(), updatedby = ? " +
      "        WHERE CA_ASSETSSTOCKEDBOM.CA_Assetsstockedbom_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (CA_Assetsstocked_ID, M_Product_ID, Price, Qty, Amt, Description, Bomqty, CA_Assetsstockedbom_ID, AD_Client_ID, AD_Org_ID, Dateofsale, Isactive, created, createdby, updated, updatedBy)" +
