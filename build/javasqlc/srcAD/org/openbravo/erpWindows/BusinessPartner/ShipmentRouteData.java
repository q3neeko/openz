 ...
 */
  public static ShipmentRouteData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBpartnerId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Warehouse_Shipper.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Warehouse_Shipper left join (select C_BPartner_ID, Value, Name from C_BPartner) table1 on (M_Warehouse_Shipper.C_BPartner_ID = table1.C_BPartner_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table2 on (M_Warehouse_Shipper.M_Warehouse_ID = table2.M_Warehouse_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
