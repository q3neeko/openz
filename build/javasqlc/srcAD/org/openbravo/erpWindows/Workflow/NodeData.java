 ...
 */
  public static NodeData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adWorkflowId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_WF_Node.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_WF_Node.AD_Workflow_ID, " +
      "(CASE WHEN AD_WF_Node.AD_Workflow_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Workflow_IDR, " +
      "AD_WF_Node.Name, " +
      ...
      "(CASE WHEN AD_WF_Node.AD_Window_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS AD_Window_IDR, " +
      "AD_WF_Node.Workflow_ID, " +
      "(CASE WHEN AD_WF_Node.Workflow_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS Workflow_IDR, " +
      "AD_WF_Node.AD_Process_ID, " +
      "(CASE WHEN AD_WF_Node.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      "AD_WF_Node.AD_Task_ID, " +
      "(CASE WHEN AD_WF_Node.AD_Task_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS AD_Task_IDR, " +
      "AD_WF_Node.AD_Form_ID, " +
      "(CASE WHEN AD_WF_Node.AD_Form_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS AD_Form_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_WF_Node left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_WF_Node.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Workflow_ID, Name from AD_Workflow) table2 on (AD_WF_Node.AD_Workflow_ID = table2.AD_Workflow_ID) left join (select AD_Workflow_ID,AD_Language, Name from AD_Workflow_TRL) tableTRL2 on (table2.AD_Workflow_ID = tableTRL2.AD_Workflow_ID and tableTRL2.AD_Language = ?)  left join ad_ref_list_v list1 on (AD_WF_Node.Action = list1.value and list1.ad_reference_id = '104' and list1.ad_language = ?)  left join (select AD_Window_ID, Name from AD_Window) table3 on (AD_WF_Node.AD_Window_ID = table3.AD_Window_ID) left join (select AD_Window_ID,AD_Language, Name from AD_Window_TRL) tableTRL3 on (table3.AD_Window_ID = tableTRL3.AD_Window_ID and tableTRL3.AD_Language = ?)  left join (select AD_Workflow_ID, Name from AD_Workflow) table4 on (AD_WF_Node.Workflow_ID =  table4.AD_Workflow_ID) left join (select AD_Workflow_ID,AD_Language, Name from AD_Workflow_TRL) tableTRL4 on (table4.AD_Workflow_ID = tableTRL4.AD_Workflow_ID and tableTRL4.AD_Language = ?)  left join (select AD_Process_ID, Name from AD_Process) table5 on (AD_WF_Node.AD_Process_ID = table5.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL5 on (table5.AD_Process_ID = tableTRL5.AD_Process_ID and tableTRL5.AD_Language = ?)  left join (select AD_Task_ID, Name from AD_Task) table6 on (AD_WF_Node.AD_Task_ID = table6.AD_Task_ID) left join (select AD_Task_ID,AD_Language, Name from AD_Task_TRL) tableTRL6 on (table6.AD_Task_ID = tableTRL6.AD_Task_ID and tableTRL6.AD_Language = ?)  left join (select AD_Form_ID, Name from AD_Form) table7 on (AD_WF_Node.AD_Form_ID = table7.AD_Form_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static NodeData[] set(String adWorkflowId, String description, String help, String action, String workflowId, String adProcessId, String iscentrallymaintained, String xposition, String adWfNodeId, String adWindowId, String yposition, String isactive, String adFormId, String updatedby, String updatedbyr, String name, String createdby, String createdbyr, String adTaskId, String adOrgId, String adClientId)    throws ServletException {
