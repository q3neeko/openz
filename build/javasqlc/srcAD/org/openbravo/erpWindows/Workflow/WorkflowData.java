 ...
 */
  public static WorkflowData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Workflow.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Workflow.AD_Module_ID, " +
      "(CASE WHEN AD_Workflow.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Workflow.Name, " +
      ...
      "(CASE WHEN AD_Workflow.AD_WF_Node_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_WF_Node_IDR, " +
      "AD_Workflow.AD_Workflow_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Workflow left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Workflow.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_Workflow.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language) left join ad_ref_list_v list1 on (AD_Workflow.AccessLevel = list1.value and list1.ad_reference_id = '5' and list1.ad_language = ?)  left join (select AD_WF_Node_ID, Name from AD_WF_Node) table4 on (AD_Workflow.AD_WF_Node_ID = table4.AD_WF_Node_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static WorkflowData[] set(String help, String name, String isactive, String createdby, String createdbyr, String accesslevel, String adWorkflowId, String adOrgId, String adWfNodeId, String adModuleId, String updatedby, String updatedbyr, String adClientId, String description)    throws ServletException {
