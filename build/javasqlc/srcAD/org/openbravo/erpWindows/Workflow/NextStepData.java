 ...
 */
  public static NextStepData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adWfNodeId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_WF_NodeNext.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_WF_NodeNext.AD_WF_Node_ID, " +
      "(CASE WHEN AD_WF_NodeNext.AD_WF_Node_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_WF_Node_IDR, " +
      "AD_WF_NodeNext.AD_WF_Next_ID, " +
      "(CASE WHEN AD_WF_NodeNext.AD_WF_Next_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_WF_Next_IDR, " +
      "COALESCE(AD_WF_NodeNext.IsActive, 'N') AS IsActive, " +
      "AD_WF_NodeNext.Condition, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_WF_NodeNext left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_WF_NodeNext.AD_Org_ID = table1.AD_Org_ID) left join (select AD_WF_Node_ID, Name from AD_WF_Node) table2 on (AD_WF_NodeNext.AD_WF_Node_ID = table2.AD_WF_Node_ID) left join (select AD_WF_Node_ID, Name from AD_WF_Node) table3 on (AD_WF_NodeNext.AD_WF_Next_ID =  table3.AD_WF_Node_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static NextStepData[] set(String adWfNodeId, String adWfNodenextId, String isactive, String adClientId, String condition, String adOrgId, String adWfNextId, String updatedby, String updatedbyr, String createdby, String createdbyr)    throws ServletException {
