 ...
 */
  public static ProcessSchedulingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Process_Scheduling.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Process_Scheduling.IsActive, 'N') AS IsActive, " +
      "AD_Process_Scheduling.AD_Process_ID, " +
      "(CASE WHEN AD_Process_Scheduling.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Process_Scheduling left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Process_Scheduling.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Process_ID, Name from AD_Process) table2 on (AD_Process_Scheduling.AD_Process_ID = table2.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL2 on (table2.AD_Process_ID = tableTRL2.AD_Process_ID and tableTRL2.AD_Language = ?)  left join ad_ref_list_v list1 on (AD_Process_Scheduling.WeekDay = list1.value and list1.ad_reference_id = '167' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static ProcessSchedulingData[] set(String adProcessSchedulingId, String isactive, String timefrom, String updatedby, String updatedbyr, String createdby, String createdbyr, String description, String weekday, String isfulltime, String adClientId, String adOrgId, String adProcessId, String timeto)    throws ServletException {
