 ...
 */
  public static InternalMaterialMovementsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Internal_Consumption.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "M_Internal_Consumption.C_Project_ID, " +
      "(CASE WHEN M_Internal_Consumption.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "M_Internal_Consumption.C_Projecttask_ID, " +
      "(CASE WHEN M_Internal_Consumption.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Internal_Consumption left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Internal_Consumption.AD_Org_ID = table1.AD_Org_ID) left join ad_ref_list_v list1 on (M_Internal_Consumption.Movementtype = list1.value and list1.ad_reference_id = '362FC668AFAA4120A2E7F9B6A22FAE4C' and list1.ad_language = ?)  left join (select A_Asset_ID, Name from A_Asset) table2 on (M_Internal_Consumption.A_Asset_ID = table2.A_Asset_ID) left join (select C_Project_ID, Value, Name from C_Project) table3 on (M_Internal_Consumption.C_Project_ID = table3.C_Project_ID) left join (select C_Projecttask_ID, C_Project_ID, Name from C_Projecttask) table4 on (M_Internal_Consumption.C_Projecttask_ID = table4.C_Projecttask_ID) left join (select C_Project_ID, Value, Name from C_Project) table5 on (table4.C_Project_ID = table5.C_Project_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static InternalMaterialMovementsData[] set(String processing, String movementdate, String mInternalConsumptionId, String aAssetId, String copyfromprojecttask, String name, String movementtype, String processed, String description, String adOrgId, String cProjectId, String cProjectIdr, String updatedby, String updatedbyr, String cProjectphaseId, String adClientId, String createdby, String createdbyr, String isactive, String documentno, String cProjecttaskId, String dateacct, String posted)    throws ServletException {
