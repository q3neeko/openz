 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mInternalConsumptionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Internal_ConsumptionLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_Internal_ConsumptionLine.M_Internal_Consumption_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.M_Internal_Consumption_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_Internal_Consumption_IDR, " +
      ...
      "(CASE WHEN M_Internal_ConsumptionLine.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_Internal_ConsumptionLine.M_AttributeSetInstance_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.M_AttributeSetInstance_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Description), ''))),'') ) END) AS M_AttributeSetInstance_IDR, " +
      "M_Internal_ConsumptionLine.M_Locator_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      "M_Internal_ConsumptionLine.QuantityOrder, " +
      "M_Internal_ConsumptionLine.M_Product_Uom_Id, " +
      "(CASE WHEN M_Internal_ConsumptionLine.M_Product_Uom_Id IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL7.Name IS NULL THEN TO_CHAR(table7.Name) ELSE TO_CHAR(tableTRL7.Name) END)), ''))),'') ) END) AS M_Product_Uom_IdR, " +
      "M_Internal_ConsumptionLine.MovementQty, " +
      "M_Internal_ConsumptionLine.C_UOM_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL8.Name IS NULL THEN TO_CHAR(table8.Name) ELSE TO_CHAR(tableTRL8.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "M_Internal_ConsumptionLine.A_Asset_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "M_Internal_ConsumptionLine.C_Project_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "M_Internal_ConsumptionLine.C_Projectphase_ID, " +
      "M_Internal_ConsumptionLine.C_Projecttask_ID, " +
      "(CASE WHEN M_Internal_ConsumptionLine.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "COALESCE(M_Internal_ConsumptionLine.Cost2project, 'N') AS Cost2project, " +
      "M_Internal_ConsumptionLine.Reinvoicedby_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM M_Internal_ConsumptionLine left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Internal_ConsumptionLine.AD_Org_ID = table1.AD_Org_ID) left join (select M_Internal_Consumption_ID, Name from M_Internal_Consumption) table2 on (M_Internal_ConsumptionLine.M_Internal_Consumption_ID = table2.M_Internal_Consumption_ID) left join (select M_Product_ID, Value, Name from M_Product) table3 on (M_Internal_ConsumptionLine.M_Product_ID = table3.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL3 on (table3.M_Product_ID = tableTRL3.M_Product_ID and tableTRL3.AD_Language = ?)  left join (select M_AttributeSetInstance_ID, Description from M_AttributeSetInstance) table4 on (M_Internal_ConsumptionLine.M_AttributeSetInstance_ID = table4.M_AttributeSetInstance_ID) left join (select M_Locator_ID, Value from M_Locator) table5 on (M_Internal_ConsumptionLine.M_Locator_ID = table5.M_Locator_ID) left join (select M_Product_Uom_Id, C_UOM_ID from M_Product_Uom) table6 on (M_Internal_ConsumptionLine.M_Product_Uom_Id = table6.M_Product_Uom_Id) left join (select C_UOM_ID, Name from C_UOM) table7 on (table6.C_UOM_ID = table7.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL7 on (table7.C_UOM_ID = tableTRL7.C_UOM_ID and tableTRL7.AD_Language = ?)  left join (select C_UOM_ID, Name from C_UOM) table8 on (M_Internal_ConsumptionLine.C_UOM_ID = table8.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL8 on (table8.C_UOM_ID = tableTRL8.C_UOM_ID and tableTRL8.AD_Language = ?)  left join (select A_Asset_ID, Name from A_Asset) table9 on (M_Internal_ConsumptionLine.A_Asset_ID = table9.A_Asset_ID) left join (select C_Project_ID, Value, Name from C_Project) table10 on (M_Internal_ConsumptionLine.C_Project_ID = table10.C_Project_ID) left join (select C_Projecttask_ID, C_Project_ID, Name from C_Projecttask) table11 on (M_Internal_ConsumptionLine.C_Projecttask_ID = table11.C_Projecttask_ID) left join (select C_Project_ID, Value, Name from C_Project) table12 on (table11.C_Project_ID = table12.C_Project_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String mInternalConsumptionId, String cProjecttaskId, String mProductId, String mProductIdr, String cProjectId, String cProjectIdr, String adOrgId, String cUomId, String isactive, String description, String quantityorder, String mInternalConsumptionlineId, String mLocatorId, String mLocatorIdr, String reinvoicedbyId, String cost2project, String line, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String adClientId, String aAssetId, String movementqty, String createdby, String createdbyr, String cProjectphaseId, String updatedby, String updatedbyr, String mProductUomId)    throws ServletException {
