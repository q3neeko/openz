 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Movement.AD_OrgTrx_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_OrgTrx_IDR, " +
      "M_Movement.User1_ID, " +
      "M_Movement.User2_ID, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Movement left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Movement.AD_Org_ID = table1.AD_Org_ID) left join (select C_Project_ID, Value, Name from C_Project) table2 on (M_Movement.C_Project_ID = table2.C_Project_ID) left join (select C_Activity_ID, Name from C_Activity) table3 on (M_Movement.C_Activity_ID = table3.C_Activity_ID) left join (select C_Campaign_ID, Name from C_Campaign) table4 on (M_Movement.C_Campaign_ID = table4.C_Campaign_ID) left join (select AD_Org_ID, value, Name from AD_Org) table5 on (M_Movement.AD_OrgTrx_ID =  table5.AD_Org_ID) left join ad_ref_list_v list1 on (list1.ad_reference_id = '234' and list1.ad_language = ?  AND M_Movement.Posted = TO_CHAR(list1.value))" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static HeaderData[] set(String adOrgId, String description, String user2Id, String cCampaignId, String posted, String postedBtn, String processed, String adOrgtrxId, String moveFromtoLocator, String mMovementId, String adClientId, String isactive, String processing, String updatedby, String updatedbyr, String cProjectId, String cProjectIdr, String createdby, String createdbyr, String movementdate, String documentno, String name, String cActivityId, String user1Id)    throws ServletException {
