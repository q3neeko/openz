 ...
 */
  public static ReversedInvoicesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cInvoiceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Invoice_Reverse.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Invoice_Reverse.Reversed_C_Invoice_ID, " +
      "(CASE WHEN C_Invoice_Reverse.Reversed_C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.GrandTotal), ''))),'') ) END) AS Reversed_C_Invoice_IDR, " +
      "C_Invoice_Reverse.C_Invoice_ID, " +
      "(CASE WHEN C_Invoice_Reverse.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table3.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "COALESCE(C_Invoice_Reverse.Isactive, 'N') AS Isactive, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Invoice_Reverse left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Invoice_Reverse.AD_Org_ID = table1.AD_Org_ID) left join (select C_Invoice_ID, DocumentNo, DateInvoiced, GrandTotal from C_Invoice) table2 on (C_Invoice_Reverse.Reversed_C_Invoice_ID = table2.C_Invoice_ID) left join (select C_Invoice_ID, DocumentNo, DateInvoiced, GrandTotal from C_Invoice) table3 on (C_Invoice_Reverse.C_Invoice_ID = table3.C_Invoice_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ReversedInvoicesData[] set(String cInvoiceId, String adClientId, String isactive, String reversedCInvoiceId, String reversedCInvoiceIdr, String createdby, String createdbyr, String cInvoiceReverseId, String updatedby, String updatedbyr, String adOrgId)    throws ServletException {
