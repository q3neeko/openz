 ...
 */
  public static TaxData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cInvoiceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_InvoiceTax.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_InvoiceTax.C_Invoice_ID, " +
      "(CASE WHEN C_InvoiceTax.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "C_InvoiceTax.C_InvoiceTax_ID, " +
      "C_InvoiceTax.C_Tax_ID, " +
      "(CASE WHEN C_InvoiceTax.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "C_InvoiceTax.TaxAmt, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_InvoiceTax left join (select AD_Org_ID, Name from AD_Org) table1 on (C_InvoiceTax.AD_Org_ID = table1.AD_Org_ID) left join (select C_Invoice_ID, DocumentNo, DateInvoiced, GrandTotal from C_Invoice) table2 on (C_InvoiceTax.C_Invoice_ID = table2.C_Invoice_ID) left join (select C_Tax_ID, Name from C_Tax) table3 on (C_InvoiceTax.C_Tax_ID = table3.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL3 on (table3.C_Tax_ID = tableTRL3.C_Tax_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TaxData[] set(String cInvoiceId, String taxbaseamt, String createdby, String createdbyr, String adOrgId, String cInvoicetaxId, String reversetaxamt, String adClientId, String line, String recalculate, String cInvoiceIdr, String updatedby, String updatedbyr, String cTaxId, String isactive, String taxamt)    throws ServletException {
