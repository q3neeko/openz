 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cInvoiceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_InvoiceLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_InvoiceLine.C_Invoice_ID, " +
      "(CASE WHEN C_InvoiceLine.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "COALESCE(C_InvoiceLine.Ispagebreak, 'N') AS Ispagebreak, " +
      ...
      "(CASE WHEN C_InvoiceLine.M_InOutLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table6.MovementDate, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL7.Name IS NULL THEN TO_CHAR(table7.Name) ELSE TO_CHAR(tableTRL7.Name) END)), ''))),'') ) END) AS M_InOutLine_IDR, " +
      "C_InvoiceLine.Textposition, " +
      "C_InvoiceLine.M_Product_ID, " +
      "(CASE WHEN C_InvoiceLine.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL8.Name IS NULL THEN TO_CHAR(table8.Name) ELSE TO_CHAR(tableTRL8.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "C_InvoiceLine.M_AttributeSetInstance_ID, " +
      "(CASE WHEN C_InvoiceLine.M_AttributeSetInstance_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Description), ''))),'') ) END) AS M_AttributeSetInstance_IDR, " +
      "C_InvoiceLine.Description, " +
      "C_InvoiceLine.QuantityOrder, " +
      "C_InvoiceLine.M_Product_Uom_Id, " +
      "(CASE WHEN C_InvoiceLine.M_Product_Uom_Id IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL11.Name IS NULL THEN TO_CHAR(table11.Name) ELSE TO_CHAR(tableTRL11.Name) END)), ''))),'') ) END) AS M_Product_Uom_IdR, " +
      "C_InvoiceLine.QtyInvoiced, " +
      "C_InvoiceLine.C_UOM_ID, " +
      "(CASE WHEN C_InvoiceLine.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL12.Name IS NULL THEN TO_CHAR(table12.Name) ELSE TO_CHAR(tableTRL12.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      ...
      "(CASE WHEN C_InvoiceLine.C_Tax_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL13.Name IS NULL THEN TO_CHAR(table13.Name) ELSE TO_CHAR(tableTRL13.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      ...
      "(CASE WHEN C_InvoiceLine.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table15.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
...
Create a registry
 */
  public static LinesData[] set(String cInvoiceId, String cUomId, String pricelimit, String cInvoiceIdr, String quantityorder, String mOfferId, String description, String pricestd, String adOrgId, String line, String pricelist, String cChargeId, String created, String sResourceassignmentId, String linegrossamt, String linenetamt, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String reinvoicedbyId, String cOrderlineId, String chargeamt, String cProjectphaseId, String textposition, String mProductUomId, String updatedby, String updatedbyr, String excludeforwithholding, String isactive, String linetaxamt, String cProjecttaskId, String cInvoicelineId, String isgrossprice, String adClientId, String mInoutlineId, String mInoutlineIdr, String cProjectId, String cProjectIdr, String aAssetId, String isdescription, String mProductId, String mProductIdr, String taxamt, String cTaxId, String ispagebreak, String createdby, String createdbyr, String qtyinvoiced, String priceactual)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_Invoice_ID = (?) , Ispagebreak = (?) , C_OrderLine_ID = (?) , Line = TO_NUMBER(?) , M_InOutLine_ID = (?) , Textposition = (?) , M_Product_ID = (?) , M_AttributeSetInstance_ID = (?) , Description = (?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , QtyInvoiced = TO_NUMBER(?) , C_UOM_ID = (?) , PriceActual = TO_NUMBER(?) , Isgrossprice = (?) , PriceList = TO_NUMBER(?) , C_Tax_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , LineNetAmt = TO_NUMBER(?) , Linegrossamt = TO_NUMBER(?) , C_Charge_ID = (?) , M_Offer_ID = (?) , PriceStd = TO_NUMBER(?) , IsDescription = (?) , Reinvoicedby_ID = (?) , IsActive = (?) , A_Asset_ID = (?) , ChargeAmt = TO_NUMBER(?) , Excludeforwithholding = (?) , PriceLimit = TO_NUMBER(?) , C_Projectphase_ID = (?) , S_ResourceAssignment_ID = (?) , TaxAmt = TO_NUMBER(?) , C_InvoiceLine_ID = (?) , Linetaxamt = TO_NUMBER(?) , updated = now(), updatedby = ? " +
      "        WHERE C_InvoiceLine.C_InvoiceLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, C_Invoice_ID, Ispagebreak, C_OrderLine_ID, Line, M_InOutLine_ID, Textposition, M_Product_ID, M_AttributeSetInstance_ID, Description, QuantityOrder, M_Product_Uom_Id, QtyInvoiced, C_UOM_ID, PriceActual, Isgrossprice, PriceList, C_Tax_ID, C_Project_ID, C_Projecttask_ID, LineNetAmt, Linegrossamt, ChargeAmt, C_Projectphase_ID, S_ResourceAssignment_ID, TaxAmt, C_InvoiceLine_ID, IsActive, Linetaxamt, C_Charge_ID, M_Offer_ID, PriceStd, IsDescription, A_Asset_ID, Reinvoicedby_ID, PriceLimit, Excludeforwithholding, created, createdby, updated, updatedBy)" +
