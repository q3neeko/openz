 ...
 */
  public static ProcessClassData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adProcessId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Model_Object.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_Model_Object.IsActive, 'N') AS IsActive, " +
      "AD_Model_Object.AD_Process_ID, " +
      "(CASE WHEN AD_Model_Object.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Model_Object left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table1 on (AD_Model_Object.AD_Module_ID = table1.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table2 on (table1.AD_Language =  table2.AD_Language) left join (select AD_Org_ID, Name from AD_Org) table3 on (AD_Model_Object.AD_Org_ID = table3.AD_Org_ID) left join (select AD_Process_ID, Name from AD_Process) table4 on (AD_Model_Object.AD_Process_ID = table4.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL4 on (table4.AD_Process_ID = tableTRL4.AD_Process_ID and tableTRL4.AD_Language = ?)  left join ad_ref_list_v list1 on (AD_Model_Object.Action = list1.value and list1.ad_reference_id = '104' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static ProcessClassData[] set(String adProcessId, String adTaskId, String adTabId, String adWorkflowId, String adFormId, String adReferenceId, String action, String isactive, String adClientId, String isdefault, String adCalloutId, String adModuleId, String classname, String adModelObjectId, String adOrgId, String updatedby, String updatedbyr, String objectType, String loadonstartup, String createdby, String createdbyr, String name, String seqno)    throws ServletException {
