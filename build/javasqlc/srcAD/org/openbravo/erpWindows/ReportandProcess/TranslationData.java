 ...
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adProcessId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Process_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Process_Trl.AD_Process_ID, " +
      "(CASE WHEN AD_Process_Trl.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      "AD_Process_Trl.AD_Language, " +
      "(CASE WHEN AD_Process_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "AD_Process_Trl.Help, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Process_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Process_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Process_ID, Name from AD_Process) table2 on (AD_Process_Trl.AD_Process_ID = table2.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL2 on (table2.AD_Process_ID = tableTRL2.AD_Process_ID and tableTRL2.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table3 on (AD_Process_Trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TranslationData[] set(String adProcessId, String name, String updatedby, String updatedbyr, String adClientId, String description, String createdby, String createdbyr, String adOrgId, String adLanguage, String adProcessTrlId, String help, String isactive, String istranslated)    throws ServletException {
