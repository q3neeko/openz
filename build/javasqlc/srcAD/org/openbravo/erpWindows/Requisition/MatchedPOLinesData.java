 ...
 */
  public static MatchedPOLinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mRequisitionlineId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_RequisitionOrder.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_RequisitionOrder.M_Requisitionline_ID, " +
      "(CASE WHEN M_RequisitionOrder.M_Requisitionline_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Qty), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.Needbydate, 'DD-MM-YYYY')),'') ) END) AS M_Requisitionline_IDR, " +
      "COALESCE(M_RequisitionOrder.IsActive, 'N') AS IsActive, " +
      "M_RequisitionOrder.C_OrderLine_ID, " +
      "(CASE WHEN M_RequisitionOrder.C_OrderLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table6.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.GrandTotal), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.LineNetAmt), ''))),'') ) END) AS C_OrderLine_IDR, " +
      "M_RequisitionOrder.Qty, " +
      "        ? AS LANGUAGE " +
      "        FROM M_RequisitionOrder left join (select AD_Org_ID, Name from AD_Org) table1 on (M_RequisitionOrder.AD_Org_ID = table1.AD_Org_ID) left join (select M_Requisitionline_ID, M_Requisition_ID, M_Product_ID, Qty, Needbydate from M_Requisitionline) table2 on (M_RequisitionOrder.M_Requisitionline_ID = table2.M_Requisitionline_ID) left join (select M_Requisition_ID, DocumentNo from M_Requisition) table3 on (table2.M_Requisition_ID = table3.M_Requisition_ID) left join (select M_Product_ID, Value, Name from M_Product) table4 on (table2.M_Product_ID = table4.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL4 on (table4.M_Product_ID = tableTRL4.M_Product_ID and tableTRL4.AD_Language = ?)  left join (select C_OrderLine_ID, C_Order_ID, Line, LineNetAmt from C_OrderLine) table5 on (M_RequisitionOrder.C_OrderLine_ID = table5.C_OrderLine_ID) left join (select C_Order_ID, DocumentNo, Name, DateOrdered, GrandTotal from C_Order) table6 on (table5.C_Order_ID = table6.C_Order_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static MatchedPOLinesData[] set(String mRequisitionlineId, String cOrderlineId, String cOrderlineIdr, String qty, String isactive, String updatedby, String updatedbyr, String createdby, String createdbyr, String mRequisitionorderId, String adClientId, String adOrgId)    throws ServletException {
