 ...
 */
  public static ProductDetailsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBpartnerId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product_PO.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectProductDetailsData.minimpositionvalue = UtilSql.getValue(result, "minimpositionvalue");
...
Create a registry
 */
  public static ProductDetailsData[] set(String cBpartnerId, String createdby, String createdbyr, String cBpartnerIdr, String cUomId, String discontinuedby, String minimpositionvalue, String orderMin, String iscurrentvendor, String mProductPoId, String mProductId, String mProductIdr, String vendorcategory, String pricelastinv, String pricelist, String isactive, String pricepo, String updatedby, String updatedbyr, String discontinued, String orderPack, String mManufacturerId, String pricelastpo, String priceeffective, String ismultipleofminimumqty, String vendorproductno, String qtystd, String cCurrencyId, String costperorder, String upc, String qualityrating, String royaltyamt, String adClientId, String capacity, String adOrgId, String deliverytimeActual, String deliverytimePromised, String manufacturernumber)    throws ServletException {
    ...
    objectProductDetailsData[0].minimpositionvalue = minimpositionvalue;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Product_PO" +
      "        SET AD_Org_ID = (?) , C_BPartner_ID = (?) , QualityRating = TO_NUMBER(?) , M_Product_ID = (?) , IsActive = (?) , IsCurrentVendor = (?) , UPC = (?) , C_Currency_ID = (?) , PriceList = TO_NUMBER(?) , PriceEffective = TO_DATE(?) , PricePO = TO_NUMBER(?) , PriceLastPO = TO_NUMBER(?) , PriceLastInv = TO_NUMBER(?) , C_UOM_ID = (?) , Qtystd = TO_NUMBER(?) , Order_Min = TO_NUMBER(?) , ismultipleofminimumqty = (?) , Minimpositionvalue = TO_NUMBER(?) , DeliveryTime_Promised = TO_NUMBER(?) , DeliveryTime_Actual = TO_NUMBER(?) , CostPerOrder = TO_NUMBER(?) , VendorProductNo = (?) , VendorCategory = (?) , M_Manufacturer_ID = (?) , manufacturernumber = (?) , Discontinued = (?) , DiscontinuedBy = TO_DATE(?) , RoyaltyAmt = TO_NUMBER(?) , M_Product_PO_ID = (?) , Order_Pack = TO_NUMBER(?) , Capacity = TO_NUMBER(?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_Product_PO.M_Product_PO_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, C_BPartner_ID, QualityRating, M_Product_ID, IsActive, IsCurrentVendor, UPC, C_Currency_ID, PriceList, PriceEffective, PricePO, PriceLastPO, PriceLastInv, C_UOM_ID, Qtystd, Order_Min, ismultipleofminimumqty, Minimpositionvalue, DeliveryTime_Promised, DeliveryTime_Actual, CostPerOrder, VendorProductNo, VendorCategory, M_Manufacturer_ID, manufacturernumber, Discontinued, DiscontinuedBy, RoyaltyAmt, M_Product_PO_ID, Order_Pack, Capacity, AD_Client_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_DATE(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), now(), ?, now(), ?)";
