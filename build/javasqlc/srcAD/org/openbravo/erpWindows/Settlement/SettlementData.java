 ...
 */
  public static SettlementData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Settlement.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Settlement.DocumentNo, " +
      "C_Settlement.C_Currency_ID, " +
      "(CASE WHEN C_Settlement.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "C_Settlement.C_DocType_ID, " +
      "(CASE WHEN C_Settlement.C_DocType_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_DocType_IDR, " +
      ...
      "(CASE WHEN C_Settlement.C_Activity_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Activity_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Settlement left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Settlement.AD_Org_ID = table1.AD_Org_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table2 on (C_Settlement.C_Currency_ID = table2.C_Currency_ID) left join (select C_DocType_ID, Name from C_DocType) table3 on (C_Settlement.C_DocType_ID = table3.C_DocType_ID) left join (select C_DocType_ID,AD_Language, Name from C_DocType_TRL) tableTRL3 on (table3.C_DocType_ID = tableTRL3.C_DocType_ID and tableTRL3.AD_Language = ?)  left join ad_ref_list_v list1 on (C_Settlement.Settlementtype = list1.value and list1.ad_reference_id = '800010' and list1.ad_language = ?)  left join (select C_Activity_ID, Name from C_Activity) table4 on (C_Settlement.C_Activity_ID = table4.C_Activity_ID) left join ad_ref_list_v list2 on (list2.ad_reference_id = '800024' and list2.ad_language = ?  AND C_Settlement.Processed = TO_CHAR(list2.value)) left join ad_ref_list_v list3 on (list3.ad_reference_id = '234' and list3.ad_language = ?  AND C_Settlement.Posted = TO_CHAR(list3.value))" +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static SettlementData[] set(String cSettlementId, String settlementopen, String createdby, String createdbyr, String cDoctypeId, String documentno, String user1Id, String adClientId, String cCampaignId, String cProjectId, String cActivityId, String createfrom, String chargedamt, String description, String processing, String cancelednotchargeamt, String generatedamt, String user2Id, String settlementtype, String updatedby, String updatedbyr, String processed, String processedBtn, String adOrgId, String cCurrencyId, String isactive, String datetrx, String posted, String postedBtn, String dateacct)    throws ServletException {
