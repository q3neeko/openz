 ...
 */
  public static CancelledPaymentsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cSettlementId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Debt_Payment_Cancel.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Debt_Payment_Cancel.C_Settlement_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_Settlement_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateTrx, 'DD-MM-YYYY')),'') ) END) AS C_Settlement_IDR, " +
      ...
      "(CASE WHEN C_Debt_Payment_Cancel.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Debt_Payment_Cancel.C_Invoice_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_Invoice_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table4.DateInvoiced, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.GrandTotal), ''))),'') ) END) AS C_Invoice_IDR, " +
      "C_Debt_Payment_Cancel.Description, " +
      "C_Debt_Payment_Cancel.C_CashBook_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_CashBook_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_CashBook_IDR, " +
      "C_Debt_Payment_Cancel.C_CashLine_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_CashLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Amount), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Description), ''))),'') ) END) AS C_CashLine_IDR, " +
      "C_Debt_Payment_Cancel.C_BankAccount_ID, " +
      "C_Debt_Payment_Cancel.C_BankStatementLine_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_BankStatementLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.StmtAmt), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS C_BankStatementLine_IDR, " +
      "C_Debt_Payment_Cancel.Amount, " +
      "C_Debt_Payment_Cancel.C_Currency_ID, " +
      "(CASE WHEN C_Debt_Payment_Cancel.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Debt_Payment_Cancel left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Debt_Payment_Cancel.AD_Org_ID = table1.AD_Org_ID) left join (select C_Settlement_ID, DocumentNo, DateTrx from C_Settlement) table2 on (C_Debt_Payment_Cancel.C_Settlement_ID = table2.C_Settlement_ID) left join ad_ref_list_v list1 on (C_Debt_Payment_Cancel.PaymentRule = list1.value and list1.ad_reference_id = '195' and list1.ad_language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (C_Debt_Payment_Cancel.C_BPartner_ID = table3.C_BPartner_ID) left join (select C_Invoice_ID, DocumentNo, DateInvoiced, GrandTotal from C_Invoice) table4 on (C_Debt_Payment_Cancel.C_Invoice_ID = table4.C_Invoice_ID) left join (select C_CashBook_ID, Name from C_CashBook) table5 on (C_Debt_Payment_Cancel.C_CashBook_ID = table5.C_CashBook_ID) left join (select C_CashLine_ID, C_Cash_ID, Line, Amount, Description from C_CashLine) table6 on (C_Debt_Payment_Cancel.C_CashLine_ID = table6.C_CashLine_ID) left join (select C_Cash_ID, Name from C_Cash) table7 on (table6.C_Cash_ID = table7.C_Cash_ID) left join (select C_BankStatementLine_ID, Line, StmtAmt, C_BankStatement_ID from C_BankStatementLine) table8 on (C_Debt_Payment_Cancel.C_BankStatementLine_ID = table8.C_BankStatementLine_ID) left join (select C_BankStatement_ID, Name from C_BankStatement) table9 on (table8.C_BankStatement_ID = table9.C_BankStatement_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table10 on (C_Debt_Payment_Cancel.C_Currency_ID = table10.C_Currency_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static CancelledPaymentsData[] set(String cSettlementId, String ispaid, String writeoffamt, String cBankaccountId, String createdby, String createdbyr, String cDebtPaymentId, String cInvoiceId, String cInvoiceIdr, String cCurrencyId, String amount, String dateplanned, String adClientId, String adOrgId, String cBankstatementlineId, String discountamt, String created, String changesettlementcancel, String cCashlineId, String cancelProcessed, String cCashbookId, String isreceipt, String paymentrule, String updatedby, String updatedbyr, String ismanual, String isactive, String isvalid, String description, String cBpartnerId, String cBpartnerIdr)    throws ServletException {
