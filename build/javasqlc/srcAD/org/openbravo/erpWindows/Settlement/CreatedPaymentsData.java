 ...
 */
  public static CreatedPaymentsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cSettlementId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Debt_Payment_Generate.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Debt_Payment_Generate.C_Settlement_ID, " +
      "(CASE WHEN C_Debt_Payment_Generate.C_Settlement_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.DateTrx, 'DD-MM-YYYY')),'') ) END) AS C_Settlement_IDR, " +
      "COALESCE(C_Debt_Payment_Generate.IsActive, 'N') AS IsActive, " +
      "C_Debt_Payment_Generate.C_Settlement_Cancel_ID, " +
      "(CASE WHEN C_Debt_Payment_Generate.C_Settlement_Cancel_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table3.DateTrx, 'DD-MM-YYYY')),'') ) END) AS C_Settlement_Cancel_IDR, " +
      ...
      "(CASE WHEN C_Debt_Payment_Generate.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Debt_Payment_Generate.Description, " +
      "C_Debt_Payment_Generate.C_CashBook_ID, " +
      "(CASE WHEN C_Debt_Payment_Generate.C_CashBook_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_CashBook_IDR, " +
      ...
      "(CASE WHEN C_Debt_Payment_Generate.C_BankStatementLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.StmtAmt), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS C_BankStatementLine_IDR, " +
      "C_Debt_Payment_Generate.Amount, " +
      "C_Debt_Payment_Generate.C_Currency_ID, " +
      "(CASE WHEN C_Debt_Payment_Generate.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
...
Create a registry
 */
  public static CreatedPaymentsData[] set(String cSettlementId, String cBankaccountId, String dateplanned, String ispaid, String cDebtPaymentId, String cInvoiceId, String description, String discountamt, String createdby, String createdbyr, String cBankstatementlineId, String isvalid, String paymentrule, String writeoffamt, String cSettlementCancelId, String isactive, String isreceipt, String ismanual, String cCashbookId, String adOrgId, String cBpartnerId, String cBpartnerIdr, String isdirectposting, String updatedby, String updatedbyr, String amount, String generateProcessed, String adClientId, String cCashlineId, String cCurrencyId, String created)    throws ServletException {
