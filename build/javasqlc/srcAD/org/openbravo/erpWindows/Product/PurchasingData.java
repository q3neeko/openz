...
static Logger log4j = Logger.getLogger(PurchasingData.class);
  ...
  public String minimpositionvalue;
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return pricelastinv;
    else if (fieldName.equalsIgnoreCase("qtylastpo"))
      return qtylastpo;
    ...
    else if (fieldName.equalsIgnoreCase("minimpositionvalue"))
      return minimpositionvalue;
 ...
 */
  public static PurchasingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product_PO.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_Product_PO.C_BPartner_ID, " +
      "(CASE WHEN M_Product_PO.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectPurchasingData.qtylastpo = UtilSql.getValue(result, "qtylastpo");
        ...
        objectPurchasingData.minimpositionvalue = UtilSql.getValue(result, "minimpositionvalue");
...
Create a registry
 */
  public static PurchasingData[] set(String mProductId, String createdby, String createdbyr, String costperorder, String mProductIdr, String vendorcategory, String pricelastinv, String deliverytimeActual, String isactive, String updatedby, String updatedbyr, String qtylastpo, String ismultipleofminimumqty, String upc, String qualityrating, String adClientId, String capacity, String adOrgId, String manufacturernumber, String cBpartnerId, String cBpartnerIdr, String cUomId, String discontinuedby, String minimpositionvalue, String orderMin, String iscurrentvendor, String mProductPoId, String priceeffective, String pricelist, String pricepo, String discontinued, String orderPack, String mManufacturerId, String pricelastpo, String vendorproductno, String cCurrencyId, String qtystd, String royaltyamt, String deliverytimePromised)    throws ServletException {
    ...
    objectPurchasingData[0].qtylastpo = qtylastpo;
    ...
    objectPurchasingData[0].minimpositionvalue = minimpositionvalue;
...
Select for auxiliar field
 */
  public static String selectDef1420_1(ConnectionProvider connectionProvider, String M_Product_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef2316_2(ConnectionProvider connectionProvider, String UpdatedByR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef2705_3(ConnectionProvider connectionProvider, String C_BPartner_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Product_PO" +
      "        SET AD_Org_ID = (?) , M_Product_ID = (?) , C_BPartner_ID = (?) , QualityRating = TO_NUMBER(?) , IsActive = (?) , IsCurrentVendor = (?) , UPC = (?) , C_Currency_ID = (?) , C_UOM_ID = (?) , PriceList = TO_NUMBER(?) , PricePO = TO_NUMBER(?) , PriceLastPO = TO_NUMBER(?) , PriceLastInv = TO_NUMBER(?) , Qtylastpo = TO_NUMBER(?) , DeliveryTime_Promised = TO_NUMBER(?) , VendorProductNo = (?) , VendorCategory = (?) , M_Manufacturer_ID = (?) , manufacturernumber = (?) , Discontinued = (?) , DiscontinuedBy = TO_DATE(?) , Qtystd = TO_NUMBER(?) , Order_Min = TO_NUMBER(?) , ismultipleofminimumqty = (?) , Minimpositionvalue = TO_NUMBER(?) , RoyaltyAmt = TO_NUMBER(?) , M_Product_PO_ID = (?) , DeliveryTime_Actual = TO_NUMBER(?) , CostPerOrder = TO_NUMBER(?) , PriceEffective = TO_DATE(?) , Capacity = TO_NUMBER(?) , AD_Client_ID = (?) , Order_Pack = TO_NUMBER(?) , updated = now(), updatedby = ? " +
      "        WHERE M_Product_PO.M_Product_PO_ID = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, qtylastpo);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, minimpositionvalue);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, M_Product_ID, C_BPartner_ID, QualityRating, IsActive, IsCurrentVendor, UPC, C_Currency_ID, C_UOM_ID, PriceList, PricePO, PriceLastPO, PriceLastInv, Qtylastpo, DeliveryTime_Promised, VendorProductNo, VendorCategory, M_Manufacturer_ID, manufacturernumber, Discontinued, DiscontinuedBy, Qtystd, Order_Min, ismultipleofminimumqty, Minimpositionvalue, RoyaltyAmt, M_Product_PO_ID, DeliveryTime_Actual, CostPerOrder, PriceEffective, Capacity, AD_Client_ID, Order_Pack, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), TO_NUMBER(?), (?), TO_NUMBER(?), now(), ?, now(), ?)";
