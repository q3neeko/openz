 ...
 */
  public static ProductData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product.M_Product_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_Product_Category_IDR, " +
      ...
      "M_Product.C_UOM_ID, " +
      "(CASE WHEN M_Product.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "M_Product.M_AttributeSet_ID, " +
      "(CASE WHEN M_Product.M_AttributeSet_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS M_AttributeSet_IDR, " +
      ...
      "(CASE WHEN M_Product.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      ...
      "(CASE WHEN M_Product.C_Country_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_Country_IDR, " +
      "COALESCE(M_Product.Calculated, 'N') AS Calculated, " +
      "M_Product.C_BPartner_ID, " +
      "(CASE WHEN M_Product.C_BPartner_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "M_Product.VendorProductNo, " +
      "COALESCE(M_Product.IsSold, 'N') AS IsSold, " +
      "M_Product.C_Tax_ID, " +
      "(CASE WHEN M_Product.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL8.Name IS NULL THEN TO_CHAR(table8.Name) ELSE TO_CHAR(tableTRL8.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "M_Product.Customerproducttext, " +
      "M_Product.Basepriceunit, " +
      "(CASE WHEN M_Product.Basepriceunit IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL9.Name IS NULL THEN TO_CHAR(table9.Name) ELSE TO_CHAR(tableTRL9.Name) END)), ''))),'') ) END) AS BasepriceunitR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Product left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Product.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_Category_ID, Name from M_Product_Category) table2 on (M_Product.M_Product_Category_ID = table2.M_Product_Category_ID) left join ad_ref_list_v list1 on (M_Product.Typeofproduct = list1.value and list1.ad_reference_id = 'D3CE5ED8E56C43E19FA09D10B616BCAA' and list1.ad_language = ?)  left join (select C_UOM_ID, Name from C_UOM) table3 on (M_Product.C_UOM_ID = table3.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL3 on (table3.C_UOM_ID = tableTRL3.C_UOM_ID and tableTRL3.AD_Language = ?)  left join (select M_AttributeSet_ID, Name from M_AttributeSet) table4 on (M_Product.M_AttributeSet_ID = table4.M_AttributeSet_ID) left join ad_ref_list_v list2 on (M_Product.ProductType = list2.value and list2.ad_reference_id = '270' and list2.ad_language = ?)  left join (select M_Locator_ID, Value from M_Locator) table5 on (M_Product.M_Locator_ID = table5.M_Locator_ID) left join (select C_Country_ID, Name from C_Country) table6 on (M_Product.C_Country_ID = table6.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL6 on (table6.C_Country_ID = tableTRL6.C_Country_ID and tableTRL6.AD_Language = ?)  left join (select C_BPartner_ID, value, Name from C_BPartner) table7 on (M_Product.C_BPartner_ID =  table7.C_BPartner_ID) left join (select C_Tax_ID, Name from C_Tax) table8 on (M_Product.C_Tax_ID = table8.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL8 on (table8.C_Tax_ID = tableTRL8.C_Tax_ID and tableTRL8.AD_Language = ?)  left join (select C_UOM_ID, Name from C_UOM) table9 on (M_Product.Basepriceunit =  table9.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL9 on (table9.C_UOM_ID = tableTRL9.C_UOM_ID and tableTRL9.AD_Language = ?) " +
      "        WHERE 2=2 " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectProductData.generateorder = UtilSql.getValue(result, "generateorder");
 ...
 */
  public static ProductData[] set(String ispurchased, String issummaryitem, String basepriceunit, String corediameter, String issetitem, String mProductCategoryId, String isactive, String isverified, String volume, String shelfdepth, String stockmin, String costtype, String basepricemultiplicator, String cTaxId, String descriptionurl, String createdby, String createdbyr, String imageurl, String value, String theme, String cBpartnerId, String cUomId, String isinvoiceprintdetails, String unitsperpallet, String mrpPlannerId, String setready4production, String delaymin, String shelfheight, String isserialtracking, String maProcessplanId, String help, String name2, String production, String name, String isbatchtracking, String manufacturernumber, String ispriceprinted, String mProductId, String upc, String sku, String issparepart, String downloadurl, String adImageId, String isserviceitem, String issold, String mLocatorId, String mLocatorIdr, String issummary, String calculated, String mAttributesetinstanceId, String classification, String customerproductno, String versionno, String buttonprintbom, String colorcount, String discontinued, String qtystd, String text, String isconsumable, String qtymax, String description, String palettetype, String typeofproduct, String generateorder, String processing, String adClientId, String qtytype, String width, String discontinuedby, String enforceAttribute, String isstocked, String adOrgId, String buttoncopyitem, String producttype, String stockMin, String customerproducttext, String salesrepId, String isfreightproduct, String mrpPlanningmethodId, String isbom, String vendorproductno, String guaranteedays, String shelfwidth, String updatedby, String updatedbyr, String capacity, String manufacturer, String qtymin, String cutoff, String mFreightcategoryId, String cusomstarifno, String cCountryId, String weight, String documentnote, String coststd, String mAttributesetId, String ispicklistprintdetails, String length)    throws ServletException {
    ...
    objectProductData[0].generateorder = generateorder;
 ...
 */
  public static String selectActP744F145CB414442C9AE94CD0E0306A3A_m_product_po_id(ConnectionProvider connectionProvider, String m_product_id)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select m_product_po_id as retval from m_product_po where m_product_id=? and iscurrentvendor='Y' order by coalesce(qualityrating,0) desc limit 1 ";
...
Select for auxiliar field
 */
  public static String selectActP744F145CB414442C9AE94CD0E0306A3A_ad_org_id(ConnectionProvider connectionProvider, String AD_ORG_ID)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "         select case when ?='0' then '' else ? end as retval from dual ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Product" +
      "        SET AD_Org_ID = (?) , AD_Image_ID = (?) , ImageURL = (?) , Value = (?) , UPC = (?) , Name = (?) , DescriptionURL = (?) , Description = (?) , DocumentNote = (?) , M_Product_Category_ID = (?) , Typeofproduct = (?) , C_UOM_ID = (?) , M_AttributeSet_ID = (?) , ProductType = (?) , IsStocked = (?) , M_Locator_ID = (?) , IsSerialTracking = (?) , Isbatchtracking = (?) , Weight = TO_NUMBER(?) , Volume = TO_NUMBER(?) , ShelfWidth = TO_NUMBER(?) , ShelfHeight = TO_NUMBER(?) , ShelfDepth = TO_NUMBER(?) , UnitsPerPallet = TO_NUMBER(?) , Cutoff = TO_NUMBER(?) , isConsumable = (?) , isSparePart = (?) , isSetItem = (?) , isFreightProduct = (?) , Production = (?) , IsBOM = (?) , isVerified = (?) , Name2 = (?) , IsPurchased = (?) , Manufacturer = (?) , Manufacturernumber = (?) , Cusomstarifno = (?) , C_Country_ID = (?) , Calculated = (?) , C_BPartner_ID = (?) , VendorProductNo = (?) , IsSold = (?) , C_Tax_ID = (?) , Customerproducttext = (?) , Basepriceunit = (?) , Basepricemultiplicator = TO_NUMBER(?) , Discontinued = (?) , DiscontinuedBy = TO_DATE(?) , Buttoncopyitem = (?) , Generateorder = (?) , Setready4production = (?) , SKU = (?) , Stockmin = TO_NUMBER(?) , DownloadURL = (?) , CoreDiameter = TO_NUMBER(?) , Ispriceprinted = (?) , VersionNo = (?) , CustomerProductNo = (?) , M_AttributeSetInstance_ID = (?) , SalesRep_ID = (?) , Palettetype = (?) , IsPickListPrintDetails = (?) , Qtytype = (?) , Enforce_Attribute = (?) , Width = TO_NUMBER(?) , Help = (?) , Stock_Min = TO_NUMBER(?) , Theme = (?) , Qtymax = TO_NUMBER(?) , AD_Client_ID = (?) , Costtype = (?) , MA_Processplan_ID = (?) , MRP_Planningmethod_ID = (?) , Qtymin = TO_NUMBER(?) , isServiceItem = (?) , Length = TO_NUMBER(?) , MRP_Planner_ID = (?) , M_Product_ID = (?) , Qtystd = TO_NUMBER(?) , Text = (?) , IsSummary = (?) , Classification = (?) , Colorcount = TO_NUMBER(?) , Delaymin = TO_NUMBER(?) , IsInvoicePrintDetails = (?) , Coststd = TO_NUMBER(?) , Buttonprintbom = (?) , M_FreightCategory_ID = (?) , IsActive = (?) , Capacity = TO_NUMBER(?) , IsSummaryItem = (?) , GuaranteeDays = TO_NUMBER(?) , Processing = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, AD_Image_ID, ImageURL, Value, UPC, Name, DescriptionURL, Description, DocumentNote, M_Product_Category_ID, Typeofproduct, C_UOM_ID, M_AttributeSet_ID, ProductType, IsStocked, M_Locator_ID, IsSerialTracking, Isbatchtracking, Weight, Volume, ShelfWidth, ShelfHeight, ShelfDepth, UnitsPerPallet, Cutoff, isConsumable, isSparePart, isSetItem, isFreightProduct, Production, IsBOM, isVerified, Name2, IsPurchased, Manufacturer, Manufacturernumber, Cusomstarifno, C_Country_ID, Calculated, C_BPartner_ID, VendorProductNo, IsSold, C_Tax_ID, Customerproducttext, Basepriceunit, Basepricemultiplicator, Discontinued, DiscontinuedBy, Buttoncopyitem, Generateorder, Setready4production, SKU, Stockmin, DownloadURL, CoreDiameter, Ispriceprinted, VersionNo, CustomerProductNo, M_AttributeSetInstance_ID, SalesRep_ID, Palettetype, IsPickListPrintDetails, Qtytype, Enforce_Attribute, Width, Help, Stock_Min, Theme, Qtymax, AD_Client_ID, Costtype, MA_Processplan_ID, MRP_Planningmethod_ID, Qtymin, isServiceItem, Length, MRP_Planner_ID, M_Product_ID, Qtystd, Text, IsSummary, Classification, Colorcount, Delaymin, IsInvoicePrintDetails, Coststd, Buttonprintbom, M_FreightCategory_ID, IsActive, Capacity, IsSummaryItem, GuaranteeDays, Processing, created, createdby, updated, updatedBy)" +
