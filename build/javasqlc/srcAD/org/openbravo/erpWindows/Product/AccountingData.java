 ...
 */
  public static AccountingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_Product_Acct.M_Product_ID, " +
      "(CASE WHEN M_Product_Acct.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_Product_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN M_Product_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "COALESCE(M_Product_Acct.IsActive, 'N') AS IsActive, " +
      "M_Product_Acct.P_Asset_Acct, " +
      "M_Product_Acct.P_Expense_Acct, " +
      "(CASE WHEN M_Product_Acct.P_Expense_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS P_Expense_AcctR, " +
      "M_Product_Acct.P_Revenue_Acct, " +
      "(CASE WHEN M_Product_Acct.P_Revenue_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Combination), ''))),'') ) END) AS P_Revenue_AcctR, " +
      "M_Product_Acct.P_Cogs_Acct, " +
      "M_Product_Acct.P_PurchasePriceVariance_Acct, " +
      "M_Product_Acct.P_InvoicePriceVariance_Acct, " +
      "(CASE WHEN M_Product_Acct.P_InvoicePriceVariance_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Combination), ''))),'') ) END) AS P_InvoicePriceVariance_AcctR, " +
      "M_Product_Acct.P_TradeDiscountRec_Acct, " +
      "M_Product_Acct.P_TradeDiscountGrant_Acct, " +
      "M_Product_Acct.M_Product_Acct_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM M_Product_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Product_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_ID, Value, Name from M_Product) table2 on (M_Product_Acct.M_Product_ID = table2.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL2 on (table2.M_Product_ID = tableTRL2.M_Product_ID and tableTRL2.AD_Language = ?)  left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (M_Product_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (M_Product_Acct.P_Expense_Acct = table4.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table5 on (M_Product_Acct.P_Revenue_Acct = table5.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table6 on (M_Product_Acct.P_InvoicePriceVariance_Acct = table6.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
