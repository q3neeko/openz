 ...
 */
  public static CostingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "M_Costing.AD_Org_ID, " +
      "(CASE WHEN M_Costing.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_Costing.M_Product_ID, " +
      "(CASE WHEN M_Costing.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_Costing.M_InOutLine_ID, " +
      "(CASE WHEN M_Costing.M_InOutLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table4.MovementDate, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS M_InOutLine_IDR, " +
      "M_Costing.C_Orderline_ID, " +
      "(CASE WHEN M_Costing.C_Orderline_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table7.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.GrandTotal), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Line), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.LineNetAmt), ''))),'') ) END) AS C_Orderline_IDR, " +
      "M_Costing.Costtype, " +
      ...
      "M_Costing.Qty, " +
      "        ? AS LANGUAGE " +
      "        FROM M_Costing left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Costing.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_ID, Value, Name from M_Product) table2 on (M_Costing.M_Product_ID = table2.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL2 on (table2.M_Product_ID = tableTRL2.M_Product_ID and tableTRL2.AD_Language = ?)  left join (select M_InOutLine_ID, M_InOut_ID, M_Product_ID from M_InOutLine) table3 on (M_Costing.M_InOutLine_ID = table3.M_InOutLine_ID) left join (select M_InOut_ID, DocumentNo, MovementDate from M_InOut) table4 on (table3.M_InOut_ID = table4.M_InOut_ID) left join (select M_Product_ID, Value, Name from M_Product) table5 on (table3.M_Product_ID = table5.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL5 on (table5.M_Product_ID = tableTRL5.M_Product_ID and tableTRL5.AD_Language = ?)  left join (select C_Orderline_ID, C_Order_ID, Line, LineNetAmt from C_Orderline) table6 on (M_Costing.C_Orderline_ID = table6.C_Orderline_ID) left join (select C_Order_ID, DocumentNo, Name, DateOrdered, GrandTotal from C_Order) table7 on (table6.C_Order_ID = table7.C_Order_ID) left join ad_ref_list_v list1 on (M_Costing.Costtype = list1.value and list1.ad_reference_id = '800025' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectCostingData.cOrderlineId = UtilSql.getValue(result, "c_orderline_id");
        objectCostingData.cOrderlineIdr = UtilSql.getValue(result, "c_orderline_idr");
...
Create a registry
 */
  public static CostingData[] set(String mProductId, String mProductIdr, String cumqty, String cOrderlineId, String ispermanent, String createdby, String createdbyr, String isproduction, String adClientId, String updatedby, String updatedbyr, String adOrgId, String mInoutlineId, String mInoutlineIdr, String cost, String mCostingId, String datefrom, String dateto, String price, String qty, String costtype, String ismanual, String cInvoicelineId)    throws ServletException {
    ...
    objectCostingData[0].cOrderlineId = cOrderlineId;
    objectCostingData[0].cOrderlineIdr = "";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Costing" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , M_Product_ID = (?) , M_InOutLine_ID = (?) , C_Orderline_ID = (?) , Costtype = (?) , DateFrom = TO_DATE(?) , DateTo = TO_DATE(?) , Cost = TO_NUMBER(?) , Isproduction = (?) , IsManual = (?) , Ispermanent = (?) , Price = TO_NUMBER(?) , C_InvoiceLine_ID = (?) , M_Costing_ID = (?) , Cumqty = TO_NUMBER(?) , Qty = TO_NUMBER(?) , updated = now(), updatedby = ? " +
      "        WHERE M_Costing.M_Costing_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, M_Product_ID, M_InOutLine_ID, C_Orderline_ID, Costtype, DateFrom, DateTo, Cost, Isproduction, IsManual, Ispermanent, Price, C_InvoiceLine_ID, M_Costing_ID, Cumqty, Qty, created, createdby, updated, updatedBy)" +
