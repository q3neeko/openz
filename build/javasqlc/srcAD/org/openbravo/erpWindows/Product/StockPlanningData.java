 ...
 */
  public static StockPlanningData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Product_Org.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(M_Product_Org.IsActive, 'N') AS IsActive, " +
      "M_Product_Org.M_Product_ID, " +
      "(CASE WHEN M_Product_Org.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "M_Product_Org.M_Locator_ID, " +
      "(CASE WHEN M_Product_Org.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      "COALESCE(M_Product_Org.Isvendorreceiptlocator, 'N') AS Isvendorreceiptlocator, " +
      "M_Product_Org.MRP_Planningmethod_ID, " +
      "(CASE WHEN M_Product_Org.MRP_Planningmethod_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS MRP_Planningmethod_IDR, " +
      "M_Product_Org.MRP_Planner_ID, " +
      "(CASE WHEN M_Product_Org.MRP_Planner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS MRP_Planner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Product_Org left join (select AD_Org_ID, Name from AD_Org) table1 on (M_Product_Org.AD_Org_ID = table1.AD_Org_ID) left join (select M_Product_ID, Value, Name from M_Product) table2 on (M_Product_Org.M_Product_ID = table2.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL2 on (table2.M_Product_ID = tableTRL2.M_Product_ID and tableTRL2.AD_Language = ?)  left join (select M_Locator_ID, Value from M_Locator) table3 on (M_Product_Org.M_Locator_ID = table3.M_Locator_ID) left join (select MRP_Planningmethod_ID, Name from MRP_Planningmethod) table4 on (M_Product_Org.MRP_Planningmethod_ID = table4.MRP_Planningmethod_ID) left join (select MRP_Planner_ID, Name from MRP_Planner) table5 on (M_Product_Org.MRP_Planner_ID = table5.MRP_Planner_ID) left join ad_ref_list_v list1 on (M_Product_Org.Abc = list1.value and list1.ad_reference_id = '1000500000' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static StockPlanningData[] set(String mProductId, String adOrgId, String qtytype, String isactive, String qtymax, String stockmin, String capacity, String createdby, String createdbyr, String adClientId, String mrpPlannerId, String mrpPlanningmethodId, String qtyoptimal, String mProductOrgId, String updatedby, String updatedbyr, String created, String mProductIdr, String abc, String qtymin, String mLocatorId, String mLocatorIdr, String isvendorreceiptlocator, String qtystd, String delaymin)    throws ServletException {
