 ...
 */
  public static PriceAdjustmentsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductPoId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Offer_V.C_Bpartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Bpartner_IDR, " +
      "M_Offer_V.Name, " +
      ...
      "M_Offer_V.Manufacturernumber, " +
      "M_Offer_V.M_Attributesetinstance_ID, " +
      "(CASE WHEN M_Offer_V.M_Attributesetinstance_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Description), ''))),'') ) END) AS M_Attributesetinstance_IDR, " +
      "COALESCE(M_Offer_V.Graterequal, 'N') AS Graterequal, " +
      "COALESCE(M_Offer_V.Lessequal, 'N') AS Lessequal, " +
      "M_Offer_V.Datefrom, " +
      "M_Offer_V.Dateto, " +
      "M_Offer_V.QTY_From, " +
      "M_Offer_V.QTY_To, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectPriceAdjustmentsData.manufacturernumber = UtilSql.getValue(result, "manufacturernumber");
        objectPriceAdjustmentsData.mAttributesetinstanceId = UtilSql.getValue(result, "m_attributesetinstance_id");
        objectPriceAdjustmentsData.mAttributesetinstanceIdr = UtilSql.getValue(result, "m_attributesetinstance_idr");
        objectPriceAdjustmentsData.graterequal = UtilSql.getValue(result, "graterequal");
        objectPriceAdjustmentsData.lessequal = UtilSql.getValue(result, "lessequal");
...
Create a registry
 */
  public static PriceAdjustmentsData[] set(String mProductPoId, String issalesoffer, String pricelistSelection, String mManufacturerId, String mOfferId, String productSelection, String createdby, String createdbyr, String addamt, String fixed, String graterequal, String name, String description, String adOrgId, String mProductId, String mProductIdr, String priority, String manufacturernumber, String cUomId, String mOfferVId, String bpGroupSelection, String isactive, String updatedby, String updatedbyr, String adClientId, String lessequal, String dateto, String directpurchasecalc, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String qtyFrom, String qtyTo, String prodCatSelection, String bpartnerSelection, String datefrom, String discount, String cBpartnerId, String cBpartnerIdr)    throws ServletException {
    ...
    objectPriceAdjustmentsData[0].manufacturernumber = manufacturernumber;
    objectPriceAdjustmentsData[0].mAttributesetinstanceId = mAttributesetinstanceId;
    objectPriceAdjustmentsData[0].mAttributesetinstanceIdr = mAttributesetinstanceIdr;
    objectPriceAdjustmentsData[0].graterequal = graterequal;
    objectPriceAdjustmentsData[0].lessequal = lessequal;
...
Select for auxiliar field
 */
  public static String selectDef750DF271F07D40F8988763216EDA4705_2(ConnectionProvider connectionProvider, String UpdatedbyR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef674D7A5B357A4E888EED98723E5B4CBC_3(ConnectionProvider connectionProvider, String M_Attributesetinstance_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef51865513CFB94F8594B1C98E292088E9_4(ConnectionProvider connectionProvider, String C_Bpartner_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  public static String selectParent(ConnectionProvider connectionProvider, String paramLanguage, String mProductPoId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT (TO_CHAR(COALESCE(TO_CHAR(table2.Value), '')) || ' - ' || TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), '')) || ' - ' || TO_CHAR(COALESCE(TO_CHAR(table3.Value), '')) || ' - ' || TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))) AS NAME FROM M_Product_PO left join (select M_Product_Po_ID, M_Product_ID, C_BPartner_ID from M_Product_Po) table1 on (M_Product_PO.M_Product_Po_ID = table1.M_Product_Po_ID) left join (select M_Product_ID, Value, Name from M_Product) table2 on (table1.M_Product_ID = table2.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL2 on (table2.M_Product_ID = tableTRL2.M_Product_ID and tableTRL2.AD_Language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (table1.C_BPartner_ID = table3.C_BPartner_ID) WHERE M_Product_PO.M_Product_Po_ID = ?  ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Offer_V" +
      "        SET M_Product_ID = (?) , C_Bpartner_ID = (?) , Name = (?) , Isactive = (?) , Directpurchasecalc = (?) , C_Uom_ID = (?) , M_Manufacturer_ID = (?) , Manufacturernumber = (?) , M_Attributesetinstance_ID = (?) , Graterequal = (?) , Lessequal = (?) , Datefrom = TO_DATE(?) , Dateto = TO_DATE(?) , QTY_From = TO_NUMBER(?) , QTY_To = TO_NUMBER(?) , Priority = TO_NUMBER(?) , Discount = TO_NUMBER(?) , Addamt = TO_NUMBER(?) , Fixed = TO_NUMBER(?) , Product_Selection = (?) , Pricelist_Selection = (?) , AD_Org_ID = (?) , Description = (?) , BP_Group_Selection = (?) , M_Offer_ID = (?) , M_Product_Po_ID = (?) , Prod_Cat_Selection = (?) , Bpartner_Selection = (?) , AD_Client_ID = (?) , Issalesoffer = (?) , M_Offer_V_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_Offer_V.M_Offer_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (M_Product_ID, C_Bpartner_ID, Name, Isactive, Directpurchasecalc, C_Uom_ID, M_Manufacturer_ID, Manufacturernumber, M_Attributesetinstance_ID, Graterequal, Lessequal, Datefrom, Dateto, QTY_From, QTY_To, Priority, Discount, Addamt, Fixed, Product_Selection, Pricelist_Selection, AD_Org_ID, Description, BP_Group_Selection, M_Offer_ID, M_Product_Po_ID, Prod_Cat_Selection, Bpartner_Selection, AD_Client_ID, Issalesoffer, M_Offer_V_ID, created, createdby, updated, updatedBy)" +
