...
static Logger log4j = Logger.getLogger(PriceHistoryData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return pricelastpo;
    else if (fieldName.equalsIgnoreCase("qtypo"))
      return qtypo;
 ...
 */
  public static PriceHistoryData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mProductPoId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN m_product_po_history.M_Product_Po_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS M_Product_Po_IDR, " +
      "m_product_po_history.Pricelist, " +
      "m_product_po_history.Pricepo, " +
      "m_product_po_history.Pricelastpo, " +
      "m_product_po_history.Qtypo, " +
      "m_product_po_history.Validfrom, " +
      "m_product_po_history.Qualityrating, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectPriceHistoryData.pricelastpo = UtilSql.getValue(result, "pricelastpo");
        objectPriceHistoryData.qtypo = UtilSql.getValue(result, "qtypo");
        objectPriceHistoryData.validfrom = UtilSql.getDateValue(result, "validfrom", "dd-MM-yyyy");
        objectPriceHistoryData.qualityrating = UtilSql.getValue(result, "qualityrating");
...
Create a registry
 */
  public static PriceHistoryData[] set(String mProductPoId, String validfrom, String updatedby, String updatedbyr, String adClientId, String pricelist, String mProductId, String createdby, String createdbyr, String pricepo, String adOrgId, String cBpartnerId, String qtypo, String mProductPoHistoryId, String pricelastpo, String isactive, String qualityrating)    throws ServletException {
    ...
    objectPriceHistoryData[0].pricelastpo = pricelastpo;
    objectPriceHistoryData[0].qtypo = qtypo;
    objectPriceHistoryData[0].validfrom = validfrom;
    objectPriceHistoryData[0].qualityrating = qualityrating;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET M_Product_Po_ID = (?) , Pricelist = TO_NUMBER(?) , Pricepo = TO_NUMBER(?) , Pricelastpo = TO_NUMBER(?) , Qtypo = TO_NUMBER(?) , Validfrom = TO_DATE(?) , Qualityrating = TO_NUMBER(?) , C_Bpartner_ID = (?) , AD_Org_ID = (?) , Isactive = (?) , M_Product_Po_History_ID = (?) , AD_Client_ID = (?) , M_Product_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE m_product_po_history.M_Product_Po_History_ID = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, qtypo);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (M_Product_Po_ID, Pricelist, Pricepo, Pricelastpo, Qtypo, Validfrom, Qualityrating, C_Bpartner_ID, AD_Org_ID, Isactive, M_Product_Po_History_ID, AD_Client_ID, M_Product_ID, created, createdby, updated, updatedBy)" +
