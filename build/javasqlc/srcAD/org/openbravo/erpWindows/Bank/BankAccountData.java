 ...
 */
  public static BankAccountData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBankId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_BankAccount.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_BankAccount.C_Bank_ID, " +
      "(CASE WHEN C_BankAccount.C_Bank_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Bank_IDR, " +
      ...
      "(CASE WHEN C_BankAccount.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_BankAccount left join (select AD_Org_ID, Name from AD_Org) table1 on (C_BankAccount.AD_Org_ID = table1.AD_Org_ID) left join (select C_Bank_ID, Name from C_Bank) table2 on (C_BankAccount.C_Bank_ID = table2.C_Bank_ID) left join ad_ref_list_v list1 on (C_BankAccount.BankAccountType = list1.value and list1.ad_reference_id = '216' and list1.ad_language = ?)  left join (select C_Currency_ID, ISO_Code from C_Currency) table3 on (C_BankAccount.C_Currency_ID = table3.C_Currency_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static BankAccountData[] set(String cBankId, String currentbalance, String cCurrencyId, String adClientId, String accountno, String codeaccount, String digitcontrol, String genericaccount, String iban, String cdtrschmeident, String showiban, String adOrgId, String isdefault, String updatedby, String updatedbyr, String created, String showgeneric, String showspanish, String creditlimit, String isactive, String bankaccounttype, String displayedaccount, String cBankaccountId, String createdby, String createdbyr)    throws ServletException {
