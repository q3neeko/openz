 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN Fact_Acct.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "Fact_Acct.Record_ID, " +
      ...
      "(CASE WHEN Fact_Acct.Account_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS Account_IDR, " +
      "Fact_Acct.C_BPartner_ID, " +
      "(CASE WHEN Fact_Acct.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "Fact_Acct.M_Product_ID, " +
      "(CASE WHEN Fact_Acct.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL7.Name IS NULL THEN TO_CHAR(table7.Name) ELSE TO_CHAR(tableTRL7.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "Fact_Acct.C_Project_ID, " +
      "(CASE WHEN Fact_Acct.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "Fact_Acct.C_SalesRegion_ID, " +
      "(CASE WHEN Fact_Acct.C_SalesRegion_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Name), ''))),'') ) END) AS C_SalesRegion_IDR, " +
      "Fact_Acct.C_Campaign_ID, " +
      "(CASE WHEN Fact_Acct.C_Campaign_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'') ) END) AS C_Campaign_IDR, " +
      "Fact_Acct.C_LocFrom_ID, " +
      "(CASE WHEN Fact_Acct.C_LocFrom_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.City), ''))),'') ) END) AS C_LocFrom_IDR, " +
      "Fact_Acct.C_LocTo_ID, " +
      "(CASE WHEN Fact_Acct.C_LocTo_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.City), ''))),'') ) END) AS C_LocTo_IDR, " +
      "Fact_Acct.C_Activity_ID, " +
      "(CASE WHEN Fact_Acct.C_Activity_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table13.Name), ''))),'') ) END) AS C_Activity_IDR, " +
      "Fact_Acct.GL_Category_ID, " +
      "(CASE WHEN Fact_Acct.GL_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table14.Name), ''))),'') ) END) AS GL_Category_IDR, " +
      "Fact_Acct.C_Tax_ID, " +
      "(CASE WHEN Fact_Acct.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL15.Name IS NULL THEN TO_CHAR(table15.Name) ELSE TO_CHAR(tableTRL15.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "Fact_Acct.M_Locator_ID, " +
      "(CASE WHEN Fact_Acct.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      "Fact_Acct.C_Currency_ID, " +
      "(CASE WHEN Fact_Acct.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "Fact_Acct.AmtAcctDr, " +
      "Fact_Acct.AmtAcctCr, " +
      "Fact_Acct.C_UOM_ID, " +
      "(CASE WHEN Fact_Acct.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL18.Name IS NULL THEN TO_CHAR(table18.Name) ELSE TO_CHAR(tableTRL18.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "Fact_Acct.Qty, " +
      "        ? AS LANGUAGE " +
      "        FROM Fact_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (Fact_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table2 on (Fact_Acct.C_AcctSchema_ID = table2.C_AcctSchema_ID) left join (select AD_Table_ID, TableName from AD_Table) table3 on (Fact_Acct.AD_Table_ID = table3.AD_Table_ID) left join (select C_Period_ID, Name from C_Period) table4 on (Fact_Acct.C_Period_ID = table4.C_Period_ID) left join (select C_ElementValue_ID, value, Name from C_ElementValue) table5 on (Fact_Acct.Account_ID =  table5.C_ElementValue_ID) left join (select C_ElementValue_ID,AD_Language, Name from C_ElementValue_TRL) tableTRL5 on (table5.C_ElementValue_ID = tableTRL5.C_ElementValue_ID and tableTRL5.AD_Language = ?)  left join (select C_BPartner_ID, Value, Name from C_BPartner) table6 on (Fact_Acct.C_BPartner_ID = table6.C_BPartner_ID) left join (select M_Product_ID, Value, Name from M_Product) table7 on (Fact_Acct.M_Product_ID = table7.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL7 on (table7.M_Product_ID = tableTRL7.M_Product_ID and tableTRL7.AD_Language = ?)  left join (select C_Project_ID, Value, Name from C_Project) table8 on (Fact_Acct.C_Project_ID = table8.C_Project_ID) left join (select C_SalesRegion_ID, Name from C_SalesRegion) table9 on (Fact_Acct.C_SalesRegion_ID = table9.C_SalesRegion_ID) left join (select C_Campaign_ID, Name from C_Campaign) table10 on (Fact_Acct.C_Campaign_ID = table10.C_Campaign_ID) left join (select C_Location_ID, City from C_Location) table11 on (Fact_Acct.C_LocFrom_ID =  table11.C_Location_ID) left join (select C_Location_ID, City from C_Location) table12 on (Fact_Acct.C_LocTo_ID =  table12.C_Location_ID) left join (select C_Activity_ID, Name from C_Activity) table13 on (Fact_Acct.C_Activity_ID = table13.C_Activity_ID) left join (select GL_Category_ID, Name from GL_Category) table14 on (Fact_Acct.GL_Category_ID = table14.GL_Category_ID) left join (select C_Tax_ID, Name from C_Tax) table15 on (Fact_Acct.C_Tax_ID = table15.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL15 on (table15.C_Tax_ID = tableTRL15.C_Tax_ID and tableTRL15.AD_Language = ?)  left join (select M_Locator_ID, Value from M_Locator) table16 on (Fact_Acct.M_Locator_ID = table16.M_Locator_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table17 on (Fact_Acct.C_Currency_ID = table17.C_Currency_ID) left join (select C_UOM_ID, Name from C_UOM) table18 on (Fact_Acct.C_UOM_ID = table18.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL18 on (table18.C_UOM_ID = tableTRL18.C_UOM_ID and tableTRL18.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static HeaderData[] set(String lineId, String cSalesregionId, String datetrx, String cCampaignId, String accountId, String cTaxId, String cPeriodId, String dateacct, String cAcctschemaId, String adClientId, String glCategoryId, String isactive, String aAssetId, String amtacctcr, String adOrgId, String qty, String recordId, String mLocatorId, String cLoctoId, String adTableId, String updatedby, String updatedbyr, String cProjectId, String mProductId, String mProductIdr, String cBpartnerId, String cBpartnerIdr, String cLocfromId, String createdby, String createdbyr, String cActivityId, String description, String cUomId, String factAcctId, String amtacctdr, String cCurrencyId)    throws ServletException {
