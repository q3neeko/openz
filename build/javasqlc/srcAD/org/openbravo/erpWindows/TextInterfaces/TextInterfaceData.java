 ...
 */
  public static TextInterfaceData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_TextInterfaces.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(AD_TextInterfaces.IsActive, 'N') AS IsActive, " +
      "AD_TextInterfaces.AD_Module_ID, " +
      "(CASE WHEN AD_TextInterfaces.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_TextInterfaces.Text, " +
      "AD_TextInterfaces.Filename, " +
      "COALESCE(AD_TextInterfaces.Isused, 'N') AS Isused, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_TextInterfaces left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_TextInterfaces.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table2 on (AD_TextInterfaces.AD_Module_ID = table2.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table3 on (table2.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static TextInterfaceData[] set(String updatedby, String updatedbyr, String adModuleId, String adOrgId, String text, String isused, String filename, String stillused, String adTextinterfacesId, String isactive, String createdby, String createdbyr, String adClientId)    throws ServletException {
