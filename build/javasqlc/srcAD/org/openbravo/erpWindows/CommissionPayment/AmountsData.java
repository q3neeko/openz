 ...
 */
  public static AmountsData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCommissionrunId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_CommissionAmt.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_CommissionAmt.C_CommissionRun_ID, " +
      "(CASE WHEN C_CommissionAmt.C_CommissionRun_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'') ) END) AS C_CommissionRun_IDR, " +
      "C_CommissionAmt.C_CommissionLine_ID, " +
      "(CASE WHEN C_CommissionAmt.C_CommissionLine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Line), ''))),'') ) END) AS C_CommissionLine_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_CommissionAmt left join (select AD_Org_ID, Name from AD_Org) table1 on (C_CommissionAmt.AD_Org_ID = table1.AD_Org_ID) left join (select C_CommissionRun_ID, DocumentNo from C_CommissionRun) table2 on (C_CommissionAmt.C_CommissionRun_ID = table2.C_CommissionRun_ID) left join (select C_CommissionLine_ID, C_Commission_ID, Line from C_CommissionLine) table3 on (C_CommissionAmt.C_CommissionLine_ID = table3.C_CommissionLine_ID) left join (select C_Commission_ID, Name from C_Commission) table4 on (table3.C_Commission_ID = table4.C_Commission_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
