 ...
 */
  public static TaxZoneData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cTaxId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Tax_Zone.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Tax_Zone.C_Tax_ID, " +
      "(CASE WHEN C_Tax_Zone.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "COALESCE(C_Tax_Zone.IsActive, 'N') AS IsActive, " +
      "C_Tax_Zone.From_Country_ID, " +
      "(CASE WHEN C_Tax_Zone.From_Country_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS From_Country_IDR, " +
      "C_Tax_Zone.To_Country_ID, " +
      "(CASE WHEN C_Tax_Zone.To_Country_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS To_Country_IDR, " +
      "C_Tax_Zone.From_Region_ID, " +
      "(CASE WHEN C_Tax_Zone.From_Region_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS From_Region_IDR, " +
      "C_Tax_Zone.To_Region_ID, " +
      "(CASE WHEN C_Tax_Zone.To_Region_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS To_Region_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Tax_Zone left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Tax_Zone.AD_Org_ID = table1.AD_Org_ID) left join (select C_Tax_ID, Name from C_Tax) table2 on (C_Tax_Zone.C_Tax_ID = table2.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL2 on (table2.C_Tax_ID = tableTRL2.C_Tax_ID and tableTRL2.AD_Language = ?)  left join (select C_Country_ID, Name from C_Country) table3 on (C_Tax_Zone.From_Country_ID =  table3.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL3 on (table3.C_Country_ID = tableTRL3.C_Country_ID and tableTRL3.AD_Language = ?)  left join (select C_Country_ID, Name from C_Country) table4 on (C_Tax_Zone.To_Country_ID =  table4.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL4 on (table4.C_Country_ID = tableTRL4.C_Country_ID and tableTRL4.AD_Language = ?)  left join (select C_Region_ID, Name from C_Region) table5 on (C_Tax_Zone.From_Region_ID =  table5.C_Region_ID) left join (select C_Region_ID, Name from C_Region) table6 on (C_Tax_Zone.To_Region_ID =  table6.C_Region_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TaxZoneData[] set(String cTaxId, String toRegionId, String fromCountryId, String cTaxZoneId, String updatedby, String updatedbyr, String createdby, String createdbyr, String adClientId, String isactive, String toCountryId, String adOrgId, String fromRegionId)    throws ServletException {
