 ...
 */
  public static AccountingData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cTaxId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Tax_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Tax_Acct.C_Tax_ID, " +
      "(CASE WHEN C_Tax_Acct.C_Tax_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "C_Tax_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN C_Tax_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "COALESCE(C_Tax_Acct.IsActive, 'N') AS IsActive, " +
      "C_Tax_Acct.T_Due_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_Due_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS T_Due_AcctR, " +
      "C_Tax_Acct.T_Liability_Acct, " +
      "C_Tax_Acct.T_Credit_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_Credit_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Combination), ''))),'') ) END) AS T_Credit_AcctR, " +
      "C_Tax_Acct.T_Receivables_Acct, " +
      "C_Tax_Acct.T_Expense_Acct, " +
      "C_Tax_Acct.T_P_Expense_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_P_Expense_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Combination), ''))),'') ) END) AS T_P_Expense_AcctR, " +
      "C_Tax_Acct.T_P_Revenue_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_P_Revenue_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Combination), ''))),'') ) END) AS T_P_Revenue_AcctR, " +
      "C_Tax_Acct.T_Ar_Discount_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_Ar_Discount_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Combination), ''))),'') ) END) AS T_Ar_Discount_AcctR, " +
      "C_Tax_Acct.T_Ap_Discount_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_Ap_Discount_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table9.Combination), ''))),'') ) END) AS T_Ap_Discount_AcctR, " +
      "C_Tax_Acct.T_Writeoff_Acct, " +
      "(CASE WHEN C_Tax_Acct.T_Writeoff_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Combination), ''))),'') ) END) AS T_Writeoff_AcctR, " +
      "C_Tax_Acct.C_Tax_Acct_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Tax_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Tax_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select C_Tax_ID, Name from C_Tax) table2 on (C_Tax_Acct.C_Tax_ID = table2.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL2 on (table2.C_Tax_ID = tableTRL2.C_Tax_ID and tableTRL2.AD_Language = ?)  left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (C_Tax_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (C_Tax_Acct.T_Due_Acct = table4.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table5 on (C_Tax_Acct.T_Credit_Acct = table5.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table6 on (C_Tax_Acct.T_P_Expense_Acct = table6.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table7 on (C_Tax_Acct.T_P_Revenue_Acct = table7.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table8 on (C_Tax_Acct.T_Ar_Discount_Acct = table8.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table9 on (C_Tax_Acct.T_Ap_Discount_Acct = table9.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table10 on (C_Tax_Acct.T_Writeoff_Acct = table10.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
