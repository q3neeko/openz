 ...
 */
  public static ConversionData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cUomId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_UOM_Conversion.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_UOM_Conversion.C_UOM_ID, " +
      "(CASE WHEN C_UOM_Conversion.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "C_UOM_Conversion.C_UOM_To_ID, " +
      "(CASE WHEN C_UOM_Conversion.C_UOM_To_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_UOM_To_IDR, " +
      "COALESCE(C_UOM_Conversion.IsActive, 'N') AS IsActive, " +
      "C_UOM_Conversion.MultiplyRate, " +
      "C_UOM_Conversion.DivideRate, " +
      "        ? AS LANGUAGE " +
      "        FROM C_UOM_Conversion left join (select AD_Org_ID, Name from AD_Org) table1 on (C_UOM_Conversion.AD_Org_ID = table1.AD_Org_ID) left join (select C_UOM_ID, Name from C_UOM) table2 on (C_UOM_Conversion.C_UOM_ID = table2.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL2 on (table2.C_UOM_ID = tableTRL2.C_UOM_ID and tableTRL2.AD_Language = ?)  left join (select C_UOM_ID, Name from C_UOM) table3 on (C_UOM_Conversion.C_UOM_To_ID =  table3.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL3 on (table3.C_UOM_ID = tableTRL3.C_UOM_ID and tableTRL3.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ConversionData[] set(String cUomId, String adOrgId, String adClientId, String dividerate, String cUomToId, String isactive, String multiplyrate, String cUomConversionId, String updatedby, String updatedbyr, String createdby, String createdbyr)    throws ServletException {
