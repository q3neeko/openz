 ...
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cElementvalueId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_ElementValue_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_ElementValue_Trl.C_ElementValue_ID, " +
      "(CASE WHEN C_ElementValue_Trl.C_ElementValue_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_ElementValue_IDR, " +
      "C_ElementValue_Trl.AD_Language, " +
      "(CASE WHEN C_ElementValue_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_ElementValue_Trl left join (select AD_Org_ID, Name from AD_Org) table1 on (C_ElementValue_Trl.AD_Org_ID = table1.AD_Org_ID) left join (select C_ElementValue_ID, Value, Name from C_ElementValue) table2 on (C_ElementValue_Trl.C_ElementValue_ID = table2.C_ElementValue_ID) left join (select C_ElementValue_ID,AD_Language, Name from C_ElementValue_TRL) tableTRL2 on (table2.C_ElementValue_ID = tableTRL2.C_ElementValue_ID and tableTRL2.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table3 on (C_ElementValue_Trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static TranslationData[] set(String cElementvalueId, String description, String name, String adClientId, String createdby, String createdbyr, String updatedby, String updatedbyr, String adOrgId, String isactive, String adLanguage, String cElementvalueTrlId, String istranslated)    throws ServletException {
