 ...
 */
  public static AssignedAttributeData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mAttributesetId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_AttributeUse.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "M_AttributeUse.M_AttributeSet_ID, " +
      "(CASE WHEN M_AttributeUse.M_AttributeSet_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS M_AttributeSet_IDR, " +
      "M_AttributeUse.M_Attribute_ID, " +
      "(CASE WHEN M_AttributeUse.M_Attribute_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS M_Attribute_IDR, " +
      "M_AttributeUse.SeqNo, " +
      "M_AttributeUse.M_AttributeUse_ID, " +
      "COALESCE(M_AttributeUse.IsActive, 'N') AS IsActive, " +
      "        ? AS LANGUAGE " +
      "        FROM M_AttributeUse left join (select AD_Org_ID, Name from AD_Org) table1 on (M_AttributeUse.AD_Org_ID = table1.AD_Org_ID) left join (select M_AttributeSet_ID, Name from M_AttributeSet) table2 on (M_AttributeUse.M_AttributeSet_ID = table2.M_AttributeSet_ID) left join (select M_Attribute_ID, Name from M_Attribute) table3 on (M_AttributeUse.M_Attribute_ID = table3.M_Attribute_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static AssignedAttributeData[] set(String mAttributesetId, String updatedby, String updatedbyr, String adOrgId, String mAttributeId, String adClientId, String seqno, String isactive, String createdby, String createdbyr, String mAttributeuseId)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , M_AttributeSet_ID = (?) , M_Attribute_ID = (?) , SeqNo = TO_NUMBER(?) , M_AttributeUse_ID = (?) , IsActive = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_AttributeUse.M_AttributeUse_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, M_AttributeSet_ID, M_Attribute_ID, SeqNo, M_AttributeUse_ID, IsActive, created, createdby, updated, updatedBy)" +
