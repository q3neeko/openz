 ...
 */
  public static TableReferenceData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adReferenceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Ref_Table.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Ref_Table.AD_Reference_ID, " +
      "(CASE WHEN AD_Ref_Table.AD_Reference_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(list1.name),'') ) END) AS AD_Reference_IDR, " +
      "AD_Ref_Table.AD_Table_ID, " +
      "(CASE WHEN AD_Ref_Table.AD_Table_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.TableName), ''))),'') ) END) AS AD_Table_IDR, " +
      "AD_Ref_Table.AD_Key, " +
      "(CASE WHEN AD_Ref_Table.AD_Key IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.ColumnName), ''))),'') ) END) AS AD_KeyR, " +
      "AD_Ref_Table.AD_Display, " +
      "(CASE WHEN AD_Ref_Table.AD_Display IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.ColumnName), ''))),'') ) END) AS AD_DisplayR, " +
...
Create a registry
 */
  public static TableReferenceData[] set(String adReferenceId, String adKey, String isvaluedisplayed, String orderbyclause, String adOrgId, String whereclause, String adDisplay, String adTableId, String isactive, String adClientId, String updatedby, String updatedbyr, String createdby, String createdbyr)    throws ServletException {
