 ...
 */
  public static ReferenceListData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String adReferenceId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Ref_List.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Ref_List.AD_Reference_ID, " +
      "(CASE WHEN AD_Ref_List.AD_Reference_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(list1.name),'') ) END) AS AD_Reference_IDR, " +
      "AD_Ref_List.SeqNo, " +
      "AD_Ref_List.AD_Module_ID, " +
      "(CASE WHEN AD_Ref_List.AD_Module_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Version), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_Module_IDR, " +
      "AD_Ref_List.Value, " +
      "AD_Ref_List.Name, " +
      ...
      "(CASE WHEN AD_Ref_List.AD_Window_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS AD_Window_IDR, " +
      "AD_Ref_List.AD_Tab_ID, " +
      "(CASE WHEN AD_Ref_List.AD_Tab_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL7.Name IS NULL THEN TO_CHAR(table7.Name) ELSE TO_CHAR(tableTRL7.Name) END)), ''))),'') ) END) AS AD_Tab_IDR, " +
      "AD_Ref_List.AD_Ref_List_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM AD_Ref_List left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Ref_List.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Reference_ID, Name, ValidationType from AD_Reference) table2 on (AD_Ref_List.AD_Reference_ID = table2.AD_Reference_ID) left join (select AD_Reference_ID,AD_Language, Name from AD_Reference_TRL) tableTRL2 on (table2.AD_Reference_ID = tableTRL2.AD_Reference_ID and tableTRL2.AD_Language = ?)  left join ad_ref_list_v list1 on (table2.ValidationType = list1.value and list1.ad_reference_id = '2' and list1.ad_language = ?)  left join (select AD_Module_ID, Name, Version, AD_Language from AD_Module) table3 on (AD_Ref_List.AD_Module_ID = table3.AD_Module_ID) left join (select AD_Language, Name from AD_Language) table4 on (table3.AD_Language =  table4.AD_Language) left join (select AD_Window_ID, Name from AD_Window) table5 on (AD_Ref_List.AD_Window_ID = table5.AD_Window_ID) left join (select AD_Window_ID,AD_Language, Name from AD_Window_TRL) tableTRL5 on (table5.AD_Window_ID = tableTRL5.AD_Window_ID and tableTRL5.AD_Language = ?)  left join (select AD_Tab_ID, Name, AD_Window_ID from AD_Tab) table6 on (AD_Ref_List.AD_Tab_ID = table6.AD_Tab_ID) left join (select AD_Tab_ID,AD_Language, Name from AD_Tab_TRL) tableTRL6 on (table6.AD_Tab_ID = tableTRL6.AD_Tab_ID and tableTRL6.AD_Language = ?)  left join (select AD_Window_ID, Name from AD_Window) table7 on (table6.AD_Window_ID = table7.AD_Window_ID) left join (select AD_Window_ID,AD_Language, Name from AD_Window_TRL) tableTRL7 on (table7.AD_Window_ID = tableTRL7.AD_Window_ID and tableTRL7.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ReferenceListData[] set(String adReferenceId, String isdefault, String adClientId, String description, String adWindowId, String adModuleId, String adTabId, String createdby, String createdbyr, String adRefListId, String seqno, String updatedby, String updatedbyr, String value, String name, String adOrgId, String isactive)    throws ServletException {
