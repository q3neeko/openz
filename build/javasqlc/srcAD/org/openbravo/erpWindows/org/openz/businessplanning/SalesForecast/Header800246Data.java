 ...
 */
  public static Header800246Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN bpl_salesforecast.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "bpl_salesforecast.C_Bpartner_ID, " +
      "(CASE WHEN bpl_salesforecast.C_Bpartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Bpartner_IDR, " +
      "bpl_salesforecast.Rating, " +
      "bpl_salesforecast.Description, " +
      "bpl_salesforecast.Salesrep_ID, " +
      "(CASE WHEN bpl_salesforecast.Salesrep_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS Salesrep_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM bpl_salesforecast left join (select AD_Org_ID, Name from AD_Org) table1 on (bpl_salesforecast.AD_Org_ID = table1.AD_Org_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table2 on (bpl_salesforecast.C_Bpartner_ID = table2.C_BPartner_ID) left join (select AD_User_ID, Name from AD_User) table3 on (bpl_salesforecast.Salesrep_ID =  table3.AD_User_ID) left join ad_ref_list_v list1 on (bpl_salesforecast.Estpropability = list1.value and list1.ad_reference_id = 'DD6AA16AB1DD4DF5AD11A2665AE665BC' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static Header800246Data[] set(String orderedamtfirstquarter, String offeredamtfourthquarter, String linenetamt, String invoicedamtsecondquarter, String invoicedamtfourthquarter, String salesrepId, String createdby, String createdbyr, String orderedamtfourthquarter, String bplSalesforecastId, String orderedamtsecondquarter, String description, String offeredamtthirdquarter, String orderedamtthirdquarter, String adjustedAmt, String rating, String startdate, String invoicedamtfirstquarter, String offeredamtsecondquarter, String adClientId, String offeredamtfirstquarter, String adjustedStartdate, String updatedby, String updatedbyr, String isactive, String cBpartnerId, String cBpartnerIdr, String adjustedEnddate, String invoicedamtthirdquarter, String estpropability, String adOrgId)    throws ServletException {
