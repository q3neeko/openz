 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBankstatementId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_BankStatementLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_BankStatementLine.C_BankStatement_ID, " +
      "(CASE WHEN C_BankStatementLine.C_BankStatement_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BankStatement_IDR, " +
      ...
      "(CASE WHEN C_BankStatementLine.C_Debt_Payment_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(list1.name),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table3.Dateplanned, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Amount), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.IsReceipt), ''))),'') ) END) AS C_Debt_Payment_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_BankStatementLine left join (select AD_Org_ID, Name from AD_Org) table1 on (C_BankStatementLine.AD_Org_ID = table1.AD_Org_ID) left join (select C_BankStatement_ID, Name from C_BankStatement) table2 on (C_BankStatementLine.C_BankStatement_ID = table2.C_BankStatement_ID) left join (select C_Debt_Payment_ID, C_BPartner_ID, PaymentRule, Dateplanned, Amount, IsReceipt from C_Debt_Payment) table3 on (C_BankStatementLine.C_Debt_Payment_ID = table3.C_Debt_Payment_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table4 on (table3.C_BPartner_ID = table4.C_BPartner_ID) left join ad_ref_list_v list1 on (table3.PaymentRule = list1.value and list1.ad_reference_id = '195' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static LinesData[] set(String cBankstatementId, String cBankstatementlineId, String trxamt, String adClientId, String isvoided, String referenceno, String foreigncurrency, String cCurrencyId, String voidline, String foreigncurrencyamt, String convertchargeamt, String line, String isactive, String valutadate, String cGlitemId, String foreigncurrencyrate, String chargeamt, String memo, String createdby, String createdbyr, String cDebtPaymentId, String cDebtPaymentIdr, String dateacct, String stmtamt, String cChargeId, String discountamt, String description, String adOrgId, String sepaexportenabled, String cDebtPaymentCreate, String updatedby, String updatedbyr)    throws ServletException {
 ...
 */
  public static String selectDef800404_1(ConnectionProvider connectionProvider, String C_Debt_Payment_IDR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT  ( COALESCE(TO_CHAR(table2.Value), '')  || ' - ' || COALESCE(TO_CHAR(table2.Name), '')  || ' - ' || COALESCE(TO_CHAR(table1.PaymentRule), '')  || ' - ' || COALESCE(TO_CHAR(table1.Dateplanned), '')  || ' - ' || COALESCE(TO_CHAR(table1.Amount), '')  || ' - ' || COALESCE(TO_CHAR(table1.IsReceipt), '') ) as C_Debt_Payment_ID FROM C_Debt_Payment table1 left join C_BPartner table2 on (table1.C_BPartner_ID = table2.C_BPartner_ID) WHERE table1.isActive='Y' AND table1.C_Debt_Payment_ID = ?  ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_BankStatement_ID = (?) , Line = TO_NUMBER(?) , Description = (?) , ValutaDate = TO_DATE(?) , DateAcct = TO_DATE(?) , C_Debt_Payment_ID = (?) , TrxAmt = TO_NUMBER(?) , Memo = (?) , Sepaexportenabled = (?) , C_Currency_ID = (?) , Discountamt = TO_NUMBER(?) , StmtAmt = TO_NUMBER(?) , Foreigncurrencyamt = TO_NUMBER(?) , C_Glitem_ID = (?) , IsActive = (?) , Foreigncurrencyrate = TO_NUMBER(?) , C_Charge_ID = (?) , Isvoided = (?) , Foreigncurrency = (?) , ChargeAmt = TO_NUMBER(?) , C_Debt_Payment_Create = (?) , C_BankStatementLine_ID = (?) , ConvertChargeAmt = TO_NUMBER(?) , Voidline = (?) , ReferenceNo = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_BankStatementLine.C_BankStatementLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, C_BankStatement_ID, Line, Description, ValutaDate, DateAcct, C_Debt_Payment_ID, TrxAmt, Memo, Sepaexportenabled, C_Currency_ID, Discountamt, StmtAmt, Foreigncurrencyamt, C_Glitem_ID, IsActive, Foreigncurrencyrate, C_Charge_ID, Isvoided, Foreigncurrency, ChargeAmt, C_Debt_Payment_Create, C_BankStatementLine_ID, ConvertChargeAmt, Voidline, ReferenceNo, created, createdby, updated, updatedBy)" +
