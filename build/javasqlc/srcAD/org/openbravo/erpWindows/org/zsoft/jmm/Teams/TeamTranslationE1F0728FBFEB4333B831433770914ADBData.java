 ...
 */
  public static TeamTranslationE1F0728FBFEB4333B831433770914ADBData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String juwimmTeamId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN juwimm_team_trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "juwimm_team_trl.Juwimm_Team_ID, " +
      "(CASE WHEN juwimm_team_trl.Juwimm_Team_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS Juwimm_Team_IDR, " +
      "juwimm_team_trl.AD_Language, " +
      "(CASE WHEN juwimm_team_trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_LanguageR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM juwimm_team_trl left join (select AD_Org_ID, Name from AD_Org) table1 on (juwimm_team_trl.AD_Org_ID = table1.AD_Org_ID) left join (select Juwimm_Team_ID, Name from Juwimm_Team) table2 on (juwimm_team_trl.Juwimm_Team_ID = table2.Juwimm_Team_ID) left join (select Juwimm_Team_ID,AD_Language, Name from juwimm_team_TRL) tableTRL2 on (table2.Juwimm_Team_ID = tableTRL2.Juwimm_Team_ID and tableTRL2.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table3 on (juwimm_team_trl.AD_Language =  table3.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
