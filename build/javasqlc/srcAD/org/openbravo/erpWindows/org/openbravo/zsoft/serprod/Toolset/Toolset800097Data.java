 ...
 */
  public static Toolset800097Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maToolsetTypeId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Toolset.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(MA_Toolset.IsActive, 'N') AS IsActive, " +
      "MA_Toolset.MA_Toolset_Type_ID, " +
      "(CASE WHEN MA_Toolset.MA_Toolset_Type_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Toolset_Type_IDR, " +
      "MA_Toolset.Value, " +
      "COALESCE(MA_Toolset.Discarded, 'N') AS Discarded, " +
      "MA_Toolset.Numberuses, " +
      "MA_Toolset.M_Locator_ID, " +
      "(CASE WHEN MA_Toolset.M_Locator_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'') ) END) AS M_Locator_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Toolset left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Toolset.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Toolset_Type_ID, Name from MA_Toolset_Type) table2 on (MA_Toolset.MA_Toolset_Type_ID = table2.MA_Toolset_Type_ID) left join (select M_Locator_ID, Value from M_Locator) table3 on (MA_Toolset.M_Locator_ID = table3.M_Locator_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
