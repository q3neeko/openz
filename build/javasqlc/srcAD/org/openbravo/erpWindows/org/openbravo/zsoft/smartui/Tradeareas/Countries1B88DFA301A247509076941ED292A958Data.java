 ...
 */
  public static Countries1B88DFA301A247509076941ED292A958Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String zssiTradeareaId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN zssi_tradearea_country.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "zssi_tradearea_country.Zssi_Tradearea_ID, " +
      "zssi_tradearea_country.C_Country_ID, " +
      "(CASE WHEN zssi_tradearea_country.C_Country_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS C_Country_IDR, " +
      "COALESCE(zssi_tradearea_country.Isactive, 'N') AS Isactive, " +
      "        ? AS LANGUAGE " +
      "        FROM zssi_tradearea_country left join (select AD_Org_ID, Name from AD_Org) table1 on (zssi_tradearea_country.AD_Org_ID = table1.AD_Org_ID) left join (select C_Country_ID, Name from C_Country) table2 on (zssi_tradearea_country.C_Country_ID = table2.C_Country_ID) left join (select C_Country_ID,AD_Language, Name from C_Country_TRL) tableTRL2 on (table2.C_Country_ID = tableTRL2.C_Country_ID and tableTRL2.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Countries1B88DFA301A247509076941ED292A958Data[] set(String zssiTradeareaId, String isactive, String zssiTradeareaCountryId, String adOrgId, String adClientId, String createdby, String createdbyr, String updatedby, String updatedbyr, String cCountryId)    throws ServletException {
