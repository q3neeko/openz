 ...
 */
  public static Lines800077Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String aAmortizationId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN A_Amortizationline.A_Amortization_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.StartDate, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.EndDate, 'DD-MM-YYYY')),'') ) END) AS A_Amortization_IDR, " +
      "COALESCE(A_Amortizationline.IsActive, 'N') AS IsActive, " +
      "A_Amortizationline.Line, " +
      "A_Amortizationline.A_Asset_ID, " +
      "(CASE WHEN A_Amortizationline.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM A_Amortizationline left join (select AD_Org_ID, Name from AD_Org) table1 on (A_Amortizationline.AD_Org_ID = table1.AD_Org_ID) left join (select A_Amortization_ID, Name, StartDate, EndDate from A_Amortization) table2 on (A_Amortizationline.A_Amortization_ID = table2.A_Amortization_ID) left join (select A_Asset_ID, Name from A_Asset) table3 on (A_Amortizationline.A_Asset_ID = table3.A_Asset_ID) left join (select C_Currency_ID, ISO_Code from C_Currency) table4 on (A_Amortizationline.C_Currency_ID = table4.C_Currency_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Lines800077Data[] set(String aAmortizationId, String aAssetId, String adClientId, String amortizationPercentage, String aAmortizationlineId, String isactive, String line, String updatedby, String updatedbyr, String adOrgId, String amortizationamt, String createdby, String createdbyr, String cCurrencyId)    throws ServletException {
