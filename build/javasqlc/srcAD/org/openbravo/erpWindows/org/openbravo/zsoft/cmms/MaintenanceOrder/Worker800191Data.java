 ...
 */
  public static Worker800191Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maMaintPartId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Maint_Worker.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Maint_Worker.MA_Maint_Part_ID, " +
      "(CASE WHEN MA_Maint_Worker.MA_Maint_Part_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'') ) END) AS MA_Maint_Part_IDR, " +
      "COALESCE(MA_Maint_Worker.IsActive, 'N') AS IsActive, " +
      "MA_Maint_Worker.C_BPartner_ID, " +
      "(CASE WHEN MA_Maint_Worker.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Maint_Worker left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Maint_Worker.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Maint_Part_ID, DocumentNo from MA_Maint_Part) table2 on (MA_Maint_Worker.MA_Maint_Part_ID = table2.MA_Maint_Part_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (MA_Maint_Worker.C_BPartner_ID = table3.C_BPartner_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Worker800191Data[] set(String maMaintPartId, String adClientId, String adOrgId, String maMaintWorkerId, String isactive, String updatedby, String updatedbyr, String cBpartnerId, String createdby, String createdbyr)    throws ServletException {
