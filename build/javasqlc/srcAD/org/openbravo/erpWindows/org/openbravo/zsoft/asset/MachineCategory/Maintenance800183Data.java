 ...
 */
  public static Maintenance800183Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maMachineTypeId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Maintenance.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Maintenance.MA_Machine_Type_ID, " +
      "(CASE WHEN MA_Maintenance.MA_Machine_Type_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Machine_Type_IDR, " +
      "MA_Maintenance.Name, " +
      ...
      "(CASE WHEN MA_Maintenance.MA_Maint_Operation_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS MA_Maint_Operation_IDR, " +
      "MA_Maintenance.Plannedtime, " +
      "COALESCE(MA_Maintenance.Explode, 'N') AS Explode, " +
      "MA_Maintenance.MA_Machine_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Maintenance left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Maintenance.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Machine_Type_ID, Name from MA_Machine_Type) table2 on (MA_Maintenance.MA_Machine_Type_ID = table2.MA_Machine_Type_ID) left join ad_ref_list_v list1 on (MA_Maintenance.Maintenance_Type = list1.value and list1.ad_reference_id = '800033' and list1.ad_language = ?)  left join (select MA_Maint_Operation_ID, Name from MA_Maint_Operation) table3 on (MA_Maintenance.MA_Maint_Operation_ID = table3.MA_Maint_Operation_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Maintenance800183Data[] set(String maMachineTypeId, String maMachineId, String isactive, String adOrgId, String createdby, String createdbyr, String adClientId, String plannedtime, String maMaintenanceId, String description, String name, String explode, String updatedby, String updatedbyr, String maMaintOperationId, String maintenanceType)    throws ServletException {
