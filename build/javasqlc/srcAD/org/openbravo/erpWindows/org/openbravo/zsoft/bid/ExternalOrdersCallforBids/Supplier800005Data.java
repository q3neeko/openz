 ...
 */
  public static Supplier800005Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cProjectId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Project_Vendor.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Project_Vendor.C_Project_ID, " +
      "(CASE WHEN C_Project_Vendor.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_Project_Vendor.C_BPartner_ID, " +
      "(CASE WHEN C_Project_Vendor.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "COALESCE(C_Project_Vendor.IsActive, 'N') AS IsActive, " +
      "C_Project_Vendor.Note, " +
      "C_Project_Vendor.M_PriceList_ID, " +
      "(CASE WHEN C_Project_Vendor.M_PriceList_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS M_PriceList_IDR, " +
      "C_Project_Vendor.GenerateOrder, " +
      "C_Project_Vendor.C_Incoterms_ID, " +
      "(CASE WHEN C_Project_Vendor.C_Incoterms_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_Incoterms_IDR, " +
      "C_Project_Vendor.Incoterms_Description, " +
      "C_Project_Vendor.M_PriceList_Version_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Project_Vendor left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Project_Vendor.AD_Org_ID = table1.AD_Org_ID) left join (select C_Project_ID, Value, Name from C_Project) table2 on (C_Project_Vendor.C_Project_ID = table2.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (C_Project_Vendor.C_BPartner_ID = table3.C_BPartner_ID) left join (select M_PriceList_ID, Name from M_PriceList) table4 on (C_Project_Vendor.M_PriceList_ID = table4.M_PriceList_ID) left join (select C_Incoterms_ID, Name from C_Incoterms) table5 on (C_Project_Vendor.C_Incoterms_ID = table5.C_Incoterms_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Supplier800005Data[] set(String cProjectId, String note, String mPricelistId, String adClientId, String updatedby, String updatedbyr, String cBpartnerId, String cBpartnerIdr, String incotermsDescription, String isactive, String mPricelistVersionId, String cProjectVendorId, String cIncotermsId, String adOrgId, String generateorder, String createdby, String createdbyr)    throws ServletException {
