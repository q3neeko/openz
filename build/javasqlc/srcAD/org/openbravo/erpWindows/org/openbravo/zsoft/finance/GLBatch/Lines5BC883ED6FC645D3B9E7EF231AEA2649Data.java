 ...
 */
  public static Lines5BC883ED6FC645D3B9E7EF231AEA2649Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String zsfiManualacctId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN zsfi_macctline.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET Zsfi_Manualacct_ID = (?) , Line = TO_NUMBER(?) , Acctdate = TO_DATE(?) , Acctcr = (?) , Acctdr = (?) , Amt = TO_NUMBER(?) , Isdr2cr = (?) , Isgross = (?) , C_Tax_ID = (?) , Description = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , M_Product_ID = (?) , Glstatus = (?) , Cancel = (?) , Zsfi_Macctline_ID = (?) , C_Projectphase_ID = (?) , AD_Client_ID = (?) , Isactive = (?) , AD_Org_ID = (?) , Processing = (?) , updated = now(), updatedby = ? " +
      "        WHERE zsfi_macctline.Zsfi_Macctline_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Zsfi_Manualacct_ID, Line, Acctdate, Acctcr, Acctdr, Amt, Isdr2cr, Isgross, C_Tax_ID, Description, A_Asset_ID, C_Project_ID, C_Projecttask_ID, M_Product_ID, Glstatus, Cancel, Zsfi_Macctline_ID, C_Projectphase_ID, AD_Client_ID, Isactive, AD_Org_ID, Processing, created, createdby, updated, updatedBy)" +
