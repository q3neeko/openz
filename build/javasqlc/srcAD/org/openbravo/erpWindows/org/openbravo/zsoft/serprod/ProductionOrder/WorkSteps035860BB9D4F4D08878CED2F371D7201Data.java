...
static Logger log4j = Logger.getLogger(WorkSteps035860BB9D4F4D08878CED2F371D7201Data.class);
  ...
  public String isautocloseworkstep;
  ...
  public String getField(String fieldName) {
    ...
    else if (fieldName.equalsIgnoreCase("isautocloseworkstep"))
      return isautocloseworkstep;
 ...
 */
  public static WorkSteps035860BB9D4F4D08878CED2F371D7201Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String zssmProductionorderVId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "zssm_workstep_v.C_Color_ID, " +
      "(CASE WHEN zssm_workstep_v.C_Color_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Color_IDR, " +
      ...
      "(CASE WHEN zssm_workstep_v.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      ...
      "COALESCE(zssm_workstep_v.Isautocloseworkstep, 'N') AS Isautocloseworkstep, " +
      "zssm_workstep_v.StartDate, " +
      "zssm_workstep_v.Enddate, " +
      "zssm_workstep_v.Receiving_Locator, " +
      "(CASE WHEN zssm_workstep_v.Receiving_Locator IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'') ) END) AS Receiving_LocatorR, " +
      "zssm_workstep_v.Issuing_Locator, " +
      "(CASE WHEN zssm_workstep_v.Issuing_Locator IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'') ) END) AS Issuing_LocatorR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM zssm_workstep_v left join (select zssm_productionorder_v_id, Value, Name from zssm_productionorder_v) table1 on (zssm_workstep_v.zssm_productionorder_v_id = table1.zssm_productionorder_v_id) left join (select C_Color_ID, Name from C_Color) table2 on (zssm_workstep_v.C_Color_ID = table2.C_Color_ID) left join (select M_Product_ID, Value, Name from M_Product) table3 on (zssm_workstep_v.M_Product_ID = table3.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL3 on (table3.M_Product_ID = tableTRL3.M_Product_ID and tableTRL3.AD_Language = ?)  left join (select M_Locator_ID, Value from M_Locator) table4 on (zssm_workstep_v.Receiving_Locator = table4.M_Locator_ID) left join (select M_Locator_ID, Value from M_Locator) table5 on (zssm_workstep_v.Issuing_Locator = table5.M_Locator_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data.cColorId = UtilSql.getValue(result, "c_color_id");
        objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data.cColorIdr = UtilSql.getValue(result, "c_color_idr");
        ...
        objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data.isautocloseworkstep = UtilSql.getValue(result, "isautocloseworkstep");
 ...
 */
  public static WorkSteps035860BB9D4F4D08878CED2F371D7201Data[] set(String zssmProductionorderVId, String weightproportion, String materialcost, String priceactual, String updatedby, String updatedbyr, String cProjecttaskId, String outsourcing, String unplanmaterial, String adOrgId, String zssmPrjName, String triggerreason, String cColorId, String canceltask, String actualcost, String timeplanned, String mProductId, String mProductIdr, String ismaterialdisposed, String servcost, String startdate, String createdby, String createdbyr, String returnmaterialtostock, String schedulestatus, String issuingLocator, String issuingLocatorr, String datecontract, String qtyproduced, String machinecost, String iscomplete, String cTaskId, String servcostplan, String assembly, String startonlywithcompletematerial, String materialcostplan, String qtyleft, String percentdone, String value, String percentrejects, String gotopurchasing, String taskbegun, String help, String setuptime, String expenses, String planmaterial, String machinecostplan, String isautotriggered, String forcematerialscan, String isautocloseworkstep, String timeperpiece, String indirectcostplan, String createbom, String name, String invoicedamt, String qty, String receivingLocator, String receivingLocatorr, String zssmWorkstepVId, String zssmPrjProjectcategory, String getmaterialfromstock, String iscommitceiling, String ended, String cOrderlineId, String adClientId, String begintask, String committedamt, String plannedcost, String seqno, String isactive, String endtask, String started, String zssmProductionplanTaskId, String description, String enddate, String istaskcancelled, String indirectcost)    throws ServletException {
    ...
    objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data[0].cColorId = cColorId;
    objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data[0].cColorIdr = "";
    ...
    objectWorkSteps035860BB9D4F4D08878CED2F371D7201Data[0].isautocloseworkstep = isautocloseworkstep;
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET zssm_productionorder_v_id = (?) , SeqNo = TO_NUMBER(?) , C_Color_ID = (?) , Value = (?) , Name = (?) , Description = (?) , Triggerreason = (?) , Qty = TO_NUMBER(?) , Qtyproduced = TO_NUMBER(?) , Weightproportion = TO_NUMBER(?) , Assembly = (?) , M_Product_ID = (?) , Forcematerialscan = (?) , Startonlywithcompletematerial = (?) , Isautocloseworkstep = (?) , StartDate = TO_DATE(?) , Enddate = TO_DATE(?) , Receiving_Locator = (?) , Issuing_Locator = (?) , Timeperpiece = TO_NUMBER(?) , Setuptime = TO_NUMBER(?) , Timeplanned = TO_NUMBER(?) , Materialcostplan = TO_NUMBER(?) , Materialcost = TO_NUMBER(?) , Servcostplan = TO_NUMBER(?) , Servcost = TO_NUMBER(?) , Machinecostplan = TO_NUMBER(?) , Machinecost = TO_NUMBER(?) , Plannedcost = TO_NUMBER(?) , Actualcost = TO_NUMBER(?) , Taskbegun = (?) , Started = TO_DATE(?) , Iscomplete = (?) , Ended = TO_DATE(?) , BeginTask = (?) , Endtask = (?) , Help = (?) , Zssm_Prj_Name = (?) , AD_Client_ID = (?) , Zssm_Workstep_V_ID = (?) , Unplanmaterial = (?) , Percentrejects = TO_NUMBER(?) , Invoicedamt = TO_NUMBER(?) , Istaskcancelled = (?) , Datecontract = TO_DATE(?) , Schedulestatus = (?) , Committedamt = TO_NUMBER(?) , Getmaterialfromstock = (?) , AD_Org_ID = (?) , Iscommitceiling = (?) , Indirectcostplan = TO_NUMBER(?) , Indirectcost = TO_NUMBER(?) , C_Task_ID = (?) , Createbom = (?) , IsActive = (?) , Zssm_Prj_Projectcategory = (?) , Outsourcing = (?) , Canceltask = (?) , Gotopurchasing = (?) , Qtyleft = TO_NUMBER(?) , Ismaterialdisposed = (?) , Zssm_Productionplan_Task_ID = (?) , Isautotriggered = (?) , Returnmaterialtostock = (?) , Expenses = TO_NUMBER(?) , Planmaterial = (?) , Percentdone = TO_NUMBER(?) , PriceActual = TO_NUMBER(?) , C_Orderline_ID = (?) , C_ProjectTask_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE zssm_workstep_v.Zssm_Workstep_V_ID = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, isautocloseworkstep);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (zssm_productionorder_v_id, SeqNo, C_Color_ID, Value, Name, Description, Triggerreason, Qty, Qtyproduced, Weightproportion, Assembly, M_Product_ID, Forcematerialscan, Startonlywithcompletematerial, Isautocloseworkstep, StartDate, Enddate, Receiving_Locator, Issuing_Locator, Timeperpiece, Setuptime, Timeplanned, Materialcostplan, Materialcost, Servcostplan, Servcost, Machinecostplan, Machinecost, Plannedcost, Actualcost, Taskbegun, Started, Iscomplete, Ended, BeginTask, Endtask, Help, Zssm_Prj_Name, AD_Client_ID, Zssm_Workstep_V_ID, Unplanmaterial, Percentrejects, Invoicedamt, Istaskcancelled, Datecontract, Schedulestatus, Committedamt, Getmaterialfromstock, AD_Org_ID, Iscommitceiling, Indirectcostplan, Indirectcost, C_Task_ID, Createbom, IsActive, Zssm_Prj_Projectcategory, Outsourcing, Canceltask, Gotopurchasing, Qtyleft, Ismaterialdisposed, Zssm_Productionplan_Task_ID, Isautotriggered, Returnmaterialtostock, Expenses, Planmaterial, Percentdone, PriceActual, C_Orderline_ID, C_ProjectTask_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_DATE(?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_DATE(?), (?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), now(), ?, now(), ?)";
