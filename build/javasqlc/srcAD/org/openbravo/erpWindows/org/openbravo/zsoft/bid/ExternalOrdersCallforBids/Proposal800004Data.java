 ...
 */
  public static Proposal800004Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cProjectId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_ProjectProposal.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_ProjectProposal.C_Project_ID, " +
      "(CASE WHEN C_ProjectProposal.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_ProjectProposal.C_BPartner_ID, " +
      "(CASE WHEN C_ProjectProposal.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      ...
      "(CASE WHEN C_ProjectProposal.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "C_ProjectProposal.AD_User_ID, " +
      "(CASE WHEN C_ProjectProposal.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_User_IDR, " +
      ...
      "(CASE WHEN C_ProjectProposal.C_PaymentTerm_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL6.Name IS NULL THEN TO_CHAR(table6.Name) ELSE TO_CHAR(tableTRL6.Name) END)), ''))),'') ) END) AS C_PaymentTerm_IDR, " +
      "C_ProjectProposal.HeaderNote, " +
      "C_ProjectProposal.FootNote, " +
      "C_ProjectProposal.CopyFrom, " +
      "C_ProjectProposal.Projectwon, " +
      "        ? AS LANGUAGE " +
      "        FROM C_ProjectProposal left join (select AD_Org_ID, Name from AD_Org) table1 on (C_ProjectProposal.AD_Org_ID = table1.AD_Org_ID) left join (select C_Project_ID, Value, Name from C_Project) table2 on (C_ProjectProposal.C_Project_ID = table2.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (C_ProjectProposal.C_BPartner_ID = table3.C_BPartner_ID) left join (select C_BPartner_Location_ID, Name, Deviant_Bp_Name from C_BPartner_Location) table4 on (C_ProjectProposal.C_BPartner_Location_ID = table4.C_BPartner_Location_ID) left join (select AD_User_ID, Name from AD_User) table5 on (C_ProjectProposal.AD_User_ID = table5.AD_User_ID) left join ad_ref_list_v list1 on (C_ProjectProposal.PaymentRule = list1.value and list1.ad_reference_id = '195' and list1.ad_language = ?)  left join (select C_PaymentTerm_ID, Name from C_PaymentTerm) table6 on (C_ProjectProposal.C_PaymentTerm_ID = table6.C_PaymentTerm_ID) left join (select C_PaymentTerm_ID,AD_Language, Name from C_PaymentTerm_TRL) tableTRL6 on (table6.C_PaymentTerm_ID = tableTRL6.C_PaymentTerm_ID and tableTRL6.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Proposal800004Data[] set(String cProjectId, String cPaymenttermId, String isactive, String createdby, String createdbyr, String issotrx, String cBpartnerLocationId, String adOrgId, String cProjectproposalId, String datesend, String projectwon, String updatedby, String updatedbyr, String paymentrule, String adUserId, String adClientId, String headernote, String footnote, String copyfrom, String cBpartnerId, String cBpartnerIdr)    throws ServletException {
