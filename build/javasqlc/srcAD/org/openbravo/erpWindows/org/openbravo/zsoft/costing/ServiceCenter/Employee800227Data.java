 ...
 */
  public static Employee800227Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maCostcenterVersionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Costcenter_Employee.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Costcenter_Employee.MA_Costcenter_Version_ID, " +
      "(CASE WHEN MA_Costcenter_Employee.MA_Costcenter_Version_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.ValidFrom, 'DD-MM-YYYY')),'') ) END) AS MA_Costcenter_Version_IDR, " +
      "COALESCE(MA_Costcenter_Employee.IsActive, 'N') AS IsActive, " +
      "MA_Costcenter_Employee.C_Salary_Category_ID, " +
      "(CASE WHEN MA_Costcenter_Employee.C_Salary_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS C_Salary_Category_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM MA_Costcenter_Employee left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Costcenter_Employee.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Costcenter_Version_ID, DocumentNo, MA_Costcenter_ID, ValidFrom from MA_Costcenter_Version) table2 on (MA_Costcenter_Employee.MA_Costcenter_Version_ID = table2.MA_Costcenter_Version_ID) left join (select MA_Costcenter_ID, Name from MA_Costcenter) table3 on (table2.MA_Costcenter_ID = table3.MA_Costcenter_ID) left join (select C_Salary_Category_ID, Name from C_Salary_Category) table4 on (MA_Costcenter_Employee.C_Salary_Category_ID = table4.C_Salary_Category_ID) left join (select C_Salary_Category_ID,AD_Language, Name from C_Salary_Category_TRL) tableTRL4 on (table4.C_Salary_Category_ID = tableTRL4.C_Salary_Category_ID and tableTRL4.AD_Language = ?)  left join ad_ref_list_v list1 on (MA_Costcenter_Employee.Costuom = list1.value and list1.ad_reference_id = '800088' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Employee800227Data[] set(String maCostcenterVersionId, String cSalaryCategoryId, String updatedby, String updatedbyr, String adClientId, String maCostcenterEmployeeId, String isactive, String costuom, String createdby, String createdbyr, String quantity, String adOrgId)    throws ServletException {
