 ...
 */
  public static Machine800086Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Machine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "COALESCE(MA_Machine.IsActive, 'N') AS IsActive, " +
      "MA_Machine.Value, " +
      "MA_Machine.Name, " +
      "MA_Machine.MA_Machine_Type_ID, " +
      "(CASE WHEN MA_Machine.MA_Machine_Type_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Machine_Type_IDR, " +
      "MA_Machine.A_Asset_ID, " +
      "(CASE WHEN MA_Machine.A_Asset_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "MA_Machine.SNR_Masterdata_ID, " +
      "(CASE WHEN MA_Machine.SNR_Masterdata_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Serialnumber), ''))),'') ) END) AS SNR_Masterdata_IDR, " +
      ...
      "(CASE WHEN MA_Machine.C_Color_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_Color_IDR, " +
      "MA_Machine.MA_Machine_ID, " +
      "MA_Machine.Usecycle, " +
      "MA_Machine.Purchaseyear, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Machine left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Machine.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Machine_Type_ID, Name from MA_Machine_Type) table2 on (MA_Machine.MA_Machine_Type_ID = table2.MA_Machine_Type_ID) left join (select A_Asset_ID, Name from A_Asset) table3 on (MA_Machine.A_Asset_ID =  table3.A_Asset_ID) left join (select SNR_Masterdata_ID, M_Product_ID, Serialnumber from SNR_Masterdata) table4 on (MA_Machine.SNR_Masterdata_ID = table4.SNR_Masterdata_ID) left join (select M_Product_ID, Value, Name from M_Product) table5 on (table4.M_Product_ID = table5.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL5 on (table5.M_Product_ID = tableTRL5.M_Product_ID and tableTRL5.AD_Language = ?)  left join (select C_Color_ID, Name from C_Color) table6 on (MA_Machine.C_Color_ID = table6.C_Color_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
