 ...
 */
  public static Accounting800190Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String aAssetId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN A_Asset_Acct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "A_Asset_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN A_Asset_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "COALESCE(A_Asset_Acct.IsActive, 'N') AS IsActive, " +
      "A_Asset_Acct.A_Accumdepreciation_Acct, " +
      "(CASE WHEN A_Asset_Acct.A_Accumdepreciation_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Combination), ''))),'') ) END) AS A_Accumdepreciation_AcctR, " +
      "A_Asset_Acct.A_Depreciation_Acct, " +
      "(CASE WHEN A_Asset_Acct.A_Depreciation_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS A_Depreciation_AcctR, " +
      "A_Asset_Acct.A_Disposal_Loss, " +
      "A_Asset_Acct.A_Disposal_Gain, " +
      "        ? AS LANGUAGE " +
      "        FROM A_Asset_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (A_Asset_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table2 on (A_Asset_Acct.C_AcctSchema_ID = table2.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table3 on (A_Asset_Acct.A_Accumdepreciation_Acct = table3.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (A_Asset_Acct.A_Depreciation_Acct = table4.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
