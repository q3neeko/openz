 ...
 */
  public static ExternalOrder800002Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Project.C_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Address1), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Address2), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Postal), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.City), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS C_Location_IDR, " +
      "C_Project.LastPlannedProposalDate, " +
      "C_Project.SalesRep_ID, " +
      "(CASE WHEN C_Project.SalesRep_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS SalesRep_IDR, " +
      "C_Project.Document_Copies, " +
      "C_Project.C_BPartner_ID, " +
      "(CASE WHEN C_Project.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "C_Project.AccountNo, " +
      "C_Project.C_BPartner_Location_ID, " +
      "(CASE WHEN C_Project.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "C_Project.Responsible_ID, " +
      "C_Project.AD_User_ID, " +
      "(CASE WHEN C_Project.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table8.Name), ''))),'') ) END) AS AD_User_IDR, " +
      ...
      "(CASE WHEN C_Project.M_PriceList_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table10.Name), ''))),'') ) END) AS M_PriceList_IDR, " +
      "C_Project.C_Currency_ID, " +
      "(CASE WHEN C_Project.C_Currency_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.ISO_Code), ''))),'') ) END) AS C_Currency_IDR, " +
      "C_Project.Expmargin, " +
      "C_Project.M_Warehouse_ID, " +
      "(CASE WHEN C_Project.M_Warehouse_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table12.Name), ''))),'') ) END) AS M_Warehouse_IDR, " +
...
Create a registry
 */
  public static ExternalOrder800002Data[] set(String createdby, String createdbyr, String billtoId, String invoicedamt, String processed, String adClientId, String name, String servrevenue, String publicprivate, String iscommitment, String percentdoneyet, String schedulestatus, String closeproject, String mPricelistId, String cBpartnerId, String cBpartnerIdr, String aAssetId, String salesrepId, String cLocationId, String cLocationIdr, String expreinvoicing, String cCurrencyId, String generateto, String plannedqty, String poreference, String lastplannedproposaldate, String expexpenses, String adOrgId, String projectstatus, String datefinish, String actualcostamount, String projectcategory, String cProjecttypeId, String projectbalanceamt, String invoiceToproject, String expmargin, String updatedby, String updatedbyr, String datecontract, String cCampaignId, String isactive, String createtemppricelist, String committedamt, String value, String invoicedqty, String paymentrule, String plannedamt, String cProjectphaseId, String mPricelistVersionId, String plannedpoamt, String note, String cPaymenttermId, String projectkind, String servcost, String servmargin, String projectphase, String setprojecttype, String accountno, String cBpartnerLocationId, String adUserId, String cPhaseId, String startdate, String committedqty, String iscommitceiling, String estimatedamt, String documentCopies, String cProjectId, String responsibleId, String mWarehouseId, String issummary, String generateorder, String description, String changeprojectstatus, String changeprojectstatusBtn, String copyfrom, String created, String plannedmarginamt)    throws ServletException {
