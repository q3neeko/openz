 ...
 */
  public static GLBatchHeaderB67F4C4E5C064996B264A86E3622EF58Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN zsfi_manualacct.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      ...
      "(CASE WHEN zsfi_manualacct.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "zsfi_manualacct.C_Project_ID, " +
      "(CASE WHEN zsfi_manualacct.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "zsfi_manualacct.C_Projectphase_ID, " +
      "zsfi_manualacct.C_Projecttask_ID, " +
      "(CASE WHEN zsfi_manualacct.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "zsfi_manualacct.Post, " +
      "COALESCE(zsfi_manualacct.Processing, 'N') AS Processing, " +
      "zsfi_manualacct.Unpost, " +
      "zsfi_manualacct.Cancel, " +
      "        ? AS LANGUAGE " +
      "        FROM zsfi_manualacct left join (select AD_Org_ID, Name from AD_Org) table1 on (zsfi_manualacct.AD_Org_ID = table1.AD_Org_ID) left join ad_ref_list_v list1 on (zsfi_manualacct.Glstatus = list1.value and list1.ad_reference_id = '44E25BEF5DE54641AD01F956A1F1C096' and list1.ad_language = ?)  left join (select A_Asset_ID, Name from A_Asset) table2 on (zsfi_manualacct.A_Asset_ID = table2.A_Asset_ID) left join (select C_Project_ID, Value, Name from C_Project) table3 on (zsfi_manualacct.C_Project_ID = table3.C_Project_ID) left join (select C_Projecttask_ID, C_Project_ID, Name from C_Projecttask) table4 on (zsfi_manualacct.C_Projecttask_ID = table4.C_Projecttask_ID) left join (select C_Project_ID, Value, Name from C_Project) table5 on (table4.C_Project_ID = table5.C_Project_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static GLBatchHeaderB67F4C4E5C064996B264A86E3622EF58Data[] set(String unpost, String cancel, String isactive, String adClientId, String description, String post, String cProjectphaseId, String cProjecttaskId, String glstatus, String documentno, String zsfiManualacctId, String aAssetId, String cProjectId, String adOrgId, String acctdate, String createdby, String createdbyr, String processing, String updatedby, String updatedbyr)    throws ServletException {
