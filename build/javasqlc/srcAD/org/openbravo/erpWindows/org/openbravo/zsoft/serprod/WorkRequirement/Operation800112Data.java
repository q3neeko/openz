 ...
 */
  public static Operation800112Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maWorkrequirementId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_WRPhase.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_WRPhase.MA_Workrequirement_ID, " +
      "(CASE WHEN MA_WRPhase.MA_Workrequirement_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.StartDate, 'DD-MM-YYYY')),'') ) END) AS MA_Workrequirement_IDR, " +
      "MA_WRPhase.Description, " +
      "COALESCE(MA_WRPhase.IsActive, 'N') AS IsActive, " +
      "MA_WRPhase.MA_Sequence_ID, " +
      "(CASE WHEN MA_WRPhase.MA_Sequence_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS MA_Sequence_IDR, " +
      "MA_WRPhase.MA_Process_ID, " +
      "(CASE WHEN MA_WRPhase.MA_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS MA_Process_IDR, " +
      "MA_WRPhase.SeqNo, " +
      "MA_WRPhase.Quantity, " +
      ...
      "MA_WRPhase.Name, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_WRPhase left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_WRPhase.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Workrequirement_ID, DocumentNo, StartDate from MA_Workrequirement) table2 on (MA_WRPhase.MA_Workrequirement_ID = table2.MA_Workrequirement_ID) left join (select MA_Sequence_ID, Name from MA_Sequence) table3 on (MA_WRPhase.MA_Sequence_ID = table3.MA_Sequence_ID) left join (select MA_Process_ID, Name from MA_Process) table4 on (MA_WRPhase.MA_Process_ID = table4.MA_Process_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Operation800112Data[] set(String maWorkrequirementId, String createdby, String createdbyr, String maWrphaseId, String preptime, String groupuse, String name, String closed, String isactive, String outsourced, String seqno, String adOrgId, String maProcessId, String costcenteruse, String maSequenceId, String noqty, String adClientId, String quantity, String description, String donequantity, String updatedby, String updatedbyr)    throws ServletException {
