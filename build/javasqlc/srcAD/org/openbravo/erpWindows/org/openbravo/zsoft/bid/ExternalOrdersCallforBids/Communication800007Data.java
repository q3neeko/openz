 ...
 */
  public static Communication800007Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cProjectproposalId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_ProjectProposalTask.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_ProjectProposalTask.C_Projectproposal_ID, " +
      "(CASE WHEN C_ProjectProposalTask.C_Projectproposal_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS C_Projectproposal_IDR, " +
      "C_ProjectProposalTask.AD_User_ID, " +
      "(CASE WHEN C_ProjectProposalTask.AD_User_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "COALESCE(C_ProjectProposalTask.IsActive, 'N') AS IsActive, " +
      "C_ProjectProposalTask.Note, " +
      "        ? AS LANGUAGE " +
      "        FROM C_ProjectProposalTask left join (select AD_Org_ID, Name from AD_Org) table1 on (C_ProjectProposalTask.AD_Org_ID = table1.AD_Org_ID) left join (select C_Projectproposal_ID, C_Project_ID, C_BPartner_ID from C_Projectproposal) table2 on (C_ProjectProposalTask.C_Projectproposal_ID = table2.C_Projectproposal_ID) left join (select C_Project_ID, Value, Name from C_Project) table3 on (table2.C_Project_ID = table3.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table4 on (table2.C_BPartner_ID = table4.C_BPartner_ID) left join (select AD_User_ID, Name from AD_User) table5 on (C_ProjectProposalTask.AD_User_ID =  table5.AD_User_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Communication800007Data[] set(String cProjectproposalId, String updatedby, String updatedbyr, String created, String adOrgId, String isactive, String cProjectproposaltaskId, String adClientId, String note, String createdby, String createdbyr, String adUserId)    throws ServletException {
