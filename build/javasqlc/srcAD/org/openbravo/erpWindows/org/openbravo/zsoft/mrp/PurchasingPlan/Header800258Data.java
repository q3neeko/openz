...
static Logger log4j = Logger.getLogger(Header800258Data.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return timehorizon;
    else if (fieldName.equalsIgnoreCase("isfirstneed"))
      return isfirstneed;
 ...
 */
  public static Header800258Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "COALESCE(MRP_Run_Purchase.Isfirstneed, 'N') AS Isfirstneed, " +
      ...
      "(CASE WHEN MRP_Run_Purchase.Vendor_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS Vendor_IDR, " +
      "MRP_Run_Purchase.M_Warehouse_ID, " +
      "(CASE WHEN MRP_Run_Purchase.M_Warehouse_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS M_Warehouse_IDR, " +
      "MRP_Run_Purchase.M_Product_ID, " +
      "(CASE WHEN MRP_Run_Purchase.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL5.Name IS NULL THEN TO_CHAR(table5.Name) ELSE TO_CHAR(tableTRL5.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      "MRP_Run_Purchase.M_Product_Category_ID, " +
      "(CASE WHEN MRP_Run_Purchase.M_Product_Category_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS M_Product_Category_IDR, " +
      "MRP_Run_Purchase.C_BPartner_ID, " +
      "(CASE WHEN MRP_Run_Purchase.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM MRP_Run_Purchase left join (select AD_Org_ID, Name from AD_Org) table1 on (MRP_Run_Purchase.AD_Org_ID = table1.AD_Org_ID) left join (select MRP_Planner_ID, Name from MRP_Planner) table2 on (MRP_Run_Purchase.MRP_Planner_ID = table2.MRP_Planner_ID) left join (select C_BPartner_ID, Name from C_BPartner) table3 on (MRP_Run_Purchase.Vendor_ID =  table3.C_BPartner_ID) left join (select M_Warehouse_ID, Name from M_Warehouse) table4 on (MRP_Run_Purchase.M_Warehouse_ID = table4.M_Warehouse_ID) left join (select M_Product_ID, Value, Name from M_Product) table5 on (MRP_Run_Purchase.M_Product_ID = table5.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL5 on (table5.M_Product_ID = tableTRL5.M_Product_ID and tableTRL5.AD_Language = ?)  left join (select M_Product_Category_ID, Name from M_Product_Category) table6 on (MRP_Run_Purchase.M_Product_Category_ID = table6.M_Product_Category_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table7 on (MRP_Run_Purchase.C_BPartner_ID = table7.C_BPartner_ID) left join (select C_BP_Group_ID, Name from C_BP_Group) table8 on (MRP_Run_Purchase.C_BP_Group_ID = table8.C_BP_Group_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectHeader800258Data.isfirstneed = UtilSql.getValue(result, "isfirstneed");
...
Create a registry
 */
  public static Header800258Data[] set(String isfirstneed, String simulate, String cBpGroupId, String updatedby, String updatedbyr, String vendorId, String description, String mProductId, String mProductIdr, String launchpo, String datedoc, String createdby, String createdbyr, String securitymargin, String mProductCategoryId, String timehorizon, String adOrgId, String mrpPlannerId, String cBpartnerId, String cBpartnerIdr, String mrpRunPurchaseId, String adClientId, String isactive, String mWarehouseId, String name)    throws ServletException {
    ...
    objectHeader800258Data[0].isfirstneed = isfirstneed;
 ...
 */
  public static String selectDef803659(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select null as def from dual ";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "def");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
...
Select for auxiliar field
 */
  public static String selectDef803657(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select null as def from dual ";
...
Select for auxiliar field
 */
  public static String selectDefF5D6CA9B312D45F187C049EA4D5CD21D(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select null as def from dual ";
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Org_ID = (?) , Name = (?) , Description = (?) , IsActive = (?) , DateDoc = TO_DATE(?) , Timehorizon = TO_NUMBER(?) , Isfirstneed = (?) , MRP_Planner_ID = (?) , Vendor_ID = (?) , M_Warehouse_ID = (?) , M_Product_ID = (?) , M_Product_Category_ID = (?) , C_BPartner_ID = (?) , C_BP_Group_ID = (?) , Simulate = (?) , Launchpo = (?) , Securitymargin = TO_NUMBER(?) , MRP_Run_Purchase_ID = (?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, isfirstneed);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, Name, Description, IsActive, DateDoc, Timehorizon, Isfirstneed, MRP_Planner_ID, Vendor_ID, M_Warehouse_ID, M_Product_ID, M_Product_Category_ID, C_BPartner_ID, C_BP_Group_ID, Simulate, Launchpo, Securitymargin, MRP_Run_Purchase_ID, AD_Client_ID, created, createdby, updated, updatedBy)" +
