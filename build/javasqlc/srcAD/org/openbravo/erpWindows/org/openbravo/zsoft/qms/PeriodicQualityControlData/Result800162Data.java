 ...
 */
  public static Result800162Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maPcCaseId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_PC_Value.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_PC_Value.MA_Pc_Case_ID, " +
      "(CASE WHEN MA_PC_Value.MA_Pc_Case_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Pc_Case_IDR, " +
      "COALESCE(MA_PC_Value.IsActive, 'N') AS IsActive, " +
      "MA_PC_Value.MA_Pc_Test_ID, " +
      "(CASE WHEN MA_PC_Value.MA_Pc_Test_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS MA_Pc_Test_IDR, " +
      "MA_PC_Value.Datadate, " +
      "MA_PC_Value.TestResult, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_PC_Value left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_PC_Value.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Pc_Case_ID, Name from MA_Pc_Case) table2 on (MA_PC_Value.MA_Pc_Case_ID = table2.MA_Pc_Case_ID) left join (select MA_Pc_Test_ID, Name from MA_Pc_Test) table3 on (MA_PC_Value.MA_Pc_Test_ID = table3.MA_Pc_Test_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Result800162Data[] set(String maPcCaseId, String adClientId, String maPcTestId, String createdby, String createdbyr, String maPcValueId, String datadate, String updatedby, String updatedbyr, String adOrgId, String isactive, String testresult)    throws ServletException {
