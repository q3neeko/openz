 ...
 */
  public static VacationAccountFCC9BC855DB84CB88AB610AB388ED4DDData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cBpartnerId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN c_vacation_account.C_Bpartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS C_Bpartner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM c_vacation_account left join (select C_BPartner_ID, Value, Name from C_BPartner) table1 on (c_vacation_account.C_Bpartner_ID = table1.C_BPartner_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static VacationAccountFCC9BC855DB84CB88AB610AB388ED4DDData[] set(String cBpartnerId, String isactive, String wmonth, String wyear, String updatedby, String updatedbyr, String reason, String cBpartnerIdr, String adOrgId, String cVacationAccountId, String createdby, String createdbyr, String remaining, String adClientId)    throws ServletException {
 ...
 */
  public static String selectDef63BD76D2FFCF4283BD1CD3B8EC0DF4CF(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select case when (extract(month from now())::numeric < 10) then '0' || to_char(extract(month from now())::numeric) else to_char(extract(month from now())::numeric) end as defaultvalue ";
...
Select for auxiliar field
 */
  public static String selectDefB0A818B4C8C3435B8143294E0253B63A(ConnectionProvider connectionProvider)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select EXTRACT(year from now()) as defaultvalue ";
...
Select for auxiliar field
 */
  public static String selectDefE1BB2874FA354713B32D43B0E60A02F1(ConnectionProvider connectionProvider, String c_bpartner_id)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select zssi_getHolidayEntitlement(?, to_char(EXTRACT(month from now())::numeric), to_char(EXTRACT(year from now())::numeric)) as defaultvalue ";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET C_Bpartner_ID = (?) , Wmonth = (?) , Wyear = TO_NUMBER(?) , Remaining = TO_NUMBER(?) , Reason = (?) , C_Vacation_Account_ID = (?) , AD_Client_ID = (?) , Isactive = (?) , AD_Org_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE c_vacation_account.C_Vacation_Account_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_Bpartner_ID, Wmonth, Wyear, Remaining, Reason, C_Vacation_Account_ID, AD_Client_ID, Isactive, AD_Org_ID, created, createdby, updated, updatedBy)" +
