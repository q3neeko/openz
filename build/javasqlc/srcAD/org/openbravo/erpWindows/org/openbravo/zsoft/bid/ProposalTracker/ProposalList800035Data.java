 ...
 */
  public static ProposalList800035Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_Projectproposal_V.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Projectproposal_V.C_Project_ID, " +
      "(CASE WHEN C_Projectproposal_V.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_Projectproposal_V.C_BPartner_ID, " +
      "(CASE WHEN C_Projectproposal_V.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "COALESCE(C_Projectproposal_V.IsSOTrx, 'N') AS IsSOTrx, " +
      "C_Projectproposal_V.Datesend, " +
      "C_Projectproposal_V.C_BPartner_Location_ID, " +
      "(CASE WHEN C_Projectproposal_V.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      "C_Projectproposal_V.AD_User_ID, " +
      "(CASE WHEN C_Projectproposal_V.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Projectproposal_V left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Projectproposal_V.AD_Org_ID = table1.AD_Org_ID) left join (select C_Project_ID, Value, Name from C_Project) table2 on (C_Projectproposal_V.C_Project_ID = table2.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table3 on (C_Projectproposal_V.C_BPartner_ID = table3.C_BPartner_ID) left join (select C_BPartner_Location_ID, Name, Deviant_Bp_Name from C_BPartner_Location) table4 on (C_Projectproposal_V.C_BPartner_Location_ID = table4.C_BPartner_Location_ID) left join (select AD_User_ID, Name from AD_User) table5 on (C_Projectproposal_V.AD_User_ID = table5.AD_User_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static ProposalList800035Data[] set(String todatesend, String fromdatesend, String cProjectId, String issotrx, String lastname, String firstname, String adUserId, String datesend, String cProjectproposalId, String cBpartnerId, String cBpartnerIdr, String adClientId, String adOrgId, String cBpartnerLocationId)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET Todatesend = TO_DATE(?) , C_Projectproposal_ID = (?) , Firstname = (?) , Lastname = (?) , Fromdatesend = TO_DATE(?) , AD_Client_ID = (?) , AD_Org_ID = (?) , C_Project_ID = (?) , C_BPartner_ID = (?) , IsSOTrx = (?) , Datesend = TO_DATE(?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Todatesend, C_Projectproposal_ID, Firstname, Lastname, Fromdatesend, AD_Client_ID, AD_Org_ID, C_Project_ID, C_BPartner_ID, IsSOTrx, Datesend, C_BPartner_Location_ID, AD_User_ID, created, createdby, updated, updatedBy)" +
