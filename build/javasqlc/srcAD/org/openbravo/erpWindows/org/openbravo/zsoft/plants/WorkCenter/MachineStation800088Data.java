 ...
 */
  public static MachineStation800088Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maWorkstationId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Machinestation.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Machinestation.MA_Workstation_ID, " +
      "(CASE WHEN MA_Machinestation.MA_Workstation_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Workstation_IDR, " +
      "COALESCE(MA_Machinestation.IsActive, 'N') AS IsActive, " +
      "MA_Machinestation.MA_Machine_ID, " +
      "(CASE WHEN MA_Machinestation.MA_Machine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS MA_Machine_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Machinestation left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Machinestation.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Workstation_ID, Name from MA_Workstation) table2 on (MA_Machinestation.MA_Workstation_ID = table2.MA_Workstation_ID) left join (select MA_Machine_ID, Value, Name from MA_Machine) table3 on (MA_Machinestation.MA_Machine_ID = table3.MA_Machine_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static MachineStation800088Data[] set(String maWorkstationId, String adOrgId, String maMachineId, String createdby, String createdbyr, String updatedby, String updatedbyr, String isactive, String adClientId, String maMachinestationId)    throws ServletException {
