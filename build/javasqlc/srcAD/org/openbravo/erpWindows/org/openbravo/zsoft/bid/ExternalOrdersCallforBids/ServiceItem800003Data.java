 ...
 */
  public static ServiceItem800003Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cProjectId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_ProjectLine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_ProjectLine.C_Project_ID, " +
      "(CASE WHEN C_ProjectLine.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_ProjectLine.Line, " +
      "COALESCE(C_ProjectLine.IsActive, 'N') AS IsActive, " +
      "C_ProjectLine.Description, " +
      "C_ProjectLine.M_Product_ID, " +
      "(CASE WHEN C_ProjectLine.M_Product_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS M_Product_IDR, " +
      ...
      "(CASE WHEN C_ProjectLine.C_Tax_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'') ) END) AS C_Tax_IDR, " +
      "C_ProjectLine.PlannedPrice, " +
      "C_ProjectLine.Plannedpoprice, " +
      "C_ProjectLine.PlannedAmt, " +
      "C_ProjectLine.PlannedMarginAmt, " +
      "        ? AS LANGUAGE " +
      "        FROM C_ProjectLine left join (select AD_Org_ID, Name from AD_Org) table1 on (C_ProjectLine.AD_Org_ID = table1.AD_Org_ID) left join (select C_Project_ID, Value, Name from C_Project) table2 on (C_ProjectLine.C_Project_ID = table2.C_Project_ID) left join (select M_Product_ID, Value, Name from M_Product) table3 on (C_ProjectLine.M_Product_ID = table3.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL3 on (table3.M_Product_ID = tableTRL3.M_Product_ID and tableTRL3.AD_Language = ?)  left join (select C_Tax_ID, Name from C_Tax) table4 on (C_ProjectLine.C_Tax_ID =  table4.C_Tax_ID) left join (select C_Tax_ID,AD_Language, Name from C_Tax_TRL) tableTRL4 on (table4.C_Tax_ID = tableTRL4.C_Tax_ID and tableTRL4.AD_Language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static ServiceItem800003Data[] set(String cProjectId, String mProductId, String mProductIdr, String mProductCategoryId, String description, String committedqty, String cTaxId, String committedamt, String createdby, String createdbyr, String isactive, String productValue, String plannedprice, String processed, String plannedpoprice, String cOrderpoId, String productName, String updatedby, String updatedbyr, String cProjectissueId, String cOrderId, String productDescription, String adClientId, String plannedmarginamt, String plannedqty, String isprinted, String plannedamt, String invoicedqty, String dopricing, String line, String invoicedamt, String cProjectlineId, String adOrgId)    throws ServletException {
