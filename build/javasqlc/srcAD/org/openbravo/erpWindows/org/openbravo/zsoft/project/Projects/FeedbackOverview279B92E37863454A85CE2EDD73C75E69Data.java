 ...
 */
  public static FeedbackOverview279B92E37863454A85CE2EDD73C75E69Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cProjecttaskId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN zspm_ptaskfeedbackline.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET C_Projecttask_ID = (?) , Workdate = TO_DATE(?) , Hours = TO_NUMBER(?) , Costuom = (?) , C_Salary_Category_ID = (?) , AD_User_ID = (?) , MA_Machine_ID = (?) , Description = (?) , Actualcostamount = TO_NUMBER(?) , Url = (?) , Hour_From = TO_TIMESTAMP(?,'HH24:MI:SS') , Hour_To = TO_TIMESTAMP(?,'HH24:MI:SS') , Zspm_Ptaskfeedbackline_ID = (?) , Isactive = (?) , Isprocessed = (?) , C_Project_ID = (?) , Triggeramt = TO_NUMBER(?) , C_Calendarevent_ID = (?) , Specialtime = TO_NUMBER(?) , Dayhours = (?) , AD_Org_ID = (?) , Specialtime2 = TO_NUMBER(?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE zspm_ptaskfeedbackline.Zspm_Ptaskfeedbackline_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_Projecttask_ID, Workdate, Hours, Costuom, C_Salary_Category_ID, AD_User_ID, MA_Machine_ID, Description, Actualcostamount, Url, Hour_From, Hour_To, Zspm_Ptaskfeedbackline_ID, Isactive, Isprocessed, C_Project_ID, Triggeramt, C_Calendarevent_ID, Specialtime, Dayhours, AD_Org_ID, Specialtime2, AD_Client_ID, created, createdby, updated, updatedBy)" +
