 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET zssm_workstep_v_id = (?) , Line = TO_NUMBER(?) , SNR_Batchmasterdata_ID = (?) , M_Product_ID = (?) , Description = (?) , Rawmaterial = (?) , Constuctivemeasure = (?) , Quantity = TO_NUMBER(?) , Qtyreceived = TO_NUMBER(?) , QTY_Available = TO_NUMBER(?) , QTY_Instock = TO_NUMBER(?) , Issuing_Locator = (?) , Receiving_Locator = (?) , M_Locator_ID = (?) , Qtyinrequisition = TO_NUMBER(?) , AD_Client_ID = (?) , QTY_Plan = TO_NUMBER(?) , Date_Plan = TO_DATE(?) , Zspm_Projecttaskbom_ID = (?) , Qtyreserved = TO_NUMBER(?) , Cutoff = TO_NUMBER(?) , IsActive = (?) , Actualcosamount = TO_NUMBER(?) , Zssm_WorkstepBOM_V_ID = (?) , AD_Org_ID = (?) , Planrequisition = (?) , updated = now(), updatedby = ? " +
      "        WHERE zssm_workstepbom_v.Zssm_WorkstepBOM_V_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (zssm_workstep_v_id, Line, SNR_Batchmasterdata_ID, M_Product_ID, Description, Rawmaterial, Constuctivemeasure, Quantity, Qtyreceived, QTY_Available, QTY_Instock, Issuing_Locator, Receiving_Locator, M_Locator_ID, Qtyinrequisition, AD_Client_ID, QTY_Plan, Date_Plan, Zspm_Projecttaskbom_ID, Qtyreserved, Cutoff, IsActive, Actualcosamount, Zssm_WorkstepBOM_V_ID, AD_Org_ID, Planrequisition, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_DATE(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), (?), now(), ?, now(), ?)";
