 ...
 */
  public static CheckPointSet800153Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maMeasureShiftId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Measure_Group.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Measure_Group.MA_Measure_Shift_ID, " +
      "(CASE WHEN MA_Measure_Group.MA_Measure_Shift_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(table2.Measuredate, 'DD-MM-YYYY')),'') ) END) AS MA_Measure_Shift_IDR, " +
      "COALESCE(MA_Measure_Group.IsActive, 'N') AS IsActive, " +
      "MA_Measure_Group.MA_CCP_Group_ID, " +
      "(CASE WHEN MA_Measure_Group.MA_CCP_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS MA_CCP_Group_IDR, " +
      "MA_Measure_Group.SeqNo, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Measure_Group left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Measure_Group.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Measure_Shift_ID, Measuredate from MA_Measure_Shift) table2 on (MA_Measure_Group.MA_Measure_Shift_ID = table2.MA_Measure_Shift_ID) left join (select MA_CCP_Group_ID, Name from MA_CCP_Group) table3 on (MA_Measure_Group.MA_CCP_Group_ID = table3.MA_CCP_Group_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
