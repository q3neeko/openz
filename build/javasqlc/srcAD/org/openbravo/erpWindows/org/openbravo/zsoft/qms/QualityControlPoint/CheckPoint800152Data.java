 ...
 */
  public static CheckPoint800152Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maCcpGroupId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "MA_CCP.AD_Client_ID, " +
      "MA_CCP.AD_Org_ID, " +
      "(CASE WHEN MA_CCP.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_CCP.MA_CCP_Group_ID, " +
      "(CASE WHEN MA_CCP.MA_CCP_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_CCP_Group_IDR, " +
      ...
      "MA_CCP.SeqNo, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_CCP left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_CCP.AD_Org_ID = table1.AD_Org_ID) left join (select MA_CCP_Group_ID, Name from MA_CCP_Group) table2 on (MA_CCP.MA_CCP_Group_ID = table2.MA_CCP_Group_ID) left join ad_ref_list_v list1 on (MA_CCP.Valuetype = list1.value and list1.ad_reference_id = '800039' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
