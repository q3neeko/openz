 ...
 */
  public static Cost800236Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maMachineId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Machine_Cost.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Machine_Cost.MA_Machine_ID, " +
      "(CASE WHEN MA_Machine_Cost.MA_Machine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Machine_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM MA_Machine_Cost left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Machine_Cost.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Machine_ID, Value, Name from MA_Machine) table2 on (MA_Machine_Cost.MA_Machine_ID = table2.MA_Machine_ID) left join ad_ref_list_v list1 on (MA_Machine_Cost.Costuom = list1.value and list1.ad_reference_id = 'DE781226949D4A9F8782BA610C3621C6' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Cost800236Data[] set(String maMachineId, String purchaseamt, String toolsetamt, String yearvalue, String costuomyear, String improductivehoursyear, String dayhours, String consume, String maMachineCostId, String isactive, String costuom, String createdby, String createdbyr, String adOrgId, String updatedby, String updatedbyr, String cost, String adClientId, String validfrom, String amortization, String daysyear)    throws ServletException {
