 ...
 */
  public static IndirectCost800228Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maCostcenterVersionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Costcenter_IC.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Costcenter_IC.MA_Costcenter_Version_ID, " +
      "(CASE WHEN MA_Costcenter_IC.MA_Costcenter_Version_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.ValidFrom, 'DD-MM-YYYY')),'') ) END) AS MA_Costcenter_Version_IDR, " +
      "COALESCE(MA_Costcenter_IC.IsActive, 'N') AS IsActive, " +
      "MA_Costcenter_IC.MA_Indirect_Cost_ID, " +
      "(CASE WHEN MA_Costcenter_IC.MA_Indirect_Cost_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS MA_Indirect_Cost_IDR, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_Costcenter_IC left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Costcenter_IC.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Costcenter_Version_ID, DocumentNo, MA_Costcenter_ID, ValidFrom from MA_Costcenter_Version) table2 on (MA_Costcenter_IC.MA_Costcenter_Version_ID = table2.MA_Costcenter_Version_ID) left join (select MA_Costcenter_ID, Name from MA_Costcenter) table3 on (table2.MA_Costcenter_ID = table3.MA_Costcenter_ID) left join (select MA_Indirect_Cost_ID, Name from MA_Indirect_Cost) table4 on (MA_Costcenter_IC.MA_Indirect_Cost_ID = table4.MA_Indirect_Cost_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static IndirectCost800228Data[] set(String maCostcenterVersionId, String updatedby, String updatedbyr, String maCostcenterIcId, String adClientId, String adOrgId, String isactive, String createdby, String createdbyr, String maIndirectCostId)    throws ServletException {
