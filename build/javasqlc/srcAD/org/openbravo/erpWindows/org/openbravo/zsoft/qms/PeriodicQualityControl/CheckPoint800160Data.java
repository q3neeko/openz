 ...
 */
  public static CheckPoint800160Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maPeriodicControlId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_PC_Test.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_PC_Test.MA_Periodic_Control_ID, " +
      "(CASE WHEN MA_PC_Test.MA_Periodic_Control_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS MA_Periodic_Control_IDR, " +
      "COALESCE(MA_PC_Test.IsActive, 'N') AS IsActive, " +
      "MA_PC_Test.Name, " +
      "MA_PC_Test.Description, " +
      "MA_PC_Test.Days, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_PC_Test left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_PC_Test.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Periodic_Control_ID, Name from MA_Periodic_Control) table2 on (MA_PC_Test.MA_Periodic_Control_ID = table2.MA_Periodic_Control_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static CheckPoint800160Data[] set(String maPeriodicControlId, String name, String updatedby, String updatedbyr, String isactive, String createdby, String createdbyr, String adClientId, String description, String maPcTestId, String adOrgId, String days)    throws ServletException {
