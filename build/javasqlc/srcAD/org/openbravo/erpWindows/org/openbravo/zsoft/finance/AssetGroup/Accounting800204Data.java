 ...
 */
  public static Accounting800204Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String aAssetGroupId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN A_Asset_Group_Acct.A_Asset_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS A_Asset_Group_IDR, " +
      "A_Asset_Group_Acct.C_AcctSchema_ID, " +
      "(CASE WHEN A_Asset_Group_Acct.C_AcctSchema_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS C_AcctSchema_IDR, " +
      "A_Asset_Group_Acct.A_Accumdepreciation_Acct, " +
      "(CASE WHEN A_Asset_Group_Acct.A_Accumdepreciation_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Combination), ''))),'') ) END) AS A_Accumdepreciation_AcctR, " +
      "A_Asset_Group_Acct.A_Depreciation_Acct, " +
      "(CASE WHEN A_Asset_Group_Acct.A_Depreciation_Acct IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Combination), ''))),'') ) END) AS A_Depreciation_AcctR, " +
      "A_Asset_Group_Acct.A_Disposal_Gain, " +
      "A_Asset_Group_Acct.A_Disposal_Loss, " +
      "A_Asset_Group_Acct.Processing, " +
      "        ? AS LANGUAGE " +
      "        FROM A_Asset_Group_Acct left join (select AD_Org_ID, Name from AD_Org) table1 on (A_Asset_Group_Acct.AD_Org_ID = table1.AD_Org_ID) left join (select A_Asset_Group_ID, Name from A_Asset_Group) table2 on (A_Asset_Group_Acct.A_Asset_Group_ID = table2.A_Asset_Group_ID) left join (select C_AcctSchema_ID, Name from C_AcctSchema) table3 on (A_Asset_Group_Acct.C_AcctSchema_ID = table3.C_AcctSchema_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table4 on (A_Asset_Group_Acct.A_Accumdepreciation_Acct = table4.C_ValidCombination_ID) left join (select C_ValidCombination_ID, Combination from C_ValidCombination) table5 on (A_Asset_Group_Acct.A_Depreciation_Acct = table5.C_ValidCombination_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
