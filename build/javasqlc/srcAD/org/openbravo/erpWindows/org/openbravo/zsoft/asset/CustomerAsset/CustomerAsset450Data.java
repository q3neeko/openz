 ...
 */
  public static CustomerAsset450Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "A_Asset.AD_User_ID, " +
      "(CASE WHEN A_Asset.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "A_Asset.Value, " +
      "A_Asset.Name, " +
      "A_Asset.Help, " +
      "A_Asset.Description, " +
      ...
      "(CASE WHEN A_Asset.SNR_Masterdata_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL4.Name IS NULL THEN TO_CHAR(table4.Name) ELSE TO_CHAR(tableTRL4.Name) END)), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Serialnumber), ''))),'') ) END) AS SNR_Masterdata_IDR, " +
      "A_Asset.C_Project_ID, " +
      "(CASE WHEN A_Asset.C_Project_ID IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table5.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "A_Asset.C_BPartner_ID, " +
      "(CASE WHEN A_Asset.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table6.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      "A_Asset.C_BPartner_Location_ID, " +
      "(CASE WHEN A_Asset.C_BPartner_Location_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table7.Deviant_Bp_Name), ''))),'') ) END) AS C_BPartner_Location_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM A_Asset left join (select AD_Org_ID, Name from AD_Org) table1 on (A_Asset.AD_Org_ID = table1.AD_Org_ID) left join ad_ref_list_v list1 on (A_Asset.Assettype = list1.value and list1.ad_reference_id = '65DF5707867B448CB985D4ED8FE6CB17' and list1.ad_language = ?)  left join (select AD_User_ID, Name from AD_User) table2 on (A_Asset.AD_User_ID = table2.AD_User_ID) left join (select SNR_Masterdata_ID, M_Product_ID, Serialnumber from SNR_Masterdata) table3 on (A_Asset.SNR_Masterdata_ID = table3.SNR_Masterdata_ID) left join (select M_Product_ID, Value, Name from M_Product) table4 on (table3.M_Product_ID = table4.M_Product_ID) left join (select M_Product_ID,AD_Language, Name from M_Product_TRL) tableTRL4 on (table4.M_Product_ID = tableTRL4.M_Product_ID and tableTRL4.AD_Language = ?)  left join (select C_Project_ID, Name from C_Project) table5 on (A_Asset.C_Project_ID =  table5.C_Project_ID) left join (select C_BPartner_ID, Value, Name from C_BPartner) table6 on (A_Asset.C_BPartner_ID = table6.C_BPartner_ID) left join (select C_BPartner_Location_ID, Name, Deviant_Bp_Name from C_BPartner_Location) table7 on (A_Asset.C_BPartner_Location_ID = table7.C_BPartner_Location_ID)" +
      "        WHERE 2=2 " +
...
Create a registry
 */
  public static CustomerAsset450Data[] set(String name, String cBpartnerLocationId, String guaranteedate, String isfullydepreciated, String adOrgId, String isowned, String snrMasterdataId, String assetdisposaldate, String assetservicedate, String cCurrencyId, String depreciatedvalue, String assetschedule, String aAssetId, String amortizationenddate, String isdisposed, String lot, String issummary, String uselifemonths, String createdby, String createdbyr, String value, String amortizationtype, String amortizationvalueamt, String isinposession, String adUserId, String acctvalueamt, String useunits, String cProjectId, String documentno, String cBpartnerId, String cBpartnerIdr, String residualassetvalueamt, String uselifeyears, String depreciatedpreviousamt, String amortizationstartdate, String depreciatedplan, String datecancelled, String amortizationcalctype, String serno, String isdepreciated, String description, String datepurchased, String versionno, String updatedby, String updatedbyr, String adClientId, String assetdepreciationdate, String isactive, String locationcomment, String annualamortizationpercentage, String cLocationId, String processed, String iscostcenter, String assettype, String assetvalueamt, String aAssetGroupId, String help, String lifeuseunits, String profit)    throws ServletException {
 ...
 */
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO A_Asset " +
      "        (AD_Client_ID, AD_Org_ID, Assettype, IsActive, AD_User_ID, Value, Name, Help, Description, Lot, VersionNo, SNR_Masterdata_ID, C_Project_ID, C_BPartner_ID, C_BPartner_Location_ID, LocationComment, AssetDisposalDate, GuaranteeDate, UseLifeYears, UseLifeMonths, AssetServiceDate, SerNo, DocumentNo, Datepurchased, Datecancelled, Amortizationstartdate, Amortizationenddate, Annualamortizationpercentage, C_Currency_ID, AssetValueAmt, Residualassetvalueamt, LifeUseUnits, C_Location_ID, UseUnits, Amortizationvalueamt, Depreciatedvalue, A_Asset_ID, Depreciatedpreviousamt, IsFullyDepreciated, IsOwned, Acctvalueamt, IsInPosession, Amortizationcalctype, Profit, IsDepreciated, A_Asset_Group_ID, Processed, Iscostcenter, Depreciatedplan, IsDisposed, AssetDepreciationDate, Amortizationtype, Assetschedule, Issummary, created, createdby, updated, updatedBy)" +
