 ...
 */
  public static Machine800235Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maCostcenterVersionId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_CostCenter_Machine.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_CostCenter_Machine.MA_Costcenter_Version_ID, " +
      "(CASE WHEN MA_CostCenter_Machine.MA_Costcenter_Version_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table2.ValidFrom, 'DD-MM-YYYY')),'') ) END) AS MA_Costcenter_Version_IDR, " +
      "COALESCE(MA_CostCenter_Machine.IsActive, 'N') AS IsActive, " +
      "MA_CostCenter_Machine.MA_Machine_ID, " +
      "(CASE WHEN MA_CostCenter_Machine.MA_Machine_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS MA_Machine_IDR, " +
      "MA_CostCenter_Machine.Use, " +
      "MA_CostCenter_Machine.MA_Costcenter_Machine_ID, " +
      "        ? AS LANGUAGE " +
      "        FROM MA_CostCenter_Machine left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_CostCenter_Machine.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Costcenter_Version_ID, DocumentNo, MA_Costcenter_ID, ValidFrom from MA_Costcenter_Version) table2 on (MA_CostCenter_Machine.MA_Costcenter_Version_ID = table2.MA_Costcenter_Version_ID) left join (select MA_Costcenter_ID, Name from MA_Costcenter) table3 on (table2.MA_Costcenter_ID = table3.MA_Costcenter_ID) left join (select MA_Machine_ID, Value, Name from MA_Machine) table4 on (MA_CostCenter_Machine.MA_Machine_ID = table4.MA_Machine_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Machine800235Data[] set(String maCostcenterVersionId, String isactive, String createdby, String createdbyr, String maCostcenterMachineId, String adOrgId, String updatedby, String updatedbyr, String adClientId, String maMachineId, String use)    throws ServletException {
