 ...
 */
  public static Time800154Data[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String maMeasureGroupId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MA_Measure_Time.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "MA_Measure_Time.MA_Measure_Group_ID, " +
      "(CASE WHEN MA_Measure_Time.MA_Measure_Group_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table4.Measuredate, 'DD-MM-YYYY')),'') ) END) AS MA_Measure_Group_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM MA_Measure_Time left join (select AD_Org_ID, Name from AD_Org) table1 on (MA_Measure_Time.AD_Org_ID = table1.AD_Org_ID) left join (select MA_Measure_Group_ID, MA_CCP_Group_ID, MA_Measure_Shift_ID from MA_Measure_Group) table2 on (MA_Measure_Time.MA_Measure_Group_ID = table2.MA_Measure_Group_ID) left join (select MA_CCP_Group_ID, Name from MA_CCP_Group) table3 on (table2.MA_CCP_Group_ID = table3.MA_CCP_Group_ID) left join (select MA_Measure_Shift_ID, Measuredate from MA_Measure_Shift) table4 on (table2.MA_Measure_Shift_ID = table4.MA_Measure_Shift_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static Time800154Data[] set(String maMeasureGroupId, String isactive, String createdby, String createdbyr, String explode, String maMeasureTimeId, String updatedby, String updatedbyr, String adClientId, String measurehour, String adOrgId, String processed)    throws ServletException {
