 ...
 */
  public static PeriodData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cYearId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_Period.AD_Org_ID, " +
      "(CASE WHEN C_Period.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Period.C_Year_ID, " +
      "(CASE WHEN C_Period.C_Year_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Year), ''))),'') ) END) AS C_Year_IDR, " +
      "C_Period.PeriodNo, " +
      "C_Period.Name, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM C_Period left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Period.AD_Org_ID = table1.AD_Org_ID) left join (select C_Year_ID, Year from C_Year) table2 on (C_Period.C_Year_ID = table2.C_Year_ID) left join ad_ref_list_v list1 on (C_Period.PeriodType = list1.value and list1.ad_reference_id = '115' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static PeriodData[] set(String cYearId, String name, String periodtype, String adOrgId, String updatedby, String updatedbyr, String startdate, String enddate, String adClientId, String isactive, String processing, String cPeriodId, String periodno, String createdby, String createdbyr)    throws ServletException {
