 ...
 */
  public static NonBusinessDayData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCalendarId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN C_NonBusinessDay.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_NonBusinessDay.C_Calendar_ID, " +
      "(CASE WHEN C_NonBusinessDay.C_Calendar_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Calendar_IDR, " +
      "COALESCE(C_NonBusinessDay.IsActive, 'N') AS IsActive, " +
      "C_NonBusinessDay.Name, " +
      "C_NonBusinessDay.Date1, " +
      "        ? AS LANGUAGE " +
      "        FROM C_NonBusinessDay left join (select AD_Org_ID, Name from AD_Org) table1 on (C_NonBusinessDay.AD_Org_ID = table1.AD_Org_ID) left join (select C_Calendar_ID, Name from C_Calendar) table2 on (C_NonBusinessDay.C_Calendar_ID = table2.C_Calendar_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static NonBusinessDayData[] set(String cCalendarId, String isactive, String date1, String name, String adClientId, String createdby, String createdbyr, String adOrgId, String cNonbusinessdayId, String updatedby, String updatedbyr)    throws ServletException {
