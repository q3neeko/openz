 ...
 */
  public static YearData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cCalendarId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_Year.AD_Client_ID, " +
      "C_Year.AD_Org_ID, " +
      "(CASE WHEN C_Year.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_Year.C_Calendar_ID, " +
      "(CASE WHEN C_Year.C_Calendar_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_Calendar_IDR, " +
      "C_Year.Year, " +
      "C_Year.Description, " +
      "COALESCE(C_Year.IsActive, 'N') AS IsActive, " +
      "C_Year.Processing, " +
      "        ? AS LANGUAGE " +
      "        FROM C_Year left join (select AD_Org_ID, Name from AD_Org) table1 on (C_Year.AD_Org_ID = table1.AD_Org_ID) left join (select C_Calendar_ID, Name from C_Calendar) table2 on (C_Year.C_Calendar_ID = table2.C_Calendar_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static YearData[] set(String cCalendarId, String description, String isactive, String adOrgId, String year, String processing, String createdby, String createdbyr, String adClientId, String updatedby, String updatedbyr, String cYearId)    throws ServletException {
