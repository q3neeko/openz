 ...
 */
  public static ProcessExecutionData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN AD_Process_Execution_V.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "AD_Process_Execution_V.AD_Process_ID, " +
      "(CASE WHEN AD_Process_Execution_V.AD_Process_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL2.Name IS NULL THEN TO_CHAR(table2.Name) ELSE TO_CHAR(tableTRL2.Name) END)), ''))),'') ) END) AS AD_Process_IDR, " +
      "COALESCE(AD_Process_Execution_V.Isactive, 'N') AS Isactive, " +
      "AD_Process_Execution_V.AD_User_ID, " +
      "(CASE WHEN AD_Process_Execution_V.AD_User_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table3.Name), ''))),'') ) END) AS AD_User_IDR, " +
      "TO_CHAR(AD_Process_Execution_V.Start_Time, ?) AS Start_Time, " +
      "TO_CHAR(AD_Process_Execution_V.End_Time, ?) AS End_Time, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM AD_Process_Execution_V left join (select AD_Org_ID, Name from AD_Org) table1 on (AD_Process_Execution_V.AD_Org_ID = table1.AD_Org_ID) left join (select AD_Process_ID, Name from AD_Process) table2 on (AD_Process_Execution_V.AD_Process_ID = table2.AD_Process_ID) left join (select AD_Process_ID,AD_Language, Name from AD_Process_TRL) tableTRL2 on (table2.AD_Process_ID = tableTRL2.AD_Process_ID and tableTRL2.AD_Language = ?)  left join (select AD_User_ID, Name from AD_User) table3 on (AD_Process_Execution_V.AD_User_ID = table3.AD_User_ID) left join ad_ref_list_v list1 on (AD_Process_Execution_V.Status = list1.value and list1.ad_reference_id = '7DB6D4792CDC4ED297C70A4F4282A52E' and list1.ad_language = ?) " +
      "        WHERE 2=2 " +
      "        AND 1=1 " +
...
Create a registry
 */
  public static ProcessExecutionData[] set(String isactive, String status, String adUserId, String adProcessId, String adClientId, String updatedby, String updatedbyr, String log, String isrolesecurity, String adProcessRunId, String report, String adProcessRequestId, String channel, String endTime, String startTime, String params, String result, String createdby, String createdbyr, String adOrgId, String runtime)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Client_ID = (?) , AD_Process_Run_ID = (?) , AD_Org_ID = (?) , AD_Process_ID = (?) , Isactive = (?) , AD_User_ID = (?) , Start_Time = TO_DATE(?, ?) , End_Time = TO_DATE(?, ?) , Runtime = (?) , Status = (?) , Channel = (?) , Log = (?) , Result = (?) , AD_Process_Request_ID = (?) , Report = (?) , Params = (?) , Isrolesecurity = (?) , updated = now(), updatedby = ? " +
