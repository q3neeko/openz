...
function submitFormGetParams(Command, action) {
  ...
  frm.target="_self";
  removeOnUnloadHandler(frm); // Prevents opener reload Danny Änderungen
  frm.submit();
  return true;
}
...
/**
* Callout Triggering Functions
*/
function reloadCombos(changedField) {
         submitCommandForm(changedField, false, null, '../callouts/ComboReloads.html', 'hiddenFrame', null, null, true);
         return true;
}
        
function reloadCallout(changedField, strurl) {
         submitCommandFormParameter('DEFAULT', document.forms[0].inpLastFieldChanged, changedField, false, frmMain, strurl, 'hiddenFrame', null, null, true);
         return true;
}
/**
...
function submitForm(field, value, form, bolOneFormSubmission, isCallOut, frameName) {
  ...
  if (document.getElementById('length'))
    document.getElementById('length').id = 'gollumimbalength';
  for (index = 0; index < form.length; ++index) {
    if (form[index].className.indexOf("disabledobject")==-1 && form[index].disabled==true)  {
           form[index].className = form[index].className + " disabledobject";
    }
    form[index].disabled=false;
  }
  if (document.getElementById('gollumimbalength'))
    document.getElementById('gollumimbalength').id = 'length';
...
function checkForChanges(f) {
    ...
    console.error("%o", e);
        ...
        // AUTOSAVE 
        if (form.inpenabledautosave && form.inpLastFieldChanged && (hasUserChanges || isButtonClick || isTabClick)) { // if the inpLastFieldChanged exists and there is a user change
                var autoSaveFlag = autosave;
                var continueFlag ;
                if (form.inpenabledautosave.value=='Y') {
...
function sendDirectLink(form, columnName, parentKey, url, keyId, tableId, newTarget, bolCheckChanges) {
  ...
  if (arrGeneralChange!=null && arrGeneralChange.length>0 && bolCheckChanges) {
    var strFunction = "sendDirectLink('frmMain', '" + columnName + "', '" + parentKey + "', '" + url + "', '" + keyId + "', '" + tableId + "', " + ((newTarget==null)?"null":"'" + newTarget + "'") + ", " + bolCheckChanges + ")";
...
function submitCommandForm(action, bolValidation, form, newAction, newTarget, bolOneFormSubmission, bolCheckChanges, isCallOut, controlEvt, evt) {
  ...
  if (gWaitingCallOut || (arrGeneralChange!=null && arrGeneralChange.length>0 && bolCheckChanges)) {
    var strFunction = "submitCommandForm('" + action + "', " + bolValidation + ", frmMain, " + ((newAction!=null)?("'" + newAction + "'"):"null") + ", " + ((newTarget!=null)?("'" + newTarget + "'"):"null") + ", " + bolOneFormSubmission + ", " + bolCheckChanges + ")";
    reloadFunction(strFunction);
    return false;
  }
...
function submitCommandFormParameter(action, field, value, bolValidation, form, formAction, newTarget, bolOneFormSubmission, bolCheckChanges, isCallOut, controlEvt, evt) {
  ...
  if (gWaitingCallOut || (arrGeneralChange!=null && arrGeneralChange.length>0 && bolCheckChanges)) {
    ...
    var strFunction = "submitCommandFormParameter('" + action + "', frmMain." + field.name + ", '" + value + "', " + bolValidation + ", frmMain, " + ((formAction!=null)?("'" + formAction + "'"):"null") + ", " + ((newTarget!=null)?("'" + newTarget + "'"):"null") + ", " + bolOneFormSubmission + ", " + bolCheckChanges + ", " + isCallOut + ")";
    //alert(strFunction);
    reloadFunction(strFunction);
    return false;
  }
...
/**
* Function Description
* @param {String} Command A Reference to an Command - Value - Can be evaluated in the called Servlet
* @param {Boolean} bolValidation
* @param {String} url
* @param {String} _name - appFrame or frameMenu to Indicate it is in those frames Otherwise Popup (Opens new Window)
* @param {String} processId - A Reference to an ID Value - Can be evaluated in the called Servlet
* @returns An ID reference pointing to the newly opened browser window.
* @type Object 
* @see #openPopUp 
* @see #addArrayValue
...
function formElementValue(form, ElementName, Value) {
  ...
  if (ElementName=="MESSAGE") {
    ...
    eval(Value);
  }  else if (ElementName=="TINYDESCRIPTION") {
    fillthetinyMCEdescription(Value);
    ...
    var obj = eval("document." + "frmMain" + "." + Value + ";");
    ...
    var obj = eval("document." + "frmMain" + "." + ElementName + ";");
    if (obj==null || !obj || !obj.type) {
    		return false;
    }	else{
    			fillthetinyMCE(ElementName,Value); 
...
function fillElementsFromArray(arrElements, calloutName, form) {
  ...
  if (arrElements!=null) {
    ...
    var total = arrElements.length;
    for (var x=0;x<total;x++) {
        arrGeneralChange[x+n] = new fillElements("frmMain" , arrElements[x][0], calloutName);
    }
  }
...
* @returns True if everything goes right, otherwise false. (SZ: Always return true, no care if Object exist...)
* @type Boolean
*/
...
function closeHandler(cal) {
  // Deactivated by Zisoft, function cal.sel.onchange() doesnt exist  
...
/**
* Start NEW Functions in OpenZ
*/
/** 
 * 
 * Used in Tabs together with a grid.
 * If No row was selected in the Grid, The first Row will be selected to navigate 
 * In the next TAB
*/              
function checkgridselect() {
    var obj=dijit.byId('grid');
    if (obj!=null)
        if (obj.getSelectedRows()=='') 
           obj.selectRow(0);
 }
...
function zeige(names){						//zeigen des Elements mit classname 
