...
private String processRequest(ProcessBundle bundle) throws Exception {
    ...
    for (int i=0;i<data.length;i++) {
      ...
      fi = new File(fileDir + "/tmp/content/" + BprocessCommonData.getTableName(connp, bundle.getContext().getLanguage(), data[i].adTableId) + "/" + BprocessCommonData.getIdentifierfromTabIdr(connp, data[i].adTableId, data[i].adRecordId,lang) + "-" + data[i].name );
      if (fi.exists())
        fi = new File(fileDir + "/tmp/content/" + BprocessCommonData.getTableName(connp, bundle.getContext().getLanguage(), data[i].adTableId) + "/" + BprocessCommonData.getIdentifierfromTabIdr(connp, data[i].adTableId, data[i].adRecordId,lang) + "-" + UtilsData.getUUID(connp) + "_" + data[i].name );
