...
public class ConfigureTextareaEditableAdvanced {
  ...
  private static StringBuilder doConfigureAll(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String fieldname, int leadingemptycols,int colstotal,int maxlength,boolean required,boolean readonly,String callout,String currentvalue,String labeltext, boolean isGrid, String jssettings, String action, String title, String style,String elementId) throws Exception{
    ...
    if (jssettings.equals(""))
      ...
      stdJSSettings =  "onchange=\"validateTextArea(this.id);logChanges(this);return true;\" oncontextmenu=\"changeToEditingMode('oncontextmenu');\" onpaste=\"changeToEditingMode('onpaste');\" oncut=\"changeToEditingMode('oncut');\" onkeypress=\"changeToEditingMode('onkeypress');\" onkeydown=\"changeToEditingMode('onkeydown');\" onblur=\"document.getElementById('@FIELDNAME@').innerHTML=tinyMCE.activeEditor.contentDocument.body.innerHTML;\" onkeyup=\"\"";
    for (int i = 0; i < leadingemptycols; i++) {
      retval.append("<td class=\"leadingemptycolseditadv\"></td>");
    }
    if (! isGrid)
    ...
    Replace.replace(retval, "@EDITOR@", "EditorAdvanced");
