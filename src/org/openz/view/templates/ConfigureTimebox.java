...
public class ConfigureTimebox {
  public static StringBuilder doConfigure(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String fieldname, int leadingemptycols,int colstotal,int maxlength,boolean required,boolean readonly,String callout,String currentvalue,String tooltip,String elementId,Boolean isListBased) throws Exception{
    return doConfigureAll(servlet,vars,script,fieldname, leadingemptycols,colstotal,maxlength,required,readonly,callout,currentvalue,"", false, "",tooltip,elementId,isListBased);
    
  }
  public static StringBuilder doConfigureGrid(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String fieldname, int colspan,int maxlength,boolean required,boolean readonly,String callout,String currentvalue,String jssettings, String tooltip,Boolean isListBased) throws Exception{
    return doConfigureAll(servlet,vars,script,fieldname, 0,colspan+1,maxlength,required,readonly,callout,currentvalue,"", true,jssettings, tooltip,"",isListBased);
  ...
  //TODO Implement Time Format from format.xml file or Database
    String strTimeformat="%H:%M:%S";
    
    if (jssettings.equals(""))
      stdJSSettings =  " onkeyup=\"autoCompleteTime(this);\" onblur=\"expandHour(this.id);\" onkeydown=\"changeToEditingMode('onkeydown');\" onkeypress=\"changeToEditingMode('onkeypress');\" oncut=\"changeToEditingMode('oncut');\" onpaste=\"changeToEditingMode('onpaste');\" oncontextmenu=\"changeToEditingMode('oncontextmenu');\" onchange=\"validateTimeTextBox(this.id);logChanges(this); @CALLOUT@ displayLogic(); return true;\"";
    for (int i = 0; i < leadingemptycols; i++) {
      retval.append("<td class=\"leadingemptycolstime\"></td>");
    }
    if (isListBased) {
	    retval=retval.append(ConfigureListBasedEntry.doConfigure(servlet,vars,script,fieldname,elementId,currentvalue,isGrid,colstotal,callout,null));
	    return retval;
    }
    if (! isGrid)
      retval.append(ConfigureLabel.doConfigure(servlet,vars,fieldname,"Label_ContentCell",labeltext,elementId,""));
    Object template =  servlet.getServletContext().getAttribute("timeboxTEMPLATE");
    if (template==null) {
      template = new String(FileUtils.readFile("Timebox.xml", directory + "/src-loc/design/org/openz/view/templates/"));
      servlet.getServletContext().setAttribute("timeboxTEMPLATE", template);
    }
    ...
    Replace.replace(retval, "@TIMEFORMAT@",strTimeformat);
    if (readonly)
