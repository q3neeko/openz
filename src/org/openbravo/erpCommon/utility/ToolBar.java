...
public class ToolBar {
  ...
  public void prepareEditionTemplate(boolean hasTree, boolean isFiltered, boolean isTest,
    ...
    if (pdf != null && !pdf.contains("orders") && !pdf.contains("invoices") && !pdf.contains("shipments") && !pdf.contains("subscription")) {
      removeElement("EMAIL");
    }
    if (!hasTree)
      removeElement("TREE");
...
//          .append(Utility.messageBD(conn, "Linked Items", language)).append("</a></td>\n");
  ...
  public String toString() {
    ...
    try {
      ...
      //toolbar.append("<table class=\"Main_ContentPane_ToolBar Main_ToolBar_bg\" ondrop=\"drop( openServletNewWindow('NEW', false, '../businessUtility/TabAttachments_FS.html?inpKey=' + dijit.byId('grid').getSelectedRows()+'&inpEditable=Y&amp;inpname='+event.dataTransfer.files[0].name, 'ATTACHMENT', null, true, 600, 700, true))\" ondragover=\"allowDrop(event)\" id=\"tdToolBar\">\n");
      if (!isNew && UtilsData.getOrgConfigOption(conn, "toolbardragndrop","0").equals("Y")) {
        if (grid_id == null || grid_id.equals("")){
        toolbar.append("<table class=\"Main_ContentPane_ToolBar Main_ToolBar_bg\" ondragenter=\"drop(openServletNewWindow('NEW', false, '../businessUtility/TabAttachments_FS.html?inpKey='+" +keyfield+".value+'&amp;inpEditable="+ (isEditable ? "Y" : "N") +"', 'ATTACHMENT', null, true, 600, 700, true))\" ondrop=\"allowDrop(event)\" id=\"tdToolBar\">\n");}
        else
          toolbar.append("<table class=\"Main_ContentPane_ToolBar Main_ToolBar_bg\" ondragenter=\"drop(openServletNewWindow('NEW', false, '../businessUtility/TabAttachments_FS.html?inpKey=' + dijit.byId('grid').getSelectedRows()+'&amp;inpEditable="+ (isEditable ? "Y" : "N") +"', 'ATTACHMENT', null, true, 600, 700, true))\" ondrop=\"allowDrop(event)\" id=\"tdToolBar\">\n");
      } else
