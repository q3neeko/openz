...
public class ModelSQLGeneration {
    ...
    if (sortCols != null && sortCols.length() > 0) {
      ...
      while (datas.hasMoreTokens()) {
        ...
        if (!pos.equals(""))
        	pos=pos+",";
        try {
        	Integer position=new Integer(tableSQL.getColumnPosition(token).getProperty("Position"))+1;
        ...
        if (!aux.equals("")) {
          if (!aux.equals("%")
        	  ...
        	  if (prop.getProperty("Template").equals("SQLFIELD")) {
                  ...
                  filter.addElement(sqlFieldSearch(defaultstmt,tableSQL,vars));
        	  } else
 ...
 private static String sqlFieldSearch(String _defaultstmt, TableSQLData tableSQL,VariablesSecureApp vars) {
