...
public class SE_Order_BPartner extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strBPartner,
    ...
    String strPDisc,strdViaRule,strIncoterms,strDeliveryRule, strPaymentrule, strPaymentterm, strMwarehouse, strInvoiceRule, strPriceList, strUserRep;
    strPDisc=strdViaRule=strIncoterms=strDeliveryRule = strPaymentrule = strPaymentterm = strMwarehouse = strInvoiceRule = strPriceList = strUserRep = "";
    ...
    if (data != null && data.length > 0) {
      ...
      strIncoterms = strIsSOTrx.equals("Y") ? data[0].cIncotermsId:"";
      strdViaRule=  strIsSOTrx.equals("Y") ? data[0].deliveryviarule:"";
      strPDisc=strIsSOTrx.equals("Y") ?data[0].isdiscountprinted:"";
    }
    ...
    if (!strIncoterms.isEmpty()) {
      try {
        ComboTableDataWrapper comboTableDataInc = new ComboTableDataWrapper(this, vars, "c_incoterms_id","","egal", null,null,null);
        tdv = comboTableDataInc.select(false);
      } catch (Exception ex) {
        throw new ServletException(ex);
      }
      resultado.append("new Array(\"inpcIncotermsId\", ");
      if (tdv != null && tdv.length > 0) {
        ...
        for (int i = 0; i < tdv.length; i++) {
          resultado.append("new Array(\"" + tdv[i].getField("id") + "\", \""
              ...
              + (tdv[i].getField("id").equalsIgnoreCase(strIncoterms) ? "true" : "false") + "\")");
          if (i < tdv.length - 1)
            resultado.append(",\n");
        }
        resultado.append("\n)");
    ...
    if (!strdViaRule.isEmpty()) {
      try {
        ComboTableDataWrapper comboTableDatals = new ComboTableDataWrapper(this,vars, "C_Order DeliveryViaRule","");
        tdv = comboTableDatals.select(false);
      } catch (Exception ex) {
        throw new ServletException(ex);
      }
      ...
      if (tdv != null && tdv.length > 0) {
        ...
        for (int i = 0; i < tdv.length; i++) {
          resultado.append("new Array(\"" + tdv[i].getField("id") + "\", \""
              + FormatUtilities.replaceJS(tdv[i].getField("name")) + "\", \""
              + (tdv[i].getField("id").equalsIgnoreCase(strdViaRule) ? "true" : "false") + "\")");
          if (i < tdv.length - 1)
            resultado.append(",\n");
        }
        resultado.append("\n)");
    ...
    // Min Order Value Function
    if (data != null && data.length > 0 && strIsSOTrx.equals("N") && new BigDecimal(data[0].minimumordervalue).compareTo(BigDecimal.ZERO) > 0) {
    	if (message.length() > 0)
            message.append("<br>");
    	try {
			message.append(LocalizationUtils.getElementTextByElementName(this, "minimumordervalue", vars.getLanguage()) + ": " + data[0].minimumordervalue +
					",-" + SLOrderPriceListData.select(this, strPriceList)[0].cursymbol);
			if (data[0].minimumsurchage.length()>0) {
				message.append("<br>");
				message.append(LocalizationUtils.getElementTextByElementName(this, "minimumsurchage", vars.getLanguage()) + ": " + data[0].minimumsurchage);
			}
		} catch (Exception e) {
