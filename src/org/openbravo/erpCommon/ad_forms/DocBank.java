...
public class DocBank extends AcctServer {
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    for (int i = 0; p_lines != null && i < p_lines.length; i++) {
      ...
      // SZ Stmt Amt is not used anymore
      // TrxAmt + Charge Amt is the real Transaction Amount
      fact.createLine(line, getAccount(AcctServer.ACCTTYPE_BankAsset, as, conn),
          line.m_C_Currency_ID, TrxAmt.add(ChargeAmt).toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
          conn);
      // BankInTransit DR CR (Payment) (Overloaded Method - can be CR or DR)
      // Break down in Charge and Transaction Amount
        ...
        // Charge DR (Charge)
        // Charge is always a payment, never a receipt
        // Charge only occurs in Payments and is negative there
        fact.createLine(lineAux, new Account(
            conn, DocLineBankData.selectChargeAccount(conn, C_BankAccount_ID, as.m_C_AcctSchema_ID)),
            line.m_C_Currency_ID, ChargeAmt.abs().toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo),
            DocumentType, conn);
      // Interest DR CR (Interest)
      /*
