...
public class WindowTabs {
  ...
  private void getTabs() throws Exception {
    ...
    if (tabsAux == null || tabsAux.length == 0) {
    	tabsAux = WindowTabsData.selectManualwithoutMenu(this.conn, this.TabID, this.vars.getLanguage());
    }
    if (tabsAux == null || tabsAux.length == 0) {
  ...
  public String parentTabs() {
    ...
    for (int i = 0; i < this.level; i++) {
      ...
      if (isFirst)
        text.append("</div>\n");
      if (isFirst)
        text.append("</td></tr><tr class=\"tabLeveltr\"><td class=\"tabBackGround\">");
      ...
      if (isFirst)
        ...
        text.append("  <table class=\"tabTable\"><tr class=\"tabTabletr\">\n");
      ...
      if (this.action.equals("W")) {
        while (!aux.empty()) {
          ...
          try {
            isactive = new WindowTabsData().isModuleActive(conn, data.adTabId);
            if (isactive.equals("Y"))
              // Test, if Tab is deactivated in Instance specific settings
              isactive = new WindowTabsData().isInstanceActive(conn, data.adTabId);
            if (isactive.equals("Y"))
              // Test, if Tab is deactivated in Role
              isactive = new WindowTabsData().isRoleActive(conn, vars.getRole(),data.adTabId);
          } catch (final Exception ex) {
            ex.printStackTrace();
            log4j.error(ex);
          }
          if (!data.adTabId.equals(this.TabID) && strShowAcct.equals("N")
            ...
            continue;
          // SZ: Also don't show tabs of deactivated Modules or without Role Permissions
          else if (data.isactive.equals("N") || isactive.equals("N"))
            continue;
          if (!isFirstTab)
          ...
          text
              ...
              .append(
                  ((data.adTabId.equals(this.TabID) || data.isbreadcrumb.equals("Y")) ? (isFirst ? "NULL dojoTabparentfirst"
                      : " dojoTabparent")
                      : "")).append("\">");
          text.append("<div><span><a class=\"dojoTabLink tabIsFirster\" href=\"#\" onclick=\"");
  ...
  public String mainTabs() {
    ...
    if (!hasParent)
      text.append("</div>\n");
    if (!hasParent)
      text.append("</td></tr><tr class=\"tabBackgroundtr\">");
    text.append("<td class=\"tabBackGround\">\n");
    if (!hasParent)
      text.append("  <div class=\"marginLeft\">\n");
    else {
        ...
        text
            ...
            .append(
                ((data.adTabId.equals(this.TabID) || data.isbreadcrumb.equals("Y")) ? (!hasParent ? "dojoTabcurrentfirst"
                    : "dojoTabcurrent")
                    : "dojoTab")).append("\">");
        text.append("<div><span><a class=\"dojoTabLink tabFirstestTab \" href=\"#\" onclick=\"");
        text.append(getUrlCommand(data.adTabId, data.name, Integer.valueOf(data.tablevel)
            .intValue()));
        text.append(
            "\" onMouseOver=\"return true;\" onMouseOut=\"return true;\" id=\"" + data.tabnameid
  ...
  public String childTabs(boolean readOnly) {
    ...
    while (!aux.empty()) {
      ...
      text.append("<span class=\"dojoTab").append(
          ...
          .append("\">");
      text.append("<div><a class=\"dojoTabLink tabFirst\" href=\"#\" onclick=\"");
      if (readOnly)
  ...
  private String getUrlCommand(String _tabId, String _tabName, int _level) {
    ...
    if (!_tabId.equals(this.TabID) && this.level + 1 >= _level) {
      ...
      if (! this.editView) // Grid
        text.append("checkgridselect();");      
