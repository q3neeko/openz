...
CREATE or replace FUNCTION zssi_GetReportTitle(v_printout_config_id character varying, v_doctype_id character varying, v_lang character varying) RETURNS character varying
AS $_$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
compliance with the License. You may obtain a copy of the License at http://www.mozilla.org/MPL/MPL-1.1.html
...
$_$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION zssi_GetReportTitle(v_printout_config_id character varying, v_doctype_id character varying, v_lang character varying) OWNER TO tad;
...
v_uom varchar;
BEGIN
        select c_doctype, ad_org_id, c_bpartner_id, m_product_id, c_orderline_id, c_invoiceline_id, m_inoutline_id,m_product_po_id,m_product_uom_id
        into
        v_doctype, v_org, v_bpartner_id, v_product_id, v_orderline_id, v_invoiceline_id, v_inoutline_id,v_manu,v_uom
        from (select 'order' as c_doctype, c_orderline.ad_org_id, c_order.c_bpartner_id, c_orderline.m_product_id, p_docline_id as c_orderline_id, null as c_invoiceline_id, null as m_inoutline_id,m_product_po_id,m_product_uom_id
        from c_orderline, c_order
        where p_docline_id=c_orderline.c_orderline_id and c_order.c_order_id=c_orderline.c_order_id
        union
        select 'invoice' as c_doctype, c_invoiceline.ad_org_id, c_invoice.c_bpartner_id, c_invoiceline.m_product_id, c_invoiceline.c_orderline_id, p_docline_id as c_invoiceline_id, c_invoiceline.m_inoutline_id,null as m_product_po_id,m_product_uom_id
        ...
        union
        select 'shipment' as c_doctype, m_inoutline.ad_org_id, m_inout.c_bpartner_id, m_inoutline.m_product_id, m_inoutline.c_orderline_id, null as c_invoiceline_id, p_docline_id as m_inoutline_id,null as m_product_po_id,m_product_uom_id
    ...
    for v_cur in (select attrs_ordernum as ordernum, 'attributes' as field from zspr_printinfo where printattrsondocs='Y' and ad_org_id = v_org
                    ...
                    union
                    select 1000 as ordernum, 'serialnumberondocs' as field from zspr_printinfo where printserialnumberondocs='Y' and ad_org_id = v_org and (v_doctype = 'shipment' or v_doctype = 'invoice' or v_doctype='order')
                    union
    ...
    loop
                if v_cur.field = 'attributes' then
                        ...
                        select case when p.vendorproductno is not null then zssi_getText('zssi_vendorproductno',p_language)||p.vendorproductno else '' end||
                                    case when p.m_manufacturer_id is not null or p.manufacturernumber is not null then '<br/>'||zssi_getText('zssi_manufacturertext',p_language)||coalesce(m.name,'')||'-'||coalesce(p.manufacturernumber,'') else '' end
...
$_$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION zssi_getlocationline1_new(character varying) OWNER TO tad;
...
$_$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION zssi_getlocationline2_new(character varying) OWNER TO tad;
...
BEGIN
  ...
  LOOP
    v_textval:=v_textval||v_cur.movementqty||' - '||zssi_getproductprintouttext(v_cur.m_product_id, 'de_DE', null, v_cur.ad_org_id,null)||' - ';
  END LOOP;
  for v_cur in (select * from zssi_textmodule where c_doctype_id='C993D1D33D494B44BA8842110876D417' and islower='Y' order by position)
  LOOP
    v_textval:=v_textval||replace(replace(replace(replace(replace(v_cur.text,'@FULLNAME@',coalesce(v_fullname,'n/a')),'@SHIPPER@',coalesce(v_logistic,'n/a')),'@TRACKINGURL@',coalesce(v_trackingurl,'n/a')),'@TRACKINGNO@',coalesce(v_tracking,'n/a')),'@REFERENCENO@',coalesce(v_refno,'n/a'))||'<br/>';
...
and (v_cur2.v_need)>coalesce((select sum(qtyordered-qtydelivered) from c_orderline, c_order where c_order.issotrx='N' and c_order.docstatus='CO' and c_orderline.c_order_id=c_order.c_order_id and c_orderline.m_product_id=v_cur2.v_id and ad_get_docbasetype(c_doctype_id) in  ('POO','SOO') group by c_orderline.m_product_id),0)
and m_product_org.m_product_id=m_product.m_product_id 
and m_product_org.m_locator_id=m_locator.m_locator_id 
and m_locator.m_warehouse_id=m_warehouse.m_warehouse_id 
...
select zsse_DropTrigger ('c_printout_config_insert_bef_insert_trg','c_printout_config');  
CREATE TRIGGER c_printout_config_insert_bef_insert_trg
  before INSERT
