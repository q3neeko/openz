...
BEGIN
    ...
    if p_warehouse is null then
        RAISE exception '%', '@zssm_simulationnotpossiblelocatorundefined@'||coalesce(v_wsname,zssi_getproductname(p_productid,'de_DE')||' undefined');
    end if;
    RETURN;
END;
$_$  LANGUAGE 'plpgsql';
...
BEGIN
        --v_qty:=p_qty;
        select  p_movementqtyOut into v_qty from zssm_getrequiredqty(p_productid,p_qty);
        ...
        LOOP            
            ...
            v_estdeliverydate:=v_estdeliverydate+ (trunc(now())-v_beginproductiondate);
...
BEGIN
        ...
        select min(stockdate) into v_estdeliverydateplan from mrp_inoutplanbase where m_product_id=p_productid and m_warehouse_id=p_warehouse and estimated_stock_qty >= p_qty;
        ...
        if zssm_getproductionplanIDofproduct(p_productid)!='' then
            ...
            v_estdeliverydate := v_estdeliverydate + zssi_NumofWorkdays2CaleandarDaysFromGivenDate(1,v_org,v_estdeliverydate);
        else
            -- OR Buy Product
            if length(mrp_getsheddeliverydate4vendorProduct(null,p_productid,v_org,null,null))>0 then
                v_estdeliverydate := to_timestamp(to_timestamp(mrp_getsheddeliverydate4vendorProduct(null,p_productid,v_org,null,null),'DD-MM-YYYY'));
            end if;
        end if;
...
v_time varchar;
BEGIN
    select o.issotrx,ol.m_product_id,o.m_warehouse_id,ol.qtyordered,o.c_bpartner_id,ol.m_product_uom_id,ol.m_product_po_id,o.ad_org_id
           into v_issotrx,v_product,v_warehouse,v_qty,v_partner,v_puom,v_manufac,v_org
    ...
    if v_issotrx='Y' then
        RETURN zssm_getfastestdeliverydate(v_product,v_qty,v_warehouse) ;
    else
...
CREATE OR REPLACE FUNCTION zssm_getprunreadonly(p_orderline_id varchar,p_projecttask_id varchar) RETURNS varchar
AS $_$
DECLARE 
v_tasktext varchar:='';
v_prod varchar;
BEGIN
    select replace(zssm_getpruncausetext(p_orderline_id,p_projecttask_id),'Order:','') into v_tasktext;
    ...
    if (select count(*) from c_projecttask where  m_product_id=v_prod and triggerreason like '%'||v_tasktext||'%')>0  then
        return 'Y';
    else
       return 'N';
    end if;
...
CREATE OR REPLACE FUNCTION zssm_getrequiredqty(p_productid IN varchar,p_movementqtyIn IN numeric,p_movementqtyOut OUT numeric,p_lotetxt OUT VARCHAR)  RETURNS  RECORD
AS $_$
DECLARE 
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
BEGIN
    select c_projecttask_id into v_workstepid from zssm_productionplan_task_v 
                                                         where m_product_id=p_productid and assembly='Y' and isactive='Y' 
                                                         and zssm_productionplan_v_id=zssm_getproductionplanIDofproduct(p_productid);
    --
    select mimimumqty,multipleofmimimumqty into v_mini,v_multi from c_projecttask where c_projecttask_id=v_workstepid;
    --
    p_lotetxt:='';
...
SELECT a.ad_org_id,a.ad_client_id,a.updated,a.updatedby,a.created,a.createdby,'Y'::text as isactive,
       ...
       (select p_lotetxt from zssm_getrequiredqty(a.m_product_id,a.movementqty))::varchar(60) as  lottext,
       a.requiredqty,
       a.cause ,a.currOnhandQty,
       (select p_movementqtyOut from zssm_getrequiredqty(a.m_product_id,a.movementqty))::numeric as movementqty,
       a.causetext,a.readonly
FROM
(SELECT
  v.ad_org_id,
  v.ad_client_id,
...
SELECT
  ...
  zssm_getprunreadonly(v.c_orderline_id,v.c_projecttask_id) as readonly
...
order by m_product_id,requiredqty,causetext;
...
BEGIN
    ...
    for v_cur in (select  v.c_projecttask_id,v.m_product_id, (select p_movementqtyOut from zssm_getrequiredqty(v.m_product_id,v.estimated_stock_qty*(-1)))::numeric   as requiredqty,v.planneddate as needbydate,
                  zssm_getpruncausetext(v.c_orderline_id,v.c_projecttask_id) as causetext
    ...
    LOOP 
            ...
            if v_isauto='Y'  and (v_cur.causetext like v_cause||'%' or  v_cause  like  v_cur.causetext||'%') then
                ...
                insert into zssm_productionrun(zssm_productionrun_id,ad_client_id,ad_org_id,createdby, updatedby,
                            requiredqty,needbydate,m_product_id, isautotriggered,pinstance,cause,enddate,hoursinrecursion,issuing_locator)
                    VALUES (get_uuid(),'C726FEC915A54A0995C568555DA5BB3C',v_Org,v_user,v_user,
                            v_cur.requiredqty,v_startdate,v_cur.m_product_id,'Y',p_pinstance_id,v_cur.CAUSETEXT,v_cur.needbydate, v_hoursinrecursion,v_locator);
