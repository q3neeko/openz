            ...
            IF coalesce(v_IsStocked,'N')='Y' THEN
              ...
              INTO M_TRANSACTION
                ...
                (
                  ...
                  Cur_InOut.MovementDate, v_Qty,Cur_InOutLine.C_UOM_ID
                )
                ;
            END IF;
...
BEGIN
  ...
  if p_issotrx='Y' then
    ...
    END LOOP;
    -- If no Locator with needed QTY found and there is  allowing negative Stock 
    if (select allownegativestock from ad_clientinfo where ad_client_id='C726FEC915A54A0995C568555DA5BB3C')='Y' then
            select m_locator_id into v_plocator from m_locator where isactive='Y' and isdefault='Y' and m_warehouse_id=p_warehouse;
            return v_plocator;
    end if;
  end if;
                    ...
                    if v_issotrx='Y' then     
                       ...
                       INSERT INTO M_INOUT
                          ...
                          Processed, SALESREP_ID,
                                      ...
                                      c_project_id,
                                      c_projecttask_id,
                          DELIVERY_LOCATION_ID,c_incoterms_id,weight,weight_uom,deliverylocationtext) 
                       select p_InOut_ID,C_Order_ID, IsSOTrx,Cur_lines.AD_Client_ID,Cur_lines.AD_Org_ID,v_user,v_user,
                               ...
                               'N', SALESREP_ID, a_asset_id, c_project_id, c_projecttask_id,delivery_location_id,c_incoterms_id,weight,weight_uom,deliverylocationtext
          ...
          if V_STOCKED='Y' and v_type='I' then
              ...
              INTO M_Transaction
                (
                  ...
                  MovementDate, MovementQty, M_Internal_ConsumptionLine_ID,C_UOM_ID,c_project_id,c_projecttask_id,snr_masterdata_id
                )
                VALUES
                (
                  ...
                  Cur_MoveLine.snr_masterdata_id
                );
              -- TO
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
    ...
    WHERE M_Product_ID=OLD.M_PRODUCT_ID
      ...
      AND ProductType='I';
    IF V_STOCKED > 0 THEN
      PERFORM M_UPDATE_INVENTORY(OLD.AD_CLIENT_ID, OLD.AD_ORG_ID, OLD.UPDATEDBY, OLD.M_PRODUCT_ID, OLD.M_LOCATOR_ID, OLD.M_ATTRIBUTESETINSTANCE_ID, OLD.C_UOM_ID, OLD.M_PRODUCT_UOM_ID, NULL, NULL, NULL, -(OLD.QTYCOUNT-OLD.QTYBOOK), OLD.QUANTITYORDER) ;
    END IF;
  END IF;
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    SELECT COUNT(*)
    ...
    WHERE M_Product_ID=NEW.M_PRODUCT_ID
      ...
      AND ProductType='I';
    IF V_STOCKED > 0 THEN
      PERFORM M_UPDATE_INVENTORY(NEW.AD_CLIENT_ID, NEW.AD_ORG_ID, NEW.UPDATEDBY, NEW.M_PRODUCT_ID, NEW.M_LOCATOR_ID, NEW.M_ATTRIBUTESETINSTANCE_ID, NEW.C_UOM_ID, NEW.M_PRODUCT_UOM_ID, NULL, NULL, NULL,(NEW.QTYCOUNT-NEW.QTYBOOK),(NEW.QUANTITYORDER-NEW.QUANTITYORDERBOOK)) ;
    END IF;
  END IF;
...
UNION 
        ...
        SELECT ol.m_product_id,
                ...
                case when ol.m_attributesetinstance_id = '0' then null else ol.m_attributesetinstance_id end as m_attributesetinstance_id,
                ol.c_uom_id,
                ...
                0::numeric AS qtyinsaleframe,
                o.ad_client_id,
                ol.c_orderline_id AS  zssi_onhanqty_id
        FROM c_orderline ol,c_order o,c_doctype d where o.c_order_id=ol.c_order_id and o.c_doctype_id=d.c_doctype_id 
                                                        and d.docbasetype in ('SOO','POO') and o.docstatus='CO' and o.deliverycomplete='N'
UNION 
        -- Frame Contracts
        SELECT ol.m_product_id,
                o.m_warehouse_id,
                ...
                ol.ad_client_id,
                ol.c_orderline_id AS zssi_onhanqty_id
        FROM c_orderline ol,c_order o where ol.c_order_id=o.c_order_id 
                      and o.docstatus='CO' and o.c_doctype_id in ('56913A519BA94EB59DAE5BF9A82F5F7D' ,'559A80F2E27742D4B2C476045F5C834F')
                      and trunc(now()) between o.contractdate and o.enddate
...
create view mrp_deliveries_unneeded as
        ...
        from (
                        ...
                        from  c_order o,c_orderline ol left join m_product_po po on po.m_product_id=ol.m_product_id and po.c_bpartner_id=(select c_bpartner_id from c_order where c_order_id=ol.c_order_id) and 
                                                                                   ...
                                                                                   left join m_product_org og on og.ad_org_id=ol.ad_org_id and og.m_product_id=ol.m_product_id 
...
************************************************************************/
  ...
  v_UOM_ID VARCHAR(32) ; --OBTG:VARCHAR2--
  v_attrstocked VARCHAR(32); --OBTG:VARCHAR2--
  v_attrmandat VARCHAR(32);
  v_ATTRIBUTESETINSTANCE_ID VARCHAR(32); --OBTG:VARCHAR2--
  v_Name VARCHAR(60); --OBTG:NVARCHAR2--
...
BEGIN
  ...
  -- Updating inventory
  IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
    PERFORM M_UPDATE_INVENTORY(OLD.AD_CLIENT_ID, OLD.AD_ORG_ID, OLD.UPDATEDBY, OLD.M_PRODUCT_ID, OLD.M_LOCATOR_ID, OLD.M_ATTRIBUTESETINSTANCE_ID, OLD.C_UOM_ID, OLD.M_PRODUCT_UOM_ID, -OLD.MOVEMENTQTY, -OLD.QUANTITYORDER, NULL, OLD.MOVEMENTQTY, OLD.QUANTITYORDER) ;
  END IF;
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    PERFORM M_UPDATE_INVENTORY(NEW.AD_CLIENT_ID, NEW.AD_ORG_ID, NEW.UPDATEDBY, NEW.M_PRODUCT_ID, NEW.M_LOCATOR_ID, NEW.M_ATTRIBUTESETINSTANCE_ID, NEW.C_UOM_ID, NEW.M_PRODUCT_UOM_ID, NEW.MOVEMENTQTY, NEW.QUANTITYORDER, v_DATEINVENTORY, -NEW.MOVEMENTQTY, -NEW.QUANTITYORDER) ;
  END IF;
IF TG_OP = 'DELETE' THEN RETURN OLD; ELSE RETURN NEW; END IF; 
