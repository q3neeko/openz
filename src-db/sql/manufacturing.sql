...
ELR BOM HELPER FUNCTIONS - For Excel - BOM - Report
...
CREATE OR REPLACE FUNCTION elr_getproductuomdiv(v_product character varying)
  RETURNS numeric AS
$BODY$
...
-- RAISE EXCEPTION '% = %', TG_NAME, TG_OP;
  ...
  IF TG_OP != 'DELETE' then
      ...
      select count(*) into v_count from zspm_projecttaskbom where c_projecttask_id=new.c_projecttask_id and zspm_projecttaskbom_id!=new.zspm_projecttaskbom_id and line=new.line;
      if v_count!=0 then
...
BEGIN 
    select c_project.c_project_id into v_project
           from c_project,c_projecttask 
           where c_project.c_project_id=c_projecttask.c_project_id and c_projecttask.c_projecttask_id=p_projecttask_id;
    select m_product_id,qty,qtyproduced,ad_client_id,ad_org_id,name into v_product,v_qty,v_qtyproduced,v_client,v_org,v_projecttask_name from c_projecttask where  c_projecttask_id=p_projecttask_id;
    ...
    if v_product is not null and coalesce(v_qty,0)!=0 then
        ...
        ELSE 
          IF v_method='AV' THEN
              ...
              IF (v_curr_onhand_qty+(v_qty-v_qtyproduced)>=0) and v_curr_onhand_qty>=0 THEN
                  v_actual_cost:=((v_curr_cost*v_curr_onhand_qty)+(v_itemcost*(v_qty-v_qtyproduced)))/(v_curr_onhand_qty+(v_qty-v_qtyproduced));
        ...
        if v_qty-v_qtyproduced>0 then
            ...
            insert into M_INTERNAL_CONSUMPTIONLINE(M_INTERNAL_CONSUMPTIONLINE_ID, AD_CLIENT_ID, AD_ORG_ID, CREATED, CREATEDBY, UPDATED, UPDATEDBY, M_INTERNAL_CONSUMPTION_ID, 
                  ...
                  values (v_lineUUId,v_client,v_org,NOW(), v_User_ID, NOW(),v_User_ID,v_uid,
                          v_locator,v_product,10,v_qty-v_qtyproduced,'Generated by Production->Send Material on Stock',v_uom,v_project, p_projecttask_id);
            -- seruial Number Tracking?
           if v_serial='Y'  or v_batch='Y' then
