...
CREATE VIEW pdc_barcode_v AS
  SELECT bp.value::varchar(200) AS barcode, 'EMPLOYEE' AS type, u.ad_user_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM ad_user u, c_bpartner bp
  WHERE 1=1
   AND bp.c_bpartner_id = u.c_bpartner_id
   AND bp.isemployee = 'Y'
 UNION
  SELECT l.value::varchar(200) AS barcode, 'LOCATOR' AS type, l.m_locator_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM m_locator l
 UNION
  SELECT p.value::varchar(200) AS barcode, 'PRODUCT' AS type, p.m_product_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM m_product p
 UNION
  SELECT ws.value::varchar(200) AS barcode, 'WORKSTEP' AS type, ws.c_projecttask_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM c_projecttask ws, c_project pro WHERE ws.c_project_id = pro.c_project_id and pro.projectcategory = 'PRO'
 UNION
  SELECT e.columnname::varchar(200) AS barcode, 'CONTROL' AS type, e.ad_element_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM ad_element e WHERE e.ad_module_id = '000CDBE191604F5A835A3EC3213719E8' AND description like 'CODE-128-code action%'
 UNION 
  SELECT e.columnname::varchar(200) AS barcode, 'CALCULATION' AS type, e.ad_element_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM ad_element e WHERE e.ad_module_id = '000CDBE191604F5A835A3EC3213719E8' AND description='scan calc'
 UNION
  SELECT l.serialnumber::varchar(200) AS barcode, 'SERIALNUMBER' AS type, l.snr_masterdata_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
  FROM snr_masterdata l
 UNION
  SELECT l.batchnumber::varchar(200) AS barcode, 'BATCHNUMBER' AS type, l.snr_batchmasterdata_id AS id, '' AS snrmasterdata_id,'' as batchmasterdata_id
...
BEGIN
  BEGIN
    if instr(p_value,'|')>0 and c_getconfigoption('kombibarcode','0')='Y' then
        select isserialtracking,isbatchtracking  into v_serial,v_batch from m_product where value=substr(p_value,1,instr(p_value,'|')-1);
        if coalesce(v_serial,'')='Y' then
           select snr.snr_masterdata_id,snr.snr_batchmasterdata_id ,snr.m_product_id
                  into v_serial,v_batch,v_product from snr_masterdata snr,m_product p where p.m_product_id=snr.m_product_id and p.value=substr(p_value,1,instr(p_value,'|')-1)
                                             and snr.serialnumber=substr(p_value,instr(p_value,'|')+1,length(p_value));
           select count(*) into v_count from  snr_masterdata snr,m_product p where p.m_product_id=snr.m_product_id and p.value=substr(p_value,1,instr(p_value,'|')-1)
                                             and snr.serialnumber=substr(p_value,instr(p_value,'|')+1,length(p_value));
        else
           select snr.snr_batchmasterdata_id,snr.m_product_id into v_batch,v_product from snr_batchmasterdata snr,m_product p where p.m_product_id=snr.m_product_id and p.value=substr(p_value,1,instr(p_value,'|')-1)
                                             and snr.batchnumber=substr(p_value,instr(p_value,'|')+1,length(p_value));
           select count(*) into v_count from  snr_batchmasterdata snr,m_product p where p.m_product_id=snr.m_product_id and p.value=substr(p_value,1,instr(p_value,'|')-1)
                                             and snr.batchnumber=substr(p_value,instr(p_value,'|')+1,length(p_value));
           v_serial:=null;
        end if;
        if v_count=1 then
            ...
            v_pdc_barcode_v.snrmasterdata_id := v_serial;
            v_pdc_barcode_v.batchmasterdata_id := v_batch;
            y :=1;
