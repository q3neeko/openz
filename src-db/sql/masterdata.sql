...
BEGIN
  ...
  IF (TG_OP = 'UPDATE') THEN
    IF (new.value!=old.value) THEN
      ...
      UPDATE zssm_productionplan_v set value=new.value where value=old.value;
    END IF;
    IF (new.name!=old.name) then
	UPDATE zssm_workstep_prp_v set name=new.name where name=old.name;
	UPDATE zssm_productionplan_v set name=new.name where name=old.name;
    END IF;
 ...
 END IF;
  ...
  if new.typeofproduct in ('CD','SA','AS','UA')  and new.isactive='Y' and 
     new.discontinued='N' and new.production='Y' then
         if (select count(*) from zssm_workstep_prp_v where m_product_id=new.m_product_id)=0 then
...
BEGIN
  ...
  If (new.c_location_id is not null) then    
     select c_country_id,city,postal,address1,c_region_id into v_countryID,v_city,v_zip,v_adr1,v_regId from c_location where c_location_id=new.c_location_id;
...
BEGIN
  ...
  IF (TG_OP <> 'DELETE') then
    if c_getconfigoption('synchronizeworkstepboms',new.ad_org_id)='Y' then
        ...
        if v_count>=1 then
            raise exception '%', '@duplicatelinenumber@';
        end if;
        select  m_locator_id into v_locator from m_product where m_product_id=new.m_productbom_id;
        IF (TG_OP = 'INSERT') THEN
            ...
            LOOP 
            select count(*) into v_count from zspm_projecttaskbom where c_projecttask_id=v_cur.c_projecttask_id and line=new.line;
            if v_count=1 then
                UPDATE zspm_projecttaskbom set isactive=new.isactive, updatedby=NEW.updatedby,updated=now(), m_product_id=new.m_productbom_id, quantity=new.bomqty,description=new.description,line=new.line,receiving_locator=coalesce(v_locator,v_cur.receiving_locator)
                where c_projecttask_id=v_cur.c_projecttask_id and m_product_id=new.m_productbom_id and line=new.line;
            else
                ...
                VALUES (get_uuid(), new.isactive,v_cur.c_projecttask_id, v_cur.ad_client_id, v_cur.ad_org_id, new.createdby, NEW.updatedby, new.m_productbom_id, new.bomqty,v_cur.issuing_locator,coalesce(v_locator,v_cur.receiving_locator),new.line,new.description);
            end if;
            END LOOP;
        END IF; -- TG_OP = 'INSERT'
        IF (TG_OP = 'UPDATE') THEN
           --if (old.line!=new.line or old.m_productbom_id!=new.m_productbom_id or old.bomqty!=new.bomqty or coalesce(old.description,'')!=coalesce(new.description,'') or 
            --    coalesce(old.constuctivemeasure,'')!=coalesce(new.constuctivemeasure,'') or  coalesce(old.rawmaterial,'')!=coalesce(new.rawmaterial,'')) 
            --then
                    for v_cur in (select * from c_projecttask where assembly='Y' and c_project_id is null and m_product_id=new.m_product_id)
                    LOOP 
                    select count(*) into v_count from zspm_projecttaskbom where c_projecttask_id=v_cur.c_projecttask_id and line=old.line;
                    if v_count=1 then
                        UPDATE zspm_projecttaskbom set isactive=new.isactive, updatedby=NEW.updatedby,updated=now(), m_product_id=new.m_productbom_id, quantity=new.bomqty,description=new.description,line=new.line,receiving_locator=coalesce(v_locator,v_cur.receiving_locator)
