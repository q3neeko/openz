...
BEGIN
    ...
    ELSE
        ...
        if (select count(*) from ils_snrsinoutflat  where m_inoutline_id=new.m_inoutline_id)>0 then
           update ils_snrsinoutflat set serialstext=v_snrs,snr_masterdata_id=v_snrmaster where m_inoutline_id=new.m_inoutline_id; 
        else
           insert into ils_snrsinoutflat(m_inoutline_id,snr_masterdata_id,serialstext) values (new.m_inoutline_id,v_snrmaster,v_snrs);
        end if;
    END IF;
