...
select mrp_deliveries_expected_id,created,createdby,updated,updatedby,isactive,ad_client_id,ad_org_id,
       ...
       c_project_id,c_projecttask_id ,a_asset_id,overdue,description,salesrep_id,scheddeliverydate,c_bpartner_id
from
( 
      ...
      IF (NOT isempty(NEW.m_product_id)) THEN 
        ...
        IF (NOT isempty(v_product_id)) THEN -- product was found
         -- check required settings
          ...
          ELSEIF (v_isactive <> 'Y' and TG_OP = 'INSERT') THEN
...
*/
      IF  new.isactive='Y' THEN
        ...
        LOOP
           ...
           -- Machine Costs (flat)
           select cv.materialcost,cv.vendorcost,cv.empcost into v_percent,v_percent1,v_percent2
           from ma_indirect_cost_value cv, ma_indirect_cost c where cv.ma_indirect_cost_id=c.ma_indirect_cost_id
                       and c.ma_indirect_cost_id=v_cur.ma_indirect_cost_id and cv.datefrom<=v_taskdate and cv.cost_uom='P' and cv.isactive='Y'
                       and c.isactive='Y' and c.cost_type='MA' and cv.cost_uom='P' order by datefrom desc LIMIT 1;
           if coalesce(v_percent,0)>0 or coalesce(v_percent1,0)>0 or coalesce(v_percent2,0)>0 then
              if (select count(*) from ad_org where ad_org_id='242A39E90C034F7AAA2C6BF09F715F6B')>0 then -- TS wants this....
                v_maamt:=coalesce(v_maamt,0)+round((coalesce(v_bomamt,0)*coalesce(v_percent,0)/100) + (coalesce(v_extserviceamt,0)*coalesce(v_percent1,0)/100) + (coalesce(v_hramt,0)*coalesce(v_percent2,0)/100),2);
              else
                v_maamt:=coalesce(v_maamt,0)+round((coalesce(v_bomamt,0)*coalesce(v_percent,0)/100) + ((coalesce(v_expenseamt,0)+coalesce(v_extserviceamt,0))*coalesce(v_percent1,0)/100) + (coalesce(v_hramt,0)*coalesce(v_percent2,0)/100),2);
              end if;
           end if;
        END LOOP;
        -- Sum on Task so far
  ...
  IF (TG_OP = 'UPDATE') THEN
    IF new.c_orderline_id is not null and old.c_orderline_id is not null and new.directship ='Y'  THEN
     if new.quantity != old.quantity or
        ...
        trunc(new.date_plan) != trunc(old.date_plan) or
    ...
    if  (select requested from c_project_processstatus limit 1)='Y'  then -- Process ist requested
        ...
        loop
            ...
            loop
                ...
                -- Machine Costs (flat)
                select cv.materialcost,cv.vendorcost,cv.empcost into v_percent,v_percent1,v_percent2 from ma_indirect_cost_value cv, ma_indirect_cost c 
                where cv.ma_indirect_cost_id=c.ma_indirect_cost_id
                       and c.ma_indirect_cost_id=v_cur2.ma_indirect_cost_id and cv.datefrom<=v_cur.startdate and cv.cost_uom='P' and cv.isactive='Y'
                       and c.isactive='Y' and c.cost_type='MA' order by datefrom desc LIMIT 1;
                if coalesce(v_percent,0)>0 or coalesce(v_percent1,0)>0 or coalesce(v_percent2,0)>0 then
                    if (select count(*) from ad_org where ad_org_id='242A39E90C034F7AAA2C6BF09F715F6B')>0 then -- TS wants this....
                        v_machinecost:=v_machinecost+round((v_materialcost*coalesce(v_percent,0)/100) + ((v_extservicecost+v_glextservice)*coalesce(v_percent1,0)/100) + (v_workcost*coalesce(v_percent2,0)/100),2);
                    else
                        v_machinecost:=v_machinecost+round((v_materialcost*coalesce(v_percent,0)/100) + ((v_extservicecost+v_glextservice+v_invoicecost+v_glcost)*coalesce(v_percent1,0)/100) + (v_workcost*coalesce(v_percent2,0)/100),2);
                    end if;
                end if;
            end loop;
...
-- Positionen (nur fuer Produktion) von c_orderline nach c_projecttask kopieren 10.05.2012
    IF (c_getconfigoption('generatestdtaskfromso',v_org_id)='Y') THEN
      ...
      LOOP
        ...
        if v_cur_ol.production='Y' then
            ...
            select c_projecttask_id into v_stdtask from c_projecttask where c_project_id=v_uuid and name = 'Standard'; 
    ...
    then
        PERFORM zspm_salesMaterial2projecttask(v_Record_ID,'N');
...
BEGIN
    ...
    RETURN zspm_DOconsumption4project(p_M_INOUT_ID ,1,v_org,v_User,v_client);
...
BEGIN
    ...
    */
    ...
    LOOP
        ...
        if v_currentqty <= v_cur.lineqty and v_cur.reqqty-coalesce(v_proj_qty,0) > 0 then
...
SELECT	zspm_projecttaskbom.zspm_projecttaskbom_id as zspm_projecttaskbom_view_id,
                ...
                case when c_project.projectstatus = 'OR' and zspm_projecttaskbom.m_locator_id is not null then
                    M_BOM_qty_Available(zspm_projecttaskbom.m_product_id,c_project.m_warehouse_id ,null) 
                    + case when    (zspm_projecttaskbom.quantity-zspm_projecttaskbom.qtyreceived)>0 then (zspm_projecttaskbom.quantity-zspm_projecttaskbom.qtyreceived) else 0 end
                else
                    M_BOM_qty_Available(zspm_projecttaskbom.m_product_id,c_project.m_warehouse_id ,null) 
                end
                as qty_available,
...
BEGIN
      ...
      LOOP
        ...
        if v_count>1 then
            select sum(ol.linenetamt) into p_offernetamt from c_orderline ol,c_order o where o.c_order_id=ol.c_order_id and ol.c_project_id=v_pcur.c_project_id
                      and ad_get_docbasetype(o.c_doctype_id) = 'SALESOFFER' and o.docstatus in ('CO','CL');
            select sum(ol.linenetamt) into p_ordernetamt from c_orderline ol,c_order o where o.c_order_id=ol.c_order_id and ol.c_project_id=v_pcur.c_project_id
                      and ad_get_docbasetype(o.c_doctype_id) = 'SOO' and o.docstatus in ('CO');
            select sum(il.linenetamt * case when ad_get_docbasetype(i.c_doctype_id)='ARC' then (-1) else 1 end) into  p_invoicenetamt
        ...
        for v_cur in (select o.orderselfjoin,sum(ol.linenetamt) as amt,zssi_strDate(o.dateordered,i_lang) as dateordered,o.documentno,o.c_order_id from c_order o,c_orderline  ol
        ...
        LOOP
            ...
            if (select count(*) from c_order o where ad_get_docbasetype(o.c_doctype_id) = 'SOO' and o.docstatus='CO' and o.c_order_id=coalesce(v_cur.orderselfjoin,''))=0 then
                ...
                select sum(ol.linenetamt) as amt,o.dateordered,o.documentno,o.c_order_id into  p_ordernetamt,p_orderdate,p_orderdocno,v_orderid
                ...
                for v_icur in (select sum(il.linenetamt) * case when ad_get_docbasetype(i.c_doctype_id)='ARC' then (-1) else 1 end as amt,zssi_strDate(i.dateinvoiced,i_lang) as dateinvoiced,i.documentno 
                      from c_invoice i,c_invoiceline il  
                      where i.c_invoice_id=il.c_invoice_id and il.c_project_id=v_pcur.c_project_id
                      and ad_get_docbasetype(i.c_doctype_id) in ('ARI','ARC') and i.docstatus='CO' and i.c_order_id=v_orderid
                      group by i.dateinvoiced,i.documentno,i.c_invoice_id,i.c_doctype_id,i.c_order_id)
                LOOP
        ...
        p_offernetamt:=null;
        p_offerdate:=null;
        for v_cur in (select sum(ol.linenetamt) as amt,zssi_strDate(o.dateordered,i_lang) as dateordered,o.documentno,o.c_order_id from c_order o,c_orderline  ol
                      where o.c_order_id=ol.c_order_id and ol.c_project_id=v_pcur.c_project_id
                      ...
                      group by o.dateordered,o.documentno,o.c_order_id,o.c_doctype_id)
        LOOP 
...
CREATE OR REPLACE FUNCTION zspm_salesMaterial2projecttask(p_c_order_ID varchar,p_isreopen varchar)
  RETURNS void AS
$BODY$ 
DECLARE 
/***************************************************************************************************************************************************
...
BEGIN
    ...
    for v_cur in (select ol.c_project_id,ol.c_projecttask_id ,  ol.c_orderline_id,ol.qtyordered,ol.m_product_id,ol.priceactual
                  from c_orderline ol  where ol.c_order_id=p_c_order_ID and ol.c_projecttask_id is not null)
    LOOP
       if p_isreopen='N' and (select producttype from m_product where m_product_id=v_cur.m_product_id)='I' and 
          (select typeofproduct from m_product where m_product_id=v_cur.m_product_id) not in ('CD','SA','AS','UA') then 
                ...
                INSERT INTO zspm_projecttaskbom (
                    ...
                    VALUES (
                            ...
                            v_cost, v_cost,
                            (select coalesce(max(line)+10,10) from zspm_projecttaskbom where c_projecttask_id=v_cur.c_projecttask_id),
                            'N',v_cur.c_orderline_id);
       end if; -- Items, No Production
       if p_isreopen='Y'  and (select producttype from m_product where m_product_id=v_cur.m_product_id)='I' and 
          (select typeofproduct from m_product where m_product_id=v_cur.m_product_id) not in ('CD','SA','AS','UA') then
                -- delete
                if (select count(*) from zspm_projecttaskbom where c_salesorderline_id = v_cur.c_orderline_id and (qtyreceived!=0 or quantity!=v_cur.qtyordered))=0 then
                    delete from zspm_projecttaskbom where c_salesorderline_id = v_cur.c_orderline_id;
                else
                    update zspm_projecttaskbom set c_salesorderline_id =null,quantity=quantity-v_cur.qtyordered where c_salesorderline_id=v_cur.c_orderline_id;
                end if;
       end if;
    END LOOP;
    RETURN;
