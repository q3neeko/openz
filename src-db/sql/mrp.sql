...
BEGIN
  BEGIN --BODY
    ...
    if (pp_timehorizon>0) then
     for v_cur in (select distinct m_product_id from  MRP_RUN_PURCHASELINE where MRP_RUN_PURCHASE_id=p_Run)
     LOOP
        ...
        select sum(qty) into v_correction from MRP_RUN_PURCHASELINE where mrp_run_purchase_id=p_run and m_product_id=v_cur.M_Product_ID and planneddate>v_scheddeliverydate and planneddate>p_planningdate;
        delete from MRP_RUN_PURCHASELINE where mrp_run_purchase_id=p_run and m_product_id=v_cur.M_Product_ID and planneddate>v_scheddeliverydate and planneddate>p_planningdate;
        if coalesce(v_correction,0) > 0 then -- Positiv -> Zugang in der Zukunft. Dieser Zugang führt zu Bestand.
                        SELECT * INTO  v_Aux_ID FROM Mrp_Run_Insertlines(p_Client_ID, p_Org_ID, p_User_ID, p_Run, v_cur.M_Product_ID, v_correction, 'TA', NULL, NULL, NULL,NULL,  to_date('01.01.9999','dd.mm.yyyy'),  NULL,p_Warehouse_ID);
        end if;
     END LOOP;
    end if;
  END; --BODY
...
BEGIN
  BEGIN --BODY
    ...
    FOR Cur_PlanProduct IN (
        SELECT MRP_RUN_PURCHASELINE.M_PRODUCT_ID,MRP_RUN_PURCHASELINE.AD_ORG_ID,MRP_RUN_PURCHASE.m_warehouse_id
      ...
      GROUP BY MRP_RUN_PURCHASELINE.M_PRODUCT_ID, MRP_RUN_PURCHASELINE.AD_ORG_ID,MRP_RUN_PURCHASE.m_warehouse_id
    ) LOOP
    -- Gruppenwechsel Produkt/Artikel verarbeiten,
        ...
        select greatest(min(trunc(planneddate)) , v_scheddeliverydate) into v_scheddeliverydate from mrp_inoutplan_v where m_product_id=Cur_PlanProduct.M_PRODUCT_ID and ad_org_id=Cur_PlanProduct.ad_org_id and
        ...
        if   v_NeededQtyInTimeNOStock< v_neededQty_New then
            -- Hier folgen Lieferungen noch nach dem jetzt zu beliefernden Daum.
            ...
            select sum(qty) into  v_NeededQtyInFuture from MRP_RUN_PURCHASELINE where MRP_RUN_PURCHASE_ID = p_Run_ID  and M_PRODUCT_ID=Cur_PlanProduct.M_PRODUCT_ID
             ...
             select sum(qty) into  v_QtyStockMin from MRP_RUN_PURCHASELINE where MRP_RUN_PURCHASE_ID = p_Run_ID  and M_PRODUCT_ID=Cur_PlanProduct.M_PRODUCT_ID and inouttrxtype ='MS';
            if v_NeededQtyInFuture>=v_QtyStockMin then -- Lager kann konsumiert werden, keine Beschaffung von Lagerbedarfen notwendig
             ...
             -- Nur wenn keine Folgenden Lieferungen sind, Optimalen Lagerbestand auffüllen
              select sum(qty) into  v_correction from MRP_RUN_PURCHASELINE where MRP_RUN_PURCHASE_ID = p_Run_ID  and M_PRODUCT_ID=Cur_PlanProduct.M_PRODUCT_ID and inouttrxtype='OS'; 
        end if;
        ...
        -- Nur ersten Bedarf bestellen.
                select sum(qty) into v_correction2 from MRP_RUN_PURCHASELINE where mrp_run_purchase_id=p_Run_ID and m_product_id=Cur_PlanProduct.M_Product_ID and planneddate>v_scheddeliverydate and planneddate>p_planningdate
                ...
                if coalesce(v_correction2,0)<>0 then
                    SELECT * INTO  v_Aux_New FROM Mrp_Run_Insertlines(p_Client_ID, p_Org_ID, p_User_ID, p_Run_ID, Cur_PlanProduct.M_Product_ID, v_correction2 *(-1), 'TA', NULL, NULL, NULL,NULL, to_date('01.01.9999','dd.mm.yyyy'),  NULL,Cur_PlanProduct.m_Warehouse_ID);
                end if;
        ...
        IF (v_neededQty_New < 0) THEN            
            ...
            if v_neededQty_New < 0 then
                ...
                select sum(qty)*-1 into v_auxQty from MRP_RUN_PURCHASELINE where inouttrxtype not in ('PP','IQ','OS','MS','SF') and MRP_RUN_PURCHASE_ID = p_Run_ID
                       and m_product_id=Cur_PlanProduct.M_Product_ID and planneddate<=v_Planneddate_new;
                ...
                -- Get Offers Price
                v_price:=M_Get_Offers_Price(v_Planneddate_new,v_vendor,Cur_PlanProduct.M_Product_ID,v_Qty_New, v_pricelistID,'N',null,'N',null,v_uom,v_productPoId);
                v_frameqty:=0;
                ...
                for v_cur in (select ol.c_orderline_id, ol.qtyordered-coalesce(ol.calloffqty,0) as qtyleft
                       ...
                       ol.qtyordered-coalesce(ol.calloffqty,0) >= 0 and o.contractdate <= v_Planneddate_new and o.enddate >= v_Planneddate_new order by o.contractdate)
                LOOP
                       ...
                       v_frameqtyleft:=v_cur.qtyleft;
                        ...
                        SELECT * INTO  v_Aux_New FROM Mrp_Run_Insertlines(p_Client_ID, p_Org_ID, p_User_ID, p_Run_ID, Cur_PlanProduct.M_Product_ID, v_frameqty2buy,'PP',  NULL, NULL, NULL, NULL,v_Planneddate_new, v_vendor,p_warehouse);
                ...
                if v_frameqty<v_Qty_New then
                        SELECT * INTO  v_Aux_New FROM Mrp_Run_Insertlines(p_Client_ID, p_Org_ID, p_User_ID, p_Run_ID, Cur_PlanProduct.M_Product_ID, v_Qty_New-v_frameqty,'PP',  NULL, NULL, NULL, NULL,v_Planneddate_new, v_vendor,p_warehouse);
    ...
    FOR Cur_workproposal IN (
      ...
      IF (v_count > 0) THEN
        SELECT PriceList, Pricepo as PriceStd,
               M_Get_Offers_Price(v_DateDoc,Cur_workproposal.C_BPartner_ID,Cur_workproposal.M_Product_ID,coalesce(Cur_workproposal.quantityorder,Cur_workproposal.QTY), Cur_workproposal.PO_PRICELIST_ID,'N',null,'N',null,v_2nduom,Cur_workproposal.m_product_po_id),
      ...
      INSERT INTO C_OrderLine
        (C_ORDERLINE_ID, AD_CLIENT_ID, AD_ORG_ID, ISACTIVE,
         ...
         C_TAX_ID,quantityorder,m_product_uom_id,m_product_po_id,orderlineselfjoin)
     VALUES
...
select trunc(coalesce(p.enddate,LOCALTIMESTAMP)) as stockdate,
       ...
       p.qty-p.qtyproduced as movementqty,null as c_bpartner_id,null as m_attributesetinstance_id
...
CREATE or replace FUNCTION mrp_getnextincomingDate(p_product_id varchar,p_planneddate timestamp,p_estimated_stock_qty numeric)  RETURNS DATE
AS $_$
declare
v_odate date;
v_pdate date;
...
select 
        *,mrp_inoutplan_v_id as mrp_criticalitems_v_id,
        mrp_getnextincomingDate(mrp_inoutplan_v.m_product_id,mrp_inoutplan_v.planneddate,mrp_inoutplan_v.estimated_stock_qty) as nextincomingdate
from 
        mrp_inoutplan_v
where
