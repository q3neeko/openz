...
BEGIN
   ...
   if v_count>0 then
        ...
        LOOP
            ...
            select m_product_id,c_uom_id,isserialtracking,isbatchtracking into v_productid,v_uom,v_isserial,v_isbatch from m_product where value=v_cur.productvalue_key;
            ...
            if v_productid is not null then 
               -- update m_product set isstocked='Y',producttype='I' where m_product_id=v_productid;
                ...
                LOOP   
                if (v_cur2.serialnumber is null and v_cur2.isunavailable='N' and v_cur2.batchnunber is null) or v_cur2.qtycount is null then 
                        raise exception '%',v_cur.productvalue_key||' No Batch or serial given or qty is null';
                end if;
                    ...
                    values (get_uuid(),ad_client,v_org,creator,creator,v_lineid,v_cur2.qtycount,v_cur2.ISUNAVAILABLE,v_cur2.batchnunber, v_cur2.SERIALNUMBER);
