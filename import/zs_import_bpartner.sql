  ...
  LOOP
    ...
    if v_bpid is null then
        ...
        if (select count(*) from zsi_bp_location where bp_value_key=v_cur.value)>0 then
            select count(*) into v_count from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_bpid;
                    if v_count=1 then 
                       -- Forget the automatically created entry
                       select C_LOCATION_id into v_temp from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_bpid;
                       update C_BPARTNER set C_LOCATION_id=null where c_bpartner_id=v_bpid;
                       delete  from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_bpid;
...
BEGIN
  ...
  LOOP
        ...
        if (select count(*) from C_BPARTNER_LOCATION where c_bpartner_id=v_cur.c_bpartner_id)>0 then
            select count(*) into v_count from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_cur.c_bpartner_id;
                    if v_count=1 then 
                       -- Forget the automatically created entry
                       select C_LOCATION_id into v_temp from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_cur.c_bpartner_id;
                       update C_BPARTNER set C_LOCATION_id=null where c_bpartner_id=v_cur.c_bpartner_id;
                       delete  from C_BPARTNER_LOCATION where name='Germany,  , Standard' and c_bpartner_id=v_cur.c_bpartner_id;
                       delete from c_location where c_location_id=v_temp;
                       v_i:=v_i+1;
                    end if;
        end if;
