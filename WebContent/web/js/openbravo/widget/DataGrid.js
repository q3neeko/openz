...
*/
  postMixInProperties: function(args, frag, parentComp) {
    ...
    if (this.calculateNumRows == true) {
      this.numRows = calculateNumRows(intRowHeight);
    }
    this.editor = null;
    this.editing = false;
...
*/
  onResize: function() {
    ...
    if (this.numRows != calculateNumRows(intRowHeight)) {
...
*/
  render: function() {
    ...
    gridContainer.innerHTML = "<div style='float:left;-webkit-user-drag: auto'></div>";
    ...
    this.setBounds();
    //table.style.height = (row.offsetHeight ) * (this.numRows);    
...
*/
  goToPreviousRow: function() {
    if (this.checkEditingRow()){
     ...
     this.moveCurrentPosition(-this.numRows+1);
     //this.moveCurrentPosition(-1);
    }
  },

  goToPreviousRowKey: function() {
    if (this.checkEditingRow()){
     // this.moveFromLastSelected(-1);
     
     //this.moveCurrentPosition(-this.numRows);
     this.moveCurrentPosition(-2);
    }
  },
...
*/
  goToNextRow: function() {
    if (this.checkEditingRow()){
     // this.moveFromLastSelected(1);
    // alert("NEXTOF:" + this.getCurrentOffset() + "     NumROWS:" +this.numRows + "  TotalRows:" + this.metaData.totalRows + "  Vis Rows:"+this.visibleRows );
    this.moveCurrentPosition(this.numRows-1);
    //this.moveCurrentPosition(1);
    }
  },
  
  goToNextRowKey: function() {
    if (this.checkEditingRow()){
     // this.moveFromLastSelected(1);
    // alert("NEXTOF:" + this.getCurrentOffset() + "     NumROWS:" +this.numRows + "  TotalRows:" + this.metaData.totalRows + "  Vis Rows:"+this.visibleRows );
    //this.moveCurrentPosition(this.numRows-1);
...
*/
  goToLastRow: function() {
    if (this.checkEditingRow()){
      this.scroller.moveScroll(this.metaData.totalRows - 1);
...
*/
  deleteRow: function() {
    if (document.activeElement.tagName=='BODY' || document.activeElement.id=='linkButtonDelete') {
        if (this.selectedRows.count() > 0 &&
...
*/
  selectRow: function(rowNo, hitch) {
    ...
    //  this.selectedRows.clear();
      ...
      this.showSelection();
      //console.log("DOSEL!"+rowNo);
    }
   // checkAttachmentIconRelation();
   var value = this.getSelectedRows();
   if (document.frmMain) setInputValue(document.frmMain.inpKeyName.value, value);
  },
...
*/
  moveCurrentPosition: function(mov) {
    ...
    this.scroller.moveScroll(newPosition);
    this.goToRow(newPosition); 
  },
...
goToRow: function(rowNo) {
    ...
    if (rowNo >= 0) {
      if (this.lastscroll==null){
            // Not comming from Scroller - So set the scroller!
            this.scroller.moveScroll(rowNo);
            this.lastscroll = new Date().getTime()
      }
      var minOffset = this.getCurrentOffset();
      var maxOffset = minOffset + this.numRows;
      var lastselrow = this.selectedRows.getLastSelected();
      var hitch;
      if (lastselrow != null) {
            hitch=  dojo.hitch(this, "showSelection", hitch);
      } 
      if (hitch != null){
      ...
      //console.log("SELROW:"+selrow+" COF:"+this.getCurrentOffset()+" NR:"+this.numRows+" ROW:" +rowNo);
     // if (selrow != null){
...
*/
  postRendering: function(options) {
    ...
    if ( this.options.prefetchBuffer || this.options.prefetchOffset > 0) {
      var offset = 0;
      var hitch;
      if (options.defaultRow) {
          offset = options.defaultRow;
          hitch=  dojo.hitch(this, "selectRow", offset, true);
          this.options.onRefreshComplete.push(hitch);
      }
...
*/
  fetchBuffer: function(offset) {
    if ( this.buffer.isInRange(offset)  && !this.newfilter) {
      return;
    }
    if (this.lastoffset==offset && !this.newfilter) {
      return;
    }
    ...
    //console.log("FBufPROCESS" +offset );
    //alert("FB" + offset + "-" + this.metaData.totalRows);
    setCalloutProcessing(true);
...
*/
  processQueuedRequest: function() {
    if (this.unprocessedRequest != null && this.unprocessedRequest.requestOffset > 0) {
      this.fetchBuffer(this.unprocessedRequest.requestOffset);
      this.unprocessedRequest = null
    }
  },
...
*/
  parseStructure: function(rootElem) {
    try {
      ...
      intRowHeight=parseInt(datagrids[0].getAttribute('backendRowHeight'));
      
      var nrows=calculateNumRows(intRowHeight);
      this.datagrid.numRows = nrows;
      this.datagrid.visibleRows = nrows;
      this.datagrid.visibleRowsMax = nrows;
...
/**
 * Here to Set Lineheight for Pictures
...
*/
    this.updateGrid = function() {
        var picheight=intRowHeight-3+"px";
      if (this.rowNode && (!this.offset || isVisible(this.offset))) {
        for (var i = 0; i < values.length; i++) {
          ...
          if (column.type.name == 'img' && value != '') { this.rowNode.cells[i].innerHTML = "<a onclick=\"openPopUp('" + value + "', 'Image', '70%', '70%'); return false;\" href=\"" + value + "\"><img src=\"" + value + "\" border=\"0\" height=\""+picheight+"\"></a>"; }
...
*/
  updateHeight: function() {
    ...
    var totalHeight;
    if (is_chrome){
    scrollerStyle.top         = -3+"px";} //- 16;}
    else{
    scrollerStyle.top         = -15+"px"; //- 16;}
        
    }
    // alert(intRowHeight);
    scrollerStyle.height      = visibleHeight + "px";
    //var totalHeight = parseInt(visibleHeight * this.metaData.getTotalRows()/this.metaData.getPageSize());
    //totalHeight=parseInt((19*this.metaData.getTotalRows()+38));
    totalHeight=parseInt((intRowHeight*this.metaData.getTotalRows())+38);
    //alert(totalHeight);
    this.heightDiv.style.height = totalHeight + "px" ;
    //alert(visibleHeight+" "+totalHeight);
...
*/
  createScrollBar: function() {
   ...
   if (is_mobile){
    ...
    scrollerStyle.left        = this.isIE ? "-6px" : "-0px";}
    scrollerStyle.position    = "relative";
    scrollerStyle.left        = this.isIE ? "-6px" : "-5px";
   ...
   //this.scrolandRangeDiv.appendChild(this.nextRange);
    ...
    var eventName = !this.isIE ?  "DOMMouseScroll":"onmousewheel" ;
    ...
    if (window.addEventListener){
      ...
      this.liveGrid.domNode.addEventListener('mousewheel', handler, false);}
    }else{
...
*/
  updateSize: function() {
    ...
    this.heightDiv.style.height=parseInt((intRowHeight*this.metaData.getTotalRows()) + 38)+"px";
    //oldClass
...
*/
  moveScroll: function(rowOffset) {
    this.scrollerDiv.scrollTop = (this.viewPort.rowHeight * rowOffset)
...
*/
  handleScroll: function() {
    if (this.metaData.totalRows != '0') {
      if (this.lastscroll==null){
            this.lastscroll = new Date().getTime()-1000;
      }
      if (this.lastscrolldiv==null) this.lastscrolldiv=0;
      var scrollDiff = this.lastScrollPos-this.scrollerDiv.scrollTop;
      this.lastscrolldiv=this.lastscrolldiv + scrollDiff;
      this.lastscroll = new Date().getTime();
      clearTimeout(this.scrollTimeout);
      var _this = this;
      if (this.lastscrolldiv != 0.00) {
            this.scrollTimeout = setTimeout(function() {_this.doscrollaction(); }, 0 );
      }
    }
  },
  ...
  doscrollaction: function() {
      ...
      //console.log("GOTO:"+contentOffset);
      this.liveGrid.goToRow(contentOffset); 
      // Display rowcount in the Toolbar..
      if ( this.metaData.options.onscroll ){
              this.metaData.options.onscroll( this.liveGrid, contentOffset );
      }
      this.lastScrollPos = this.scrollerDiv.scrollTop;
      this.lastscrolldiv = 0;
  },
...
*/
  loadRows: function(ajaxResponse) {
    ...
    if(this.isFilter || this.metaData.totalRows == 0) {
...
*/
  isInRange: function(position) {
     //   alert("pos"+position +"X"+ this.startPos + "M"+this.metaData.getPageSize());
...
*/
  constructor: function(table, rowHeight, visibleRows, buffer, liveGrid) {
    ...
    this.table = table;
    this.rowHeight = intRowHeight;
  //  alert (this.div.scrollWidth);
  //  alert (this.div.clientWidth);
    var scrollwidh = parseInt(this.div.scrollWidth);
    var viewwidth  = parseInt(this.div.clientWidth);
    var rowadd = visibleRows+6;
   // alert (scrollwidh  + viewwidth);

// oldClass
this.div.style.height = (intRowHeight * visibleRows) + this.div.firstChild.offsetHeight + 15 + "px";
    this.buffer = buffer;
    this.liveGrid = liveGrid;
...
*/
  bufferChanged: function() {
    this.refreshContents( parseInt(this.liveGrid.scroller.scrollerDiv.scrollTop /  this.rowHeight));
  },
...
*/
  scrollTo: function(offset) {
    if (this.lastPixelOffset == offset){
      return;
    }
    this.refreshContents(offset)
    //this.div.scrollTop = pixelOffset % this.rowHeight
    this.lastPixelOffset = offset;
  },
...
*/
  visibleHeight: function() {
    ...
    if (is_chrome) {
        ...
        visibleHeight = visibleHeight + 26;
    }
    
   // alert(dojo.getComputedStyle(this.div).height);
//    if (getBrowserInfo('name').toUpperCase().indexOf("CHROME") != -1 || getBrowserInfo('name').toUpperCase().indexOf("SAFARI") != -1) {
//      if (parent.frameMenu) {
//        visibleHeight = visibleHeight + 18;
//      } else {
