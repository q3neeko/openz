...
function sumselectedcontent() { 
	...
	var all_obj=getElementsByClassName("hpfr_input");
	    ...
	    var priceobj=document.getElementById("hiddenprice" + guid);
            if (isaNumber(all_obj[i].value.replace(',', '.')) && isaNumber(priceobj.value.replace(',', '.'))){
             if (parseInt(all_obj[i].value.replace(',', '.'))>0) {
                ...
                lines=lines + '<div class="hpfr_rdrs_mn">' + document.getElementById("descr"+ guid).value.substring(1,35) + '</div><div class="hpfr_rdrs_prc1">' + priceobj.value + " €</div>";
                SumCalc=SumCalc + su;
             }
            }
        }
        ...
        //alert(oldval);
        if (SumCalc==0 && isordered=="Y")
            odbtn.value="ABBESTELLEN";
        else
            odbtn.value="BESTELLUNG AUFGEBEN";
        //alert(ges);
