...
function windowSearch(strPage, strHeight, strWidth, strTop, strLeft, strWindow, parameters, strValueID) {
  ...
  winSelector=null;
  if (strHeight==null)
    strHeight=(screen.height - 150);
  if (strWidth==null)
    strWidth=(screen.width - 10);
  if (strTop==null) 
    strTop= 5;
    //parseInt((screen.height - strHeight)/2);
  if (strLeft==null) 
...
function calculateNumRows(intRowHeight) {
   resizeAreaInfo();
   ...
   var grid_row_height = intRowHeight +1 ; //document.getElementById("grid_sample_row").clientHeight + 1;
