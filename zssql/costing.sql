    ...
    LOOP
        ...
        if coalesce(p_planorfact,'Ä')='fact' and coalesce(p_costuom,'H')='P' and p_projecttask_id is not null then
            select sum((case when ad_get_docbasetype(c_invoice.c_doctype_id)='APC' then 
                                ...
                                case when c_invoice.isgrossinvoice='Y' and t.rate>0 then round(c_currency_convert(linegrossamt,c_currency_id,v_curr,dateacct)-c_currency_convert(linegrossamt,c_currency_id,v_curr,dateacct)/(1+100/t.rate),2) else case when isgrossinvoice='Y' and t.rate=0 then coalesce(c_currency_convert(linegrossamt,c_currency_id,v_curr,dateacct),0) else coalesce(c_currency_convert(linenetamt,c_currency_id,v_curr,dateacct),0) end end end)*v_cost.d/100)  
                                into v_invoicecost
