      ...
      LOOP
          ...
          if v_count>0 and coalesce(v_onlydebcred,'N')='N' then 
              ...
              LOOP
                ...
                v_desc:=replace(replace(substr(v_cur2.description,1,30),E'\r\n',''),';','');
                v_ldesc:=replace(replace(substr(v_cur2.description,1,210),E'\r\n',''),';','');
                if v_cur2.c_bpartner_id is not null then
                    v_ziart2:='KreditorDebitorNr';
                ...
                v_satz:= ';'||v_sollhaben||';'||v_stramt||';;'||v_dracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';;0,00;;;;'||v_ziart1||';'||v_ldesc||';'||v_ziart2||';'||v_ziart2text||';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                v_line:=v_line+1;
                ...
                LOOP
                    ...
                    if v_cur2.ad_table_id='800019' then
                        ...
                        if v_count>0 and v_newgroup='Y' then
                            ...
                            select acctvalue into v_gegenkonto from fact_acct where  fact_acct_group_id=v_cur1.fact_acct_group_id 
                            ...
                            v_satz:= ';'||v_source||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;'||v_ziart1||';'||v_ldesc||';'||v_ziart2||';'||v_ziart2text||';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                            if v_satz is null then
                    ...
                    -- Steuer-Bezeichnung bei Rechnungen 
                    ...
                    -- Falls die c_tax_id doch im Rechnungszeilensatz gesetzt sein sollte.. (Steuer-Bezeichnung bei Brutto-Rechnungen )
                    if v_cur2.ad_table_id='318' and v_cur2.c_tax_id is not null and v_cur2.line_id is not null then
                        select t.name  into  v_belegfeld1 from c_tax t where t.c_tax_id=v_cur2.c_tax_id and t.rate!=0;
                    end if;
                    ...
                    if (v_cur2.seqno=10  or (v_lineid!='' and v_newgroup='Y')) then
                        ...
                        then
                            ...
                            if v_count!=1 and v_cur2.ad_table_id='318' then
                                -- Hier erweitern wir, damit das alte Verhalten ansonsten bleibt...
                                ...
                                LOOP
                                    ...
                                    v_satz:= ';'||v_xsource||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;'||v_ziart1||';'||v_ldesc||';'||v_ziart2||';'||v_ziart2text||';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                                    if v_satz is null then
                                        raise exception '%','Datensatz ist NULL e: '||v_cur2.fact_acct_id;
                                    end if;
                                    insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group,lineno, export_data )
                    ...
                    --seqno=10
                        ...
                        if v_taxbooked='N' then
                            ...
                            v_satz:= ';'||v_sollhaben||';'||v_stramt||';;'||v_taxdracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_taxcracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;'||v_ziart1||';'||v_ldesc||';'||v_ziart2||';'||v_ziart2text||';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                            if v_satz is null then
                        ...
                        if v_amt=0 then
                            ...
                            v_satz:= ';'||v_sollhaben||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;'||v_ziart1||';'||v_ldesc||';'||v_ziart2||';'||v_ziart2text||';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                            v_line:=v_line+1;
                            if v_satz is null then
...
-- Kopfsatz kopieren
  ...
  for v_cur in (select * from i_susa where (length(i_01)=4 and i_01 not like '9%' and i_01!='1400' and i_01!='1600') or length(i_01)!=4)
  LOOP
    select zsfi_susadatevaccounts_id,c_elementvalue_id into v_id,v_evid from zsfi_susadatevaccounts where lpad(name,5,'0')=lpad(v_cur.i_01,5,'0') 
    ...
    if v_id is null then
       ...
       select C_ELEMENTVALUE.C_ELEMENTVALUE_ID,C_ELEMENTVALUE.value,ad_org_acctschema.c_acctschema_id ,ZSFI_SUSADATEV.datefrom,ZSFI_SUSADATEV.dateto ,
              ...
              lpad(C_ELEMENTVALUE.value,5,'0')=lpad(v_cur.i_01,5,'0') and C_ELEMENTVALUE.C_ELEMENT_id=C_ELEMENT.C_ELEMENT_id and
