...
BEGIN
  ...
  IF (TG_OP = 'INSERT') THEN
   ...
   SELECT
     -- cln.ad_client_id,
     zssi_cleanasciistring(org.name) AS Dbtr_Nm, bacc.iban AS DbtrAcct_IBAN, c_bank.swiftcode AS DbtrAgt_BIC
...
BEGIN
  ...
  -- get creditor banking information
    SELECT bacc.iban, bacc.cdtrschmeident, bank.swiftcode
    INTO v_pmtinf_CdtrAcct_IBAN, v_pmtinf_CdtrSchmeId_id, v_pmtinf_CdtrAgt_BIC
    FROM c_bankaccount bacc, c_bank bank
    WHERE 1=1
  ...
  IF (v_anzError = 0) THEN
   ...
   -- 2/2.3: Lastschrift-Bezogene (1..n) innerhalb des logischen Sammlers
        ...
        BEGIN
          ...
          FOR CUR_debit IN (
           SELECT
             ...
             TRIM(SUBSTR(drctDbtTx_RmtInf_Ustrd, 1, 140)) AS RmtInf_Ustrd   -- VARCHAR(140) NOT NULL,
            FROM 
        ...
        /*  INSERT INTO zsfi_sepa_debit_xml (daten) VALUES (' <!-- DrctDbtTxInf Datenzeile für Basislastschrift -->');  */
            ...
            INSERT INTO zsfi_sepa_debit_xml (daten) VALUES ('  <InstdAmt Ccy="EUR">' || CUR_debit.InstdAmt || '</InstdAmt>'); -- Betrag '6543.14'
