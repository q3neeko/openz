       ...
       LOOP
          FOR v_i IN 1..2 LOOP
            ...
            for v_cur3 in (select CASE WHEN v_cur.isasset='N' THEN (SUM(fact_acct.AMTACCTDR)-SUM(fact_acct.AMTACCTCR)) 
                           ...
                           fact_acct.c_acctschema_id,fact_acct.ACCTVALUE,fact_acct.account_id
                    from fact_acct,zspr_bwaprefacct bwaprefacct where 
                          ...
                          group by fact_acct.c_acctschema_id,fact_acct.ACCTVALUE,fact_acct.account_id)
            LOOP
               if (select count(*) from bwacalculation where account_id=v_cur3.account_id and lastyear=v_vj)=0 then
                insert into bwacalculation (c_acctschema_id,ACCTVALUE,account_id,summe,lastyear,ad_org_id) 
                values (v_cur3.c_acctschema_id,v_cur3.ACCTVALUE,v_cur3.account_id,v_cur3.summe,v_vj,v_org);
                v_acctschema:=v_cur3.c_acctschema_id;
               end if;
            END LOOP;
            for v_cur3 in (select 0 as summe,v_acctschema as c_acctschema_id,c_elementvalue.value as ACCTVALUE,c_elementvalue.c_elementvalue_id as account_id
                                    ...
                                    and bwaprefacct.zspr_bwaprefs_id=v_cur.zspr_bwaprefs_id 
                                     ...
                                     group by c_elementvalue.value,c_elementvalue.c_elementvalue_id)
             LOOP
                if (select count(*) from bwacalculation where account_id=v_cur3.account_id and lastyear=v_vj)=0 then
                    insert into bwacalculation (c_acctschema_id,ACCTVALUE,account_id,summe,lastyear,ad_org_id) 
                    values (v_cur3.c_acctschema_id,v_cur3.ACCTVALUE,v_cur3.account_id,v_cur3.summe,v_vj,v_org);
...
BEGIN 
      ...
      LOOP
        if v_cur.isparent='N' then
                select sum(ca.summe) into v_sum from bwacalculation ca, zspr_bwaprefacct bwaprefacct where  bwaprefacct.zspr_bwaprefs_id=v_cur.zspr_bwaprefs_id 
                                             and ca.ACCTVALUE like replace(bwaprefacct.acctmatch,'*','%')
                                             and case when instr(bwaprefacct.acctmatch,'*')>0 then length(ca.ACCTVALUE)=5 else length(ca.ACCTVALUE)=4 end
