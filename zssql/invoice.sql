  ...
  --TYPE RECORD IS REFCURSOR;
    ...
    v_qty NUMERIC;
        ...
        DECLARE Cur_ManualGeneratedLines CURSOR (Order_ID VARCHAR)  FOR
          ...
          ol.pricefluctuation,
          ol.m_attributesetinstance_id,ol.c_project_id,ol.c_projectphase_id,ol.c_projecttask_id,ol.a_asset_id,ol.textposition,ol.ispagebreak
              ...
              if v_draftexists>0 then
                    ...
                    INTO C_INVOICE
                      ...
                      (
                        ...
                        coalesce(coalesce(Cur_Order.contractdate,Cur_Order.scheddeliverydate),v_prjstartdate),coalesce(Cur_Order.enddate,v_prjenddate),v_internalnote,v_scheduledtransactiondate,v_transactionreference
                      )
                      ;
                        ...
                        IF(NOT Next_O_Line) THEN
                          ...
                          INTO C_INVOICELINE
                            (
                              ...
                              m_attributesetinstance_id,isgrossprice,c_project_id ,c_projectphase_id ,c_projecttask_id ,a_asset_id,pricefluctuation,textposition,ispagebreak
                            )
                            VALUES
                            (
                              ...
                              v_ĺinesrecord.c_projecttask_id ,v_ĺinesrecord.a_asset_id,v_ĺinesrecord.pricefluctuation,v_ĺinesrecord.textposition,v_ĺinesrecord.ispagebreak
                            )
                            ;
                            -- SZ END
                        END IF;--Next_O_Line
...
BEGIN
  ...
  if ad_get_docbasetype(v_doctype) in ('SOO','POO') then
      ...
      if p_pendingorall='PENDING' and v_invrule in ('D','O','S') then
              ...
              case when sl.M_Product_UOM_ID is null then sl.movementqty else
                    C_Uom_Convert(sl.movementqty-sl.quantityorder, sl.C_UOM_ID, (select c_uom_id from m_product_uom where m_product_uom_id=sl.M_Product_UOM_ID), 'Y') end, 
      ...
      if ad_get_docbasetype(v_doctype) in ('POO','SOO') then 
            ...
            -- After delivery , oder Complete and Customer Scheduling 
                  ...
                  if v_invrule = 'O' then
                        ...
                        if v_compare2=0 and v_ignoreresidue='N' and (v_QtyOrdered-v_QtyInvoiced > 0 or  v_amt - v_invoicedamt > 0) and coalesce(v_datepromised,now()-1)<now() then
                                return 'Y';
                        else
                                return 'N';
                        end if;
...
BEGIN
    ...
    LOOP
        ...
        then
                ...
                performanceperiodstart,performanceperiodend,c_project_id,c_projecttask_id,salesrep_Id ,ad_user_id,internalnote,description)
                VALUES
                ...
                v_cur.performanceperiodstart,v_cur.performanceperiodend,v_cur.headprj,v_cur.headpt,v_cur.salesrep_Id ,v_cur.ad_user_id,v_cur.internalnote,v_cur.description);
                v_lines:=0;
        end if;
        v_lines:=v_lines+10;
