            ...
            loop
              ...
              IF(v_NEW_AMORTIZATION='-1') THEN
                ...
                INTO A_AMORTIZATION
                  ...
                  VALUES
                  (v_NEW_AMORTIZATION, v_ENDING_DATE, v_AD_CLIENT_ID, v_AD_ORG_ID, TO_DATE(NOW()), v_CREATEDBY, TO_DATE(NOW()), v_CREATEDBY, LAST_DAY(TO_DATE('01-' || TO_CHAR(v_BEGINING_DATE, 'MM') || '-' || TO_CHAR(v_BEGINING_DATE, 'yyyy'), 'DD-MM-YYYY')), 'Y', TO_CHAR(TO_DATE(LAST_DAY(TO_DATE('01-' || TO_CHAR(v_BEGINING_DATE, 'MM') || '-' || TO_CHAR(v_BEGINING_DATE, 'YYYY'), 'DD-MM-YYYY')))), 'N', 'N', 'N', TO_DATE('01-' || TO_CHAR(v_BEGINING_DATE, 'MM') || '-' || TO_CHAR(v_BEGINING_DATE, 'yyyy'), 'DD-MM-YYYY'), v_CURRENCY_ID) ;
              END IF;
              SELECT COALESCE(MAX(LINE), 0) +10
              INTO v_LINE
              FROM A_AMORTIZATIONLINE
