...
on insert to tsrv_feedback_v do instead 
    ...
    values
            (new.tsrv_feedback_v_id,new.ad_client_id, new.ad_org_id, new.isactive, new.created, new.createdby, new.updated, new.updatedby, new.c_project_id, new.c_projecttask_id, new.c_calendarevent_id, new.employee_id, new.ma_machine_id,
             ...
             new.issaturday,new.issunday,new.isholiday);
...
BEGIN
 -- INSERT OR UPDATE
  ...
  IF (NEW.hours IS NOT NULL) THEN
    v_cost := (v_cost * (NEW.hours-new.nighthours-new.overtimehours)) + v_overtimecost + v_nightcost+(coalesce(new.specialtime,0)*v_sp1)+(coalesce(new.specialtime2,0)*v_sp2)+(coalesce(new.specialtime3,0)*v_sp3)+coalesce(new.triggeramt,0);
  ELSE                                                                                                                                   
