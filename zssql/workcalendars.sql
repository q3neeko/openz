...
BEGIN 
   ...
   LOOP     
    ...
    values (get_uuid(),'C726FEC915A54A0995C568555DA5BB3C',v_cur.ad_org_id,'0','0',v_cur.ad_user_id, v_cur.wyear, v_cur.wmonth, v_target,v_work,v_cur.value,zssi_calculatevacationaccountbalance(v_cur.c_bpartner_id ,to_char(v_cur.wmonth,'99'),to_char(v_cur.wyear,'9999')));
    i :=i+1; 
   END LOOP;
   update c_empworktimeaccount set balance=zssi_calculatetimeaccountbalance(ad_user_id,wmonth,wyear)
    where wyear >= extract (year from now()) -2;
...
dsoll:=v_time;
for v_cur in (select workdate, to_char(workdate,'DY') as vdd,to_char(workdate,'MONTH') as vm, to_char(workdate,'YYYY') as vy, to_char(workdate,'DD') as vdn,c_project_id,  hour_from, hour_to, hours, issunday, issaturday, isholiday, dayhours, breaktime, paidbreaktime, zspm_ptaskfeedbackline_id, traveltime, specialtime, specialtime2,specialtime3,triggeramt, overtimehours, c_projecttask_id, nighthours, ad_user_id 
  ...
  LOOP
    ...
    if (v_cur.issunday='Y' or v_cur.issaturday='Y' or v_cur.isholiday='Y' or (select dayname from c_workcalender where trunc(workdate)=trunc(v_curcal.datenow))='5') then
        if (v_cur.issunday='Y') then
            ...
            then     
               if coalesce(v_cur.nighthours,0)-coalesce(v_cur.breaktime,0)-coalesce(v_cur.paidbreaktime,0) > 0 then
                dsun:= v_cur.hours - extract(hour from v_cur.hour_to)- extract(minute from  v_cur.hour_to)/60+coalesce(v_cur.breaktime,0)+coalesce(v_cur.paidbreaktime, 0);
...
BEGIN
	...
	-- Logik wenn ein Eintrag im selben Jahr gefunden wurde
		...
		v_firstDayOfSpan := (p_year || '-' || (v_tmpWMonth + 1) || '-01')::timestamp without time zone;
		v_lastDayOfMonth := (p_year || '-' || p_month || '-01')::timestamp without time zone + (INTERVAL '1 month - 1 day');
