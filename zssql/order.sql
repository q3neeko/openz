...
BEGIN
  ...
  END IF;
  IF TG_OP = 'UPDATE' THEN
          IF old.documentno!=new.documentno and (select count(*) from c_orderline where (qtyinvoiced!=0 or qtydelivered!=0) and c_order_id=new.c_order_id)>0 then
         ...
         INSERT INTO C_OrderLine
          ...
          VALUES
            (p_destorder_id,get_uuid(),v_Client_ID, v_Org_ID,p_user_id, p_User_id,
             ...
             v_cur.isoptional,v_cur.textposition,v_cur.c_projecttask_id,v_cur.ispagebreak);
         v_i:=v_i+1;
    END LOOP;
    -- Payment scheduling
...
BEGIN 
    select ad_client_id,ad_org_id into  v_client_id,v_org_id from c_order where c_order_id=p_sourceorder_id;
    select c_bpartner_id,m_pricelist_id,c_bpartner_location_id into v_partner,v_pricelist,v_partner_location_id from c_order where c_order_id=p_destorder_id;
    --raise notice '%','copyorderlines - Source:'||coalesce(p_sourceorder_id,'0');
    --raise notice '%','copyorderlines - Dest:'||coalesce(p_destorder_id,'0');
    -- Copy the lines from the given Order
    for v_cur in (
    ...
    LOOP
         ...
         if v_isprodline='N' then
            ...
            VALUES
                ...
                v_cur.textposition,v_cur.ispagebreak);
            v_i:=v_i+1;
          end if;
    END LOOP;
    -- Payment scheduling
...
BEGIN 
    ...
    LOOP
         ...
         INSERT INTO C_OrderLine
            ...
            textposition,isoptional,ispagebreak)
          VALUES
            (p_destorder_id,get_uuid(),v_Client_ID, v_Org_ID,p_user_id, p_User_id,
             ...
             v_cur.textposition,case when v_copy_optionalpositions='Y' then v_cur.isoptional else 'N' end,v_cur.ispagebreak);
         v_i:=v_i+1;
    END LOOP;
    -- Payment scheduling
    for v_cur in (select * from c_order_paymentschedule where c_order_id=p_sourceorder_id)
...
loop
	...
	update c_order set 
		internalnote = zssi_getElementTextByColumname('Interval', v_language) || ': ' || zssi_strdate(v_intervalstartdate, v_language) || ' - ' || zssi_strdate(v_intervalenddate - interval '1 day', v_language) || E'\r\n' || E'\r\n' || coalesce(c_order.internalnote,''),
		datepromised = v_invoicedate,
...
BEGIN
    ...
    and (select isdropshipper from c_bpartner where c_bpartner_id=c_getcurrentvendor(ol.m_product_id,ol.ad_org_id))='Y'
    and not exists (select 0 from c_order ek,c_orderline ekl where ek.issotrx='N' and ek.orderselfjoin=o.c_order_id and ek.docstatus!='VO' and ekl.c_order_id=ek.c_order_id and ekl.m_product_id=ol.m_product_id)
