...
vars.getGlobalVariable("inpmInventorylineId", windowId + "|M_Inventoryline_ID");
  ...
  private Serialnumbers26BC2E065AFE41F9A7D99AB3FC8FC985Data getEditVariables(Connection con, VariablesSecureApp vars, String strPM_Inventoryline_ID) throws IOException,ServletException {
   ...
   private Serialnumbers26BC2E065AFE41F9A7D99AB3FC8FC985Data[] getRelationData(Serialnumbers26BC2E065AFE41F9A7D99AB3FC8FC985Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mInventorylineId = FormatUtilities.truncate(data[i].mInventorylineId, 32);        data[i].serialnumber = FormatUtilities.truncate(data[i].serialnumber, 40);        data[i].lotnumber = FormatUtilities.truncate(data[i].lotnumber, 40);        data[i].selectedbatch = FormatUtilities.truncate(data[i].selectedbatch, 0);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].rfidnumber = FormatUtilities.truncate(data[i].rfidnumber, 40);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].snrInventorylineId = FormatUtilities.truncate(data[i].snrInventorylineId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strSNR_Inventoryline_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|SNR_Inventoryline_ID", data[0].getField("snrInventorylineId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPM_Inventoryline_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.snrInventorylineId, vars, this,con);
