...
vars.getGlobalVariable("inpadRefListId", windowId + "|AD_Ref_List_ID");
  ...
  private ListReferenceInstanceSpecificData getEditVariables(Connection con, VariablesSecureApp vars, String strPAD_Ref_List_ID) throws IOException,ServletException {
   ...
   private ListReferenceInstanceSpecificData[] getRelationData(ListReferenceInstanceSpecificData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].adRefListId = FormatUtilities.truncate(data[i].adRefListId, 32);        data[i].adReferenceId = FormatUtilities.truncate(data[i].adReferenceId, 32);        data[i].value = FormatUtilities.truncate(data[i].value, 20);        data[i].name = FormatUtilities.truncate(data[i].name, 50);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].adWindowId = FormatUtilities.truncate(data[i].adWindowId, 32);        data[i].adTabId = FormatUtilities.truncate(data[i].adTabId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].adRefListinstanceId = FormatUtilities.truncate(data[i].adRefListinstanceId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strAD_Ref_Listinstance_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Ref_Listinstance_ID", data[0].getField("adRefListinstanceId"));
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPAD_Ref_List_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.adRefListinstanceId, vars, this,con);
