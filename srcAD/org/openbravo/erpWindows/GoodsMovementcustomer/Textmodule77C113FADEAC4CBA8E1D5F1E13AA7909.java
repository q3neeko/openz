...
vars.getGlobalVariable("inpmInoutId", windowId + "|M_Inout_ID");
  ...
  private Textmodule77C113FADEAC4CBA8E1D5F1E13AA7909Data getEditVariables(Connection con, VariablesSecureApp vars, String strPM_Inout_ID) throws IOException,ServletException {
   ...
   private Textmodule77C113FADEAC4CBA8E1D5F1E13AA7909Data[] getRelationData(Textmodule77C113FADEAC4CBA8E1D5F1E13AA7909Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mInoutId = FormatUtilities.truncate(data[i].mInoutId, 32);        data[i].zssiTextmoduleId = FormatUtilities.truncate(data[i].zssiTextmoduleId, 32);        data[i].text = FormatUtilities.truncate(data[i].text, 50);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].zssiMinoutTextmoduleId = FormatUtilities.truncate(data[i].zssiMinoutTextmoduleId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strZssi_Minout_Textmodule_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|Zssi_Minout_Textmodule_ID", data[0].getField("zssiMinoutTextmoduleId"));
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPM_Inout_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.zssiMinoutTextmoduleId, vars, this,con);
