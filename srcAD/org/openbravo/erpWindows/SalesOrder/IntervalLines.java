...
vars.getGlobalVariable("inpcSubscriptionintervalViewId", windowId + "|C_Subscriptioninterval_View_ID");
  ...
  private IntervalLinesData getEditVariables(Connection con, VariablesSecureApp vars, String strPC_Subscriptioninterval_View_ID) throws IOException,ServletException {
   ...
   private IntervalLinesData[] getRelationData(IntervalLinesData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 44);        data[i].cSubscriptionintervalViewId = FormatUtilities.truncate(data[i].cSubscriptionintervalViewId, 44);        data[i].cBpartnerId = FormatUtilities.truncate(data[i].cBpartnerId, 44);        data[i].cBpartnerLocationId = FormatUtilities.truncate(data[i].cBpartnerLocationId, 44);        data[i].mProductId = FormatUtilities.truncate(data[i].mProductId, 44);        data[i].mAttributesetinstanceId = FormatUtilities.truncate(data[i].mAttributesetinstanceId, 14);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].mProductUomId = FormatUtilities.truncate(data[i].mProductUomId, 44);        data[i].cUomId = FormatUtilities.truncate(data[i].cUomId, 44);        data[i].cCurrencyId = FormatUtilities.truncate(data[i].cCurrencyId, 44);        data[i].cTaxId = FormatUtilities.truncate(data[i].cTaxId, 44);        data[i].cProjectId = FormatUtilities.truncate(data[i].cProjectId, 32);        data[i].cProjecttaskId = FormatUtilities.truncate(data[i].cProjecttaskId, 32);        data[i].cSubscriptionintervallinesViewId = FormatUtilities.truncate(data[i].cSubscriptionintervallinesViewId, 0);        data[i].mWarehouseId = FormatUtilities.truncate(data[i].mWarehouseId, 22);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 22);}
    }
    return data;
  }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPC_Subscriptioninterval_View_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.cSubscriptionintervallinesViewId, vars, this,con);
