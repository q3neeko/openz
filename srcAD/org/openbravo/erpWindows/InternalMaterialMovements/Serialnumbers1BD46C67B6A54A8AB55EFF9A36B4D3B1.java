...
vars.getGlobalVariable("inpmInternalConsumptionlineId", windowId + "|M_Internal_Consumptionline_ID");
  ...
  private Serialnumbers1BD46C67B6A54A8AB55EFF9A36B4D3B1Data getEditVariables(Connection con, VariablesSecureApp vars, String strPM_Internal_Consumptionline_ID) throws IOException,ServletException {
   ...
   private Serialnumbers1BD46C67B6A54A8AB55EFF9A36B4D3B1Data[] getRelationData(Serialnumbers1BD46C67B6A54A8AB55EFF9A36B4D3B1Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mInternalConsumptionlineId = FormatUtilities.truncate(data[i].mInternalConsumptionlineId, 32);        data[i].serialnumber = FormatUtilities.truncate(data[i].serialnumber, 40);        data[i].lotnumber = FormatUtilities.truncate(data[i].lotnumber, 40);        data[i].selectedbatch = FormatUtilities.truncate(data[i].selectedbatch, 32);        data[i].snrBuiltinsnr = FormatUtilities.truncate(data[i].snrBuiltinsnr, 32);        data[i].snrInternalConsumptionlineId = FormatUtilities.truncate(data[i].snrInternalConsumptionlineId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].rfidnumber = FormatUtilities.truncate(data[i].rfidnumber, 40);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].description = FormatUtilities.truncate(data[i].description, 50);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strSNR_Internal_Consumptionline_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|SNR_Internal_Consumptionline_ID", data[0].getField("snrInternalConsumptionlineId"));
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPM_Internal_Consumptionline_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.snrInternalConsumptionlineId, vars, this,con);
