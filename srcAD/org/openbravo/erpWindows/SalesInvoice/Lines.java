...
vars.getGlobalVariable("inpcInvoiceId", windowId + "|C_Invoice_ID");
  ...
  private LinesData getEditVariables(Connection con, VariablesSecureApp vars, String strPC_Invoice_ID) throws IOException,ServletException {
   ...
   private LinesData[] getRelationData(LinesData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 44);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 44);        data[i].cInvoiceId = FormatUtilities.truncate(data[i].cInvoiceId, 44);        data[i].cOrderlineId = FormatUtilities.truncate(data[i].cOrderlineId, 44);        data[i].mInoutlineId = FormatUtilities.truncate(data[i].mInoutlineId, 44);        data[i].textposition = FormatUtilities.truncate(data[i].textposition, 50);        data[i].mProductId = FormatUtilities.truncate(data[i].mProductId, 44);        data[i].mAttributesetinstanceId = FormatUtilities.truncate(data[i].mAttributesetinstanceId, 14);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].mProductUomId = FormatUtilities.truncate(data[i].mProductUomId, 44);        data[i].cUomId = FormatUtilities.truncate(data[i].cUomId, 44);        data[i].cTaxId = FormatUtilities.truncate(data[i].cTaxId, 44);        data[i].cProjectId = FormatUtilities.truncate(data[i].cProjectId, 32);        data[i].cProjecttaskId = FormatUtilities.truncate(data[i].cProjecttaskId, 32);        data[i].cProjectphaseId = FormatUtilities.truncate(data[i].cProjectphaseId, 32);        data[i].sResourceassignmentId = FormatUtilities.truncate(data[i].sResourceassignmentId, 14);        data[i].cInvoicelineId = FormatUtilities.truncate(data[i].cInvoicelineId, 10);        data[i].cChargeId = FormatUtilities.truncate(data[i].cChargeId, 44);        data[i].mOfferId = FormatUtilities.truncate(data[i].mOfferId, 44);        data[i].aAssetId = FormatUtilities.truncate(data[i].aAssetId, 32);        data[i].reinvoicedbyId = FormatUtilities.truncate(data[i].reinvoicedbyId, 32);}
    }
    return data;
  }
...
xmlDocument.setParameter("grid_Default", selectedRow);
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean boolNew, String strC_InvoiceLine_ID, String strPC_Invoice_ID, TableSQLData tableSQL)
    ...
    String orderClause = " C_InvoiceLine.QuantityOrder,  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL11.Name IS NULL THEN TO_CHAR(table11.Name) ELSE TO_CHAR(tableTRL11.Name) END)), ''))),'') ), C_InvoiceLine.Line";
