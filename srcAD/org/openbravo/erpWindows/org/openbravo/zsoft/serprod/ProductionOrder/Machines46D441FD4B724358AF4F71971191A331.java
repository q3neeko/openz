...
vars.getGlobalVariable("inpzssmWorkstepVId", windowId + "|Zssm_Workstep_V_ID");
  ...
  private Machines46D441FD4B724358AF4F71971191A331Data getEditVariables(Connection con, VariablesSecureApp vars, String strPZssm_Workstep_V_ID) throws IOException,ServletException {
   ...
   private Machines46D441FD4B724358AF4F71971191A331Data[] getRelationData(Machines46D441FD4B724358AF4F71971191A331Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].zssmWorkstepVId = FormatUtilities.truncate(data[i].zssmWorkstepVId, 32);        data[i].maMachineId = FormatUtilities.truncate(data[i].maMachineId, 32);        data[i].durationunit = FormatUtilities.truncate(data[i].durationunit, 50);        data[i].zssmWorkstepmachinesVId = FormatUtilities.truncate(data[i].zssmWorkstepmachinesVId, 32);        data[i].zssmSectionId = FormatUtilities.truncate(data[i].zssmSectionId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].zspmPtaskmachineplanId = FormatUtilities.truncate(data[i].zspmPtaskmachineplanId, 32);        data[i].zssmWorkstepPrpVId = FormatUtilities.truncate(data[i].zssmWorkstepPrpVId, 32);        data[i].costuom = FormatUtilities.truncate(data[i].costuom, 20);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strZssm_WorkstepMachines_V_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|Zssm_WorkstepMachines_V_ID", data[0].getField("zssmWorkstepmachinesVId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPZssm_Workstep_V_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.zssmWorkstepmachinesVId, vars, this,con);
