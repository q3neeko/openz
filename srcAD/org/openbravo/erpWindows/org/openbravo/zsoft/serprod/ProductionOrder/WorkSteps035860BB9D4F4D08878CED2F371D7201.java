...
PInstanceProcessData.insertPInstanceParam(this, pinstance, "30", "M_Locator_ID", strmLocatorId, vars.getClient(), vars.getOrg(), vars.getUser());
  ...
  private WorkSteps035860BB9D4F4D08878CED2F371D7201Data getEditVariables(Connection con, VariablesSecureApp vars, String strPzssm_productionorder_v_id) throws IOException,ServletException {
   ...
   private WorkSteps035860BB9D4F4D08878CED2F371D7201Data[] getRelationData(WorkSteps035860BB9D4F4D08878CED2F371D7201Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].zssmProductionorderVId = FormatUtilities.truncate(data[i].zssmProductionorderVId, 46);        data[i].cColorId = FormatUtilities.truncate(data[i].cColorId, 32);        data[i].value = FormatUtilities.truncate(data[i].value, 20);        data[i].name = FormatUtilities.truncate(data[i].name, 50);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].triggerreason = FormatUtilities.truncate(data[i].triggerreason, 50);        data[i].mProductId = FormatUtilities.truncate(data[i].mProductId, 32);        data[i].receivingLocator = FormatUtilities.truncate(data[i].receivingLocator, 32);        data[i].issuingLocator = FormatUtilities.truncate(data[i].issuingLocator, 32);        data[i].help = FormatUtilities.truncate(data[i].help, 50);        data[i].zssmPrjName = FormatUtilities.truncate(data[i].zssmPrjName, 50);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 22);        data[i].zssmWorkstepVId = FormatUtilities.truncate(data[i].zssmWorkstepVId, 32);        data[i].schedulestatus = FormatUtilities.truncate(data[i].schedulestatus, 20);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 22);        data[i].cTaskId = FormatUtilities.truncate(data[i].cTaskId, 22);        data[i].zssmPrjProjectcategory = FormatUtilities.truncate(data[i].zssmPrjProjectcategory, 50);        data[i].zssmProductionplanTaskId = FormatUtilities.truncate(data[i].zssmProductionplanTaskId, 32);        data[i].cOrderlineId = FormatUtilities.truncate(data[i].cOrderlineId, 32);        data[i].cProjecttaskId = FormatUtilities.truncate(data[i].cProjecttaskId, 32);}
    }
    return data;
  }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPzssm_productionorder_v_id) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = vars.getStringParameter("inpnewdataseIdVal");
          ...
          CrudOperations.UpdateCustomFields(tabId, data.zssmWorkstepVId, vars, this,con);
